...
theorem simulate_eagerTrace_filteredCausalAttackerHashQueryFromHigh_support_revealsCovered
  ...
  | cached output =>
      ...
      subst result
      exact ⟨hcovered.causalRecordedState secretKey selected input,
        by simp [CausalTraceRevealsCovered]⟩
  | reveal index =>
      have hindex := filteredCausalAttackerHashPlan_reveal_mem_of_covered
        secretKey selected input state index covered hplan hcovered hforward
      ...
      subst result
      constructor
      · exact hcovered.causalRevealResultState secretKey selected input index
          (table index) _ hindex
  ...
  | conditioned digest =>
      ...
      subst result
      exact ⟨(hcovered.causalRecordedState secretKey selected input).setCache _,
        by simp [CausalTraceRevealsCovered]⟩
  | fresh =>
      rw [filteredCausalAttackerHashQueryFromHigh_run, hplan,
...
theorem simulate_eagerTrace_filteredCausalSigningQuery_support_cache_le
  ...
  rw [mem_support_bind_iff] at hresult
  obtain ⟨encoded, hencoded, hresult⟩ := hresult
  have hencodedLe : state.cache ≤ encoded.2 :=
  ...
  | some encoding =>
      ...
      let signature := replaceSignatureChainValue
        (Concrete.CacheReplay.signWithEncoding keyView.cache keyView.secretKey
          request.epoch randomness encoding) selected (table index)
      have hstable : Concrete.CacheView.encodingHash finalCache
        ...
        exact hcacheLe
      have hindex : index ∈ covered := by
        apply hdirect signature encoding
        · rfl
        · change TargetSum.decodeDigest
            (Concrete.CacheView.encodingHash finalCache
              keyView.secretKey.parameter request.epoch
              (request.message, randomness)) = some encoding
          rw [hstable]
          exact hdecode
      constructor
      · exact (hcovered.setCache encoded.2).recordReveal
          index (table index) hindex
      · intro candidate _value hmem
        simp only [List.mem_singleton] at hmem
        cases hmem
        exact hindex
...
theorem simulate_eagerTrace_filteredCausalAttackerHashQueryFromHigh_support_cache_le
  ...
  | cached output =>
      ...
      simp only [simulateQ_pure, WriterT.run_pure, support_pure,
        Set.mem_singleton_iff] at hresult
      subst result
      simpa only [causalRecordedState_cache] using (le_refl state.cache)
  | reveal index =>
      have habsent := filteredCausalAttackerHashPlan_cache_none_of_noncached
        secretKey selected input state (.reveal index) hplan
          (by intro output heq; cases heq)
      rw [filteredCausalAttackerHashQueryFromHigh_run, hplan,
        simulate_eagerTrace_causalRevealHashQueryFromHigh] at hresult
      simp only [support_pure, Set.mem_singleton_iff] at hresult
      subst result
  ...
  | conditioned digest =>
      have habsent := filteredCausalAttackerHashPlan_cache_none_of_noncached
        secretKey selected input state (.conditioned digest) hplan
          (by intro output heq; cases heq)
      rw [filteredCausalAttackerHashQueryFromHigh_run, hplan,
        simulateQ_bind, WriterT.run_bind',
        RevealProbeOracleSimulation.simulate_eagerTrace_liftProbComp,
  ...
  | fresh =>
      rw [filteredCausalAttackerHashQueryFromHigh_run, hplan] at hresult
      simpa only [causalRecordedState_cache] using
        simulate_eagerTrace_causalHashQuery_support_cache_le table input
          (causalRecordedState secretKey selected input state) result hresult
