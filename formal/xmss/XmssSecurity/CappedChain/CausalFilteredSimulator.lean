...
noncomputable def filteredCausalSigningAttempt
...
noncomputable def filteredCausalSignBoundedAttempts : Nat →
    ProgrammedFixedChainKeygenView → ChainIndex → SignRequest →
    CausalHashState →
    OracleComp (RevealProbeOracleSimulation.World ChainValueIndex)
      (Option Signature × CausalHashState)
  | 0, _keyView, _selected, _request, state => pure (none, state)
  | attempts + 1, keyView, selected, request, state => do
      let result ← filteredCausalSigningAttempt keyView selected request state
      match result.1 with
      | some signature => pure (some signature, result.2)
      | none => filteredCausalSignBoundedAttempts attempts keyView selected request result.2
...
noncomputable def filteredCausalSigningQuery
    ...
    OracleComp (RevealProbeOracleSimulation.World ChainValueIndex)
      (Option Signature × CausalHashState) :=
  filteredCausalSignBoundedAttempts signingAttemptLimit keyView selected request
    state

noncomputable def filteredCausalSignTraceContinuation
...
theorem simulate_eagerTrace_filteredCausalSignBoundedAttempts_succ
    (attempts : Nat) (table : ChainValueIndex → Digest)
    (keyView : ProgrammedFixedChainKeygenView) (selected : ChainIndex)
    (request : SignRequest) (state : CausalHashState) :
    (simulateQ (RevealProbeOracleSimulation.eagerTraceImpl table)
      (filteredCausalSignBoundedAttempts (attempts + 1) keyView selected request
        state)).run =
      (simulateQ (RevealProbeOracleSimulation.eagerTraceImpl table)
        (filteredCausalSigningAttempt keyView selected request state)).run >>=
          filteredCausalSignTraceContinuation attempts table keyView selected
            request := by
  rw [filteredCausalSignBoundedAttempts, simulateQ_bind, WriterT.run_bind']
  apply bind_congr
  intro result
  rcases result with ⟨⟨signatureOption, resultState⟩, trace⟩
  cases signatureOption with
  | none =>
      simp only [filteredCausalSignTraceContinuation]
      congr 1
  | some signature =>
      simp [filteredCausalSignTraceContinuation]
...
def FilteredSigningAttemptResultRelation
    (parameter : PublicParameter) (selected : ChainIndex)
    (leftBase rightBase : QueryCache HashSpec)
    (table : ChainValueIndex → Digest)
    (leftResult : Option Signature × QueryCache HashSpec)
    (rightResult : (Option Signature × CausalHashState) ×
      RevealProbeOracleSimulation.ActionTrace ChainValueIndex) : Prop :=
  FilteredSigningResultRelation parameter selected leftBase rightBase table
      leftResult rightResult ∧
    (leftResult.1 = none → rightResult.2 = [])

set_option maxRecDepth 100000 in
theorem relTriple_programmed_filteredCausalSigningAttempt
    (selected : ChainIndex)
    (left : ProgrammedFixedChainKeygenView)
    (right : ProgrammedFixedChainKeygenView ×
      (ChainValueIndex → Digest))
    ...
    RelTriple
      ...
      ((simulateQ (RevealProbeOracleSimulation.eagerTraceImpl right.2)
        (filteredCausalSigningAttempt right.1 selected request rightState)).run)
      (FilteredSigningAttemptResultRelation left.secretKey.parameter selected
  ...
  | none =>
      ...
      apply relTriple_pure_pure
      unfold FilteredSigningAttemptResultRelation
      refine ⟨?_, fun _ => rfl⟩
...
set_option maxRecDepth 100000 in
theorem relTriple_programmed_filteredCausalSignBoundedAttempts
    ...
    RelTriple
      ((simulateQ xmssRomImpl
        (Concrete.signBoundedAttempts attempts left.secretKey
          request.epoch request.message)).run leftCache)
      ((simulateQ (RevealProbeOracleSimulation.eagerTraceImpl right.2)
        (filteredCausalSignBoundedAttempts attempts right.1 selected request
          rightState)).run)
  ...
  | zero =>
      simp only [Concrete.signBoundedAttempts,
        filteredCausalSignBoundedAttempts, simulateQ_pure, StateT.run_pure,
        WriterT.run_pure]
      apply relTriple_pure_pure
      exact ⟨rfl, hstate⟩
  | succ attempts ih =>
      ...
      apply relTriple_bind
        (relTriple_programmed_filteredCausalSigningAttempt selected left right
      ...
      | none =>
          ...
          unfold filteredCausalSignTraceContinuation
...
theorem relTriple_programmed_filteredCausalSigningQuery
    ...
    RelTriple
      ...
      (FilteredSigningResultRelation left.secretKey.parameter selected
        left.cache right.1.cache right.2) := by
  simp only [Concrete.scheme, filteredCausalSigningQuery]
  have hleftKey := programmedWarmedFixedChainKeygen_support_keyResult
    selected left hleftSupport
  apply relTriple_of_evalDist_eq_left
