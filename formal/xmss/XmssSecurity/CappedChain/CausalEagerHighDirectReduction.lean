import XmssSecurity.CappedChain.CausalEagerHighHitTransfer
import XmssSecurity.CappedChain.CausalStrategyReduction
import XmssSecurity.CausalEagerHighDirectReduction
...
theorem filteredCausalAttackerHashQueryFromHigh_isProbeQueryBoundP
  ...
  exact
    XmssSecurity.filteredCausalAttackerHashQueryFromHigh_isProbeQueryBoundP
      high secretKey selected input state
...
theorem relTriple_sourceDirect_filteredHighMonitored_adversary_boundedHit
    ...
    (hbound : (simulateQ
      (sourceUnloggedMappedAdversaryImpl left.publicKey
        (materializedFixedChainSecretKey left))
  ...
  | query_bind input next ih =>
      ...
      have hleftInfo := sourceDirectTracedMappedAdversaryImpl_support_info
        left.publicKey (materializedFixedChainSecretKey left) input leftState
          headLeft hhead.2.1
      ...
      have hstepBound :
          (liftM (sourceUnloggedMappedAdversaryImpl left.publicKey
            (materializedFixedChainSecretKey left) input) >>=
              continuation).IsQueryBoundP
              (· matches .inr _) fuel := by
        exact hbound
      have hrestBound :=
        sourceUnloggedMappedAdversaryImpl_continuation_hashQueryBound
          left.publicKey (materializedFixedChainSecretKey left) input
            continuation fuel hstepBound headLeft.1 hleftInfo.1
  ...
  · obtain ⟨hforgery, hstates, _hcoarseCount⟩ := hgood
    ...
    have hresidual :=
      sourceDirectTracedMappedAdversary_residual_hashQueryBound
        left.publicKey (materializedFixedChainSecretKey left)
...
theorem hasBoundedFilteredHighDirectReduction_of_hashQueryBound
  ...
  rw [sourceDirectTracedProgram_eq_highProgrammedWarmedDetailedGame] at hmapped
  rw [filteredHighBoundedMonitoredProgram_projection_eq_eagerExperiment]
    at hmapped
  apply relTriple_of_evalDist_eq_left _ hmapped
  exact (evalDist_chronologicallyWarmedDetailedGame_eq_programmed adversary
    selected).symm.trans
      (evalDist_chronologicallyWarmedDetailedGame_eq_highProgrammed adversary
        selected)
