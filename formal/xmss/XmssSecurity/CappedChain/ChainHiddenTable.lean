...
theorem WinningOutcomeBadEventOccurs.chain_coordinate_ne_returned
  ...
  exact ne_of_gt
    (XmssSecurity.CappedChain.WinningOutcomeBadEventOccurs.chain_coordinate_lt_returned
      hevent forgedEncoding hforgedDecode request signature signedEncoding hreturned
      hsignedDecode hepoch)
...
theorem returned_chainValue_eq_keygenChainValueTable
    (qAdversary : Adversary Concrete.cappedScheme)
    (keyResult : (PublicKey × SecretKey) × QueryCache HashSpec)
    ...
    (hafter : execution ∈ support
      ((simulateQ xmssRomImpl
        (detailedGameAfterKeygen Concrete.cappedScheme qAdversary keyResult.1.1
  ...
  have hgame := CappedLeaf.afterKeygen_execution_mem_detailedGame qAdversary keyResult hkeygen
    execution hafter
  have hkeys := CappedLeaf.detailedGameAfterKeygen_keys_eq qAdversary keyResult.1.1 keyResult.1.2
    keyResult.2 execution hafter
  have hsignature := CappedLeaf.detailed_execution_returned_signature_eq qAdversary execution hgame
