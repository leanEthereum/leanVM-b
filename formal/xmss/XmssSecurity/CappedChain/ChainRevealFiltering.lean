...
import XmssSecurity.CappedChain.ChainHiddenTable
...
theorem WinningOutcomeChainValueHasKeygenOrigin.has_unrevealed_probe
  ...
  have hunrevealed : (outcome.forgery.epoch, encoding chain) ∉
    ...
    rw [← hsecret]
    exact
      XmssSecurity.CappedChain.WinningOutcomeBadEventOccurs.forged_chain_coordinate_not_mem_returned
        horigin.1 encoding (by simpa [hsecret] using hdecode)
