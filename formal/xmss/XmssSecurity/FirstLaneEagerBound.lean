import XmssSecurity.FirstLaneHazardEnforcement
...
noncomputable def runTracedObserved
  ...
  (fun result => runObserved table encodingState chainState result.2) <$>
...
theorem runTracedObserved_eq_runStructural
    (table : Index → Digest)
    (encodingState : Option EncodingMonitor.State)
    (chainState : AdaptiveRevealMonitor.State Index)
    ...
    evalDist (runTracedObserved table encodingState chainState computation) =
      evalDist
        (runStructural table encodingState chainState fuel computation) := by
  induction computation using OracleComp.inductionOn generalizing
      encodingState chainState fuel with
  | pure result =>
      simp [runTracedObserved, runStructural, runObserved, eagerTraceImpl]
  | query_bind input next ih =>
      ...
      | uniform n =>
          ...
          intro output
          change evalDist (runTracedObserved table encodingState chainState
              (next output)) =
            evalDist
              (runStructural table encodingState chainState fuel (next output))
          exact ih output encodingState chainState hagrees fuel
      ...
      | encodingQuery epoch =>
          ...
          | succ remaining =>
              simp only [OracleQuery.input_query, monadLift_self,
                eagerTraceImpl, QueryImpl.withTraceAppend_apply, eagerImpl,
                traceFragment, WriterT.run_bind', WriterT.run_monadLift',
                WriterT.run_tell, WriterT.run_pure', map_eq_bind_pure_comp,
                bind_assoc, pure_bind, Prod.map_apply, id_eq,
                Function.comp_apply]
              apply OracleComp.DeferredSampling.evalDist_bind_congr_left
              intro output
              simp [runObserved]
              ...
              | none =>
                  simp only
                  change evalDist (runTracedObserved table none chainState
                      (next output)) =
                    evalDist
                      (runStructural table none chainState remaining
                        (next output))
                  exact ih output none chainState hagrees remaining
                    (by simpa [IsHazardQuery] using hbound.2 output)
              | some state =>
                  cases happly :
                  ...
                  | none =>
                      ...
                      change evalDist (runTracedObserved table none chainState
                          (next output)) =
                        evalDist (runStructural table none chainState remaining
                          (next output))
                      exact ih output none chainState hagrees remaining
                        (by simpa [IsHazardQuery] using hbound.2 output)
                  | some result =>
                      ...
                      cases hit with
                      | false =>
                          simp only [happly, Bool.false_or]
                          change evalDist (runTracedObserved table
                              (some nextState) chainState (next output)) =
                            evalDist (runStructural table (some nextState)
                              chainState remaining (next output))
                          exact ih output (some nextState) chainState hagrees
                            remaining
                            (by simpa [IsHazardQuery] using hbound.2 output)
                      | true =>
                          simp only [happly, Bool.true_or, ↓reduceIte]
                          change evalDist
                              ((fun _ => true) <$>
                                (simulateQ (eagerTraceImpl table)
                                  (next output)).run) = evalDist (pure true)
                          exact
      ...
      | encodingSignAttempt epoch =>
          ...
          | none =>
              simp only
              change evalDist
                  (runTracedObserved table none chainState (next output)) =
                evalDist
                  (runStructural table none chainState fuel (next output))
              exact ih output none chainState hagrees fuel
                (by simpa [IsHazardQuery] using hbound.2 output)
          | some state =>
              ...
              | some result =>
                  ...
                  | false =>
                      simp only [happly, Bool.false_or]
                      change evalDist (runTracedObserved table (some nextState)
                          chainState (next output)) =
                        evalDist (runStructural table (some nextState)
                          chainState fuel (next output))
                      exact ih output (some nextState) chainState hagrees fuel
                        (by simpa [IsHazardQuery] using hbound.2 output)
                  | true =>
                      simp only [happly, Bool.true_or, ↓reduceIte]
                      change evalDist
      ...
      | probe index target =>
          ...
          | succ remaining =>
              ...
              | some value =>
                  simp only
                  change evalDist (runTracedObserved table encodingState
                      chainState (next ())) =
                    evalDist (runStructural table encodingState chainState
                      remaining (next ()))
                  exact ih () encodingState chainState hagrees remaining
                    (by simpa [IsHazardQuery] using hbound.2 ())
              | none =>
                  simp only
                  change evalDist (runTracedObserved table encodingState
                      (chainState.addPending index target) (next ())) =
                    evalDist (runStructural table encodingState
                      (chainState.addPending index target) remaining (next ()))
                  exact ih () encodingState
                    (chainState.addPending index target)
                      (hagrees.addPending index target) remaining
                      (by simpa [IsHazardQuery] using hbound.2 ())
      | reveal index =>
          simp only [OracleQuery.input_query, monadLift_self, eagerTraceImpl,
            QueryImpl.withTraceAppend_apply, eagerImpl, traceFragment,
          ...
          | some value =>
              ...
              change evalDist (runTracedObserved table encodingState chainState
                  (next (table index))) =
                evalDist (runStructural table encodingState chainState fuel
                  (next value))
              have hvalue := hagrees index value hrevealed
              subst value
              exact ih (table index) encodingState chainState hagrees fuel
                (by simpa [IsHazardQuery] using hbound.2 (table index))
          | none =>
              simp only
              by_cases hhit : table index ∈ chainState.pending index
              · simp only [hhit, decide_true, Bool.true_or, ↓reduceIte]
                change evalDist
                    ((fun _ => true) <$>
                      (simulateQ (eagerTraceImpl table)
                        (next (table index))).run) = evalDist (pure true)
                exact
                  OracleComp.DeferredSampling.evalDist_bind_const_neverFails
                    ((simulateQ (eagerTraceImpl table)
                      (next (table index))).run) (by simp) (pure true)
              · simp only [hhit, decide_false, Bool.false_or, ↓reduceIte]
                change evalDist (runTracedObserved table encodingState
                    (chainState.install index (table index))
                    (next (table index))) =
                  evalDist (runStructural table encodingState
...
noncomputable def tracedTableExperiment
  ...
  runTracedObserved table (some EncodingMonitor.State.empty)
    AdaptiveRevealMonitor.State.empty computation

theorem map_eagerExperiment_runObserved_eq_tracedTableExperiment
    (computation : OracleComp (World Index) α) :
    (fun result => runObserved result.1 (some EncodingMonitor.State.empty)
...
theorem eagerExperiment_combinedHit_probability_le
    (fuel : Nat) (computation : OracleComp (World Index) α)
    (hbound : computation.IsQueryBoundP IsHazardQuery fuel) :
    Pr[ExperimentHit | eagerExperiment computation] ≤
      (fuel : ENNReal) / ((2 ^ digestBits : Nat) : ENNReal) := by
  refine (probEvent_congr' (fun result _ =>
    (runObserved_empty_eq_combinedHit result.1 result.2.2).symm) rfl).le.trans ?_
  ...
  refine (show Pr[(fun hit : Bool => hit = true) |
    ...
    exact runTracedObserved_eq_runStructural table
      ...
      fuel computation hbound).le.trans
        (structuralExperiment_empty_true_probability_le fuel computation)

theorem eagerExperiment_enforced_combinedHit_probability_le
    (fuel : Nat) (computation : OracleComp (World Index) α) :
    Pr[ExperimentHit |
      eagerExperiment (enforceHazardBound fuel computation)] ≤
      (fuel : ENNReal) / ((2 ^ digestBits : Nat) : ENNReal) :=
  eagerExperiment_combinedHit_probability_le fuel
