...
noncomputable local instance globalCausalInstalledDetailedGameSampleableChainTable :
...
noncomputable def globalCausalEagerVerificationContinuation
    ...
    (handled : (((Forgery × AttackerActionTrace) × GlobalCausalHashState) ×
      RevealProbeOracleSimulation.ActionTrace GlobalChainValueIndex)) : ProbComp α := do
  let verified ← (simulateQ
    ...
    ((simulateQ (globalCausalVerifierXmssRomImpl secretKey)
      (Concrete.cappedScheme.verify publicKey handled.1.1.1.epoch
  ...
  calc
    _ = 𝒟[do
        ...
        globalCausalEagerVerificationContinuation publicKey secretKey
          continuation (globalCausalInstalledTable handled.1.2 base) handled] := by
