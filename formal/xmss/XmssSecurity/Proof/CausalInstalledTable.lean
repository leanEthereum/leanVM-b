import XmssSecurity.Proof.CausalActionResampling
...
theorem CausalRevealsLe.causalRecordedState
  ...
  rw [XmssSecurity.causalRecordedState]
  ...
  exact (CausalRevealsLe.causalRecordedState
    secretKey chain input state).trans
      ((CausalRevealsLe.recordReveal
        (XmssSecurity.causalRecordedState secretKey chain input state) index value
