...
theorem extendHashCacheWithLog_le (initialCache : QueryCache HashSpec)
    (log : QueryLog HashSpec) (cache : QueryCache HashSpec)
    (hinitial : initialCache ≤ cache)
    (hentries : ∀ entry ∈ log, cache entry.1 = some entry.2) :
    extendHashCacheWithLog initialCache log ≤ cache := by
  induction log generalizing initialCache with
  | nil => exact hinitial
  | cons entry tail ih =>
      apply ih (initialCache.cacheQuery entry.1 entry.2)
      · intro input output hlookup
        ...
        · rw [QueryCache.cacheQuery_of_ne _ _ heq] at hlookup
          exact hinitial hlookup
      · intro tailEntry htail
        exact hentries tailEntry (List.Mem.tail entry htail)

theorem hashCacheOfLog_le (log : QueryLog HashSpec)
    (cache : QueryCache HashSpec)
    (hentries : ∀ entry ∈ log, cache entry.1 = some entry.2) :
    hashCacheOfLog log ≤ cache :=
  extendHashCacheWithLog_le ∅ log cache bot_le hentries
