...
noncomputable local instance globalCausalInstalledAdversaryRunSampleableChainTable :
    ...
    QueryImpl (OracleWorld + SigningSpec)
      (WriterT AttackerActionTrace
        (StateT GlobalCausalHashState
          (WriterT (RevealProbeOracleSimulation.ActionTrace GlobalChainValueIndex)
            ProbComp))) :=
...
noncomputable def globalCausalEagerAdversaryRestContinuation
    ...
    (continuation : (GlobalChainValueIndex → Digest) →
      (((α × AttackerActionTrace) × GlobalCausalHashState) ×
        RevealProbeOracleSimulation.ActionTrace GlobalChainValueIndex) → ProbComp β)
    (table : GlobalChainValueIndex → Digest)
    (handled : ((((OracleWorld + SigningSpec).Range input ×
      AttackerActionTrace) × GlobalCausalHashState) ×
      RevealProbeOracleSimulation.ActionTrace GlobalChainValueIndex)) : ProbComp β := do
  let rest ← (simulateQ
    (RevealProbeOracleSimulation.eagerTraceImpl table)
    (((simulateQ (globalCausalActionTracedMappedAdversaryAfterRealRomImpl
