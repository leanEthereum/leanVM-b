...
The arithmetic does not call for another optimization pass: at x=2^-15 the large-budget margin after delta is exactly 2^-17 per x, which exceeds the exceptional allowance. The small-budget coefficients leave a fixed gap below two. The complete paper derivation is now written, with the adaptive witness transfers identified as the main mathematical audit target. The next condition for resuming Lean work is checking those transfers against the stated endpoint requirements; writing their formulas and passing finite examples do not replace that audit.
