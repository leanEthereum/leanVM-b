...
The recommendation from this review is to audit the two-range argument against its original-experiment requirements. The expanded [stopped coverage proof](cached-target-forecast.md) derives the local shape transitions, fresh-target charges, and complete certificate inequality from the message kernel, and identifies its instantiation in the original signer. Its deferred-secret application uses the graph simulation and inherited path budget described in small-budget-transfer.md. Lean now connects certificate accumulation, its pathwise and expected payments, the exact adaptive terminal word, and the numerical fixed-word moments to the original whole game. The full bound is E[C_full]<=3 E[A_msg]/(2N)+q/2^141; the sum of the 14 omitted-coordinate monitor expectations is at most 557q/N. The exceptional probabilities, coverage extension to deferred-secret kernels, and cryptographic transfers remain missing. The expanded [canonical-graph argument](paid-probe-quadratic.md) supplies the paper sampling order, serialized-domain audit, hidden-label induction, SUF decomposition, and large-budget assembly. The [small-budget transfer](small-budget-transfer.md) supplies the paper likelihood and cost arguments. Those concrete transfers remain the main audit target. No public 127-bit Lean theorem is claimed.
...
The arithmetic does not call for another optimization pass: the fixed-word route at x=3/2^14 and delta=2^-13 leaves exactly 2^-16 per x in the large-budget margin, while the small-budget coefficients remain below 7/4. The adaptive witness transfers are the main mathematical audit target. The OTS projection must remain explicit: the preimage-count density is false on a record retaining the starting secret. Lean now connects the adaptive terminal-word kernel, actual support invariants, accumulated forecast charges, and finite numerical moments to the original-game stopped certificate inequalities. The exception bounds, coverage extension to deferred-secret kernels, and both security transfers remain to be completed; finite examples do not replace those proofs.
