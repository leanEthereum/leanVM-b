...
The recommendation from this review is to keep this two-range route and finish its original-experiment arguments on paper. The expanded [stopped coverage proof](cached-target-forecast.md) now derives the local shape transitions, fresh-target charges, and complete certificate inequality from the message kernel, and identifies its instantiation in the original signer. Its deferred-secret application still requires the separate exact graph simulation and inherited path budget. The committed Lean branch connects certificate accumulation and its pathwise and expected payments to the original whole game; the complete numerical coverage result is not yet formalized. Existing untracked terminal-word drafts are possible ingredients, not an established coverage or security endpoint. The expanded [canonical-graph argument](paid-probe-quadratic.md) now gives the exact sampling order, serialized-domain audit, hidden-label induction, exhaustive SUF decomposition, and large-budget assembly. The next paper deliverable is the small-budget witness transfer. None of these new paper endpoints is a public 127-bit Lean theorem.
...
The expanded canonical-graph note derives this correspondence for the large branch, including the control-flow and signature-field checks below. Its argument is an induction through the original execution, beyond the local sampling identities. The small branch still needs a transfer for useful witnesses after guesses have occurred.
...
The next paper work should transfer the allocated OTS density argument through the original adaptive graph and complete the FTS first-guess comparison. The large-budget graph, posterior, SUF decomposition, and differently stopped count comparison are now written. The small branch must permit guesses and completed witnesses after them, so its transfer is not supplied merely by the large branch's induction up to the first match. For coverage formalization, use the single potential V and the concrete kernel derivation, including a fresh bridge for every forced-secret law. These milestones measure completed security arguments, rather than the number of supporting modules. A useful stopping criterion for further infrastructure is that its next endpoint must discharge one of these concrete obligations for the original game.
...
The arithmetic does not call for another optimization pass: at x=2^-15 the large-budget margin after delta is exactly 2^-17 per x, which exceeds the exceptional allowance. The small-budget coefficients leave a fixed gap below two. The remaining mathematical uncertainty is concentrated in the small-budget adaptive witness transfers, especially the allocated multi-chain OTS likelihood bound. The full 127-bit paper proof is not yet established; finishing that transfer is the next condition for resuming Lean work.
