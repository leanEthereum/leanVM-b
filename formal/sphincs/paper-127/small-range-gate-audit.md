...
The remaining coverage proof must establish uniform fresh message replies, the original finite randomizer loop, selected-view and failure rules, and the appropriate signing-cap conditions in each forced law. The original work debit is proved. The current environment contains deferred secrets, so the existing full-game coverage theorem still needs to be instantiated from these local kernels.
