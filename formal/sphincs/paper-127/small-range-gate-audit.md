...
The remaining coverage interface should quantify over the concrete projected message and signing kernels with this record-and-length monitor. Each forced-FTS instance must discharge its local hypotheses. The original native full-certificate classification, primitive potential application and exceptional-history bounds now establish the large-budget interval in [Security127LargeBudget.lean](../SphincsSecurity/Proof/Security127LargeBudget.lean). The OTS witness comparison and forced-FTS coverage are still required for the small-budget interval before the full public 127-bit claim can be stated as proved.
