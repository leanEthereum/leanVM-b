...
These checks support the paper calculation under the specified causal projection. [OtsPrefixSimulation.lean](../SphincsSecurity/Proof/OtsPrefixSimulation.lean) supplies the query interpreter and its exact fixed-oracle game reconstruction, including the original hash-cost trace. It also proves that replacing private prefix entries in the auxiliary oracle does not change the program. [OtsPrefixReferenceSeed.lean](../SphincsSecurity/Proof/OtsPrefixReferenceSeed.lean) gives the exact original oracle law from independent low prefix tables and auxiliary seeds, including a fixed-domain seed for unused encoding rows. Proving that auxiliary oracle and frontier reconstruction depends on the chosen secret and prefix tables only through the endpoint is still necessary. The additional monitoring observations, ideal execution cap, conditional restart and completed-witness estimates also remain open.
