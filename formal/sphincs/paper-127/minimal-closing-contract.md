# Mathematical contract for the 127-bit proof

This note specifies sufficient probability comparisons for the unchanged original game. See [current status](critical-path-review.md) for which concrete comparisons are proved.
...
A union over test positions covers every execution with a true guess, even when an earlier query already guessed a different coordinate. [SecretGuessForced.lean](../SphincsSecurity/Proof/SecretGuessForced.lean) and [FtsGuessForcedBudget.lean](../SphincsSecurity/Proof/FtsGuessForcedBudget.lean) prove support containment and the original budget for the forced law. The selected-hit likelihood and weighted payoff bound are proved in [SecretGuessForceLikelihood.lean](../SphincsSecurity/Proof/SecretGuessForceLikelihood.lean) and [SecretGuessForceBound.lean](../SphincsSecurity/Proof/SecretGuessForceBound.lean). [FtsGuessNearSource.lean](../SphincsSecurity/Proof/FtsGuessNearSource.lean) now connects the retained event to the sum over positions and averages the original sampling. Its `forcedNearGame` checks a final valid-log near certificate. Prove that each forced law's fresh message kernel, finite signing loop and selected-view rules meet the coverage hypotheses, attach that law's own proposal bridge, and compare this final event with the banked count including stopped-monitor cases. The target probability \(557q/N\) in each \(R_j\) then gives \(557x^2/(1-x)\). The two-guess term is already bounded by \(x^2/(2(1-x)^2)\). The original-law near estimate alone cannot justify this multiplication. Exception terms arising in the forced laws retain their likelihood factor when averaged.
...
The forced law now has an exact adaptive program and deferred residual-table presentation in [FtsGuessAuxiliaryProgram.lean](../SphincsSecurity/Proof/FtsGuessAuxiliaryProgram.lean), [FtsGuessDeferredSeed.lean](../SphincsSecurity/Proof/FtsGuessDeferredSeed.lean) and [FtsGuessNearDeferred.lean](../SphincsSecurity/Proof/FtsGuessNearDeferred.lean). This preserves the original finite signer and whole-experiment work budget. [FtsGuessDeferredMessage.lean](../SphincsSecurity/Proof/FtsGuessDeferredMessage.lean) establishes uniform fresh message replies in that presentation. Quantitative selected-view and cached-input estimates, proposal augmentation and the final-certificate comparison remain necessary.
...
## A conditional OTS restart with an explicit shared charge
