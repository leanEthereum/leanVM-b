...
The [small-range gate audit](small-range-gate-audit.md) narrows the required projection: retain signing records and independent proposal block lengths, then use the existing adaptive projection to erase rejected proposal values. It also gives a finite counterexample to retaining secret-dependent canonical high halves in the OTS endpoint density. The OTS simulator should instead keep the independent high-output function and only observe it at externally queried inputs. These findings remove an unnecessary joint posterior obligation while leaving the concrete coverage and witness comparisons to be proved.

The original cached-index exception uses full cache size and cannot be included in the projected stopping rule. The certificate monitor's readiness test uses actual spent calls instead. A failure of that test implies the original exception under the cache-size resource invariant. Keep the stronger flag only for the final original-game exceptional probability allowance, so private cache occupancy never enters the OTS comparison history.
...
The byte router and cached external-query correspondence are now proved in [CanonicalProbeExecution.lean](../SphincsSecurity/Proof/CanonicalProbeExecution.lean) and its imports. The response kernel programs each canonical input with its graph reply and gives other fresh structural inputs a uniform full reply. Its direct first-match stop equals the public router's probe stop, including for malformed payloads. This equality composes through randomized adaptive external computations with fixed reference and disclosure data, preserves the cache and counters on stops, and counts repeated and stopping calls. The cache invariant remains valid when FTS disclosures grow. [ReferenceResidualSeeds.lean](../SphincsSecurity/Proof/ReferenceResidualSeeds.lean) now proves the exact original-game sampling presentation with an independent graph and residual seed, retaining the shared table after reference conditioning. The reference family, conditional encoding rows and seed are sampled before the graph. [PublicReferenceResidual.lean](../SphincsSecurity/Proof/PublicReferenceResidual.lean) proves that the encoding blocks use only exposed graph values and leave every structural seed row unchanged. The fresh noncanonical query law retains the sampled answer and updated seed for arbitrary continuations. Composing this completion law through the actual cached external and private signing execution, with the native hidden-label frontend, original costs and full message/coverage state, is still required.

[PublicSigningNative.lean](../SphincsSecurity/Proof/PublicSigningNative.lean) implements the private-signing part of this correspondence. Exposed graph values supply every OTS frontier, FTS path and hypertree path. A plan uses the sampled first-success results to decide success and compute the original costs, including all three layers. Its successful completion requests only the selected FTS coordinates through the native disclosure interface. Failed plans issue no disclosures and keep the selected FTS view. Under the conditioned shared oracle, the completed record agrees with the original `signWithView` record and boundary trace. Given a product prior on nonempty candidate sets, the component has the native posterior and lazy-erasure identities.

[ResidualSigningProgram.lean](../SphincsSecurity/Proof/ResidualSigningProgram.lean) now gives this signer a causal program in the joint hidden-label/residual-row interface. [CausalPublicSigning.lean](../SphincsSecurity/Proof/CausalPublicSigning.lean) runs the finite message-digest loop with ordinary shared-table queries and retains the original non-message work as a charged cost. [MessageByteTrace.lean](../SphincsSecurity/Proof/MessageByteTrace.lean) proves the exact message-cache and trace transition, including repeats, and shows message inputs trigger neither structural nor encoding stops. [ResidualSigningDisclosure.lean](../SphincsSecurity/Proof/ResidualSigningDisclosure.lean) charges the remaining work and discloses selected FTS coordinates on successful plans. The combined program projects to the original signWithView result and full boundary trace from every compatible fixed-table state, retains the selected view on failure, and uses an input envelope implied by the original signer. Its joint posterior and erasure include the private message reads and subsequent secret disclosures. The retained interleaved program now updates the public disclosure information used by later byte routing and retains the complete signing log. Proving its original-game correspondence with accumulated budget and coverage statistics remains required. The component theorem alone does not close either original-game security interval.

[ReferenceCoordinateGame.lean](../SphincsSecurity/Proof/ReferenceCoordinateGame.lean) now establishes this initial prior from the original secret and graph samplers. Its sampling bijection retains every secret and both halves of each full graph reply. Sampling the public restriction first leaves independent uniform hidden coordinates, with singleton candidate sets on the public coordinates. The resulting game preserves the original SUF winning probability and whole-experiment hash bound. [PublicSigningInitial.lean](../SphincsSecurity/Proof/PublicSigningInitial.lean) proves the public-root agreement and integrates the original signing record over this concrete prior to obtain the lazy signing record. Composing residual-table completion through cached external queries, private message attempts, budget stops and retained coverage memory remains open.

The [initial-prior and composition note](initial-prior-and-composition.md) gives the finite sampling derivation underlying these initial-prior theorems. It also specifies the next joint execution invariant: the full auxiliary seed may be retained in the ambient sampler, but future residual rows must remain unobserved in the history used for fresh-query bounds. This initialization argument does not replace the remaining adaptive composition or establish an original-game security interval.

[AdaptiveResidualLabels.lean](../SphincsSecurity/Proof/AdaptiveResidualLabels.lean) now proves the joint hidden-label and residual-row posterior through an adaptive interface. A fresh probe uses the sampled residual cell, records its full reply on survival and imposes the existing label restrictions. Cached probes reuse their recorded answer; ordinary reads and secret disclosures use their conditional completion kernels. Auxiliary transitions may stop based on the retained state. [AdaptiveResidualErasure.lean](../SphincsSecurity/Proof/AdaptiveResidualErasure.lean) preserves the stopped result and visible memory after erasing both latent tables and proves total probability is retained. [ReferenceJointPrior.lean](../SphincsSecurity/Proof/ReferenceJointPrior.lean) rewrites the original reference game with its encoding auxiliary outside this joint prior, preserving the original SUF probability and hash bound. The concrete frontend must use probes for the relevant structural queries, route public canonical and conditioned encoding replies correctly, and translate private message attempts and signing completion while preserving original costs and coverage memory. The generic interface permits ordinary reads without a structural test, so its posterior theorem cannot substitute for that correspondence.

[ResidualBytePrior.lean](../SphincsSecurity/Proof/ResidualBytePrior.lean) now discharges the external byte-query part of this connection, for fixed reference and disclosure data. Its imported router distinguishes public canonical answers, conditioned encoding lookups and residual reads or structural probes. [ResidualByteRun.lean](../SphincsSecurity/Proof/ResidualByteRun.lean) proves the original fixed-table correspondence through arbitrary randomized adaptive external computations covered by the finite envelope. The initial-prior erasure and posterior retain the stopped result, visible cache and counters; the posterior also retains both latent tables on survival. A syntactic source hash bound transfers to the projected lazy execution, including repeated and stopping calls. Private message attempts, interleaved signing/disclosures, coverage records and the nonreference equal-code encoding stop remain to be connected. In particular, the current structural posterior may condition on the entire encoding auxiliary. A fresh encoding-match estimate must instead hide its unqueried rows or derive the corresponding conditional completion law; those rows are not unrestricted uniforms merely because they are routed outside the structural graph.
...
[ReferencePrefixSigning.lean](../SphincsSecurity/Proof/ReferencePrefixSigning.lean) also proves that prefix agreement preserves the canonical first-success selection and the original public signing record, including the failure cases and trace. Every protected row except the selected row decodes invalidly; the selected row is the excluded canonical encoding input. Unrestricted tails and noncanonical-message encodings can therefore use the shared residual completion law for their fresh replies. The prefix byte interpreter now uses these lookups and the nonreference equal-code stopping rule.

[PrefixBytePrior.lean](../SphincsSecurity/Proof/PrefixBytePrior.lean) proves the initial-prior erasure and joint posterior for this stopped external execution, preserving its visible cache and original hash count. [PublicEncodingMatch.lean](../SphincsSecurity/Proof/PublicEncodingMatch.lean) defines the nonreference match from exposed layer messages, proves protected prefix rows cannot match, and bounds the uniform reply event by $1/N$ using the unique encoding tweak. [PrefixEncodingRisk.lean](../SphincsSecurity/Proof/PrefixEncodingRisk.lean) transports that estimate to every next query after a supported surviving external computation. Its cache conditions follow from the execution: residual rows remain recorded in the external cache, and surviving executions have no earlier encoding match. The sampled encoding auxiliary supplies the first-success consistency condition. On an encoding stop, the last reply remains recorded in analytical memory and its hash call stays charged. The remaining original-game composition must include private message attempts, interleaved signing/disclosures and coverage records before the joint primitive potential can close the large interval. Neither interval is proved by this local encoding estimate.
...
The component correspondences now retain full native memory in [RetainedResidualSigningTrace.lean](../SphincsSecurity/Proof/RetainedResidualSigningTrace.lean). [RetainedResidualByteRun.lean](../SphincsSecurity/Proof/RetainedResidualByteRun.lean) proves the external stopped correspondence through arbitrary covered computations, with the message observation list retained rather than reconstructed from distinct cache entries. [RetainedResidualMessageTrace.lean](../SphincsSecurity/Proof/RetainedResidualMessageTrace.lean) identifies the full update for message-only computations with the original boundary trace. Charging the remaining work and completing native disclosures then gives the exact original signing record and memory update, including cached retries, failure, signing-log insertion and disclosure updates. These component equalities require compatible initial states. The source composition and its expanded-source budget transfer are now proved as described below; instantiation from the original whole experiment remains required.

The retained interleaving implementation is now in [RetainedResidualProgram.lean](../SphincsSecurity/Proof/RetainedResidualProgram.lean). Its routing query reads only retained public information, and each completed signing invocation records the original signature option and selected view before continuing. The complete native program has joint posterior and erasure laws. [RetainedResidualCorrespondence.lean](../SphincsSecurity/Proof/RetainedResidualCorrespondence.lean) projects external computations and signing invocations to the established original fixed-oracle components from compatible states. [RetainedResidualHistory.lean](../SphincsSecurity/Proof/RetainedResidualHistory.lean) retains the signing log, records and routing through component stops and proves their exact update on completed signing. The native program retains the strong-novelty log through monitoring stops. Instantiating its original-game correspondence still requires the initial sampling and the final verification step.

[RetainedResidualContext.lean](../SphincsSecurity/Proof/RetainedResidualContext.lean) proves that live external replies and completed original signing records preserve public agreement, cache consistency, structural cleanliness and encoding cleanliness. Its imported row and trace invariants retain recorded residual rows on stops and validate every original message-cache update. [RetainedResidualComposition.lean](../SphincsSecurity/Proof/RetainedResidualComposition.lean) composes the full-memory correspondence through arbitrary covered source computations, including interleaved signing, and preserves compatibility on live outcomes. [RetainedResidualSource.lean](../SphincsSecurity/Proof/RetainedResidualSource.lean) defines a finite input envelope over all source-response branches. That envelope is analytical data, not a security-budget assumption. [RetainedResidualBudget.lean](../SphincsSecurity/Proof/RetainedResidualBudget.lean) bounds the native hash counter by the original expanded source computation's syntactic budget. The proof deducts each supported original signing trace's actual cost before bounding its continuation and charges repeated and stopping hash calls. These theorems do not yet derive the complete native game's bound directly from `HasHashQueryBound`: the remaining instantiation must choose a sufficient sampling envelope, establish the initial compatible state, include final verification, and account for key-generation work. Coverage monitors and the probability inequalities still remain open.
