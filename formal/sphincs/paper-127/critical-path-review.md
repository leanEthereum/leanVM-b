...
The local signing extension is now established in `DigestCompletionMonitor.lean`. It runs the unchanged digest loop followed by an arbitrary probabilistic continuation with an arbitrary result type. Each supported successful result must use the selected randomness and view, and the continuation must preserve message-domain answers. These support conditions imply the original signing-log and mixed-moment bounds, the new-target creation bound and the complete local monitor potential bound, including recorded hash cost and arbitrary later stops. A fresh digest adds exactly one admissible cache entry; excluding that entry's own input makes its target moments equal their values before signing. The original signer's mixed-moment and new-target inequalities now instantiate the general results. This does not establish the required conditional digest-loop law after a joint history of a deferred-secret execution. Prove that law and the continuation's support conditions in the actual retained and forced interpreters before composing the local bounds through their adaptive runs.
