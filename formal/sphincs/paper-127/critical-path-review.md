...
The adaptive composition of the local inequalities is established in `RetainedResidualMonitoredCoverage.lean`. A monitor attached to the retained interpreter samples proposal-block lengths independently and uses a uniform fallback index only when no digest was selected. `RetainedResidualMonitoredErasure.lean` proves its exact erasure to the existing native execution and transfers the candidate and residual-row invariants through every supported step. Finite induction bounds the expected terminal potential, and hence the retained certificate count, by the initial potential plus accumulated creation charges. The initial potential vanishes when the message cache is empty. This leaves the quantitative payment step: bound those charges by the common stopped message count and the original hash budget, using the proposal and resource correspondences. It does not yet establish the unit-payment coverage inequality or either original-game security interval.
