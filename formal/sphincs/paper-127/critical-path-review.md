...
The concrete retained interpreter's message kernel and digest-loop law are now established in `RetainedResidualMessageKernel.lean` and `RetainedResidualDigestLaw.lean`. The equality retains the loop's output, cache and full boundary trace. `RetainedResidualMessageHistory.lean` transfers the existing observed row invariant by joint erasure, so the equality applies after every supported history from nonempty candidate sets and a covered residual cache. `RetainedResidualSigningKernel.lean` gives the actual program's digest/completion split and proves the completion's message-preservation and selection-consistency support conditions. These are local kernel results, not an adaptive certificate inequality. The next coverage step must combine these laws with the monitor expectation, original recorded costs and accumulated message payment on the retained execution. Its forced-FTS counterpart still needs its own kernel justification. The original-game event transfer, joint primitive potential and forgery-classification connection remain required to close the large interval.
