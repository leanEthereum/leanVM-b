...
1. Bound the native monitor exception. Set `stopAfter := fun _ _ _ _ => false` in `forgeAdvantage_le_monitored_bound_add_exception` and prove $\Pr[E_{\rm native}]\le\epsilon(q)$. A strong winning run must not stop for bookkeeping reasons: prove that its cached signing digests, signing cap, spent-call count and remaining macro budget satisfy the monitor's conditions. Account for the remaining cache and proposal-prefix exceptions on their actual laws, without charging the primitive stop a second time. The certificate-bank connection and the original-game probability transfer are now proved.
