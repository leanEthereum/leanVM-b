...
The joint exception game is now formalized in [CertificateJointExceptions.lean](../SphincsSecurity/Proof/CertificateJointExceptions.lean). Its cache flag persists through arbitrary supported continuations, its erased law is exactly the existing certificate game, and its cache-or-prefix event has probability at most q/2^223 + q/2^170 + 2^-704 under the original whole-experiment budget. The same augmented execution inherits the full-coverage expectation bound. A conditional readiness lemma identifies the remaining interface: once digest-log consistency, cache cardinality and spent-budget bounds are supplied, readiness failure implies a recorded cache exception. Establishing those conditions at every relevant reachable boundary and completing the cryptographic reductions remain necessary.
