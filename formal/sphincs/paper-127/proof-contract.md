...
The coverage and exception components now share one original execution in [CertificateFamilyGame.lean](../SphincsSecurity/Proof/CertificateFamilyGame.lean). Each of its full and 14 near banks projects exactly to the corresponding certificate game, including the proposal word, cache and exception flag. Its full-count expectation is at most E[messageCalls]/2^128 + 11q/2^144, and its summed near-count expectation is at most 557q/2^128. This implements the later [unit-payment plan](unit-payment-plan.md), which replaces the three-halves baseline below. The common cache-or-prefix event costs at most q/2^223 + q/2^170 + 2^-704 once. With only the proposal-prefix stop enabled, every valid original run outside that event stays live and retains the complete original signing log; a terminal certificate on a clean retained run contributes at least one to its bank. The construction permits a shared stop depending on the cache and common counters, without reading the differing bank contents or coverage prices. Instantiating that stop with the concrete primitive event, proving its hidden-label hazard, extending coverage to forced-secret laws, and completing the cryptographic reductions remain necessary; 127-bit SUF is still unproved.
