...
The reference words and frontier values are constructed from the original hash answers. The randomized frontier signer preserves responses, failures, selected FTS views and original costs. Its adaptive game retains the complete signing log and original strong-novelty test, reconstructs the public root, and runs the original verifier. The finite canonical graph now has an exact joint sampling correspondence with the completed oracle table: independent full hash outputs are sampled at distinct serialized position domains and their canonical rows are programmed into the table. The graph labels have the uniform function-table distribution for every fixed secret assignment. At positions within the tree bounds, they equal the original honest hash values. The resulting game reads its root and OTS frontiers from those labels, has exactly the original SUF winning probability, and inherits the original whole-experiment hash bound. The conditional reference-encoding presentation, hidden-label posterior and adaptive likelihood bounds remain separate obligations.
-/
