...
The reference words and frontier values are constructed from the original hash answers. The randomized frontier signer preserves responses, failures, selected FTS views and original costs. Its adaptive game retains the complete signing log and original strong-novelty test, reconstructs the public root, and runs the original verifier. The finite canonical graph has an exact joint sampling correspondence with the completed oracle table. Its labels have the uniform function-table distribution for every fixed secret assignment and agree with honest hash values within the tree bounds. The resulting game reads its root and OTS frontiers from those labels, preserves the original SUF winning probability and inherits the original whole-experiment hash bound. Its finite input envelope also contains every reference encoding row. The family of canonical counter tables is uniform independently of the graph labels; the original reference searches have the induced joint distribution, including failures and exact costs. For each fixed reference address, first-success conditioning reconstructs the entire oracle table. Its selected reference is independent of all non-encoding rows, including accessible OTS prefix functions, and agrees with the original encoding search and its cost. Given supplied endpoints and frontier values, the causal game reconstructs reference words with every chain row masked, and signs with rows below its reference digits masked. External queries and the original verifier use the complete oracle. This preserves the original SUF law, complete signing log, failures, message trace and whole-experiment hash bound. The projected endpoint density after forgetting the starting secret, its adaptive partial-table likelihood, the hidden-label posterior and the remaining probability bounds still need to be proved.
-/
