...
attribute [local irreducible] canonicalGraphInputs canonicalEncodingInputs instFintypePosition frontierRoot chainWalk
...
theorem tree_match (key : SecretKey) (f : QueryImpl HashSpec Id) (words : OtsReferenceWords) (lay : Layer) (tree : TreeIndex) (trace : Trace)
  ...
  obtain ⟨level, index, payload, hl, hi, hb, hrow, hhit⟩ := h
  let position : Position := .node lay tree ⟨level, hl.trans_le (layerHeight_le lay)⟩ ⟨index, hi⟩
  refine ⟨position, trivial, hb, payload, hrow, hhit.1, ?_⟩
...
theorem leaf_match (key : SecretKey) (f : QueryImpl HashSpec Id) (words : OtsReferenceWords)
  ...
  obtain ⟨payload, hne, hrow, hvalue⟩ := h
  refine ⟨.leaf lay tree leaf, trivial, trivial, payload, hrow, hne, ?_⟩
...
theorem forward_match (key : SecretKey) (f : QueryImpl HashSpec Id) (words : OtsReferenceWords)
  ...
  refine ⟨.chain lay tree leaf chain step, habove, trivial, digestBytes payload, hrow, ?_, ?_⟩
