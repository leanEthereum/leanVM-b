...
import SphincsSecurity.Proof.FewTimeSharedTargetBound
...
theorem probEvent_otherViewedTerminal_le_fts_allowance_sharedTargets127
...
theorem forgeAdvantage_le_answerEncoding_guess_allowances_sharedTargets127
    (adversary : Adversary) (q : Nat) (hqPos : 1 ≤ q)
  ...
  apply (add_le_add le_rfl (add_le_add le_rfl (add_le_add le_rfl
    (probEvent_otherViewedTerminal_le_fts_allowance_sharedTargets127 adversary q hqPos hq hqMax _ hfts)))).trans_eq
  rw [sampledQueryCharge_add]
  simp only [show Fintype.card Digest = 2 ^ digestBits by simp]
  ring

theorem forgeAdvantage_le_min_unconditional_guesses_sharedTargets127
    (adversary : Adversary) (q : Nat) (hqPos : 1 ≤ q)
  ...
  have hspace : q + 1 < Fintype.card Digest := by
    ...
    omega
  apply (forgeAdvantage_le_answerEncoding_guess_allowances_sharedTargets127 adversary q hqPos hq hqMax _ _
