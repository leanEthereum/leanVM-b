...
theorem probEvent_retained_liveCover_pairs_refund_of_step_le
    ...
    (hrefund : CoverageStepRefundBound exception parameter root otsTable ftsTable q refundStep)
    ...
    Pr[fun result => result.1.2.2 = false ∧ result.2 = false ∧
      ...
      expectedCoverageRefund exception parameter root otsTable ftsTable q refundStep
  ...
  rw [runWithFailure_retainedComputation_trace exception adversary parameter root otsTable ftsTable q htrace, probEvent_map,
    ...
    expectedBeforeFailureSigningCharge_retainedComputation exception _ adversary parameter root otsTable ftsTable q htrace]
  apply le_trans ?_ (probEvent_runWithFailure_liveCover_pairs_refund_of_step_le exception parameter root otsTable ftsTable q refundStep hrefund
...
theorem probEvent_retained_liveCover_pairs_refund_le_reserved_add_residual
    ...
    Pr[fun result => result.1.2.2 = false ∧ result.2 = false ∧
      ...
      (q : ENNReal) * initialRawIndexRate q +
        ...
        expectedPaidCoverageResidual exception parameter root otsTable ftsTable q
          (unloggedRetainedRestComputation adversary ⟨root, parameter⟩) q frame (cache, []) hit failed := by
  exact probEvent_retained_liveCover_pairs_refund_of_step_le exception adversary parameter root otsTable ftsTable q
    (paidRemainingCoverageStepRefund exception parameter root otsTable ftsTable q)
    (fun budget input frame state hit failed hsigned hcache hcost hhead =>
      stepWithFailure_coverage_pairs_refund_le_reserved_add_residual exception parameter root otsTable ftsTable
        q budget hq input frame state hit failed hsigned hcache hcost hhead)
    htrace hbound frame cache hit failed hnone hcache

abbrev CoverageRefundFamily :=
  PublicParameter → Digest → (OtsSecretIndex → HashOutput) → (Coordinate → Digest) → Nat → Nat →
    (OracleWorld + SigningSpec).Domain → Option Frame → CoverLogState → Bool → Bool → ENNReal

def CoverageRefundFamilyBound (refund : CoverageRefundFamily) (cap : Nat) : Prop :=
  ∀ parameter root otsTable ftsTable,
    CoverageStepRefundBound (parentException parameter otsTable ftsTable) parameter root otsTable ftsTable cap
      (refund parameter root otsTable ftsTable cap)

noncomputable def initializedCoverageRefund (refund : CoverageRefundFamily)
  ...
  ∑' initial, Pr[= initial | initializeRoot parameter otsTable ftsTable q fuel] *
    expectedCoverageRefund (parentException parameter otsTable ftsTable) parameter initial.2.1 otsTable ftsTable q
      (refund parameter initial.2.1 otsTable ftsTable q)
...
theorem probEvent_liveNonSecretResidual_pairs_refund_of_family_le
    ...
    Pr[LiveNonSecretResidual parameter otsTable ftsTable |
      ...
      initializedCoverageRefund refund adversary parameter otsTable ftsTable q fuel +
  ...
  unfold initializedCoverageRefund initializedPaidCoverageResidual initializedBeforeFailureSigningCharge
  ...
  calc
    _ ≤ ∑' initial, Pr[= initial | initializeRoot parameter otsTable ftsTable q fuel] *
      ...
      · let key := secretKey parameter initial.2.1 otsTable ftsTable
        ...
        exact mul_le_mul' le_rfl (probEvent_retained_liveCover_pairs_refund_of_step_le (parentException parameter otsTable ftsTable)
          adversary parameter initial.2.1 otsTable ftsTable q
          (refund parameter initial.2.1 otsTable ftsTable q) (hrefund parameter initial.2.1 otsTable ftsTable) htrace hbound initial.1 initial.2.2 false initial.1.isNone hconditions.1 hconditions.2)
    ...
    Pr[LiveNonSecretResidual parameter otsTable ftsTable |
      ...
      (q : ENNReal) * initialRawIndexRate q +
        initializedBeforeFailureSigningCharge (nonMessageNonEncodingHashCharge parameter)
          adversary parameter otsTable ftsTable q fuel * (Fintype.card Digest : ENNReal)⁻¹ +
        initializedPaidCoverageResidual adversary parameter otsTable ftsTable q fuel := by
  exact probEvent_liveNonSecretResidual_pairs_refund_of_family_le adversary q hq
    ...
    parameter hp otsTable ftsTable hfts fuel
