...
theorem expectedPreExceptionCharge_encode_le_preHashQueries
...
theorem expectedPreExceptionCharge_otsSignFrom_le_preHashQueries
    (secretKey : SecretKey) (lay : Layer) (tree : TreeIndex) (leafIdx : LeafIndex)
  ...
  | succ attempts ih =>
      ...
      apply expectedPreExceptionCharge_bind_le_bind _ _ _ _ _ _ _
        (expectedPreExceptionCharge_encode_le_preHashQueries exception secretKey lay tree leafIdx message _ cache hit hsettled)
      intro result hr
...
theorem expectedPreExceptionCharge_signLayer_le_preHashQueries
    (secretKey : SecretKey) (index : Index) (lay : Layer) (cache : QueryCache HashSpec) (hit : Bool) :
    expectedPreExceptionCharge exception (signingStructuralCharge secretKey)
  ...
  · intro result hr
    ...
    rw [liftM_bind]
    apply expectedPreExceptionCharge_bind_le_bind _ _ _ _ _ _ _
      (expectedPreExceptionCharge_otsSignFrom_le_preHashQueries exception secretKey lay _ _ result.1.1 _ _ result.1.2 result.2 hsettled)
    intro signed _
...
theorem expectedPreExceptionCharge_sign_le_preHashQueries
    (secretKey : SecretKey) (message : Message) (cache : QueryCache HashSpec) (hit : Bool) :
    expectedPreExceptionCharge exception (signingStructuralCharge secretKey) (sign secretKey message) cache hit ≤
      expectedPreExceptionCharge exception (fun _ _ => 1) (sign secretKey message) cache hit := by
  rw [sign]
  apply expectedPreExceptionCharge_bind_le_bind
  · rw [expectedPreExceptionCharge_eq_zero_of_queryCharge_eq_zero exception _ _ _ _
    ...
    | some selectedDigest =>
        ...
        · intro opening _
          ...
          · apply expectedPreExceptionCharge_lift_sequenceFin_le
            ...
            exact expectedPreExceptionCharge_signLayer_le_preHashQueries exception secretKey selectedDigest.2.1 lay currentCache currentHit
          · intro layers _
            split <;> simp
