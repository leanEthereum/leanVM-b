...
theorem forgeAdvantage_add_terminal_message_and_signingReserves_le_live
  ...
  have hstructural := (sampledBeforeFailureStructural_add_signingNonEncoding_le adversary q (q + 1)).trans
  ...
  have hbound : forgeAdvantage scheme adversary + sampledStructuralTerminalReserve adversary q (q + 1) +
      (sampledSigningNonEncodingReserve adversary q (q + 1) + sampledJointNonSecretQueryCharge adversary q) * (Fintype.card Digest : ENNReal)⁻¹ ≤
    ...
    calc
      _ = (sampledBeforeFailureStructuralCharge adversary q (q + 1) + sampledSigningNonEncodingReserve adversary q (q + 1)) *
            (Fintype.card Digest : ENNReal)⁻¹ +
          (sampledParentSharedFailureRisk adversary q (q + 1) + sampledJointNonSecretQueryCharge adversary q * (Fintype.card Digest : ENNReal)⁻¹) +
          sampledLiveNonSecretResidual adversary q (q + 1) := by rw [add_mul, add_mul]; ac_rfl
...
theorem forgeAdvantage_add_terminal_message_signing_executionReserves_remainder_gap_le_initialEnvelope
    (adversary : Adversary) (q : Nat) (hq : HasHashQueryBound scheme adversary q) (hqMax : q ≤ 2 ^ 127) :
    forgeAdvantage scheme adversary + sampledStructuralTerminalReserve adversary q (q + 1) +
      (sampledSelectedJointQueryCharge MessageHashInput adversary q + sampledBeforeFailureHashCharge messageHashCharge adversary q (q + 1) +
        sampledSigningNonEncodingReserve adversary q (q + 1)) * (Fintype.card Digest : ENNReal)⁻¹ +
  ...
  have hbase := (forgeAdvantage_add_terminal_message_and_signingReserves_le_live adversary q hq hqMax).trans
