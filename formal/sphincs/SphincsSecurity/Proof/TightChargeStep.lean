...
attribute [local irreducible] tightPotential
...
private theorem growth_empty (n : Nat) :
    (n + 2) + (∅ : Finset Digest).card ≤ n + 2 := by
  simp

private theorem empty_growth (n : Nat) : n + (∅ : Finset Digest).card ≤ n + 2 := by
  norm_num

private theorem singleton_card_le (n : Nat) (target : Digest) :
    ({target} : Finset Digest).card ≤ n + 2 := by
  simp only [Finset.card_singleton]
  omega

private theorem singleton_growth (n : Nat) (target : Digest) :
    n + ({target} : Finset Digest).card ≤ n + 2 := by
  simp only [Finset.card_singleton]
  omega

/-- The target set and both obligations needed by `Amortized.probEvent_bad_le_amortized`. -/
inductive TightStep (cache : QueryCache HashSpec) (input₀ : HashInput) : Prop where
  | intro (targets : Finset Digest)
      (card_le : targets.card ≤ tightPotential parameter otsSecret ftsSecret cache + 2)
      (safe : ∀ answer : HashOutput, truncateHash answer ∉ targets →
        ¬ Bad parameter otsSecret ftsSecret (cache.cacheQuery input₀ answer)
          ∧ tightPotential parameter otsSecret ftsSecret (cache.cacheQuery input₀ answer) + targets.card
            ≤ tightPotential parameter otsSecret ftsSecret cache + 2) : TightStep cache input₀
...
private theorem step_of_unsettled_after {cache : QueryCache HashSpec} {input₀ : HashInput}
    ...
    TightStep parameter otsSecret ftsSecret cache input₀ := by
  refine TightStep.intro (parameter := parameter) (otsSecret := otsSecret) (ftsSecret := ftsSecret)
  ...
  have htightPotential := tightPotential_cacheQuery_le_of_unsettled parameter otsSecret ftsSecret
    (cache := cache) (input := input₀) (answer := answer) (queried := p₀)
    huncached hposition hunsettled'
  exact (Nat.add_le_add htightPotential (Nat.le_refl _)).trans
    (growth_empty (tightPotential parameter otsSecret ftsSecret cache))
...
private theorem step_of_settling {cache : QueryCache HashSpec} (hfinite : Finite cache)
    ...
    (hsettles : ∀ answer : HashOutput,
      Settled parameter otsSecret ftsSecret (cache.cacheQuery input₀ answer) p₀) :
    TightStep parameter otsSecret ftsSecret cache input₀ := by
  have hpaid := tightPotential_add_settlingTargets_card_le parameter otsSecret ftsSecret
    (cache := cache) hfinite (input₀ := input₀) (answer := 0) (p₀ := p₀)
    huncached hposition hsettled (hsettles 0)
  have hcardBase : (tightSettlingTargets parameter otsSecret ftsSecret cache hfinite p₀).card
      ≤ tightPotential parameter otsSecret ftsSecret cache :=
    (Nat.le_add_left (tightSettlingTargets parameter otsSecret ftsSecret cache hfinite p₀).card
      (tightPotential parameter otsSecret ftsSecret (cache.cacheQuery input₀ 0))).trans hpaid
  have hcard := hcardBase.trans
    (Nat.le_add_right (tightPotential parameter otsSecret ftsSecret cache) 2)
  refine TightStep.intro (parameter := parameter) (otsSecret := otsSecret) (ftsSecret := ftsSecret)
    (cache := cache) (input₀ := input₀) (tightSettlingTargets parameter otsSecret ftsSecret cache hfinite p₀) hcard
    (fun answer hanswer => ?_)
  have hsettled' := hsettles answer
  refine ⟨clean_cacheQuery_of_settling_of_avoids_tight parameter otsSecret ftsSecret
    hfinite hclean huncached hposition hsettled hsettled' hanswer, ?_⟩
  exact (tightPotential_add_settlingTargets_card_le parameter otsSecret ftsSecret
...
private theorem step_at_position {cache : QueryCache HashSpec} (hfinite : Finite cache)
  ...
  · let target := honestValue (fromCache cache) parameter otsSecret ftsSecret p₀
    refine TightStep.intro (parameter := parameter) (otsSecret := otsSecret) (ftsSecret := ftsSecret)
      ...
      (singleton_card_le (tightPotential parameter otsSecret ftsSecret cache) target)
      (fun answer hanswer => ?_)
    ...
    have htightPotential := tightPotential_cacheQuery_le_of_settled parameter otsSecret ftsSecret
      (cache := cache) (input₀ := input₀) (answer := answer) (p₀ := p₀)
      huncached hposition hsettled
    exact (Nat.add_le_add htightPotential (Nat.le_refl _)).trans
      (singleton_growth (tightPotential parameter otsSecret ftsSecret cache) target)
...
current tightPotential and `2` new units. -/
theorem tight_bad_step (cache : QueryCache HashSpec) (hfinite : Finite cache)
    (hclean : ¬ Bad parameter otsSecret ftsSecret cache) (input₀ : HashInput)
    (huncached : cache input₀ = none) : TightStep parameter otsSecret ftsSecret cache input₀ := by
  classical
  ...
  · refine TightStep.intro (parameter := parameter) (otsSecret := otsSecret) (ftsSecret := ftsSecret)
      (cache := cache) (input₀ := input₀) ∅ (Nat.zero_le _) (fun answer _ => ?_)
    obtain ⟨hclean', htightPotential⟩ := clean_and_tightPotential_cacheQuery_of_not_atPosition
      parameter otsSecret ftsSecret hclean huncached (fun p hp => hAt ⟨p, hp⟩)
    refine ⟨hclean', ?_⟩
    exact (Nat.add_le_add htightPotential (Nat.le_refl _)).trans
      (empty_growth (tightPotential parameter otsSecret ftsSecret cache))

/-- **The amortized bad-event bound.** A `q`-query computation makes `Bad` true with probability at
most `(2q + tightPotential) / 2^128`. -/
theorem probEvent_bad_le_tight {α : Type} (oa : OracleComp OracleWorld α) (q : Nat)
    ...
    Pr[fun result => Bad parameter otsSecret ftsSecret result.2
      ...
      ≤ ((2 * q + tightPotential parameter otsSecret ftsSecret cache : Nat) : ℝ≥0∞)
          * ((2 ^ digestBits : Nat) : ℝ≥0∞)⁻¹ := by
  apply probEvent_bad_le_amortized
    (Bad := Bad parameter otsSecret ftsSecret) (Inv := Finite)
    (potential := tightPotential parameter otsSecret ftsSecret) (c := 2)
  · intro target
    rw [← probOutput_map]
  ...
  · intro cache' hfinite' hclean' input huncached
    rcases tight_bad_step parameter otsSecret ftsSecret cache' hfinite' hclean' input huncached with
      ⟨targets, hcard, hsafe⟩
...
/-- The accounted SPHINCS run starts clean with zero tightPotential, so its structural-hit probability is
at most `2q / 2^128`. -/
theorem probEvent_bad_gameAfterSecrets_le_two_mul (adversary : Adversary) (parameter : PublicParameter)
    (otsSecret : Layer → TreeIndex → LeafIndex → ChainIndex → Digest)
