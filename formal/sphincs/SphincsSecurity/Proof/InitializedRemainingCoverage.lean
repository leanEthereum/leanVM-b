...
theorem probEvent_retained_liveObservedCover_add_remainingUnused_le_initial
    ...
    Pr[fun result => result.1.2.2 = false ∧ result.2 = false ∧
      ...
      expectedRemainingUnusedCoverageCharge exception parameter root otsTable ftsTable q ∅ Finset.univ
        (unloggedRetainedRestComputation adversary ⟨root, parameter⟩) q frame (cache, []) hit failed * ((2 ^ 140 : Nat) : ENNReal)⁻¹ ≤
  ...
  apply le_trans ?_ (probEvent_runWithFailure_liveCover_add_remainingUnused_le_initial exception parameter root otsTable ftsTable q hq
...
noncomputable def initializedRemainingUnusedCoverageCharge (adversary : Adversary) (parameter : PublicParameter)
  ...
  ∑' initial, Pr[= initial | initializeRoot parameter otsTable ftsTable q fuel] *
    (expectedRemainingUnusedCoverageCharge (parentException parameter otsTable ftsTable) parameter initial.2.1 otsTable ftsTable q
      ∅ Finset.univ (unloggedRetainedRestComputation adversary ⟨initial.2.1, parameter⟩) q
      initial.1 (initial.2.2, []) false initial.1.isNone * ((2 ^ 140 : Nat) : ENNReal)⁻¹)

theorem probEvent_liveNonSecretResidual_add_initialized_remainingUnused_le_initial
    ...
    Pr[LiveNonSecretResidual parameter otsTable ftsTable |
      ...
      initializedRemainingUnusedCoverageCharge adversary parameter otsTable ftsTable q fuel ≤
      (q : ENNReal) * initialRawIndexRate q := by
  apply (add_le_add (probEvent_mono (q := fun result => result.1.2.2 = false ∧ result.2 = false ∧
    ∃ value, result.1.2.1.1 = some value ∧ ObservedRetainedCover value (messageAnswers parameter result.1.2.1.2))
    (fun result hr hres => ⟨hres.2.1, hres.1,
      liveNonSecretResidual_observed adversary q hq parameter hp otsTable ftsTable hfts fuel result hr hres⟩)) le_rfl).trans
  rw [runRetainedWithFailure, probEvent_bind_eq_tsum, initializedRemainingUnusedCoverageCharge, ← ENNReal.tsum_add]
  calc
    _ ≤ ∑' initial, Pr[= initial | initializeRoot parameter otsTable ftsTable q fuel] * ((q : ENNReal) * initialRawIndexRate q) := by
      ...
      · let key := secretKey parameter initial.2.1 otsTable ftsTable
        ...
        exact mul_le_mul' le_rfl (probEvent_retained_liveObservedCover_add_remainingUnused_le_initial (parentException parameter otsTable ftsTable)
