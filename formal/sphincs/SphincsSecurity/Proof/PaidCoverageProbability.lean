...
theorem probEvent_runWithFailure_liveCover_pairs_refund_of_step_le
    ...
    Pr[fun result => result.1.2.2 = false ∧ result.2 = false ∧ SigningTranscript.Valid result.1.2.1.1.2 ∧
      ...
      expectedCoverageRefund exception parameter root otsTable ftsTable q refundStep computation q frame (cache, []) hit failed +
  ...
  have h := expected_runWithFailure_coverage_pairs_refund_of_step_le exception parameter root otsTable ftsTable
    q q refundStep hrefund computation frame (cache, []) hit failed hbound hsigned hcache
    ...
    Pr[fun result => result.1.2.2 = false ∧ result.2 = false ∧ SigningTranscript.Valid result.1.2.1.1.2 ∧
      ...
      (q : ENNReal) * initialRawIndexRate q +
        expectedBeforeFailureSigningCharge exception (nonMessageNonEncodingHashCharge parameter)
          parameter root otsTable ftsTable computation frame cache hit failed * (Fintype.card Digest : ENNReal)⁻¹ +
        expectedPaidCoverageResidual exception parameter root otsTable ftsTable q computation q frame (cache, []) hit failed := by
  exact probEvent_runWithFailure_liveCover_pairs_refund_of_step_le exception parameter root otsTable ftsTable q
    ...
    computation hbound frame cache hit failed hnone hcache
