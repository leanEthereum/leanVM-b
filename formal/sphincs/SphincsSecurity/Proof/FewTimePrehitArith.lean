...
theorem prehit_effective_slots_inflation_pow_le_nine_eighths :
    ((129 / 128 : ℝ≥0∞) ^ 14) ≤ 9 / 8 := by
  apply (ENNReal.toReal_le_toReal (by finiteness) (by finiteness)).mp
  simp only [ENNReal.toReal_pow, ENNReal.toReal_div]
  norm_num
...
theorem prehit_origin_inflation_pow_le_nine_eighths {q distinct : Nat}
    (hq : q ≤ 2 ^ 120) (hdistinct : distinct ≤ 14) :
    (1 + q * ((2 ^ 127 : Nat) : ℝ≥0∞)⁻¹) ^ distinct ≤ 9 / 8 := by
  have hbase : 1 + (q : ℝ≥0∞) * ((2 ^ 127 : Nat) : ℝ≥0∞)⁻¹ ≤
      129 / 128 := by
    have hq' : (q : ℝ≥0∞) ≤ ((2 ^ 120 : Nat) : ℝ≥0∞) := by
      exact_mod_cast hq
