...
def positionOtsLayer? : Position → Option Layer
  | .chain lay _ _ _ _ => some lay
  | .leaf lay _ _ => some lay
  | .node lay _ _ _ => some lay
  | .ftsLeaf _ _ _ => none
  | .ftsNode _ _ _ _ => none
  | .ftsRoots _ => none

def coordinateOtsLayer? : Coordinate → Option Layer
  | .chainStart lay _ _ _ => some lay
  | .position p => positionOtsLayer? p
...
theorem completionOrdinaryInput_of_pending_node_child
    {parameter : PublicParameter} {table : OtsSecretIndex → HashOutput}
    {context : DeferredContext} {input : HashInput}
    {lay : Layer} {tree : TreeIndex} {level : Fin maxLayerHeight}
    {nodeIdx : LeafIndex} {prior remaining : List Position} {child : Position}
  ...
  have hdecodeOther : decodePosition? parameter input = some other := by
    ...
    exact (decodePosition?_eq_some_iff parameter _ other).2
      ⟨tablePayload completion other, rfl⟩
  have hother : other = .node lay tree level nodeIdx := by
    rw [hposition] at hdecodeOther
    exact Option.some.inj hdecodeOther.symm
  subst other
  have hslotLt : prior.length < (Position.node lay tree level nodeIdx).children.length := by
    simp [hchildren]
  have hslot := slotDigest_tableInput_node_getElem parameter completion lay tree level nodeIdx
    prior.length hslotLt
  have hchild : (Position.node lay tree level nodeIdx).children[prior.length] = child := by
    simp [hchildren]
  rw [hchild] at hslot
  have havoids := hcompletion.2.2.1 (.position child) (slotDigest prior.length input) hpending
  apply havoids
...
theorem completionOrdinaryInput_of_decodeProbe_none_chain
    {parameter : PublicParameter} {table : OtsSecretIndex → HashOutput}
    {context : DeferredContext} {input : HashInput}
    {lay : Layer} {tree : TreeIndex} {leafIdx : LeafIndex}
    ...
    CompletionOrdinaryInput parameter table context input := by
  intro completion _hcompletion other _hots heq
  have hdecodeOther : decodePosition? parameter input = some other := by
    rw [heq]
    exact (decodePosition?_eq_some_iff parameter _ other).2
      ⟨tablePayload completion other, rfl⟩
  have hother : other = .chain lay tree leafIdx chainIdx step := by
    rw [hposition] at hdecodeOther
    exact Option.some.inj hdecodeOther.symm
  subst other
  have hmatches := chainInputProbe_matchesInput parameter completion lay tree leafIdx chainIdx step
  apply ((decodeProbe?_eq_none_iff parameter input).1 hprobe
    (chainInputProbe completion lay tree leafIdx chainIdx step))
  simpa [heq] using hmatches

set_option maxRecDepth 100000 in
theorem completionOrdinaryInput_of_decodeProbe_none_leaf
    {parameter : PublicParameter} {table : OtsSecretIndex → HashOutput}
    {context : DeferredContext} {input : HashInput}
    {lay : Layer} {tree : TreeIndex} {leafIdx : LeafIndex}
    (hprobe : decodeProbe? parameter input = none)
  ...
  apply ((decodeProbe?_eq_none_iff parameter input).1 hprobe candidate)
  simp only [candidate, Probe.MatchesInput]
  rw [dif_neg (by simp [Position.lastChainStep, chainLength, winternitzBits])]
  exact ⟨trivial, tablePayload completion (.leaf lay tree leafIdx),
    (by simpa [tableInput, Position.domain] using heq), trivial⟩
...
theorem resolveDeferredChainPrefix_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (chainIdx : ChainIndex) :
    ∀ steps hsteps context result other,
      positionOtsLayer? other ≠ some lay →
      some result ∈ support
        (resolveDeferredChainPrefix table lay tree leafIdx chainIdx steps hsteps context) →
      result.values other = context.values other
  | 0, hsteps, context, result, other, hother, hresult => by
      simp only [resolveDeferredChainPrefix, support_pure, Set.mem_singleton_iff] at hresult
      exact congrFun (resolveDeferredChainStart_deferred_values_eq table
        ⟨lay, tree, leafIdx, chainIdx⟩ context result hresult.symm) other
  | steps + 1, hsteps, context, result, other, hother, hresult => by
      rw [resolveDeferredChainPrefix, mem_support_bind_iff] at hresult
      obtain ⟨previousOption, hprevious, hrest⟩ := hresult
      cases previousOption with
      | none => simp at hrest
      | some previous =>
          have hmiddle := resolveDeferredChainPrefix_preserves_other_layer_value table lay tree
            leafIdx chainIdx steps (by omega) context previous other hother hprevious
          have hne : other ≠ .chain lay tree leafIdx chainIdx ⟨steps, by omega⟩ := by
            intro heq
            subst other
            exact hother (by simp [positionOtsLayer?])
          exact (resolveDeferredPositionValue_preserves_other
            (.chain lay tree leafIdx chainIdx ⟨steps, by omega⟩) other
            previous.toDeferredContext result hne (by simpa using hrest)).trans hmiddle

theorem resolveDeferredChains_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) :
    ∀ chains context result other,
      positionOtsLayer? other ≠ some lay →
      some result ∈ support (resolveDeferredChains table lay tree leafIdx chains context) →
      result.values other = context.values other
  | [], context, result, other, hother, hresult => by
      simp [resolveDeferredChains] at hresult
      subst result
      rfl
  | chainIdx :: remaining, context, result, other, hother, hresult => by
      rw [resolveDeferredChains, mem_support_bind_iff] at hresult
      obtain ⟨resolvedOption, hresolved, hrest⟩ := hresult
      cases resolvedOption with
      | none => simp at hrest
      | some resolved =>
          exact (resolveDeferredChains_preserves_other_layer_value table lay tree leafIdx
            remaining resolved.toDeferredContext result other hother (by simpa using hrest)).trans
              (resolveDeferredChainPrefix_preserves_other_layer_value table lay tree leafIdx
                chainIdx (chainLength - 1) (by omega) context resolved other hother hresolved)

theorem resolveDeferredOtsLeaf_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (context : DeferredContext) (result : DeferredResolution)
    (other : Position) (hother : positionOtsLayer? other ≠ some lay)
    (hresult : some result ∈ support
      (resolveDeferredOtsLeaf table lay tree leafIdx context)) :
    result.values other = context.values other := by
  rw [resolveDeferredOtsLeaf, mem_support_bind_iff] at hresult
  ...
  | none => simp at hrest
  | some chains =>
      have hmiddle := resolveDeferredChains_preserves_other_layer_value table lay tree leafIdx
        (List.ofFn fun chainIdx : ChainIndex => chainIdx) context chains other hother hchains
      have hne : other ≠ .leaf lay tree leafIdx := by
        intro heq
        subst other
        exact hother (by simp [positionOtsLayer?])
      exact (resolveDeferredPositionValue_preserves_other (.leaf lay tree leafIdx) other chains
        result hne (by simpa using hrest)).trans hmiddle

theorem resolveDeferredTreeNode_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex) :
    ∀ level nodeIdx hlevel context result other,
      positionOtsLayer? other ≠ some lay →
      some result ∈ support
        (resolveDeferredTreeNode table lay tree level nodeIdx hlevel context) →
      result.values other = context.values other
  | 0, nodeIdx, hlevel, context, result, other, hother, hresult =>
      resolveDeferredOtsLeaf_preserves_other_layer_value table lay tree (leafOfNat nodeIdx)
        context result other hother hresult
  | level + 1, nodeIdx, hlevel, context, result, other, hother, hresult => by
      rw [resolveDeferredTreeNode, mem_support_bind_iff] at hresult
      obtain ⟨leftOption, hleft, hafterLeft⟩ := hresult
      cases leftOption with
      | none => simp at hafterLeft
      | some left =>
          ...
          | none => simp at hafterRight
          | some right =>
              have hleftValue := resolveDeferredTreeNode_preserves_other_layer_value table lay
                tree level (2 * nodeIdx) (by omega) context left other hother hleft
              have hrightValue := resolveDeferredTreeNode_preserves_other_layer_value table lay
                tree level (2 * nodeIdx + 1) (by omega) left.toDeferredContext right other
                hother hright
              have hne : other ≠ .node lay tree ⟨level, by omega⟩ (leafOfNat nodeIdx) := by
                intro heq
                subst other
                exact hother (by simp [positionOtsLayer?])
              exact (resolveDeferredPositionValue_preserves_other
                (.node lay tree ⟨level, by omega⟩ (leafOfNat nodeIdx)) other
                right.toDeferredContext result hne (by simpa using hafterRight)).trans
                  (hrightValue.trans hleftValue)

theorem resolveDeferredSelectedChainFamily_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) :
    ∀ {n : Nat} (family : Fin n → ChainIndex) (digits : Fin n → Digit)
      (context finalContext : DeferredContext) (values : Fin n → Digest) (other : Position),
      positionOtsLayer? other ≠ some lay →
      some (finalContext, values) ∈ support
        (resolveDeferredSelectedChainFamily table lay tree leafIdx family digits context) →
      finalContext.values other = context.values other
  | 0, family, digits, context, finalContext, values, other, hother, hresult => by
      simp [resolveDeferredSelectedChainFamily] at hresult
      rw [hresult.1]
  | n + 1, family, digits, context, finalContext, values, other, hother, hresult => by
      rw [resolveDeferredSelectedChainFamily, mem_support_bind_iff] at hresult
      obtain ⟨headOption, hhead, hrest⟩ := hresult
      cases headOption with
      | none => simp at hrest
      | some head =>
          ...
          | some tail =>
              ...
              have hcontext : finalContext = tailContext :=
                congrArg Prod.fst (Option.some.inj hreturn)
              rw [hcontext]
              exact (resolveDeferredSelectedChainFamily_preserves_other_layer_value table lay
                tree leafIdx (fun index : Fin n => family index.succ)
                (fun index : Fin n => digits index.succ) head.toDeferredContext tailContext
                tailValues other hother htail).trans
                  (resolveDeferredChainPrefix_preserves_other_layer_value table lay tree leafIdx
                    (family 0) (digits 0).val (by have := (digits 0).isLt; omega) context head
                    other hother hhead)

theorem resolveDeferredLayerPathFamily_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) :
    ∀ {n : Nat} (family : Fin n → Fin maxLayerHeight)
      (context finalContext : DeferredContext) (values : Fin n → Digest) (other : Position),
      ...
      some (finalContext, values) ∈ support
        (resolveDeferredLayerPathFamily table lay tree leafIdx family context) →
      finalContext.values other = context.values other
  | 0, family, context, finalContext, values, other, hother, hresult => by
      simp [resolveDeferredLayerPathFamily] at hresult
      rw [hresult.1]
  | n + 1, family, context, finalContext, values, other, hother, hresult => by
      rw [resolveDeferredLayerPathFamily] at hresult
      by_cases hinLayer : (family 0).val < layerHeight lay
      · simp only [hinLayer, ↓reduceDIte, mem_support_bind_iff] at hresult
        obtain ⟨headOption, hhead, hrest⟩ := hresult
        ...
        | some head =>
            ...
            | some tail =>
                ...
                have hcontext : finalContext = tailContext :=
                  congrArg Prod.fst (Option.some.inj hreturn)
                rw [hcontext]
                exact (resolveDeferredLayerPathFamily_preserves_other_layer_value table lay tree
                  leafIdx (fun index : Fin n => family index.succ) head.toDeferredContext
                  tailContext tailValues other hother htail).trans
                    (resolveDeferredTreeNode_preserves_other_layer_value table lay tree
                      (family 0).val (Nat.xor (leafIdx.val / 2 ^ (family 0).val) 1)
                      (by have := (family 0).isLt; omega) context head other hother hhead)
      · simp only [hinLayer, ↓reduceDIte, mem_support_bind_iff] at hresult
        obtain ⟨tailOption, htail, hreturn⟩ := hresult
        cases tailOption with
        | none => simp at hreturn
        | some tail =>
            ...
            rw [hcontext]
            exact resolveDeferredLayerPathFamily_preserves_other_layer_value table lay tree
              leafIdx (fun index : Fin n => family index.succ) context tailContext tailValues
              other hother htail

theorem resolveDeferredLayerValues_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (index : Index) (lay : Layer)
    ...
    finalContext.values other = context.values other := by
  ...
  | some chains =>
      ...
      | some path =>
          ...
          have hcontext : finalContext = afterPath :=
            congrArg Prod.fst (Option.some.inj hreturn)
          rw [hcontext]
          exact (resolveDeferredLayerPathFamily_preserves_other_layer_value table lay
            (treeIndexAt index lay) (leafIndexAt index lay)
            (fun level : Fin maxLayerHeight => level) afterChains afterPath pathValues other
            hother hpath).trans
              (resolveDeferredSelectedChainFamily_preserves_other_layer_value table lay
                (treeIndexAt index lay) (leafIndexAt index lay)
                (fun chainIdx : ChainIndex => chainIdx) encoding context afterChains chainValues
                other hother hchains)

theorem resolveDeferredChainPrefix_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (chainIdx : ChainIndex) :
    ∀ steps hsteps context result other,
      coordinateOtsLayer? other ≠ some lay →
      some result ∈ support
        (resolveDeferredChainPrefix table lay tree leafIdx chainIdx steps hsteps context) →
      result.state.pendingAt other = context.state.pendingAt other
  | 0, hsteps, context, result, other, hother, hresult => by
      simp only [resolveDeferredChainPrefix, support_pure, Set.mem_singleton_iff] at hresult
      have hstate := resolveDeferredChainStart_state_eq_clearPending table
        ⟨lay, tree, leafIdx, chainIdx⟩ context result hresult.symm
      rw [hstate]
      apply pendingAt_clearPending_of_ne
      intro heq
      subst other
      exact hother (by simp [coordinateOtsLayer?, OtsSecretIndex.coordinate])
  | steps + 1, hsteps, context, result, other, hother, hresult => by
      rw [resolveDeferredChainPrefix, mem_support_bind_iff] at hresult
      obtain ⟨previousOption, hprevious, hrest⟩ := hresult
      cases previousOption with
      | none => simp at hrest
      | some previous =>
          have hmiddle := resolveDeferredChainPrefix_preserves_other_layer_pendingAt table lay
            tree leafIdx chainIdx steps (by omega) context previous other hother hprevious
          have hrest' : some result ∈ support (resolveDeferredPositionValue
              (.chain lay tree leafIdx chainIdx ⟨steps, by omega⟩)
              previous.toDeferredContext) := by
            simpa using hrest
          rw [resolveDeferredPositionValue_state_eq_clearPending
            (.chain lay tree leafIdx chainIdx ⟨steps, by omega⟩)
            previous.toDeferredContext result hrest']
          exact (pendingAt_clearPending_of_ne previous.state
            (.position (.chain lay tree leafIdx chainIdx ⟨steps, by omega⟩)) other (by
              intro heq
              subst other
              exact hother (by simp [coordinateOtsLayer?, positionOtsLayer?]))).trans hmiddle

theorem resolveDeferredChains_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) :
    ∀ chains context result other,
      coordinateOtsLayer? other ≠ some lay →
  ...
  | chainIdx :: remaining, context, result, other, hother, hresult => by
      ...
      | none => simp at hrest
      | some resolved =>
          exact (resolveDeferredChains_preserves_other_layer_pendingAt table lay tree leafIdx
            remaining resolved.toDeferredContext result other hother (by simpa using hrest)).trans
              (resolveDeferredChainPrefix_preserves_other_layer_pendingAt table lay tree leafIdx
                chainIdx (chainLength - 1) (by omega) context resolved other hother hresolved)

theorem resolveDeferredOtsLeaf_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (context : DeferredContext) (result : DeferredResolution)
    (other : Coordinate) (hother : coordinateOtsLayer? other ≠ some lay)
    (hresult : some result ∈ support
      (resolveDeferredOtsLeaf table lay tree leafIdx context)) :
    result.state.pendingAt other = context.state.pendingAt other := by
  rw [resolveDeferredOtsLeaf, mem_support_bind_iff] at hresult
  obtain ⟨chainsOption, hchains, hrest⟩ := hresult
  cases chainsOption with
  | none => simp at hrest
  | some chains =>
      have hmiddle := resolveDeferredChains_preserves_other_layer_pendingAt table lay tree leafIdx
        (List.ofFn fun chainIdx : ChainIndex => chainIdx) context chains other hother hchains
      have hrest' : some result ∈ support
          (resolveDeferredPositionValue (.leaf lay tree leafIdx) chains) := by
        simpa using hrest
      rw [resolveDeferredPositionValue_state_eq_clearPending (.leaf lay tree leafIdx) chains
        result hrest']
      exact (pendingAt_clearPending_of_ne chains.state (.position (.leaf lay tree leafIdx))
        other (by
          intro heq
          subst other
          exact hother (by simp [coordinateOtsLayer?, positionOtsLayer?]))).trans hmiddle

theorem resolveDeferredTreeNode_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex) :
    ∀ level nodeIdx hlevel context result other,
      coordinateOtsLayer? other ≠ some lay →
      some result ∈ support
      ...
      result.state.pendingAt other = context.state.pendingAt other
  | 0, nodeIdx, hlevel, context, result, other, hother, hresult =>
      resolveDeferredOtsLeaf_preserves_other_layer_pendingAt table lay tree (leafOfNat nodeIdx)
        context result other hother hresult
  | level + 1, nodeIdx, hlevel, context, result, other, hother, hresult => by
      rw [resolveDeferredTreeNode, mem_support_bind_iff] at hresult
      obtain ⟨leftOption, hleft, hafterLeft⟩ := hresult
      ...
      | some left =>
          ...
          | none => simp at hafterRight
          | some right =>
              have hleftPending := resolveDeferredTreeNode_preserves_other_layer_pendingAt table
                lay tree level (2 * nodeIdx) (by omega) context left other hother hleft
              have hrightPending := resolveDeferredTreeNode_preserves_other_layer_pendingAt table
                lay tree level (2 * nodeIdx + 1) (by omega) left.toDeferredContext right other
                hother hright
              have hrest' : some result ∈ support (resolveDeferredPositionValue
                  (.node lay tree ⟨level, by omega⟩ (leafOfNat nodeIdx))
                  right.toDeferredContext) := by
                simpa using hafterRight
              rw [resolveDeferredPositionValue_state_eq_clearPending
                (.node lay tree ⟨level, by omega⟩ (leafOfNat nodeIdx))
                right.toDeferredContext result hrest']
              exact (pendingAt_clearPending_of_ne right.state
                (.position (.node lay tree ⟨level, by omega⟩ (leafOfNat nodeIdx))) other (by
                  intro heq
                  subst other
                  exact hother (by simp [coordinateOtsLayer?, positionOtsLayer?]))).trans
                    (hrightPending.trans hleftPending)

theorem resolveDeferredSelectedChainFamily_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) :
    ∀ {n : Nat} (family : Fin n → ChainIndex) (digits : Fin n → Digit)
      (context finalContext : DeferredContext) (values : Fin n → Digest)
      ...
      some (finalContext, values) ∈ support
        (resolveDeferredSelectedChainFamily table lay tree leafIdx family digits context) →
      finalContext.state.pendingAt other = context.state.pendingAt other
  | 0, family, digits, context, finalContext, values, other, hother, hresult => by
      simp [resolveDeferredSelectedChainFamily] at hresult
      rw [hresult.1]
  | n + 1, family, digits, context, finalContext, values, other, hother, hresult => by
      ...
      | some head =>
          ...
          | some tail =>
              ...
              have hcontext : finalContext = tailContext :=
                congrArg Prod.fst (Option.some.inj hreturn)
              rw [hcontext]
              exact (resolveDeferredSelectedChainFamily_preserves_other_layer_pendingAt table lay
                tree leafIdx (fun index : Fin n => family index.succ)
                (fun index : Fin n => digits index.succ) head.toDeferredContext tailContext
                tailValues other hother htail).trans
                  (resolveDeferredChainPrefix_preserves_other_layer_pendingAt table lay tree
                    leafIdx (family 0) (digits 0).val (by have := (digits 0).isLt; omega)
                    context head other hother hhead)

theorem resolveDeferredLayerPathFamily_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) :
    ∀ {n : Nat} (family : Fin n → Fin maxLayerHeight)
      (context finalContext : DeferredContext) (values : Fin n → Digest)
        ...
        | some head =>
            ...
            | some tail =>
                ...
                have hcontext : finalContext = tailContext :=
                  congrArg Prod.fst (Option.some.inj hreturn)
                rw [hcontext]
                exact (resolveDeferredLayerPathFamily_preserves_other_layer_pendingAt table lay
                  tree leafIdx (fun index : Fin n => family index.succ) head.toDeferredContext
                  tailContext tailValues other hother htail).trans
                    (resolveDeferredTreeNode_preserves_other_layer_pendingAt table lay tree
                      (family 0).val (Nat.xor (leafIdx.val / 2 ^ (family 0).val) 1)
                      (by have := (family 0).isLt; omega) context head other hother hhead)
      · simp only [hinLayer, ↓reduceDIte, mem_support_bind_iff] at hresult
        obtain ⟨tailOption, htail, hreturn⟩ := hresult
        ...
        | some tail =>
            ...
            have hcontext : finalContext = tailContext :=
              congrArg Prod.fst (Option.some.inj hreturn)
            rw [hcontext]
            exact resolveDeferredLayerPathFamily_preserves_other_layer_pendingAt table lay tree
              leafIdx (fun index : Fin n => family index.succ) context tailContext tailValues
              other hother htail

theorem resolveDeferredLayerValues_preserves_other_layer_pendingAt
    (table : OtsSecretIndex → HashOutput) (index : Index) (lay : Layer)
    (encoding : ChainIndex → Digit) (context finalContext : DeferredContext)
    (values : (ChainIndex → Digest) × (Fin maxLayerHeight → Digest))
    (other : Coordinate) (hother : coordinateOtsLayer? other ≠ some lay)
    (hresult : some (finalContext, values) ∈ support
      (resolveDeferredLayerValues table index lay encoding context)) :
    finalContext.state.pendingAt other = context.state.pendingAt other := by
  rw [resolveDeferredLayerValues, mem_support_bind_iff] at hresult
  obtain ⟨chainsOption, hchains, hrest⟩ := hresult
  cases chainsOption with
  | none => simp at hrest
  | some chains =>
      ...
      | some path =>
          ...
          exact (resolveDeferredLayerPathFamily_preserves_other_layer_pendingAt table lay
            ...
            (fun level : Fin maxLayerHeight => level) afterChains afterPath pathValues other
            hother hpath).trans
              (resolveDeferredSelectedChainFamily_preserves_other_layer_pendingAt table lay
                (treeIndexAt index lay) (leafIndexAt index lay)
                (fun chainIdx : ChainIndex => chainIdx) encoding context afterChains chainValues
                other hother hchains)
...
def ResolverLayerViewEq (lay : Layer) (left right : DeferredContext) : Prop :=
  (∀ coordinate, coordinateOtsLayer? coordinate = some lay →
    left.state.values coordinate = right.state.values coordinate) ∧
  (∀ position, positionOtsLayer? position = some lay →
    left.values position = right.values position) ∧
  ∀ coordinate, coordinateOtsLayer? coordinate = some lay →
    left.state.pendingAt coordinate = right.state.pendingAt coordinate

theorem ResolverLayerViewEq.refl (lay : Layer) (context : DeferredContext) :
    ResolverLayerViewEq lay context context := by
  exact ⟨fun _ _ => rfl, fun _ _ => rfl, fun _ _ => rfl⟩

theorem ResolverLayerViewEq.symm {lay : Layer} {left right : DeferredContext}
    (heq : ResolverLayerViewEq lay left right) :
    ResolverLayerViewEq lay right left := by
  exact ⟨fun coordinate hcoordinate => (heq.1 coordinate hcoordinate).symm,
    fun position hposition => (heq.2.1 position hposition).symm,
    fun coordinate hcoordinate => (heq.2.2 coordinate hcoordinate).symm⟩

theorem ResolverLayerViewEq.trans {lay : Layer} {left middle right : DeferredContext}
    (hleft : ResolverLayerViewEq lay left middle)
    (hright : ResolverLayerViewEq lay middle right) :
    ResolverLayerViewEq lay left right := by
  exact ⟨fun coordinate hcoordinate =>
      (hleft.1 coordinate hcoordinate).trans (hright.1 coordinate hcoordinate),
    fun position hposition =>
      (hleft.2.1 position hposition).trans (hright.2.1 position hposition),
    fun coordinate hcoordinate =>
      (hleft.2.2 coordinate hcoordinate).trans (hright.2.2 coordinate hcoordinate)⟩

theorem LayerViewEq.toResolverLayerViewEq {lay : Layer} {left right : DeferredContext}
    (heq : LayerViewEq lay left right) : ResolverLayerViewEq lay left right := by
  exact ⟨fun coordinate _ => congrFun heq.1 coordinate,
    heq.2.2.2.1, heq.2.2.2.2⟩

theorem ResolverLayerViewEq.hitAt_iff {lay : Layer} {left right : DeferredContext}
    (heq : ResolverLayerViewEq lay left right) (coordinate : Coordinate)
    (hcoordinate : coordinateOtsLayer? coordinate = some lay) (output : HashOutput) :
    left.state.hitAt coordinate output ↔ right.state.hitAt coordinate output := by
  unfold LazyRevealProbe.State.hitAt
  rw [heq.2.2 coordinate hcoordinate]

theorem ResolverLayerViewEq.positionValue_eq
    {lay : Layer} {left right : DeferredContext}
    (heq : ResolverLayerViewEq lay left right) (position : Position)
    (hposition : positionOtsLayer? position = some lay) :
    left.positionValue position = right.positionValue position := by
  unfold DeferredContext.positionValue
  have hcoordinate : coordinateOtsLayer? (.position position) = some lay := by
    simpa [coordinateOtsLayer?] using hposition
  rw [heq.1 (.position position) hcoordinate]
  cases right.state.values (.position position)
  · exact heq.2.1 position hposition
  · rfl

theorem ResolverLayerViewEq.clearPending
    {lay : Layer} {left right : DeferredContext}
    (heq : ResolverLayerViewEq lay left right) (coordinate : Coordinate)
    (hcoordinate : coordinateOtsLayer? coordinate = some lay) :
    ResolverLayerViewEq lay
      { left with state := left.state.clearPending coordinate }
      { right with state := right.state.clearPending coordinate } := by
  ...
  · rw [pendingAt_clearPending_of_ne left.state coordinate other hsame,
      pendingAt_clearPending_of_ne right.state coordinate other hsame]
    exact heq.2.2 other hother

theorem ResolverLayerViewEq.install
    {lay : Layer} {left right : DeferredContext}
    (heq : ResolverLayerViewEq lay left right) (position : Position)
    (hposition : positionOtsLayer? position = some lay) (output : HashOutput) :
    ResolverLayerViewEq lay
      { left with values := left.values.install position output }
      { right with values := right.values.install position output } := by
  refine ⟨heq.1, ?_, heq.2.2⟩
  intro other hother
  by_cases hsame : other = position
  · subst other
    simp [DeferredStructuralValues.install]
  · simpa [DeferredStructuralValues.install, hsame] using
      heq.2.1 other hother

def ResolverLayerResolutionEq (lay : Layer) :
    Option DeferredResolution → Option DeferredResolution → Prop
  | none, none => True
  | some left, some right =>
      left.output = right.output ∧
        ResolverLayerViewEq lay left.toDeferredContext right.toDeferredContext
  | _, _ => False

theorem relTriple_resolveDeferredPositionValue_of_resolverLayerViewEq
    (lay : Layer) (position : Position)
    (hposition : positionOtsLayer? position = some lay)
    ...
    RelTriple
      (resolveDeferredPositionValue position left)
      (resolveDeferredPositionValue position right)
      (ResolverLayerResolutionEq lay) := by
  have hcoordinate : coordinateOtsLayer? (.position position) = some lay := by
    simpa [coordinateOtsLayer?] using hposition
  have hstate := heq.1 (.position position) hcoordinate
  unfold resolveDeferredPositionValue
  dsimp only
  cases hleftState : left.state.values (.position position) with
  | some output =>
  ...
  | none =>
      have hrightState : right.state.values (.position position) = none := by
        ...
        exact hleftState
      simp only [hrightState]
      have hvalue := heq.2.1 position hposition
      cases hleftValue : left.values position with
      | some output =>
          have hrightValue : right.values position = some output := by
            rw [← hvalue]
...
theorem relTriple_resolveDeferredChainStart_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (chainIdx : ChainIndex)
    (left right : DeferredContext) (heq : ResolverLayerViewEq lay left right) :
    RelTriple
  ...
  have hcoordinate : coordinateOtsLayer? (.chainStart lay tree leafIdx chainIdx) = some lay := by
    simp [coordinateOtsLayer?]
  have hstate := heq.1 (.chainStart lay tree leafIdx chainIdx) hcoordinate
  unfold resolveDeferredChainStart
  dsimp only [OtsSecretIndex.coordinate]
  cases hleftState : left.state.values (.chainStart lay tree leafIdx chainIdx) with
  | some output =>
...
theorem relTriple_resolveDeferredChainPrefix_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (chainIdx : ChainIndex) :
    ∀ steps hsteps left right,
      ResolverLayerViewEq lay left right →
      RelTriple
        (resolveDeferredChainPrefix table lay tree leafIdx chainIdx steps hsteps left)
        (resolveDeferredChainPrefix table lay tree leafIdx chainIdx steps hsteps right)
        (ResolverLayerResolutionEq lay)
  | 0, hsteps, left, right, heq => by
      simp only [resolveDeferredChainPrefix]
      exact relTriple_resolveDeferredChainStart_of_resolverLayerViewEq table lay tree leafIdx
        chainIdx left right heq
  | steps + 1, hsteps, left, right, heq => by
      rw [resolveDeferredChainPrefix, resolveDeferredChainPrefix]
      apply relTriple_bind
        (relTriple_resolveDeferredChainPrefix_of_resolverLayerViewEq table lay tree leafIdx
          chainIdx steps (by omega) left right heq)
      intro leftPrevious rightPrevious hprevious
      cases leftPrevious with
      | none =>
          cases rightPrevious <;> simp [ResolverLayerResolutionEq] at hprevious ⊢
      | some leftPrevious =>
          cases rightPrevious with
          | none => simp [ResolverLayerResolutionEq] at hprevious
          | some rightPrevious =>
              exact relTriple_resolveDeferredPositionValue_of_resolverLayerViewEq lay
                (.chain lay tree leafIdx chainIdx ⟨steps, by omega⟩)
                (by simp [positionOtsLayer?]) leftPrevious.toDeferredContext
                rightPrevious.toDeferredContext hprevious.2

def ResolverLayerContextOptionEq (lay : Layer) :
    Option DeferredContext → Option DeferredContext → Prop
  | none, none => True
  | some left, some right => ResolverLayerViewEq lay left right
  | _, _ => False

theorem relTriple_resolveDeferredChains_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) : ∀ chains left right,
      ResolverLayerViewEq lay left right →
      RelTriple
        (resolveDeferredChains table lay tree leafIdx chains left)
        (resolveDeferredChains table lay tree leafIdx chains right)
        (ResolverLayerContextOptionEq lay)
  | [], left, right, heq => by
      simp only [resolveDeferredChains]
      exact relTriple_pure_pure heq
  | chainIdx :: remaining, left, right, heq => by
      ...
      apply relTriple_bind
        (relTriple_resolveDeferredChainPrefix_of_resolverLayerViewEq table lay tree leafIdx
          chainIdx (chainLength - 1) (by omega) left right heq)
      intro leftResolved rightResolved hresolved
      cases leftResolved with
      | none =>
          cases rightResolved with
          | none =>
              simpa [ResolverLayerContextOptionEq] using
                (relTriple_pure_pure (R := ResolverLayerContextOptionEq lay) trivial)
          | some rightResolved => simp [ResolverLayerResolutionEq] at hresolved
      | some leftResolved =>
          cases rightResolved with
          | none => simp [ResolverLayerResolutionEq] at hresolved
          | some rightResolved =>
              exact relTriple_resolveDeferredChains_of_resolverLayerViewEq table lay tree
                leafIdx remaining leftResolved.toDeferredContext
                rightResolved.toDeferredContext hresolved.2

theorem relTriple_resolveDeferredOtsLeaf_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) (left right : DeferredContext)
    (heq : ResolverLayerViewEq lay left right) :
    RelTriple
  ...
  rw [resolveDeferredOtsLeaf, resolveDeferredOtsLeaf]
  apply relTriple_bind
    (relTriple_resolveDeferredChains_of_resolverLayerViewEq table lay tree leafIdx
      (List.ofFn fun chainIdx : ChainIndex => chainIdx) left right heq)
  intro leftChains rightChains hchains
  cases leftChains with
  | none =>
      cases rightChains with
      | none =>
          simpa [ResolverLayerResolutionEq] using
            (relTriple_pure_pure (R := ResolverLayerResolutionEq lay) trivial)
      | some rightChains => simp [ResolverLayerContextOptionEq] at hchains
  | some leftChains =>
      cases rightChains with
      | none => simp [ResolverLayerContextOptionEq] at hchains
      | some rightChains =>
          exact relTriple_resolveDeferredPositionValue_of_resolverLayerViewEq lay
            (.leaf lay tree leafIdx) (by simp [positionOtsLayer?]) leftChains rightChains hchains

theorem relTriple_resolveDeferredTreeNode_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex) :
    ∀ level nodeIdx hlevel left right,
      ResolverLayerViewEq lay left right →
      RelTriple
        ...
        (ResolverLayerResolutionEq lay)
  | 0, nodeIdx, hlevel, left, right, heq =>
      relTriple_resolveDeferredOtsLeaf_of_resolverLayerViewEq table lay tree
        (leafOfNat nodeIdx) left right heq
  | level + 1, nodeIdx, hlevel, left, right, heq => by
      rw [resolveDeferredTreeNode, resolveDeferredTreeNode]
      apply relTriple_bind
        (relTriple_resolveDeferredTreeNode_of_resolverLayerViewEq table lay tree level
          (2 * nodeIdx) (by omega) left right heq)
      intro leftNode rightNode hleftNode
      cases leftNode with
      | none =>
          cases rightNode <;> simp [ResolverLayerResolutionEq] at hleftNode ⊢
      | some leftNode =>
          cases rightNode with
          | none => simp [ResolverLayerResolutionEq] at hleftNode
          | some rightNode =>
              apply relTriple_bind
                (relTriple_resolveDeferredTreeNode_of_resolverLayerViewEq table lay tree level
                  (2 * nodeIdx + 1) (by omega) leftNode.toDeferredContext
              ...
              cases leftSibling with
              | none =>
                  cases rightSibling <;> simp [ResolverLayerResolutionEq] at hsibling ⊢
              | some leftSibling =>
                  cases rightSibling with
                  | none => simp [ResolverLayerResolutionEq] at hsibling
                  | some rightSibling =>
                      exact relTriple_resolveDeferredPositionValue_of_resolverLayerViewEq lay
                        (.node lay tree ⟨level, by omega⟩ (leafOfNat nodeIdx))
                        (by simp [positionOtsLayer?]) leftSibling.toDeferredContext
...
theorem relTriple_resolveDeferredPosition_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (position : Position)
    (hposition : positionOtsLayer? position = some lay)
    (left right : DeferredContext) (heq : ResolverLayerViewEq lay left right) :
    RelTriple
  ...
  | chain selectedLay tree leafIdx chainIdx step =>
      have hlay : selectedLay = lay := Option.some.inj hposition
      subst selectedLay
      exact relTriple_resolveDeferredChainPrefix_of_resolverLayerViewEq table lay tree leafIdx
        chainIdx (step.val + 1) (by have := step.isLt; omega) left right heq
  | leaf selectedLay tree leafIdx =>
      have hlay : selectedLay = lay := Option.some.inj hposition
      subst selectedLay
      exact relTriple_resolveDeferredOtsLeaf_of_resolverLayerViewEq table lay tree leafIdx
        left right heq
  | node selectedLay tree level nodeIdx =>
      have hlay : selectedLay = lay := Option.some.inj hposition
      subst selectedLay
      exact relTriple_resolveDeferredTreeNode_of_resolverLayerViewEq table lay tree
        (level.val + 1) nodeIdx (by have := level.isLt; omega) left right heq
  | ftsLeaf index tree leafIdx => simp [positionOtsLayer?] at hposition
  | ftsNode index tree level nodeIdx => simp [positionOtsLayer?] at hposition
  | ftsRoots index => simp [positionOtsLayer?] at hposition

theorem relTriple_resolveDeferredReveal_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (position : Position)
    (hposition : positionOtsLayer? position = some lay)
    (left right : DeferredContext) (heq : ResolverLayerViewEq lay left right) :
    RelTriple
  ...
  by_cases hresolvable : ResolvableOtsPosition position
  · rw [if_pos hresolvable, if_pos hresolvable]
    exact relTriple_resolveDeferredPosition_of_resolverLayerViewEq table lay position hposition
      left right heq
  · rw [if_neg hresolvable, if_neg hresolvable]
    exact relTriple_resolveDeferredPositionValue_of_resolverLayerViewEq lay position hposition
      left right heq

def ResolverLayerContextValueEq (lay : Layer) :
    Option (DeferredContext × alpha) → Option (DeferredContext × alpha) → Prop
  | none, none => True
  | some left, some right =>
      left.2 = right.2 ∧ ResolverLayerViewEq lay left.1 right.1
  | _, _ => False

theorem relTriple_resolveDeferredSelectedChainFamily_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) : ∀ {n : Nat} (family : Fin n → ChainIndex)
      ...
      RelTriple
        ...
        (ResolverLayerContextValueEq lay)
  | 0, family, digits, left, right, heq => by
      simp only [resolveDeferredSelectedChainFamily]
      apply relTriple_pure_pure
      exact ⟨rfl, heq⟩
  | n + 1, family, digits, left, right, heq => by
      rw [resolveDeferredSelectedChainFamily, resolveDeferredSelectedChainFamily]
      apply relTriple_bind
        (relTriple_resolveDeferredChainPrefix_of_resolverLayerViewEq table lay tree leafIdx
          (family 0) (digits 0).val (by have := (digits 0).isLt; omega) left right heq)
      intro leftHead rightHead hhead
      cases leftHead with
      | none =>
          cases rightHead with
          | none =>
              simpa [ResolverLayerContextValueEq] using
                (relTriple_pure_pure (R := ResolverLayerContextValueEq lay) trivial)
          | some rightHead => simp [ResolverLayerResolutionEq] at hhead
      | some leftHead =>
          cases rightHead with
          | none => simp [ResolverLayerResolutionEq] at hhead
          | some rightHead =>
              apply relTriple_bind
                (relTriple_resolveDeferredSelectedChainFamily_of_resolverLayerViewEq table lay
                  tree leafIdx (fun index : Fin n => family index.succ)
                  (fun index : Fin n => digits index.succ) leftHead.toDeferredContext
                  rightHead.toDeferredContext hhead.2)
              ...
              | none =>
                  ...
                  | none =>
                      simpa [ResolverLayerContextValueEq] using
                        (relTriple_pure_pure (R := ResolverLayerContextValueEq lay) trivial)
                  | some rightTail => simp [ResolverLayerContextValueEq] at htail
              | some leftTail =>
                  cases rightTail with
                  | none => simp [ResolverLayerContextValueEq] at htail
                  | some rightTail =>
                      apply relTriple_pure_pure
                      refine ⟨?_, htail.2⟩
                      funext position
...
theorem relTriple_resolveDeferredLayerPathFamily_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (lay : Layer) (tree : TreeIndex)
    (leafIdx : LeafIndex) : ∀ {n : Nat} (family : Fin n → Fin maxLayerHeight)
      ...
      RelTriple
        ...
        (ResolverLayerContextValueEq lay)
  | 0, family, left, right, heq => by
      simp only [resolveDeferredLayerPathFamily]
      apply relTriple_pure_pure
      exact ⟨rfl, heq⟩
  | n + 1, family, left, right, heq => by
      rw [resolveDeferredLayerPathFamily, resolveDeferredLayerPathFamily]
      by_cases hinLayer : (family 0).val < layerHeight lay
      · simp only [hinLayer, ↓reduceDIte]
        apply relTriple_bind
          (relTriple_resolveDeferredTreeNode_of_resolverLayerViewEq table lay tree
            (family 0).val (Nat.xor (leafIdx.val / 2 ^ (family 0).val) 1)
            (by have := (family 0).isLt; omega) left right heq)
        intro leftHead rightHead hhead
        cases leftHead with
        | none =>
            cases rightHead with
        ...
        | some leftHead =>
            ...
            | some rightHead =>
                apply relTriple_bind
                  (relTriple_resolveDeferredLayerPathFamily_of_resolverLayerViewEq table lay tree
                    leafIdx (fun position : Fin n => family position.succ)
                    leftHead.toDeferredContext rightHead.toDeferredContext hhead.2)
                intro leftTail rightTail htail
                cases leftTail with
                | none =>
                    cases rightTail with
      ...
      · simp only [hinLayer, ↓reduceDIte]
        apply relTriple_bind
          (relTriple_resolveDeferredLayerPathFamily_of_resolverLayerViewEq table lay tree leafIdx
            (fun position : Fin n => family position.succ) left right heq)
        intro leftTail rightTail htail
        cases leftTail with
        | none =>
            cases rightTail with
...
theorem relTriple_resolveDeferredLayerValues_of_resolverLayerViewEq
    (table : OtsSecretIndex → HashOutput) (index : Index) (lay : Layer)
    ...
    RelTriple
      ...
      (ResolverLayerContextValueEq lay) := by
  rw [resolveDeferredLayerValues, resolveDeferredLayerValues]
  apply relTriple_bind
    (relTriple_resolveDeferredSelectedChainFamily_of_resolverLayerViewEq table lay
      (treeIndexAt index lay) (leafIndexAt index lay)
      (fun chainIdx : ChainIndex => chainIdx) encoding left right heq)
  intro leftChains rightChains hchains
  cases leftChains with
  | none =>
      cases rightChains with
      | none =>
          simpa [ResolverLayerContextValueEq] using
            (relTriple_pure_pure (R := ResolverLayerContextValueEq lay) trivial)
      | some rightChains => simp [ResolverLayerContextValueEq] at hchains
  | some leftChains =>
      cases rightChains with
      | none => simp [ResolverLayerContextValueEq] at hchains
      | some rightChains =>
          apply relTriple_bind
            (relTriple_resolveDeferredLayerPathFamily_of_resolverLayerViewEq table lay
              (treeIndexAt index lay) (leafIndexAt index lay)
              (fun level : Fin maxLayerHeight => level) leftChains.1 rightChains.1 hchains.2)
          ...
          | none =>
              ...
              | none =>
                  simpa [ResolverLayerContextValueEq] using
                    (relTriple_pure_pure (R := ResolverLayerContextValueEq lay) trivial)
              | some rightPath => simp [ResolverLayerContextValueEq] at hpath
          | some leftPath =>
              cases rightPath with
              | none => simp [ResolverLayerContextValueEq] at hpath
              | some rightPath =>
                  apply relTriple_pure_pure
                  exact ⟨by rw [hchains.1, hpath.1], hpath.2⟩

theorem resolveDeferredPosition_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (resolvedLay : Layer)
    (position : Position) (hposition : positionOtsLayer? position = some resolvedLay)
    (context : DeferredContext) (result : DeferredResolution) (other : Position)
    (hother : positionOtsLayer? other ≠ some resolvedLay)
    (hresult : some result ∈ support (resolveDeferredPosition table position context)) :
    result.values other = context.values other := by
  cases position with
  | chain lay tree leafIdx chainIdx step =>
      have hlay : lay = resolvedLay := Option.some.inj hposition
      subst lay
      exact resolveDeferredChainPrefix_preserves_other_layer_value table resolvedLay tree
        leafIdx chainIdx (step.val + 1) (by have := step.isLt; omega) context result other
        hother (by simpa [resolveDeferredPosition] using hresult)
  | leaf lay tree leafIdx =>
      have hlay : lay = resolvedLay := Option.some.inj hposition
      subst lay
      exact resolveDeferredOtsLeaf_preserves_other_layer_value table resolvedLay tree leafIdx
        context result other hother (by simpa [resolveDeferredPosition] using hresult)
  | node lay tree level nodeIdx =>
      have hlay : lay = resolvedLay := Option.some.inj hposition
      subst lay
      exact resolveDeferredTreeNode_preserves_other_layer_value table resolvedLay tree
        (level.val + 1) nodeIdx (by have := level.isLt; omega) context result other hother
        (by simpa [resolveDeferredPosition] using hresult)
  | ftsLeaf index tree leafIdx => simp [positionOtsLayer?] at hposition
  | ftsNode index tree level nodeIdx => simp [positionOtsLayer?] at hposition
  | ftsRoots index => simp [positionOtsLayer?] at hposition

theorem resolveDeferredReveal_preserves_other_layer_value
    (table : OtsSecretIndex → HashOutput) (resolvedLay : Layer)
    (position : Position) (hposition : positionOtsLayer? position = some resolvedLay)
    (context : DeferredContext) (result : DeferredResolution) (other : Position)
    (hother : positionOtsLayer? other ≠ some resolvedLay)
  ...
  classical
  by_cases hresolvable : ResolvableOtsPosition position
  · exact resolveDeferredPosition_preserves_other_layer_value table resolvedLay position
      hposition context result other hother
        (by simpa [resolveDeferredReveal, hresolvable] using hresult)
  · have hdirect : some result ∈ support (resolveDeferredPositionValue position context) := by
      simpa [resolveDeferredReveal, hresolvable] using hresult
    apply resolveDeferredPositionValue_preserves_other position other context result
    · intro heq
      subst other
      exact hother hposition
    · exact hdirect

def ResolverLayerViewPreserving (lay : Layer)
    (computation : StateT SplitHashCache
      (OracleComp (LazyRevealProbe.World Coordinate)) alpha) : Prop :=
  ∀ context fuel table cache result,
    some result ∈ support
      (runResolvedFromTable context fuel table (computation.run cache)) →
    ResolverLayerViewEq lay result.context context

theorem resolverLayerViewPreserving_pure (lay : Layer) (value : alpha) :
    ResolverLayerViewPreserving lay
      (pure value : StateT SplitHashCache
        (OracleComp (LazyRevealProbe.World Coordinate)) alpha) := by
  intro context fuel table cache result hresult
  simp [runResolvedFromTable] at hresult
  subst result
  exact ResolverLayerViewEq.refl lay context

theorem ResolverLayerViewPreserving.bind {lay : Layer}
    {left : StateT SplitHashCache
      (OracleComp (LazyRevealProbe.World Coordinate)) alpha}
    {next : alpha → StateT SplitHashCache
      (OracleComp (LazyRevealProbe.World Coordinate)) beta}
    (hleft : ResolverLayerViewPreserving lay left)
    (hnext : ∀ value, ResolverLayerViewPreserving lay (next value)) :
    ResolverLayerViewPreserving lay (left >>= next) := by
  intro context fuel table cache result hresult
  rw [StateT.run_bind, runResolvedFromTable_bind, mem_support_bind_iff] at hresult
...
theorem resolverLayerViewPreserving_ensureCoordinate
    (lay : Layer) (coordinate : Coordinate) :
    ResolverLayerViewPreserving lay (ensureCoordinate coordinate) := by
  intro context fuel table cache result hresult
  unfold ensureCoordinate at hresult
  rw [StateT.run_liftM, LazyRevealProbe.ensureQuery,
    runResolvedFromTable_ensure_query_bind] at hresult
...
theorem resolverLayerViewPreserving_splitHashQuery_ordinary
    (lay : Layer) (input : HashInput) :
    ResolverLayerViewPreserving lay (splitHashQuery (.ordinary input)) := by
  intro context fuel table cache result hresult
...
theorem resolverLayerViewPreserving_ordinaryHashImpl (lay : Layer) (input : HashInput) :
    ResolverLayerViewPreserving lay (ordinaryHashImpl input) :=
  resolverLayerViewPreserving_splitHashQuery_ordinary lay input

theorem resolverLayerViewPreserving_sequenceFin {lay : Layer} {n : Nat}
    (computation : Fin n → StateT SplitHashCache
      (OracleComp (LazyRevealProbe.World Coordinate)) alpha)
    (hcomponent : ∀ position, ResolverLayerViewPreserving lay (computation position)) :
    ResolverLayerViewPreserving lay (sequenceFin computation) := by
  induction n with
  | zero =>
      simpa [sequenceFin] using
        (resolverLayerViewPreserving_pure lay Fin.elim0 :
          ResolverLayerViewPreserving lay
            (pure Fin.elim0 : StateT SplitHashCache
              (OracleComp (LazyRevealProbe.World Coordinate)) (Fin 0 → alpha)))
  | succ n ih =>
      ...
      exact (hcomponent 0).bind fun _ =>
        (ih (fun position : Fin n => computation position.succ)
          (fun position => hcomponent position.succ)).bind fun _ =>
            resolverLayerViewPreserving_pure lay _

theorem resolverLayerViewPreserving_simulateQ {spec : OracleSpec ι}
    (lay : Layer)
    (impl : QueryImpl spec
      (StateT SplitHashCache
        (OracleComp (LazyRevealProbe.World Coordinate))))
    (hquery : ∀ query, ResolverLayerViewPreserving lay (impl query))
    (computation : OracleComp spec alpha) :
    ResolverLayerViewPreserving lay (simulateQ impl computation) := by
  induction computation using OracleComp.inductionOn with
  | pure value =>
      simp only [simulateQ_pure]
      exact resolverLayerViewPreserving_pure lay value
  | query_bind query next ih =>
      rw [simulateQ_query_bind]
      exact (hquery query).bind fun output => ih output

theorem resolverLayerViewPreserving_revealPosition_of_ne
    (preservedLay revealedLay : Layer) (hne : preservedLay ≠ revealedLay)
    (position : Position) (hposition : positionOtsLayer? position = some revealedLay) :
    ResolverLayerViewPreserving preservedLay (revealPosition position) := by
  intro context fuel table cache result hresult
  ...
  | some resolved =>
      ...
      · intro coordinate hcoordinate
        have hcoordinateNe : coordinate ≠ .position position := by
          intro heq
          subst coordinate
          have hcoordinate' : positionOtsLayer? position = some preservedLay := by
            simpa [coordinateOtsLayer?] using hcoordinate
          have hlayers : revealedLay = preservedLay := by
            apply Option.some.inj
            rw [← hposition, ← hcoordinate']
          exact hne hlayers.symm
      ...
      · intro other hother
        ...
        · intro hlayer
          have : preservedLay = revealedLay := by
            apply Option.some.inj
            rw [← hother, hlayer]
          exact hne this
        · exact hresolved
...
theorem resolverLayerViewPreserving_ensureFullChain
    (preservedLay lay : Layer) (tree : TreeIndex) (leafIdx : LeafIndex)
    (chainIdx : ChainIndex) :
    ResolverLayerViewPreserving preservedLay (ensureFullChain lay tree leafIdx chainIdx) := by
  unfold ensureFullChain
  exact (resolverLayerViewPreserving_sequenceFin
    (fun step : ChainStep =>
      ensureCoordinate (.position (.chain lay tree leafIdx chainIdx step)))
    (fun step => resolverLayerViewPreserving_ensureCoordinate preservedLay
      (.position (.chain lay tree leafIdx chainIdx step)))).bind fun _ =>
        resolverLayerViewPreserving_pure preservedLay ()

theorem resolverLayerViewPreserving_ensureOtsLeaf
    (preservedLay lay : Layer) (tree : TreeIndex) (leafIdx : LeafIndex) :
    ResolverLayerViewPreserving preservedLay (ensureOtsLeaf lay tree leafIdx) := by
  unfold ensureOtsLeaf
  exact (resolverLayerViewPreserving_sequenceFin
    (fun chainIdx : ChainIndex => ensureFullChain lay tree leafIdx chainIdx)
    (fun chainIdx => resolverLayerViewPreserving_ensureFullChain preservedLay lay tree leafIdx
      chainIdx)).bind fun _ =>
        resolverLayerViewPreserving_ensureCoordinate preservedLay
          (.position (.leaf lay tree leafIdx))

theorem resolverLayerViewPreserving_ensureTreeNode
    (preservedLay lay : Layer) (tree : TreeIndex) :
    ∀ level nodeIdx,
      ResolverLayerViewPreserving preservedLay (ensureTreeNode lay tree level nodeIdx)
  | 0, nodeIdx =>
      resolverLayerViewPreserving_ensureOtsLeaf preservedLay lay tree (leafOfNat nodeIdx)
  | level + 1, nodeIdx => by
      rw [ensureTreeNode]
      exact (resolverLayerViewPreserving_ensureTreeNode preservedLay lay tree level
        (2 * nodeIdx)).bind fun _ =>
          (resolverLayerViewPreserving_ensureTreeNode preservedLay lay tree level
            (2 * nodeIdx + 1)).bind fun _ => by
              by_cases hlevel : level < maxLayerHeight
              · rw [dif_pos hlevel]
                exact resolverLayerViewPreserving_ensureCoordinate preservedLay
                  (.position (.node lay tree ⟨level, hlevel⟩ (leafOfNat nodeIdx)))
              · rw [dif_neg hlevel]
                exact resolverLayerViewPreserving_pure preservedLay ()

theorem resolverLayerViewPreserving_maskedTreeNode_of_ne
    (preservedLay lay : Layer) (hne : preservedLay ≠ lay)
    (tree : TreeIndex) (level nodeIdx : Nat) :
    ResolverLayerViewPreserving preservedLay (maskedTreeNode lay tree level nodeIdx) := by
  unfold maskedTreeNode
  apply (resolverLayerViewPreserving_ensureTreeNode preservedLay lay tree level nodeIdx).bind
  intro _
  cases level with
  | zero =>
      exact resolverLayerViewPreserving_revealPosition_of_ne preservedLay lay hne
        (.leaf lay tree (leafOfNat nodeIdx)) (by simp [positionOtsLayer?])
  | succ current =>
      by_cases hlevel : current < maxLayerHeight
      · simp only [hlevel, ↓reduceDIte]
      ...
      · simp [hlevel]
        exact resolverLayerViewPreserving_pure preservedLay 0

theorem resolverLayerViewPreserving_maskedTreeRoot_of_ne
    (preservedLay lay : Layer) (hne : preservedLay ≠ lay) (tree : TreeIndex) :
    ResolverLayerViewPreserving preservedLay (maskedTreeRoot lay tree) := by
  unfold maskedTreeRoot
  exact resolverLayerViewPreserving_maskedTreeNode_of_ne preservedLay lay hne tree
    (layerHeight lay) 0

theorem resolverLayerViewPreserving_ensureChainPrefix
    (preservedLay lay : Layer) (tree : TreeIndex) (leafIdx : LeafIndex)
    ...
    (fun step => by
      ...
      · rw [if_pos hstep]
        exact resolverLayerViewPreserving_ensureCoordinate preservedLay
          (.position (.chain lay tree leafIdx chainIdx step))
      · rw [if_neg hstep]
        exact resolverLayerViewPreserving_pure preservedLay ())).bind fun _ =>
          resolverLayerViewPreserving_pure preservedLay ()

theorem resolverLayerViewPreserving_ensureTreePath
    (preservedLay lay : Layer) (tree : TreeIndex) (leafIdx : LeafIndex) :
    ResolverLayerViewPreserving preservedLay (ensureTreePath lay tree leafIdx) := by
  unfold ensureTreePath
  exact (resolverLayerViewPreserving_sequenceFin
    (fun level : Fin maxLayerHeight =>
      if level.val < layerHeight lay then
    ...
    (fun level => by
      by_cases hlevel : level.val < layerHeight lay
      · rw [if_pos hlevel]
        exact resolverLayerViewPreserving_ensureTreeNode preservedLay lay tree level.val
          (Nat.xor (leafIdx.val / 2 ^ level.val) 1)
      · rw [if_neg hlevel]
        exact resolverLayerViewPreserving_pure preservedLay ())).bind fun _ =>
          resolverLayerViewPreserving_pure preservedLay ()

theorem resolverLayerViewPreserving_maskedOtsSignFrom
    (preservedLay lay : Layer) (parameter : PublicParameter) (tree : TreeIndex)
    (leafIdx : LeafIndex) (message : Digest) : ∀ attempts counter,
    ResolverLayerViewPreserving preservedLay
      (maskedOtsSignFrom parameter lay tree leafIdx message attempts counter)
  | 0, counter => by
      rw [maskedOtsSignFrom]
      exact resolverLayerViewPreserving_pure preservedLay none
  | attempts + 1, counter => by
      rw [maskedOtsSignFrom]
      have hencoded := resolverLayerViewPreserving_simulateQ preservedLay ordinaryHashImpl
        (resolverLayerViewPreserving_ordinaryHashImpl preservedLay)
        (encode parameter lay tree leafIdx message
          (BitVec.ofNat counterBits counter))
      apply hencoded.bind
      intro encoded
      cases encoded with
      | none =>
          exact resolverLayerViewPreserving_maskedOtsSignFrom preservedLay lay parameter tree
            leafIdx message attempts (counter + 1)
      | some encoding =>
          apply (resolverLayerViewPreserving_sequenceFin
            (fun chainIdx => ensureChainPrefix lay tree leafIdx chainIdx
              (encoding chainIdx))
            (fun chainIdx => resolverLayerViewPreserving_ensureChainPrefix preservedLay lay tree
              leafIdx chainIdx (encoding chainIdx))).bind
          intro _
          exact resolverLayerViewPreserving_pure preservedLay
            (some (BitVec.ofNat counterBits counter, encoding))

theorem resolverLayerViewPreserving_maskedOtsSign
    (preservedLay lay : Layer) (parameter : PublicParameter) (tree : TreeIndex)
    (leafIdx : LeafIndex) (message : Digest) :
    ResolverLayerViewPreserving preservedLay
      (maskedOtsSign parameter lay tree leafIdx message) := by
  exact resolverLayerViewPreserving_maskedOtsSignFrom preservedLay lay parameter tree leafIdx
    message encodingAttemptLimit 0

theorem resolverLayerViewPreserving_maskedLayerMessage_of_below
    (preservedLay : Layer) (parameter : PublicParameter)
    (ftsSecret : Index → FtsTree → FtsLeaf → Digest) (index : Index)
    (lay : Layer) (hbelow : lay.val + 1 < numLayers)
    (hne : preservedLay ≠ ⟨lay.val + 1, hbelow⟩) :
    ResolverLayerViewPreserving preservedLay
      (maskedLayerMessage parameter ftsSecret index lay) := by
  unfold maskedLayerMessage
  rw [dif_pos hbelow]
  exact resolverLayerViewPreserving_maskedTreeRoot_of_ne preservedLay
    ⟨lay.val + 1, hbelow⟩ hne (treeIndexAt index ⟨lay.val + 1, hbelow⟩)

theorem resolverLayerViewPreserving_maskedLayerMessage_of_not_below
    (preservedLay : Layer) (parameter : PublicParameter)
    (ftsSecret : Index → FtsTree → FtsLeaf → Digest) (index : Index)
    (lay : Layer) (hbelow : ¬lay.val + 1 < numLayers) :
    ResolverLayerViewPreserving preservedLay
  ...
  unfold maskedLayerMessage
  rw [dif_neg hbelow]
  exact resolverLayerViewPreserving_simulateQ preservedLay ordinaryHashImpl
    (resolverLayerViewPreserving_ordinaryHashImpl preservedLay)
    (ftsKey parameter index (ftsSecret index))

theorem resolverLayerViewPreserving_maskedSignLayer_of_below
    (preservedLay : Layer) (parameter : PublicParameter)
    (ftsSecret : Index → FtsTree → FtsLeaf → Digest) (index : Index)
    (lay : Layer) (hbelow : lay.val + 1 < numLayers)
    (hne : preservedLay ≠ ⟨lay.val + 1, hbelow⟩) :
    ResolverLayerViewPreserving preservedLay
      (maskedSignLayer parameter ftsSecret index lay) := by
  unfold maskedSignLayer
  apply (resolverLayerViewPreserving_maskedLayerMessage_of_below preservedLay parameter
    ftsSecret index lay hbelow hne).bind
  intro message
  apply (resolverLayerViewPreserving_maskedOtsSign preservedLay lay parameter
    (treeIndexAt index lay) (leafIndexAt index lay) message).bind
  intro selected
  cases selected with
  | none => exact resolverLayerViewPreserving_pure preservedLay none
  | some selected =>
      apply (resolverLayerViewPreserving_ensureTreePath preservedLay lay
        (treeIndexAt index lay) (leafIndexAt index lay)).bind
      intro _
      exact resolverLayerViewPreserving_pure preservedLay (some selected)

theorem resolverLayerViewPreserving_maskedSignLayer_of_not_below
    (preservedLay : Layer) (parameter : PublicParameter)
    (ftsSecret : Index → FtsTree → FtsLeaf → Digest) (index : Index)
    (lay : Layer) (hbelow : ¬lay.val + 1 < numLayers) :
    ResolverLayerViewPreserving preservedLay
      (maskedSignLayer parameter ftsSecret index lay) := by
  unfold maskedSignLayer
  apply (resolverLayerViewPreserving_maskedLayerMessage_of_not_below preservedLay parameter
    ftsSecret index lay hbelow).bind
  intro message
  apply (resolverLayerViewPreserving_maskedOtsSign preservedLay lay parameter
    (treeIndexAt index lay) (leafIndexAt index lay) message).bind
  intro selected
...
theorem resolverLayerViewPreserving_maskedSignLayer_of_lt
    (preservedLay lay : Layer) (hlt : preservedLay.val < lay.val)
    (parameter : PublicParameter)
    (ftsSecret : Index → FtsTree → FtsLeaf → Digest) (index : Index) :
    ResolverLayerViewPreserving preservedLay
      (maskedSignLayer parameter ftsSecret index lay) := by
...
theorem layerViewEq_resolveDeferredLayerValues_of_ne
    ...
    (resolvedLay observedLay : Layer) (hne : observedLay ≠ resolvedLay)
    (encoding : ChainIndex → Digit) (context finalContext : DeferredContext)
    (values : (ChainIndex → Digest) × (Fin maxLayerHeight → Digest))
    (hresult : some (finalContext, values) ∈ support
      (resolveDeferredLayerValues table index resolvedLay encoding context)) :
  ...
  · intro position hposition
    ...
    · exact hresult
  · intro coordinate hcoordinate
    apply resolveDeferredLayerValues_preserves_other_layer_pendingAt table index resolvedLay
      encoding context finalContext values coordinate
    · intro heq
      rw [hcoordinate] at heq
      exact hne (Option.some.inj heq)
    · exact hresult
...
theorem resolvedRevealComputation_run_eq_finalQuery_of_available
  ...
  classical
  by_cases hresolvable : ResolvableOtsPosition position
  · simpa [resolvedRevealComputation, hresolvable] using
      resolvedPositionComputation_run_eq_finalQuery_of_available parameter table completion
        context cache position input hcompletion hresolvable havailable hclosed hinput
  · simp only [resolvedRevealComputation, hresolvable, ↓reduceIte, oracleHash,
      simulateQ_bind, HasQuery.instOfMonadLift_query, simulateQ_spec_query, simulateQ_pure,
...
set_option maxRecDepth 100000 in
theorem relTriple_runResolvedFromTable_revealPositionOutput_chronological
    (parameter : PublicParameter) (table : OtsSecretIndex → HashOutput)
    (completion : Coordinate → HashOutput) (position : Position) (input : HashInput)
    (context : DeferredContext) (fuel : Nat) (cache : SplitHashCache)
    (concreteCache : QueryCache HashSpec)
  ...
  classical
  by_cases hresolvable : ResolvableOtsPosition position
  · exact relTriple_runResolvedFromTable_revealResolvablePositionOutput_chronological
      parameter table completion position input context fuel cache concreteCache hinvariant
        hcompletion hots hresolvable havailable hclosed hpublished hinput
  · rw [runResolvedFromTable_revealCoordinateOutput]
    have hcanonical : ∀ otherCompletion, DeferredCompletion table context otherCompletion →
        input = tableInput parameter otherCompletion (.position position) := by
      intro otherCompletion hotherCompletion
    ...
    have hresolve := relTriple_resolveDeferredReveal_chronological parameter table position input
      context (ordinaryQueryCache cache) concreteCache hinvariant hots hcanonical
    have hrun := resolvedRevealComputation_run_eq_finalQuery_of_available parameter table
      completion context concreteCache position input hcompletion havailable hclosed hinput
    rw [hrun] at hresolve
    have hsupportedLeft :=
      SphincsSecurity.Concrete.FtsProbeSimulation.relTriple_and_left_support hresolve
        (fun resolved => resolved ∈ support (resolveDeferredReveal table position context))
    ...
    have hcore : RelTriple
      ...
      | some resolved =>
          ...
          have hpositionSupport : (value, finalCache) ∈ support
              ((simulateQ (randomOracle : QueryImpl HashSpec _)
                (resolvedRevealComputation parameter table position input)).run concreteCache) := by
            rw [hrun]
            exact hrightSupport
          have hreplay := replay_of_mem_support
            (resolvedRevealComputation parameter table position input) concreteCache value finalCache
              hpositionSupport (fromCache finalCache) (agreesWithFn_fromCache finalCache)
          have hclosedFinal := hclosed.mono hreplay.1
          have hvisible : VisibleResolvedComputationsCached parameter table
              (materializeResolvedPosition context position resolved) finalCache := by
            ...
            by_cases heq : other = position
            · subst other
              exact (hresolvable hotherResolvable).elim
            · apply hclosedFinal other output hotherResolvable
              simpa [materializeResolvedPosition, LazyRevealProbe.State.materialize, heq] using
                hvisibleValue
          by_cases hcompletable : DeferredCompletable table
              (materializeResolvedPosition context position resolved)
          · have hcanonicalAfter : ∀ otherCompletion,
                DeferredCompletion table resolved.toDeferredContext otherCompletion →
                  input = tableInput parameter otherCompletion (.position position) := by
              intro otherCompletion hotherCompletion
              have horiginal := hotherCompletion.of_resolveDeferredReveal
                hinvariant.2.1.valuesConsistent hinvariant.2.2.1 position resolved hresultSupport
              have hotherAvailable := havailable.changeTable horiginal.1
              exact hinput.trans (tableInput_eq_of_available parameter completion otherCompletion
                context.state (.position position) havailable hotherAvailable)
            have hcached : finalCache input = some resolved.output :=
              hresultInvariant.concreteCache_eq_of_positionValue position hots resolved.output
                hposition input hcanonicalAfter
            refine Or.inl ⟨rfl, ?_, ?_, hvisible, ?_⟩
            · simp [hcached]
            · rw [ordinaryQueryCache_update_hidden]
              exact hinvariant.materialize_resolvedReveal position resolved hresultSupport
                hresultInvariant hcompletable
            · simpa [materializeResolvedPosition] using
                hpublished.materialize (.position position) resolved.output
          · refine Or.inr ⟨rfl, ?_⟩
    ...
    have hrecover : recover <$> (((randomOracle input).run concreteCache >>= fun result =>
      ...
      | some output =>
          ...
          simp [recover, hcache]
    rw [hrecover] at hmapped
    simpa [materializeResolvedPosition] using hmapped
...
set_option maxRecDepth 100000 in
theorem relTriple_runResolvedFromTable_resolveKnownInput_completionCanonicalAny_reachable
    (parameter : PublicParameter) (table : OtsSecretIndex → HashOutput)
    (completion : Coordinate → HashOutput) (position : Position) (input : HashInput)
    (context : DeferredContext) (fuel : Nat) (cache : SplitHashCache)
    (concreteCache : QueryCache HashSpec)
  ...
  · unfold resolveKnownInput
    ...
    rw [StateT.run_bind, runResolvedFromTable_bind]
    have hreveal :=
      relTriple_runResolvedFromTable_revealPositionOutput_chronological parameter table
        completion position input context fuel cache concreteCache hinvariant hcompletion hots
          havailable hclosed hpublished hinput
    have hsupportedLeft :=
      SphincsSecurity.Concrete.FtsProbeSimulation.relTriple_and_left_support hreveal
        (fun result => result ∈ support
          (runResolvedFromTable context fuel table
...
set_option maxRecDepth 100000 in
theorem relTriple_runResolvedFromTable_resolveKnownInput_availableDecodedAny_reachable
    (parameter : PublicParameter) (table : OtsSecretIndex → HashOutput)
    (position : Position) (input : HashInput)
    (context : DeferredContext) (fuel : Nat) (cache : SplitHashCache)
    (concreteCache : QueryCache HashSpec)
    ...
    RelTriple
      ...
      (ReachableResolvedRunRel parameter table) := by
  by_cases hinput : tableInput parameter completion (.position position) = input
  · exact relTriple_runResolvedFromTable_resolveKnownInput_completionCanonicalAny_reachable
      parameter table completion position input context fuel cache concreteCache hinvariant
        hcompletion hots havailable hclosed hpublished hinput.symm
  · exact relTriple_runResolvedFromTable_resolveKnownInput_completionOrdinary_reachable
      parameter table (.position position) input context fuel cache concreteCache hinvariant hclosed
        hpublished (fun other heq => by cases heq; exact hots)
        (completionOrdinaryInput_of_available_decoded_ne hdecode havailable hinput)
...
theorem exists_decodePosition?_of_decodeProbe?
    (parameter : PublicParameter) (input : HashInput) (candidate : Probe)
    (hprobe : decodeProbe? parameter input = some candidate) :
    ∃ position : Position,
      decodePosition? parameter input = some position ∧
        candidate.outputCoordinate = .position position := by
  have hmatches := (decodeProbe?_eq_some_iff parameter input candidate).1 hprobe
  rcases candidate with ⟨coordinate, candidateDigest⟩
  cases coordinate with
  | chainStart lay tree leafIdx chainIdx =>
      rcases hmatches with ⟨step, hstep, hinput⟩
      have hzero : step = ⟨0, by norm_num [chainLength, winternitzBits]⟩ := Fin.ext hstep
      subst step
      refine ⟨.chain lay tree leafIdx chainIdx
        ⟨0, by norm_num [chainLength, winternitzBits]⟩, ?_, rfl⟩
      exact (decodePosition?_eq_some_iff parameter _ _).2 ⟨digestBytes candidateDigest, hinput⟩
  | position source =>
      cases source with
      | chain lay tree leafIdx chainIdx step =>
          simp only [Probe.MatchesInput] at hmatches
          ...
          · rw [dif_pos hnext] at hmatches
            rcases hmatches with ⟨nextStep, hnextStep, hinput⟩
            have hstepEq : nextStep = ⟨step.val + 1, hnext⟩ := Fin.ext hnextStep
            subst nextStep
            refine ⟨.chain lay tree leafIdx chainIdx ⟨step.val + 1, hnext⟩, ?_, ?_⟩
            · exact (decodePosition?_eq_some_iff parameter _ _).2
                ⟨digestBytes candidateDigest, hinput⟩
            · simp [Probe.outputCoordinate, hnext]
          · rw [dif_neg hnext] at hmatches
            rcases hmatches with ⟨_hchain, payload, hinput, _hslot⟩
            refine ⟨.leaf lay tree leafIdx, ?_, ?_⟩
            · exact (decodePosition?_eq_some_iff parameter _ _).2 ⟨payload, hinput⟩
            · simp [Probe.outputCoordinate, hnext]
      | leaf | node | ftsLeaf | ftsNode | ftsRoots =>
          simp [Probe.MatchesInput] at hmatches

theorem decodePosition?_chain_or_leaf_of_decodeProbe?
    (parameter : PublicParameter) (input : HashInput) (candidate : Probe)
    (hprobe : decodeProbe? parameter input = some candidate) :
    (∃ lay tree leafIdx chainIdx step,
      decodePosition? parameter input = some (.chain lay tree leafIdx chainIdx step)) ∨
    ∃ lay tree leafIdx,
      decodePosition? parameter input = some (.leaf lay tree leafIdx) := by
  have hmatches := (decodeProbe?_eq_some_iff parameter input candidate).1 hprobe
  rcases candidate with ⟨coordinate, candidateDigest⟩
  cases coordinate with
  | chainStart lay tree leafIdx chainIdx =>
      rcases hmatches with ⟨step, _hstep, hinput⟩
      exact Or.inl ⟨lay, tree, leafIdx, chainIdx, step,
        (decodePosition?_eq_some_iff parameter _ _).2 ⟨digestBytes candidateDigest, hinput⟩⟩
  | position source =>
      cases source with
      | chain lay tree leafIdx chainIdx step =>
          simp only [Probe.MatchesInput] at hmatches
          by_cases hnext : step.val + 1 < chainLength - 1
          · rw [dif_pos hnext] at hmatches
            rcases hmatches with ⟨nextStep, _hnextStep, hinput⟩
            exact Or.inl ⟨lay, tree, leafIdx, chainIdx, nextStep,
              (decodePosition?_eq_some_iff parameter _ _).2
                ⟨digestBytes candidateDigest, hinput⟩⟩
          · rw [dif_neg hnext] at hmatches
            rcases hmatches with ⟨_hchain, payload, hinput, _hslot⟩
            exact Or.inr ⟨lay, tree, leafIdx,
              (decodePosition?_eq_some_iff parameter _ _).2 ⟨payload, hinput⟩⟩
      | leaf | node | ftsLeaf | ftsNode | ftsRoots =>
          simp [Probe.MatchesInput] at hmatches
...
set_option maxHeartbeats 800000 in
set_option maxRecDepth 100000 in
theorem relTriple_runResolvedFromTable_probingHashQuery_node_reachable
    (parameter : PublicParameter) (table : OtsSecretIndex → HashOutput)
  ...
  · let coordinates := (Position.node lay tree level nodeIdx).children.map Coordinate.position
    ...
    rw [runResolvedFromTable_probeFirstMissingInputCoordinate_of_values completion input
      context cache fuel table 0 coordinates hcoordinateValues]
    simp only [pure_bind]
    exact relTriple_runResolvedFromTable_resolveKnownInput_availableDecodedAny_reachable
      parameter table (.node lay tree level nodeIdx) input context fuel cache concreteCache
        hinvariant hclosed hpublished (by simp [IsOtsPosition]) hposition completion hcompletion
          havailable
  · let priorCoordinates := prior.map Coordinate.position
    let remainingCoordinates := remaining.map Coordinate.position
    have hcoordinates : (Position.node lay tree level nodeIdx).children.map Coordinate.position =
        priorCoordinates ++ .position child :: remainingCoordinates := by
      simp [hchildren, priorCoordinates, remainingCoordinates]
    have hcoordinateValues : ∀ coordinate, coordinate ∈ priorCoordinates →
        context.state.values coordinate = some (completion coordinate) := by
    ...
    | succ remainingFuel =>
        ...
        · have hprobeInvariant := hinvariant.addPending_of_completable (.position child)
          ...
          have hpending : (.position child, slotDigest prior.length input) ∈
              probeContext.state.pending := by
            simp [probeContext, LazyRevealProbe.State.addPending]
          have hordinary := completionOrdinaryInput_of_pending_node_child (table := table)
            hposition hchildren hpending
          exact relTriple_runResolvedFromTable_resolveKnownInput_completionOrdinary_reachable
            parameter table (.position (.node lay tree level nodeIdx)) input probeContext
              remainingFuel cache concreteCache hprobeInvariant hprobeClosed hprobePublished
                (fun position heq => by cases heq; simp [IsOtsPosition]) hordinary
        · have hdoomed : DoomedResolvedContext table probeContext := ⟨
            hinvariant.2.1.valuesConsistent.addPending (.position child)
...
set_option maxRecDepth 100000 in
theorem reachableResolvedCouples_probingHashQuery
    (parameter : PublicParameter) (table : OtsSecretIndex → HashOutput)
    (input : HashInput) :
    ReachableResolvedCouples parameter table (probingHashQuery parameter input)
      (randomOracle input) := by
  ...
  | some candidate =>
      ...
      · exact relTriple_runResolvedFromTable_probingHashQuery_chain_reachable parameter table
          input candidate lay tree leafIdx chainIdx step hprobe hposition context fuel cache
            concreteCache hinvariant hclosed hpublished
      · exact relTriple_runResolvedFromTable_probingHashQuery_leaf_reachable parameter table
          input candidate lay tree leafIdx hprobe hposition context fuel cache concreteCache
            hinvariant hclosed hpublished
  | none =>
      cases hposition : decodePosition? parameter input with
      | none =>
          have hstable : StableOrdinaryInput parameter input := ⟨hprobe, by
            intro position hdecode _hots
            rw [hposition] at hdecode
            simp at hdecode⟩
          exact reachableResolvedCouples_probingHashQuery_of_stable parameter table input hstable
      ...
      | some position =>
              ...
              exact relTriple_runResolvedFromTable_resolveKnownInput_completionOrdinary_reachable
                parameter table (.position (.leaf lay tree leafIdx)) input context fuel cache
                  concreteCache hinvariant hclosed hpublished
                    (fun other heq => by cases heq; simp [IsOtsPosition]) hordinary
          | node lay tree level nodeIdx =>
              exact relTriple_runResolvedFromTable_probingHashQuery_node_reachable parameter table
                input lay tree level nodeIdx hprobe hposition context fuel cache concreteCache
                  hinvariant hclosed hpublished
          | ftsLeaf index tree leafIdx =>
              have hstable : StableOrdinaryInput parameter input := ⟨hprobe, by
                intro other hdecode hots
                rw [hposition] at hdecode
                have heq : other = .ftsLeaf index tree leafIdx := Option.some.inj hdecode.symm
                subst other
                simpa [IsOtsPosition] using hots⟩
              exact reachableResolvedCouples_probingHashQuery_of_stable parameter table input
                hstable context fuel cache concreteCache hinvariant hclosed hpublished
          | ftsNode index tree level nodeIdx =>
              have hstable : StableOrdinaryInput parameter input := ⟨hprobe, by
                intro other hdecode hots
                rw [hposition] at hdecode
                have heq : other = .ftsNode index tree level nodeIdx :=
          ...
          | ftsRoots index =>
              have hstable : StableOrdinaryInput parameter input := ⟨hprobe, by
                intro other hdecode hots
                rw [hposition] at hdecode
                have heq : other = .ftsRoots index := Option.some.inj hdecode.symm
                subst other
                simpa [IsOtsPosition] using hots⟩
              exact reachableResolvedCouples_probingHashQuery_of_stable parameter table input
                hstable context fuel cache concreteCache hinvariant hclosed hpublished
