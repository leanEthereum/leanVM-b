...
theorem FewTimeCover.precached_entry_has_earlier_exact_source
    ...
    ∃ (source selected : Fin result.2.2.intervals.length),
      source.val < selected.val
        ∧ AdversaryCacheEntry.signingEntry? (result.2.2.intervals.get selected) =
          some (cover.cacheEntry result.2.2.signing rfl entry)
        ∧ ((result.2.2.intervals.take selected.val).filterMap
          AdversaryCacheEntry.signingEntry?).length = (cover.logIndex entry).val
        ∧ (result.2.2.intervals.get source).initialCache
            (cover.entryDigestInput entry) = none
        ∧ (result.2.2.intervals.get source).finalCache
            (cover.entryDigestInput entry) ≠ none
  ...
  have hselectedSigning : result.2.2.signing.get selectedSigning =
    ...
    rfl
  obtain ⟨selected, hselected, hselectedRank, hearlier⟩ :=
  ...
  have hkind := FullAdversaryTrace.transition_source_kind hvalid
    (result.2.2.intervals.get source) (List.get_mem _ source)
    (cover.entryDigestInput entry) hmiss hhit
  refine ⟨source, selected, hlt, hselected', hselectedRank, hmiss, hhit, ?_⟩
...
theorem FewTimeCover.precached_entry_has_earlier_direct_source
    ...
    ∃ (source selected : Fin result.2.2.intervals.length),
      source.val < selected.val
        ...
        ∧ (result.2.2.intervals.get source).initialCache
          (cover.entryDigestInput entry) = none := by
  obtain ⟨source, selected, hlt, hselected, hselectedRank, hsourceMiss, hsourceHit,
      hkind⟩ :=
  ...
  · exact ⟨source, selected, hlt, hselected, hselectedRank, hdirect, hsourceMiss⟩
  · exfalso
...
abbrev FewTimeCover.PrecachedEntries {f : QueryImpl HashSpec Id}
...
noncomputable def FewTimeCover.precachedEntryFinset {f : QueryImpl HashSpec Id}
    {cache : QueryCache HashSpec} {secretKey : SecretKey}
    {signingLog : QueryLog SigningSpec} {index : Index}
    {targetLeaves : DigestTree → FtsLeaf}
    (cover : FewTimeCover f cache secretKey signingLog index targetLeaves)
    (trace : SigningCacheTrace) (hlog : trace.toSigningLog = signingLog) :
    Finset cover.entries := by
  classical
  exact Finset.univ.filter fun entry => cover.EntryDigestPrecached trace hlog entry

theorem adversaryIntervals_directHashCount
    (intervals : List AdversaryCacheEntry) :
    Fin.countP (fun position =>
        isDirectHashQuery (intervals.get position).input) =
  ...
  | cons entry rest ih =>
      rw [Fin.countP_succ]
      have htail : Fin.countP (fun position : Fin rest.length =>
          isDirectHashQuery ((entry :: rest).get position.succ).input) =
          Fin.countP (fun position => isDirectHashQuery (rest.get position).input) := by
        rfl
      rw [htail, ih]
      cases hinput : entry.input with
      | inl worldInput =>
          cases worldInput with
          | inl uniformInput =>
              simp [isDirectHashQuery, directHashQueries, AdversaryCacheEntry.queryEntry,
                hinput]
          | inr hashInput =>
              ...
              simp [isDirectHashQuery, directHashQueries, AdversaryCacheEntry.queryEntry,
                Nat.add_comm]
      | inr request =>
          simp [isDirectHashQuery, directHashQueries, AdversaryCacheEntry.queryEntry, hinput]
...
theorem FewTimeCover.precached_entry_has_fresh_direct_view_source
  ...
  obtain ⟨source, selected, hsourceLt, hselected, hselectedRank, hsourceInput,
      hsourceMiss⟩ :=
  ...
  refine ⟨source, selected, output, hsourceLt, hselected, hselectedRank, hsourceInput,
    hsourceMiss, hsourceRun', hadmissible, ?_⟩
  apply Prod.ext
...
theorem FewTimeCover.precached_entries_have_injective_fresh_direct_view_sources
      ...
      Function.Injective source
        ∧ ∀ entry,
          (source entry).val < (selected entry).val
            ...
            ∧ ((result.2.2.intervals.take (selected entry).val).filterMap
  ...
  have hleft := (hsource left).2.2.2.1
