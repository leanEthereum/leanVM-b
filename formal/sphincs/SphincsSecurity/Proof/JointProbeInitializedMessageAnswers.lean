...
import SphincsSecurity.Proof.JointProbeOriginalLiveMessageAnswers
...
theorem probEvent_initialized_live_message_value_le_fullShared
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop)
    (parameter : PublicParameter) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    (computation : Digest → OracleComp (OracleWorld + SigningSpec) α) (event : α → (HashInput → Option HashOutput) → Prop) (q fuel : Nat)
    ...
    (∑' initial, Pr[= initial | initializeRoot parameter otsTable ftsTable q fuel] *
      Pr[fun result => event result.1.2.1.1 (messageAnswers parameter result.1.2.1.2) ∧ result.1.2.2 = false ∧ result.2 = false |
    ...
    Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) | AdaptiveRevealProbe.runDetailed ftsTable AdaptiveRevealProbe.State.empty q
  ...
  calc
    _ ≤ ∑' raw, Pr[= raw | (rootCoupling parameter otsTable ftsTable q fuel).1] *
        Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) | resumeJointResolved ftsTable q continuation raw.1] := by
      ...
      · apply mul_le_mul' le_rfl
        ...
        | none =>
            ...
            rw [probEvent_runWithFailure_live_message_failed_eq_zero]
            exact zero_le
        | some frame =>
            ...
            apply probEvent_runWithFailure_live_message_value_le_fullShared exception parameter raw.2.1 otsTable ftsTable
    ...
    _ = _ := by
      have hm := tsum_probOutput_map_mul (rootCoupling parameter otsTable ftsTable q fuel).1 Prod.fst
        (fun result => Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) | resumeJointResolved ftsTable q continuation result])
...
theorem probEvent_sampled_initialized_live_message_value_le_erasedHistory
      ...
      ∑' initial, Pr[= initial | initializeRoot parameter otsTable ftsTable q fuel] *
        Pr[fun result => event result.1.2.1.1 (messageAnswers parameter result.1.2.1.2) ∧ result.1.2.2 = false ∧ result.2 = false |
          runWithFailure (exception otsTable) parameter initial.2.1 otsTable ftsTable (computation initial.2.1)
            initial.1 initial.2.2 false initial.1.isNone]) ≤
    Pr[JointHistoryReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) | AdaptiveRevealProbe.runDetailed ftsTable AdaptiveRevealProbe.State.empty q
  ...
  calc
    _ ≤ ∑' otsTable, Pr[= otsTable | OtsProbeSimulation.sampleOtsHashTable] *
      ...
      exact mul_le_mul' le_rfl
        (probEvent_initialized_live_message_value_le_fullShared (exception otsTable) parameter otsTable ftsTable computation event q fuel hbound)
    _ ≤ _ := probEvent_sampled_jointReturned_le_erasedHistory ∅ source (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) ftsTable AdaptiveRevealProbe.State.empty q fuel
