...
theorem probEvent_runWithFailure_live_message_failed_eq_zero
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop)
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    (computation : OracleComp (OracleWorld + SigningSpec) α) (event : α → (HashInput → Option HashOutput) → Prop)
...
theorem probEvent_runWithFailure_live_message_value_le_fullShared
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop)
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    (computation : OracleComp (OracleWorld + SigningSpec) α) (event : α → (HashInput → Option HashOutput) → Prop) (frame : Frame) (cache : QueryCache HashSpec)
    ...
    Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) | AdaptiveRevealProbe.runDetailed ftsTable frame.state frame.ftsFuel
  ...
  | pure value =>
      ...
      have hanswers := hvalid.messageAnswers
      rw [hbody, runJointResolved_pure, withNativeOrdinaryCache_prepare]
      simp [runWithFailure_pure, AdaptiveRevealProbe.runDetailed,
        hvalid.1, JointReturned, cleanJointResolved, hanswers]
  | query_bind input next ih =>
      ...
      change _ ≤ Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) | AdaptiveRevealProbe.runDetailed ftsTable frame.state frame.ftsFuel
      ...
      calc
        _ ≤ ∑' raw, Pr[= raw | (queryCoupling exception parameter root otsTable ftsTable input frame cache false he).1] *
            Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) |
        ...
        _ = _ := by
          have hm := tsum_probOutput_map_mul
            ...
            (fun result => Pr[JointReturned (fun value => event value.1 (messageAnswers parameter (ordinaryQueryCache value.2.2))) |
