import SphincsSecurity.Proof.SharedParentRefundStep
...
theorem expected_collisionPotential_add_sharedParentDiscard_run_le
  ...
  | query_bind input next ih =>
      ...
      calc
        _ = (∑' head, Pr[= head | stepWithFailure exception parameter root otsTable ftsTable input (some frame) cache false false] *
            ...
            sharedFailureDiscardStep exception reserve parameter root otsTable ftsTable input (some frame) cache false false * eps := by
        ...
        _ = ((∑' head, Pr[= head | stepWithFailure exception parameter root otsTable ftsTable input (some frame) cache false false] *
              collisionSurvivingStructuralPotential key head.1.2.1.2 head.1.2.2 head.2) +
            sharedFailureDiscardStep exception reserve parameter root otsTable ftsTable input (some frame) cache false false * eps) +
        ...
        _ ≤ (collisionStructuralRecordPotential key cache none +
            ...
            (∑' head, Pr[= head | stepWithFailure exception parameter root otsTable ftsTable input (some frame) cache false false] *
              expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable (next head.1.2.1.1)
                head.1.1 head.1.2.1.2 head.1.2.2 head.2) * eps :=
          add_le_add (expected_collisionPotential_add_sharedParentDiscard_step_le parameter root otsTable ftsTable input frame cache hfinite he hcomputed) le_rfl
        _ = _ := by
