...
theorem expected_adaptive_cappedCachedTargetEnvelope_le {α : Type} (key : SecretKey) (q : Nat) (hq : q ≤ 2 ^ 127)
  ...
  | pure value => simp only [simulateQ_pure, StateT.run_pure, tsum_probOutput_pure_mul, expectedMacroIndexCharge_pure, mul_zero, add_zero, le_refl]
  | query_bind input next ih =>
      ...
      have hstep := expected_logTraced_cappedCachedTargetEnvelope_le key q hq state hsigned hbefore input
      ...
      calc
        _ ≤ ∑' result, Pr[= result | (logTracedMappedAdversaryImpl key input).run state] *
            (cappedCachedTargetEnvelope key q result.2 groups remaining + arrival * expectedMacroIndexCharge key q groups remaining (next result.1) result.2) := by
        ...
        _ = (∑' result, Pr[= result | (logTracedMappedAdversaryImpl key input).run state] *
            ...
            arrival * ∑' result, Pr[= result | (logTracedMappedAdversaryImpl key input).run state] *
              expectedMacroIndexCharge key q groups remaining (next result.1) result.2 := by
        ...
        _ ≤ (cappedCachedTargetEnvelope key q state groups remaining +
            (signingMacroHashCost input : ENNReal) * (arrival * cappedRawIndexCacheEnvelope key q state groups remaining)) +
            arrival * ∑' result, Pr[= result | (logTracedMappedAdversaryImpl key input).run state] *
              expectedMacroIndexCharge key q groups remaining (next result.1) result.2 := add_le_add hstep le_rfl
        _ = _ := by
