...
noncomputable def cappedTargetCacheEnvelope (key : SecretKey) (q : Nat) (payload : HashInput) (target : FewTimeView) (state : CoverLogState) (groups : Finset (Finset FtsTree)) (remaining : Finset FtsTree) : ENNReal :=
  if SigningTranscript.Valid state.2 then targetCacheEnvelope key q payload target (signatureLimit - state.2.length) state groups remaining else 0

theorem expected_logTraced_cappedTargetCacheEnvelope_le (key : SecretKey) (q : Nat) (hq : q ≤ 2 ^ 127) (payload : HashInput) (target : FewTimeView)
    ...
    (∑' result, Pr[= result | (logTracedMappedAdversaryImpl key input).run state] *
      cappedTargetCacheEnvelope key q payload target result.2 groups remaining) ≤ cappedTargetCacheEnvelope key q payload target state groups remaining := by
  by_cases hactive : ValidSigningStep state.2 input
  · rw [cappedTargetCacheEnvelope, if_pos hactive.valid_before]
    cases input with
    | inl world =>
        apply le_trans ?_ (expected_logTraced_world_targetCacheEnvelope_le key q payload target (signatureLimit - state.2.length) state world hsigned hcap groups remaining hvalid)
        ...
        · apply mul_le_mul' le_rfl
          ...
          unfold cappedTargetCacheEnvelope
          split_ifs
          · rw [hlog]
          · exact bot_le
    ...
    | inr message =>
        ...
        rw [hremaining]
        apply le_trans ?_ (expected_logTraced_sign_targetCacheEnvelope_le key q payload target _ hq state hsigned hcache message hcap groups remaining hvalid)
        apply ENNReal.tsum_le_tsum
        intro result
  ...
  · have hzero : (∑' result, Pr[= result | (logTracedMappedAdversaryImpl key input).run state] *
        cappedTargetCacheEnvelope key q payload target result.2 groups remaining) = 0 := by
      ...
      · have hinvalid : ¬ SigningTranscript.Valid result.2.2 := fun h =>
        ...
        rw [cappedTargetCacheEnvelope, if_neg hinvalid, mul_zero]
      · rw [probOutput_eq_zero_of_not_mem_support hresult, zero_mul]
    rw [hzero]
    exact bot_le

theorem expected_adaptive_cappedTargetCacheEnvelope_le {α : Type} (key : SecretKey) (q : Nat) (hq : q ≤ 2 ^ 127) (payload : HashInput) (target : FewTimeView)
  ...
  | query_bind input next ih =>
      ...
      have hstep := expected_logTraced_cappedTargetCacheEnvelope_le key q hq payload target state hsigned hbefore input
...
theorem expected_adaptive_validTargetShape_le {α : Type} (key : SecretKey) (q : Nat) (hq : q ≤ 2 ^ 127) (payload : HashInput) (target : FewTimeView)
    (computation : OracleComp (OracleWorld + SigningSpec) α) (state : CoverLogState)
    (hsigned : SigningDigestsCached key.parameter state.1 key.root state.2)
    (hbudget : ∀ result ∈ support ((simulateQ (logTracedMappedAdversaryImpl key) computation).run state),
      QueryCache.enncard result.2.1 ≤ q) (groups : Finset (Finset FtsTree)) (remaining : Finset FtsTree) (hvalid : TargetShapeValid groups remaining) :
    (∑' result, Pr[= result | (simulateQ (logTracedMappedAdversaryImpl key) computation).run state] *
      (if SigningTranscript.Valid result.2.2 then observedTargetShapeVector key payload target result.2 groups remaining else 0)) ≤
      cappedTargetCacheEnvelope key q payload target state groups remaining := by
  apply le_trans ?_ (expected_adaptive_cappedTargetCacheEnvelope_le key q hq payload target computation state hsigned hbudget groups remaining hvalid)
  apply ENNReal.tsum_le_tsum
  intro result
  apply mul_le_mul' le_rfl
  unfold cappedTargetCacheEnvelope
