...
theorem gameAfterSecrets_eq (segment : OtsPrefix) (oracle : QueryImpl HashSpec Id)
    (otsSecret : Layer → TreeIndex → LeafIndex → ChainIndex → Digest)
    (ftsSecret : Index → FtsTree → FtsLeaf → Digest) (dummy : OtsReferenceWords) (adversary : Adversary)
    (hword : let root := evalWithAnswerFn oracle (treeRoot segment.parameter topLayer rootTree (otsSecret topLayer rootTree))
      let key : SecretKey := ⟨segment.parameter, root, otsSecret, ftsSecret⟩
      segment.digit.val ≤ (canonicalReferenceWords key oracle dummy segment.lay segment.tree segment.leaf segment.chainIdx).val) :
    let root := evalWithAnswerFn oracle (treeRoot segment.parameter topLayer rootTree (otsSecret topLayer rootTree))
    let key : SecretKey := ⟨segment.parameter, root, otsSecret, ftsSecret⟩
    let words := canonicalReferenceWords key oracle dummy
    fixedBoundaryRun segment.parameter oracle (gameAfterSecrets adversary segment.parameter otsSecret ftsSecret) =
      simulateQ (segment.fixedImpl (segment.lows oracle))
        (segment.game (segment.highs oracle) oracle ftsSecret words (canonicalFrontierValues key oracle words) adversary) := by
  dsimp only
  rw [fixedImpl_game_original segment oracle ftsSecret _ hword]
  exact fixedBoundaryRun_gameAfterSecrets_canonical adversary segment.parameter otsSecret ftsSecret oracle dummy

theorem hashImpl_answer (segment : OtsPrefix) (high : segment.Query → High)
    (tables : Fin segment.digit.val → Digest → Digest) (privateHigh : segment.Query → High)
    (outside : QueryImpl HashSpec Id) :
    segment.hashImpl high (segment.answer tables privateHigh outside) = segment.hashImpl high outside := by
  funext bytes
  cases hparse : segment.parse bytes <;> simp only [hashImpl, answer, hparse]

theorem worldImpl_answer (segment : OtsPrefix) (high : segment.Query → High)
    (tables : Fin segment.digit.val → Digest → Digest) (privateHigh : segment.Query → High)
    (outside : QueryImpl HashSpec Id) :
    segment.worldImpl high (segment.answer tables privateHigh outside) = segment.worldImpl high outside := by
  funext input
  cases input with
  | inl input => rfl
  | inr bytes => exact congrFun (segment.hashImpl_answer high tables privateHigh outside) bytes

theorem adversaryImpl_answer (segment : OtsPrefix) (root : Digest) (high : segment.Query → High)
    (tables : Fin segment.digit.val → Digest → Digest) (privateHigh : segment.Query → High)
    (outside : QueryImpl HashSpec Id) (ftsSecret : Index → FtsTree → FtsLeaf → Digest)
    (words : OtsReferenceWords)
    (hword : segment.digit.val ≤ (words segment.lay segment.tree segment.leaf segment.chainIdx).val)
    (frontier : OtsFrontierValues) :
    segment.adversaryImpl root high (segment.answer tables privateHigh outside) ftsSecret words frontier =
      segment.adversaryImpl root high outside ftsSecret words frontier := by
  funext input
  cases input with
  | inl input =>
      change (segment.worldImpl high (segment.answer tables privateHigh outside)).withTrace _ input =
        (segment.worldImpl high outside).withTrace _ input
      rw [worldImpl_answer]
  | inr message =>
      rw [adversaryImpl_signing, adversaryImpl_signing, segment.mask_answer words hword tables privateHigh outside]

theorem gameRest_answer (segment : OtsPrefix) (root : Digest) (high : segment.Query → High)
    (tables : Fin segment.digit.val → Digest → Digest) (privateHigh : segment.Query → High)
    (outside : QueryImpl HashSpec Id) (ftsSecret : Index → FtsTree → FtsLeaf → Digest)
    (words : OtsReferenceWords)
    (hword : segment.digit.val ≤ (words segment.lay segment.tree segment.leaf segment.chainIdx).val)
    (frontier : OtsFrontierValues) (adversary : Adversary) :
    segment.gameRest root high (segment.answer tables privateHigh outside) ftsSecret words frontier adversary =
      segment.gameRest root high outside ftsSecret words frontier adversary := by
  simp only [gameRest, adversaryRun, boundary, segment.worldImpl_answer high tables privateHigh outside,
    segment.adversaryImpl_answer root high tables privateHigh outside ftsSecret words hword frontier]

theorem game_answer (segment : OtsPrefix) (high : segment.Query → High)
    (tables : Fin segment.digit.val → Digest → Digest) (privateHigh : segment.Query → High)
    (outside : QueryImpl HashSpec Id) (ftsSecret : Index → FtsTree → FtsLeaf → Digest)
    (words : OtsReferenceWords)
    (hword : segment.digit.val ≤ (words segment.lay segment.tree segment.leaf segment.chainIdx).val)
    (frontier : OtsFrontierValues) (adversary : Adversary) :
    segment.game high (segment.answer tables privateHigh outside) ftsSecret words frontier adversary =
      segment.game high outside ftsSecret words frontier adversary := by
  rw [game, game, segment.mask_answer words hword tables privateHigh outside,
    segment.gameRest_answer _ high tables privateHigh outside ftsSecret words hword frontier adversary]
