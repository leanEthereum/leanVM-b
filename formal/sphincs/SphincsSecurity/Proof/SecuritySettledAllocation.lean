import SphincsSecurity.Proof.SampledSettledEncodingReserve
...
theorem forgeAdvantage_add_terminal_message_signing_settledReserves_le_live
    (adversary : Adversary) (q : Nat) (hq : HasHashQueryBound scheme adversary q) (hqMax : q ≤ 2 ^ 127) :
    forgeAdvantage scheme adversary + sampledStructuralTerminalReserve adversary q (q + 1) +
      (sampledSelectedJointQueryCharge MessageHashInput adversary q + sampledBeforeFailureHashCharge messageHashCharge adversary q (q + 1) +
        sampledSigningNonEncodingReserve adversary q (q + 1) + sampledSettledEncodingReserve adversary q (q + 1)) * (Fintype.card Digest : ENNReal)⁻¹ ≤
      (sampledBeforeFailureRestHashCharge adversary q (q + 1) + (q : ENNReal)) * (Fintype.card Digest : ENNReal)⁻¹ +
      (q : ENNReal) / ((2 ^ 216 : Nat) : ENNReal) + sampledLiveNonSecretResidual adversary q (q + 1) := by
  ...
  have hstructural := (sampledBeforeFailureStructural_add_signingNonEncoding_add_settledEncoding_le adversary q (q + 1)).trans
    (add_le_add le_rfl (sampledBeforeFailureSelectedCharge_le_erased NonMessageNonSecretHashInput adversary q (q + 1)))
  have hbound : forgeAdvantage scheme adversary + sampledStructuralTerminalReserve adversary q (q + 1) +
      (sampledSigningNonEncodingReserve adversary q (q + 1) + sampledSettledEncodingReserve adversary q (q + 1) + sampledJointNonSecretQueryCharge adversary q) * (Fintype.card Digest : ENNReal)⁻¹ ≤
    ...
    apply (add_le_add (forgeAdvantage_add_terminalReserve_le_sharedFailure_add_structural_add_live_residual adversary q hq (q + 1)) le_rfl).trans
    calc
      _ = (sampledBeforeFailureStructuralCharge adversary q (q + 1) + sampledSigningNonEncodingReserve adversary q (q + 1) + sampledSettledEncodingReserve adversary q (q + 1)) *
            (Fintype.card Digest : ENNReal)⁻¹ +
...
theorem forgeAdvantage_add_terminal_message_signing_settled_executionReserves_remainder_gap_le_initialEnvelope
    (adversary : Adversary) (q : Nat) (hq : HasHashQueryBound scheme adversary q) (hqMax : q ≤ 2 ^ 127) :
    forgeAdvantage scheme adversary + sampledStructuralTerminalReserve adversary q (q + 1) +
      (sampledSelectedJointQueryCharge MessageHashInput adversary q + sampledBeforeFailureHashCharge messageHashCharge adversary q (q + 1) +
        sampledSigningNonEncodingReserve adversary q (q + 1) + sampledSettledEncodingReserve adversary q (q + 1)) * (Fintype.card Digest : ENNReal)⁻¹ +
  ...
  have hbase := (forgeAdvantage_add_terminal_message_signing_settledReserves_le_live adversary q hq hqMax).trans
