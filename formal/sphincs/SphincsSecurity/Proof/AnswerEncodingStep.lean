...
theorem encodingMessageTargets_for_settling_query
  ...
  · obtain ⟨position, index, htree, hleaf, heq⟩ := hdirect
    refine ⟨encodingMessageTargets secretKey.parameter cache hfinite position, ?_, ?_⟩
...
theorem answerEncoding_step_of_settling
    ...
    (hsettles : ∀ answer : HashOutput,
      Settled secretKey.parameter secretKey.otsSecret secretKey.ftsSecret (cache.cacheQuery input answer) queried) :
    ∃ targets : Finset Digest, targets.card ≤ answerEncodingPotential cache secretKey ∧
      ∀ answer : HashOutput,
        ...
        ¬ Bad secretKey.parameter secretKey.otsSecret secretKey.ftsSecret (cache.cacheQuery input answer) ∧
          answerEncodingPotential (cache.cacheQuery input answer) secretKey + targets.card ≤ answerEncodingPotential cache secretKey ∧
  ...
  classical
  obtain ⟨messageTargets, hdirect, hstage⟩ := encodingMessageTargets_for_settling_query hfinite hfresh hat hbefore hsettles
  let targets := answerTargets secretKey.parameter cache hfinite queried ∪ messageTargets
  have hdrop (answer : HashOutput) :
      answerEncodingPotential (cache.cacheQuery input answer) secretKey + targets.card ≤ answerEncodingPotential cache secretKey := by
  ...
  apply encodingSelectionPotential_cacheQuery_le_without_parent hfinite hfresh hat hnoParent
...
theorem answerEncoding_step_of_no_new_messages
    ...
    ∃ targets : Finset Digest, targets.card ≤ answerEncodingPotential cache secretKey + 1 ∧
      ∀ answer : HashOutput,
        ¬ ParentSettlement secretKey.parameter secretKey.otsSecret secretKey.ftsSecret cache input answer →
        truncateHash answer ∉ targets →
        ¬ Bad secretKey.parameter secretKey.otsSecret secretKey.ftsSecret (cache.cacheQuery input answer) ∧
          answerEncodingPotential (cache.cacheQuery input answer) secretKey + targets.card ≤ answerEncodingPotential cache secretKey + 1 ∧
          encodingSelectionPotential (cache.cacheQuery input answer) (finite_cacheQuery hfinite input answer) secretKey ≤
            encodingSelectionPotential cache hfinite secretKey := by
...
theorem answerEncoding_step_of_structural
  ...
  · exact answerEncoding_step_of_no_new_messages hfinite hclean hfresh hnotEncoding
  ...
  · by_cases hnever : ∀ answer : HashOutput,
    ...
    · push Not at hnever
      ...
      obtain ⟨targets, hcard, hsafe⟩ := answerEncoding_step_of_settling hfinite hclean hfresh hat hbefore hsettles
