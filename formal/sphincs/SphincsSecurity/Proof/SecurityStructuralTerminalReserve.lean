import SphincsSecurity.Proof.StructuralTerminalReserve
...
theorem probEvent_original_win_add_terminalReserve_le_failure_add_charge_add_live_residual
  ...
  have hsum := add_le_add (add_le_add
    ...
    (probEvent_structuralFailure_add_terminalReserve_le_beforeFailureCharge adversary parameter otsTable ftsTable q fuel))
...
noncomputable def sampledStructuralTerminalReserve (adversary : Adversary) (q fuel : Nat) : ENNReal :=
  ∑' parameter, Pr[= parameter | sampleParameter] *
    ∑' ftsSecret, Pr[= ftsSecret | sampleFtsSecrets] *
      ∑' table, Pr[= table | OtsProbeSimulation.sampleOtsHashTable] *
        initializedStructuralTerminalReserve adversary parameter table (curryFtsTableEquiv ftsSecret) q fuel

theorem forgeAdvantage_add_terminalReserve_le_sharedFailure_add_structural_add_live_residual
    (adversary : Adversary) (q : Nat) (hq : HasHashQueryBound scheme adversary q) (fuel : Nat) :
    forgeAdvantage scheme adversary + sampledStructuralTerminalReserve adversary q fuel ≤ sampledParentSharedFailureRisk adversary q fuel +
  ...
  unfold sampledStructuralTerminalReserve sampledOriginalRecordRisk sampledParentSharedFailureRisk sampledBeforeFailureStructuralCharge sampledLiveNonSecretResidual
  ...
  calc
    _ ≤ ∑' parameter, Pr[= parameter | sampleParameter] *
      ...
      · apply mul_le_mul' le_rfl
        ...
        · exact mul_le_mul' le_rfl (probEvent_original_win_add_terminalReserve_le_failure_add_charge_add_live_residual adversary q hq parameter hp table ht
