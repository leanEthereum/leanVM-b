...
noncomputable def survivingStructuralPotential (key : SecretKey) (cache : QueryCache HashSpec) (hit failed : Bool) : ENNReal :=
  if failed then 0 else if hit then 1 else structuralRecordPotential key cache none

noncomputable def structuralEnvelope
  ...
  else structuralRecordPotential (secretKey parameter root otsTable ftsTable) result.1.2 none

theorem structuralEnvelope_le_recordPotential
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    ...
    structuralEnvelope parameter root otsTable ftsTable cache (result.1, result.2.isSome) ≤
      structuralRecordPotential (secretKey parameter root otsTable ftsTable) result.1.2 result.2 := by
  cases hs : result.2 with
  | none => simp only [structuralEnvelope, Option.isSome_none, Bool.false_eq_true, if_false, le_refl]
  | some record =>
      by_cases he : EarlyOtsParentTransition parameter otsTable ftsTable cache result.1.2
      · simp only [structuralEnvelope, Option.isSome_some, if_true, he, zero_le]
      · have hv := runFirstException_none_valid (parentException parameter otsTable ftsTable) _ cache hr record (by simp [hs])
        ...
        simp only [structuralEnvelope, Option.isSome_some, if_true, he, if_false, structuralRecordPotential,
          answerEncodingMonitorPotential, ftsParentSelectionPotential, firstExceptionSelectionPotential, hselected, zero_add, le_refl]

theorem expected_survivingStructuralPotential_step_le
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    ...
    structuralRecordPotential (secretKey parameter root otsTable ftsTable) cache none +
      expectedPreExceptionCharge (parentException parameter otsTable ftsTable) (signingStructuralCharge (secretKey parameter root otsTable ftsTable))
  ...
  let cost := structuralEnvelope parameter root otsTable ftsTable cache (α := (OracleWorld + SigningSpec).Range input)
  calc
    _ ≤ ∑' result, Pr[= result | stepWithFailure exception parameter root otsTable ftsTable input (some frame) cache false false] * cost result.1.2 := by
      ...
      · apply mul_le_mul' le_rfl
        ...
        | true => simp only [survivingStructuralPotential, if_true, zero_le]
        | false =>
            have he : ¬ EarlyOtsParentTransition parameter otsTable ftsTable cache result.1.2.1.2 := by
              ...
              simp [hf] at h
            cases ho : result.1.2.2 <;> simp [survivingStructuralPotential, cost, structuralEnvelope, he, ho]
    ...
    _ ≤ ∑' result, Pr[= result | runFirstException exception (expandedAdversaryImpl (secretKey parameter root otsTable ftsTable) input) cache none] *
      ...
      · exact mul_le_mul' le_rfl (structuralEnvelope_le_recordPotential parameter root otsTable ftsTable input cache result hr)
      · rw [probOutput_eq_zero_of_not_mem_support hr, zero_mul, zero_mul]
    _ ≤ _ := expected_structuralRecordPotential_le_preCharge (secretKey parameter root otsTable ftsTable) _ cache hfinite none
