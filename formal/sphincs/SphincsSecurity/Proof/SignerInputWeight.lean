...
theorem expected_successfulSignerInputWeight_le (key : SecretKey) (message : Message)
  ...
  calc
    ...
    _ ≤ (∑' view, Pr[= view | ($ᵗ FewTimeView : ProbComp FewTimeView)] * uniformWeight view) +
        ∑' input, digestReuseWeight q * cachedSignerInputWeight key message before weight input := by
      apply add_le_add
      · apply ENNReal.tsum_le_tsum
        intro view
        apply mul_le_mul' _ le_rfl
        simpa only [probEvent_eq_eq_probOutput] using probEvent_signWithView_freshSuccessful_le_uniform key message before (· = view)
    ...
    _ = _ := by rw [ENNReal.tsum_mul_left, mul_comm (digestReuseWeight q)]
