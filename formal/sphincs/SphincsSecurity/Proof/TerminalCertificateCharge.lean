...
theorem expected_poissonCertificateGame_count_le_mass_price (adversary : Adversary)
    (budget : Nat) (required : Finset FtsTree) (stopAfter : Nat → SecretKey → CertificateStopRule)
    (hbudget : budget ≤ 2 ^ 127) :
    (∑' result, Pr[= result | poissonCertificateGame adversary budget required stopAfter] *
      certificateBankCount result.2.1.2.2.2.bank) ≤
        ∑' result, Pr[= result | poissonCertificateGame adversary budget required stopAfter] *
          (result.2.1.2.2.2.creationMass * terminalCertificatePrice required result.2.2) := by
  rw [poissonCertificateGame, randomTerminalGame, ← PMF.monad_bind_eq_bind,
    tsum_probOutput_bind_mul, tsum_probOutput_bind_mul]
  simp_rw [← PMF.monad_map_eq_map, tsum_probOutput_map_mul]
  exact ENNReal.tsum_le_tsum fun total => mul_le_mul' le_rfl
    (expected_certificateTerminalGame_count_le_mass_price adversary budget total required (stopAfter total) hbudget)

theorem expected_poissonCertificateGame_creationMass_le_messageCalls (adversary : Adversary)
    (budget : Nat) (required : Finset FtsTree) (stopAfter : Nat → SecretKey → CertificateStopRule) :
    (∑' result, Pr[= result | poissonCertificateGame adversary budget required stopAfter] * result.2.1.2.2.2.creationMass) ≤
      ∑' result, Pr[= result | poissonCertificateGame adversary budget required stopAfter] * result.2.1.2.2.2.messageCalls := by
  rw [poissonCertificateGame, randomTerminalGame, ← PMF.monad_bind_eq_bind,
    tsum_probOutput_bind_mul, tsum_probOutput_bind_mul]
  simp_rw [← PMF.monad_map_eq_map, tsum_probOutput_map_mul]
  apply ENNReal.tsum_le_tsum
  ...
  rw [expected_certificateTerminalGame_project adversary budget required _ _ total
    ...
    expected_certificateTerminalGame_project adversary budget required _ _ total
      (fun result => (result.2.2.2.messageCalls : ENNReal))]
  exact expected_certificateGame_creationMass_le_messageCalls adversary budget required _ _

theorem expected_poissonCertificateGame_count_le_message_excess (adversary : Adversary)
    ...
    (∑' result, Pr[= result | poissonCertificateGame adversary q required stopAfter] *
      certificateBankCount result.2.1.2.2.2.bank) ≤
        baseline * (∑' result, Pr[= result | poissonCertificateGame adversary q required stopAfter] *
          result.2.1.2.2.2.messageCalls) +
          (q : ENNReal) * ∑' total, Pr[= total | targetProposalPool] *
            ∑' word, Pr[= word | independentProposalWord (PMF.uniformOfFintype Index) total] *
              (terminalCertificatePrice required word - baseline) := by
  let law := poissonCertificateGame adversary q required stopAfter
  calc
    _ ≤ ∑' result, Pr[= result | law] *
        (result.2.1.2.2.2.creationMass * terminalCertificatePrice required result.2.2) :=
      expected_poissonCertificateGame_count_le_mass_price adversary q required stopAfter hbudget
    _ ≤ ∑' result, Pr[= result | law] *
        (result.2.1.2.2.2.creationMass *
          (baseline + (terminalCertificatePrice required result.2.2 - baseline))) := by
      apply ENNReal.tsum_le_tsum
      intro result
      exact mul_le_mul' le_rfl (mul_le_mul' le_rfl le_add_tsub)
    _ = baseline * (∑' result, Pr[= result | law] * result.2.1.2.2.2.creationMass) +
        ∑' result, Pr[= result | law] *
          (result.2.1.2.2.2.creationMass * (terminalCertificatePrice required result.2.2 - baseline)) := by
      simp_rw [mul_add, ENNReal.tsum_add]
      congr 1
      calc
        _ = ∑' result, baseline * (Pr[= result | law] * result.2.1.2.2.2.creationMass) := by
          apply tsum_congr
          intro result
          ring
        _ = _ := ENNReal.tsum_mul_left
    _ ≤ _ := add_le_add
      (mul_le_mul' le_rfl (expected_poissonCertificateGame_creationMass_le_messageCalls adversary q required stopAfter))
      (expected_poissonCertificateGame_mass_payoff_le adversary q required stopAfter hbound
        (fun word => terminalCertificatePrice required word - baseline))
