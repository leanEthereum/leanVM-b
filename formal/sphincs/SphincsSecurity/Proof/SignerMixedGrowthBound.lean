...
noncomputable def newMixedMomentWeight (key : SecretKey) (before : QueryCache HashSpec) (log : QueryLog SigningSpec)
  ...
  cachePowerArrival power (cachedIndexMultiplicity key.parameter before source.1) *
    (((signingSlotsAtIndex (observedOptionalSigningViews (messageAnswers key.parameter before) key.root log) source.1).card + 1).choose degree : ENNReal)

theorem signingSlotsAtIndex_append_le_add_one (key : SecretKey) (before after : QueryCache HashSpec)
    (log : QueryLog SigningSpec) (entry : SigningEntry) (hcache : before ≤ after)
    (hsigned : SigningDigestsCached key.parameter before key.root log) (index : Index) :
    (signingSlotsAtIndex (observedOptionalSigningViews (messageAnswers key.parameter after) key.root (log ++ [entry])) index).card ≤
      (signingSlotsAtIndex (observedOptionalSigningViews (messageAnswers key.parameter before) key.root log) index).card + 1 := by
  have hstable := observedOptionalSigningViews_cache_stable key.parameter key.root before after log hcache hsigned
  have hstep := signingSlotsAtIndex_log_append_card log entry (observedSigningView? (messageAnswers key.parameter after) key.root) index
  change (signingSlotsAtIndex (observedOptionalSigningViews (messageAnswers key.parameter after) key.root (log ++ [entry])) index).card =
    (signingSlotsAtIndex (observedOptionalSigningViews (messageAnswers key.parameter after) key.root log) index).card + _ at hstep
  rw [hstep, hstable]
  split_ifs <;> omega

theorem signWithView_mixedGrowth_le_new (key : SecretKey) (message : Message) (before after : QueryCache HashSpec)
    ...
    mixedSigningGrowthMoments key before power (after, log ++ [⟨message, signature⟩]) degree ≤
      newMixedMomentWeight key before log power (hashOutputFewTimeView output) degree := by
  ...
  unfold mixedSigningGrowthMoments observedWeightedBinomialMoments weightedBinomialOccupancyMoment
  calc
    _ ≤ ∑ index : Index, if (hashOutputFewTimeView output).1 = index then
        newMixedMomentWeight key before log power (hashOutputFewTimeView output) degree else 0 := by
      ...
      · subst index
        ...
        exact mul_le_mul' hpower (Nat.cast_le.mpr (Nat.choose_le_choose degree
          (signingSlotsAtIndex_append_le_add_one key before after log ⟨message, signature⟩ hcache hsigned _)))
      · simp only [heq, if_false, add_zero, tsub_self, zero_mul, le_refl]
    _ = _ := by simp only [Finset.sum_ite_eq, Finset.mem_univ, if_true]

theorem signWithView_mixedGrowth_le_newEvents (key : SecretKey) (message : Message) (before : QueryCache HashSpec)
  ...
  · obtain ⟨payload, output, hfresh, hafter, hadmissible, _⟩ := hnew
    have hbound := signWithView_mixedGrowth_le_new key message before result.2 log power degree result.1.1 result.1.2
  ...
  · have hcache := simulateQ_romImpl_cache_le (signWithView key message) before result hresult
    ...
    simp only [mixedSigningGrowthMoments, observedWeightedBinomialMoments, weightedBinomialOccupancyMoment,
