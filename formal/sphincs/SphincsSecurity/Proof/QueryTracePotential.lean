  ...
  | query_bind input next ih =>
      ...
      have hconstant : (∑' middle, Pr[= middle | (impl input).run state] *
          ∑' result, Pr[= result | (simulateQ impl (traced observation (next middle.1))).run middle.2] *
            ((charge input : ENNReal) + (cost result.1.2 : ENNReal))) =
          (charge input : ENNReal) * (∑' middle, Pr[= middle | (impl input).run state]) +
            ∑' middle, Pr[= middle | (impl input).run state] *
              ∑' result, Pr[= result | (simulateQ impl (traced observation (next middle.1))).run middle.2] * (cost result.1.2 : ENNReal) := by
        ...
        · have hi' := hpreserve history state hi input middle hm
          rw [show (∑' result, Pr[= result | (simulateQ impl (traced observation (next middle.1))).run middle.2] *
              ((charge input : ENNReal) + (cost result.1.2 : ENNReal))) =
              (charge input : ENNReal) + ∑' result,
                Pr[= result | (simulateQ impl (traced observation (next middle.1))).run middle.2] * (cost result.1.2 : ENNReal) by
            simp only [mul_add, ENNReal.tsum_add, ENNReal.tsum_mul_right,
              tsum_probOutput_eq_one' (hmass (next middle.1) _ _ hi'), one_mul]]
          rw [mul_add, mul_comm _ (charge input : ENNReal)]
      calc
