...
theorem expectedStoppedExecutionCharge_add_remainder_scaled_le_initial
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop)
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
...
theorem expectedStoppedExecutionCharge_add_remainder_scaled_le_127
  ...
  (expectedStoppedExecutionCharge_add_remainder_scaled_le_initial exception parameter root otsTable ftsTable q hq computation hbound
    frame cache hit failed hnone hbudget).trans (mul_le_mul' le_rfl (initialRawIndexRate_le_127 q hq))

theorem expectedStoppedArrival_add_executionReserve_remainder_scaled_le_initial
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop)
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    (q : Nat) (hq : q ≤ 2 ^ 127) (computation : OracleComp (OracleWorld + SigningSpec) α)
    (hbound : (simulateQ (expandedAdversaryImpl (secretKey parameter root otsTable ftsTable)) computation).IsQueryBoundP (· matches Sum.inr _) q)
  ...
  exact expectedStoppedExecutionCharge_add_remainder_scaled_le_initial exception parameter root otsTable ftsTable q hq computation hbound
    ...
    expectedStoppedIndexCharge exception parameter root otsTable ftsTable q (targetArrivalHashCost parameter)
      ...
      (expectedStoppedIndexCharge exception parameter root otsTable ftsTable q (unusedTargetExecutionCost parameter)
        ...
        expectedStoppedIndexRemainder exception parameter root otsTable ftsTable q ∅ Finset.univ computation q
          frame (cache, []) hit failed * ((2 ^ 176 : Nat) : ENNReal)⁻¹) ≤
      (q : ENNReal) * ((29 / 64 : ENNReal) * ((2 ^ 127 : Nat) : ENNReal)⁻¹) :=
  (expectedStoppedArrival_add_executionReserve_remainder_scaled_le_initial exception parameter root otsTable ftsTable q hq computation hbound
    frame cache hit failed hnone hbudget).trans (mul_le_mul' le_rfl (initialRawIndexRate_le_127 q hq))
