...
import SphincsSecurity.Proof.InitializedDoubleSharedParentRefund
...
noncomputable def initializedCollisionDoubleParentCredit
...
theorem probEvent_structuralFailure_add_doubleParentCredit_le_beforeFailureCollisionCharge
    ...
    Pr[SurvivingStructuralFailure parameter otsTable ftsTable |
      ...
      initializedCollisionDoubleParentCredit adversary parameter otsTable ftsTable q fuel ≤
      initializedBeforeFailureCollisionCharge adversary parameter otsTable ftsTable q fuel * (Fintype.card Digest : ENNReal)⁻¹ := by
  rw [initializedCollisionDoubleParentCredit, ← add_assoc]
  apply le_trans _ (expected_collisionPotential_add_twice_sharedParentDiscard_retained_le adversary q hq hqMax parameter hp otsTable ftsTable hfts fuel)
...
theorem probEvent_original_win_add_collisionDoubleParentCredit_le_failure_add_charge_add_live_residual
    (adversary : Adversary) (q : Nat) (hq : HasHashQueryBound scheme adversary q) (hqMax : q ≤ 2 ^ 127)
  ...
  have hsum := add_le_add (add_le_add
    (le_refl (Pr[fun result => result.2 = true | runRetainedWithFailure (parentException parameter otsTable ftsTable)
      adversary parameter otsTable ftsTable q fuel]))
    (probEvent_structuralFailure_add_doubleParentCredit_le_beforeFailureCollisionCharge adversary q hq hqMax parameter hparameter otsTable ftsTable hfts fuel))
    (le_refl (Pr[LiveNonSecretResidual parameter otsTable ftsTable |
      runRetainedWithFailure (parentException parameter otsTable ftsTable) adversary parameter otsTable ftsTable q fuel]))
  simpa only [add_assoc, add_comm, add_left_comm] using hsum

noncomputable def sampledCollisionDoubleParentCredit (adversary : Adversary) (q fuel : Nat) : ENNReal :=
  ∑' parameter, Pr[= parameter | sampleParameter] *
    ∑' ftsSecret, Pr[= ftsSecret | sampleFtsSecrets] *
      ∑' table, Pr[= table | OtsProbeSimulation.sampleOtsHashTable] *
        initializedCollisionDoubleParentCredit adversary parameter table (curryFtsTableEquiv ftsSecret) q fuel

theorem forgeAdvantage_add_collisionDoubleParentCredit_le_sharedFailure_add_charge_add_live_residual
    (adversary : Adversary) (q : Nat) (hq : HasHashQueryBound scheme adversary q) (hqMax : q ≤ 2 ^ 127) (fuel : Nat) :
    forgeAdvantage scheme adversary + sampledCollisionDoubleParentCredit adversary q fuel ≤ sampledParentSharedFailureRisk adversary q fuel +
  ...
  unfold sampledCollisionDoubleParentCredit sampledOriginalRecordRisk sampledParentSharedFailureRisk sampledBeforeFailureCollisionCharge sampledLiveNonSecretResidual
  ...
  calc
    _ ≤ ∑' parameter, Pr[= parameter | sampleParameter] *
      ...
      · apply mul_le_mul' le_rfl
        ...
        · exact mul_le_mul' le_rfl (probEvent_original_win_add_collisionDoubleParentCredit_le_failure_add_charge_add_live_residual adversary q hq hqMax parameter hp table ht
...
theorem sampledCollisionDoubleParentCredit_eq_terminal_add_shared (adversary : Adversary) (q fuel : Nat) :
    sampledCollisionDoubleParentCredit adversary q fuel = sampledCollisionTerminalReserve adversary q fuel +
      sampledSharedParentDiscard adversary q fuel * (2 * (Fintype.card Digest : ENNReal)⁻¹) := by
  unfold sampledCollisionDoubleParentCredit initializedCollisionDoubleParentCredit sampledCollisionTerminalReserve sampledSharedParentDiscard
  simp only [mul_add, ENNReal.tsum_add, ← mul_assoc, ENNReal.tsum_mul_right]
