...
noncomputable def targetCacheEnvelope (key : SecretKey) (q : Nat) (payload : HashInput) (target : FewTimeView) (signatures : Nat) (state : CoverLogState) : TargetShapeVector :=
...
theorem expected_randomOracle_targetCacheEnvelope_le (key : SecretKey) (q : Nat) (payload : HashInput) (target : FewTimeView) (signatures : Nat)
    ...
    (∑' result, Pr[= result | (randomOracle input).run before] *
      targetCacheEnvelope key q payload target signatures (result.2, log) groups remaining) ≤
      targetCacheEnvelope key q payload target signatures (before, log) groups remaining := by
  ...
  · have hcap' : QueryCache.enncard before + 1 ≤ q := by
    ...
    simp only [targetCacheEnvelope, hslots, hbefore]
    exact expected_fresh_targetEnvelope_le key q slots signatures payload target before log input hfresh hsigned groups remaining hvalid
...
theorem expected_romImpl_targetCacheEnvelope_le (key : SecretKey) (q : Nat) (payload : HashInput) (target : FewTimeView) (signatures : Nat)
  ...
  | inl input =>
      ...
      change (∑' result, Pr[= result | (unifFwdImpl HashSpec input).run before] *
        targetCacheEnvelope key q payload target signatures (result.2, log) groups remaining) ≤ _
      rw [hrun, tsum_probOutput_map_mul]
      dsimp only
      rw [ENNReal.tsum_mul_right]
      exact mul_le_of_le_one_left' tsum_probOutput_le_one
  | inr input => exact expected_randomOracle_targetCacheEnvelope_le key q payload target signatures before log input hsigned hcap groups remaining hvalid

theorem expected_logTraced_world_targetCacheEnvelope_le (key : SecretKey) (q : Nat) (payload : HashInput) (target : FewTimeView) (signatures : Nat)
    ...
    (∑' result, Pr[= result | (logTracedMappedAdversaryImpl key (.inl input)).run state] *
      targetCacheEnvelope key q payload target signatures result.2 groups remaining) ≤ targetCacheEnvelope key q payload target signatures state groups remaining := by
  ...
  simpa only [signingLogFragment, List.append_nil, unloggedMappedAdversaryImpl, OracleSpec.Range, OracleSpec.add_apply_inl] using
    expected_romImpl_targetCacheEnvelope_le key q payload target signatures state.1 state.2 input hsigned hbase groups remaining hvalid

theorem expected_logTraced_sign_targetCacheEnvelope_le (key : SecretKey) (q : Nat) (payload : HashInput) (target : FewTimeView) (signatures : Nat) (hq : q ≤ 2 ^ 127)
    ...
    (∑' result, Pr[= result | (logTracedMappedAdversaryImpl key (.inr message)).run state] *
      targetCacheEnvelope key q payload target signatures result.2 groups remaining) ≤
      targetCacheEnvelope key q payload target (signatures + 1) state groups remaining := by
  apply le_trans ?_ (expected_logTraced_sign_targetEnvelope_le key q (cacheSlotCount q state.1) signatures
    hq payload target state hsigned hcache message groups remaining hvalid)
