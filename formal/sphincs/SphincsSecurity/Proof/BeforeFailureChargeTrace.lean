...
theorem expectedBeforeFailureCharge_map
    ...
    expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable (project <$> computation) frame cache hit failed =
      expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable computation frame cache hit failed := by
  induction computation using OracleComp.inductionOn generalizing frame cache hit failed with
  | pure value => simp
  | query_bind input next ih => simp only [map_bind, expectedBeforeFailureCharge_query_bind, ih]

theorem expectedBeforeFailureCharge_signingTrace
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop) (charge : QueryCache HashSpec → HashInput → ENNReal)
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    (computation : OracleComp (OracleWorld + SigningSpec) α)
    (frame : Option Frame) (cache : QueryCache HashSpec) (hit failed : Bool) :
    expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable (signingTraceComputation computation) frame cache hit failed =
      expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable computation frame cache hit failed := by
  have h := expectedBeforeFailureCharge_map exception charge parameter root otsTable ftsTable (signingTraceComputation computation)
    Prod.fst frame cache hit failed
  rw [signingTraceComputation_fst] at h
  exact h.symm

theorem expectedBeforeFailureCharge_outerCap
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop) (charge : QueryCache HashSpec → HashInput → ENNReal)
    (parameter : PublicParameter) (root : Digest) (otsTable : OtsSecretIndex → HashOutput) (ftsTable : Coordinate → Digest)
    ...
    (frame : Option Frame) (cache : QueryCache HashSpec) (hit failed : Bool) :
    expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable (OtsProbeSimulation.capOuterHashQueries computation q) frame cache hit failed =
      expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable computation frame cache hit failed := by
  ...
  | pure value => simp only [OtsProbeSimulation.capOuterHashQueries_pure, expectedBeforeFailureCharge_pure]
...
theorem expectedBeforeFailureCharge_retainedComputation
    (exception : QueryCache HashSpec → HashInput → HashOutput → Prop) (charge : QueryCache HashSpec → HashInput → ENNReal)
    ...
    (frame : Option Frame) (cache : QueryCache HashSpec) (hit failed : Bool) :
    expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable (retainedComputation adversary parameter root q) frame cache hit failed =
      expectedBeforeFailureCharge exception charge parameter root otsTable ftsTable
        (unloggedRetainedRestComputation adversary ⟨root, parameter⟩) frame cache hit failed := by
  rw [retainedComputation, expectedBeforeFailureCharge_map, expectedBeforeFailureCharge_outerCap exception charge parameter root otsTable ftsTable _ q hbound,
    retainedGameRestComputation_eq_signingTrace, expectedBeforeFailureCharge_map, expectedBeforeFailureCharge_signingTrace]

theorem probEvent_retainedComputation_failed_eq_unlogged
    ...
    (hbound : (simulateQ (expandedAdversaryImpl (secretKey parameter root otsTable ftsTable))
      (retainedGameRestComputation adversary ⟨root, parameter⟩)).IsQueryBoundP (· matches Sum.inr _) q)
    (frame : Option Frame) (cache : QueryCache HashSpec) (hit failed : Bool) :
    Pr[fun result => result.2 = true | runWithFailure exception parameter root otsTable ftsTable
  ...
  rw [runWithFailure_retainedComputation_trace exception adversary parameter root otsTable ftsTable q hbound, probEvent_map,
    ← runWithFailure_withSigningLog_fst exception parameter root otsTable ftsTable
      (unloggedRetainedRestComputation adversary ⟨root, parameter⟩) [], probEvent_map]
  rfl
