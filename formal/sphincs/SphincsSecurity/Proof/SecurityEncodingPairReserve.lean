...
theorem forgeAdvantage_add_parentFunding_afterEncodingPairs_le
    ...
    forgeAdvantage scheme adversary +
      (sampledJointCollisionCoverageCompletionCredit adversary q (q + 1) + sampledJointCollisionCoverageCredit adversary q (q + 1) +
        ...
        (sampledSelectedJointQueryCharge MessageHashInput adversary q + sampledBeforeFailureHashCharge messageHashCharge adversary q (q + 1) +
          sampledSigningNonEncodingReserve adversary q (q + 1) + sampledOuterEncodingReserveAfterPairs adversary q (q + 1)) * (Fintype.card Digest : ENNReal)⁻¹) +
      ...
      (sampledBeforeFailureRestHashCharge adversary q (q + 1) + (q : ENNReal)) * (Fintype.card Digest : ENNReal)⁻¹ +
        (q : ENNReal) / ((2 ^ 216 : Nat) : ENNReal) + sampledSigningEncodingPairCharge adversary q (q + 1) * (Fintype.card Digest : ENNReal)⁻¹ +
  ...
  have h := forgeAdvantage_add_jointParentFunding_afterTerminalRefund_le adversary q hq hqMax
  rw [sampledBeforeFailureEncodingPairCharge_eq_outer_add_signing,
    ← sampledOuterEncodingReserveAfterPairs_add_pairs adversary q hq hqMax (q + 1)] at h
  apply ENNReal.le_of_add_le_add_right (sampledOuterEncodingPairCharge_scaled_ne_top adversary q hq hqMax (q + 1))
