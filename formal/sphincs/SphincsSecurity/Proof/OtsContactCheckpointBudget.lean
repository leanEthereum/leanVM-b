...
import SphincsSecurity.Proof.OtsPrefixVisibleAccounting
...
theorem contactCheckpoint_budget (budget : Nat)
    ...
    (hmiddle : middle ∈ (lazyRun (extendAux uniformImpl (segment.seedOracle inputs hencoding hgraph auxiliary secrets ftsSecret words endpoint))
      (segment.contactCheckpointBefore inputs hencoding hgraph auxiliary secrets ftsSecret words adversary endpoint) (fun _ _ => none)).support)
  ...
  exact lazyRun_pause_budget_of_real aux program (fun output => output.2.hashCalls) budget
    ...
    hreal' hsmall endpoint _ _ 1 middle hmiddle ((result.1.1.1, result.1.2), result.2) hresult'
