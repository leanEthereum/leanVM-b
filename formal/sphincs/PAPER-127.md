...
Start with the [current mathematical plan](paper-127/minimal-closing-contract.md), reviewed against `7a0816c9`. The target is the unchanged original SUF game with probability at most `q / 2^127`. The public Lean endpoint remains 126 bits.
...
The local kernels for the large-budget invariant are now formalized. [HiddenLabelProbe.lean](SphincsSecurity/Proof/HiddenLabelProbe.lean) retains full hash replies, proves the exact surviving mass after the child and parent tests, and derives the two-test hazard. [UniformTableDisclosure.lean](SphincsSecurity/Proof/UniformTableDisclosure.lean) proves the joint conditional sampling identity for revealing a coordinate. [CanonicalHiddenCoordinates.lean](SphincsSecurity/Proof/CanonicalHiddenCoordinates.lean) identifies the concrete hidden coordinates, proves that every payload with a hidden child is unary with distinct child and parent coordinates, and connects the tests to serialized canonical inputs. [PrimitiveMessagePotential.lean](SphincsSecurity/Proof/PrimitiveMessagePotential.lean) proves the scalar probe balance and message payment. These local results still need composition through the original adaptive execution and its stopping rule before they establish the large-budget security interval.

[AdaptiveHiddenLabels.lean](SphincsSecurity/Proof/AdaptiveHiddenLabels.lean) now proves that composition for a native probe/disclosure interface, with auxiliary sampling depending on the current candidate sets and visible memory. The posterior is derived from sampling the hidden table once. A primitive match stops the computation while retaining its visible memory, so past message payments can remain recorded on matched runs. Hidden-table erasure preserves the whole stopped distribution, and the executions have no additional probability loss from a nonempty initial family. [AdaptiveHiddenCandidates.lean](SphincsSecurity/Proof/AdaptiveHiddenCandidates.lean) tracks the actual probe counter and retired coordinates through arbitrary adaptive continuations. [AdaptiveHiddenHazard.lean](SphincsSecurity/Proof/AdaptiveHiddenHazard.lean) derives the paired-probe hazard at supported states and the exact public-output match probability. Connecting the original byte-oracle execution to this interface, including its external cache and private honest costs, remains necessary before applying the potential to the original SUF game.

[CanonicalProbeExecution.lean](SphincsSecurity/Proof/CanonicalProbeExecution.lean) and its imports now prove a concrete external-byte correspondence. The router selects its test using only public values and disclosure status; malformed structural payloads still receive an output-match test. Public canonical rows return their full known replies. For a programmed-row response kernel, the routed and direct first-match-stopped executions have the same distribution through arbitrary randomized adaptive external computations with fixed reference and disclosure data. Repeated rows use the external cache, and every call, including repeats and the stopping call, consumes one hash. The retained cache remains clean, hash and probe counters obey the syntactic hash bound, and stops retain probability mass. Cache cleanliness also survives further FTS disclosures. The original residual-row law and private-signing component are now formalized. The remaining connection must compose their shared-table completion through the interleaved execution and retain the original costs, stops, message records and coverage state. This correspondence alone does not establish an original-game security interval.

[ReferenceCoordinateGame.lean](SphincsSecurity/Proof/ReferenceCoordinateGame.lean) now derives the initial hidden-coordinate prior from the original samplers and preserves the original SUF probability and hash bound. A finite bijection retains both secret tables and both halves of every graph reply; sampling the public coordinates first leaves independent uniform hidden coordinates. [PublicSigningInitial.lean](SphincsSecurity/Proof/PublicSigningInitial.lean) connects this prior to the original and lazy private-signing records. The [initial-prior note](paper-127/initial-prior-and-composition.md) explains the sampling identity and the remaining requirement to keep future residual rows unobserved in the history used for fresh-query bounds. The public endpoint remains 126 bits.

[AdaptiveResidualLabels.lean](SphincsSecurity/Proof/AdaptiveResidualLabels.lean) now proves the joint hidden-label and residual-table posterior through adaptive reads, probes, disclosures and retained auxiliary stops. [AdaptiveResidualErasure.lean](SphincsSecurity/Proof/AdaptiveResidualErasure.lean) erases both tables while preserving the stopped result and memory. [ReferenceJointPrior.lean](SphincsSecurity/Proof/ReferenceJointPrior.lean) provides the original game's matching joint sampler with its SUF and hash-bound correspondence. Connecting the concrete byte router, private signing and coverage records to this interface remains necessary; neither 127-bit interval is proved yet.
