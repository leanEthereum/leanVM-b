...
The next missing coverage step is constructing one proposal-word coupling for the adaptive original execution and proving its prefix and cache exceptions, followed by the terminal Poisson excess and banked certificate charging. The checked boundary estimate assumes its explicit clean-state and proposal-count hypotheses; it does not prove that an adaptive execution satisfies them, the adaptive coverage inequality, or any strengthened public security theorem. The [paper review](paper-127/paper-review.md) states the original-experiment endpoints needed for completion.
