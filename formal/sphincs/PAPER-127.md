...
The next missing Lean coverage step is the numerical Poisson analysis: take c=3/(2N) for full coverage and c=0 for each near-target coordinate subset, then prove the required terminal expectations and exceptional probabilities. The current accumulated endpoint instantiates the original signer; applying coverage to forced FTS kernels still requires the message-kernel generalization and its concrete deferred-secret instantiation. The graph and OTS likelihood transfers also remain to be formalized. The paper audit makes the OTS projection explicit: its preimage-count density applies only after forgetting the hidden starting secret and private canonical prefix data. No public 127-bit security theorem follows from these intermediate results.
