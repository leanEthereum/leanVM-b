...
The abstract bridge and adaptive uniform-prefix laws are checked. The next missing coverage step is their instantiation with the original signing records and their conditional index bound, preserving the recorded-history filtration and unqueried message cells. The prefix and cache exception bounds, terminal Poisson excess, and banked certificate charging also remain open. The checked boundary estimate assumes its explicit clean-state and proposal-count hypotheses; it does not prove that an adaptive original execution satisfies them, the adaptive coverage inequality, or any strengthened public security theorem. The [paper review](paper-127/paper-review.md) states the original-experiment endpoints needed for completion.
