...
The generic first-contact bound is proved in [AdaptiveChainContact.lean](SphincsSecurity/Proof/AdaptiveChainContact.lean): $\Pr_R[\mathrm{contact}]\le2\mathbb E_I Q/2^{128}$ for arbitrary adaptive prefix programs starting with no observed rows. Here contact means a queried last-level row returns the published endpoint, and $Q$ counts actual selected-prefix queries. A pending-path potential accounts for earlier preparation, including one query completing several paths. The bound still needs its observed-row connection to the capped conditional allocation.

The next work is that connection and the remaining completed-witness estimates against the joint allocation, including two-edge witnesses, distinct contacts and encoding markers in either order. The forced-FTS transitions also need their coverage estimates. The full 127-bit theorem must combine the resulting small-budget bound with the completed large-budget theorem and pass the axiom audit.
