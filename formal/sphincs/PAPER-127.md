...
The next missing coverage step is the adaptive composition of these local ledger inequalities with the concrete monitoring state and one terminal proposal variable. The two-filtration construction permits coverage to use more information than the primitive analysis, while the latter erases rejected values and retains lengths. Exact marginal erasure alone does not prove the hidden-label posterior, fresh-message law in the chosen coverage history, or independent unused terminal suffix. The accumulated charge still needs its actual expected message-cost bound, and the prefix exceptions and terminal Poisson estimates need to be assembled with that same execution. No public 127-bit security theorem follows from the local results alone. The [two-filtration audit](paper-127/two-filtration-audit.md) gives the current original-experiment obligations and work order.
