...
The next work is the small-budget interval: construct the concrete OTS prefix simulator and prove completed-witness charges with one shared query allocation, then establish coverage under forced FTS transitions. The generic endpoint likelihood and record-and-length projection are available; their concrete comparisons remain obligations. The full 127-bit theorem must combine that interval with the completed large-budget theorem and pass the axiom audit.
