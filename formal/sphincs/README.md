...
| [OtsPrefixObservedSource.lean](SphincsSecurity/Proof/OtsPrefixObservedSource.lean) and [OtsPrefixObservedBudget.lean](SphincsSecurity/Proof/OtsPrefixObservedBudget.lean) | Exact original-game representation by observed prefix runs and the original hash budget on each supported real run. Its transfer to the ideal run still requires a syntactic prefix-query bound. |
