//! Demo / benchmark: prove and verify Fibonacci in the exponent over a
//! `HeapBuf` — an unrolled `mul_range` loop (`buff[g^k] = g^{F(k)}`, the recurrence
//! `buff[i·g²] = buff[i·g] · buff[i]`). The result `g^{F(N)}` is bound as the
//! public input `m[0]`.
...
#[test]
fn test_fibonacci() {
    run_test_fibonacci::<200_000>();
}

fn run_test_fibonacci<const FIB_N: usize>() {
