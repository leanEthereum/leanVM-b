...
- The one hash function is BLAKE2s, in `primitives::blake2s`: scalar, streaming, keyed, and a lane-transposed batched form for the PCS Merkle tree. It replaced BLAKE3 because the VM proves one compression per opcode and BLAKE2s takes the byte counter and final-block flag as ordinary inputs, so a single opcode is a complete hash for any length, with no chunk tree to reproduce in-circuit.
