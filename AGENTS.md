...
- `formal/xmss/` is a Lean 4 proof (over VCVio) of that instance's classical random-oracle security, `xmss_has_127_bits_of_classical_security`. `formal/sphincs/` proves 127 bits of classical strong unforgeability in the random-oracle model for the SPHINCS instance, `sphincs_has_127_bits_of_classical_security`, with the proof route recorded in `formal/sphincs/PAPER-127.md`. In both, `*/Statement.lean` defines the concrete parameters, the byte layout of every hash input, the three algorithms, the game, and the claim. `lake exe cache get` once, then `lake build`. The SPHINCS build also audits every local declaration for unapproved axioms.
