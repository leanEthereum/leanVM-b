...
- The one hash function is `sha2_eth`, in `primitives::sha2`: SHA-256's compression `C` under a length-prefixed Merkle-Damgard. `sha2_eth(msg)` absorbs `len_block(8·|msg|) ‖ msg ‖ 0…0` from `IV_ETH = sha256("Ethereum")`, big-endian throughout. Putting the length FIRST makes the encoding prefix-free (so the construction is indifferentiable from a random oracle modulo the usual heuristic on `C`, which being Davies-Meyer is provably not an ideal fixed-input-length primitive), and it is what makes the hash cheap: the first compression depends only on the length, so `iv_for_len(n)` is a compile-time constant and an `n`-byte hash of known length is exactly `ceil(n/64)` compressions, with no counter, no final flag and no tree structure to reproduce in-circuit. The VM proves one compression per opcode. **Natively that constant is the caller's job**: `iv_for_len` is a `const fn`, but called with a runtime length it is an ordinary function and runs a whole extra compression, so use `hash_block` for the 64-byte case (the Fiat-Shamir chain, the Merkle parent, most of the hashing a proof does). Surfaces: `compress` (the opcode's relation, dispatching to SHA-NI / ARMv8 `sha256h` where the target has one), `hash_block`, `hash` / `Hasher` (which takes the total length up front), and `hash_many` for the PCS Merkle tree.
...
The third is worth understanding before touching the verifier. `guests/aggregate.py` is not Python that runs; it is the zkDSL, which `lean_compiler` lowers to the VM's seven-opcode ISA (`XOR`, `MUL`, `SET`, `DEREF`, `JUMP`, `SHA2`, `PACK64X2`) over write-once memory. So every verifier step, Fiat-Shamir absorption, sumcheck fold, Merkle path, field inverse, becomes VM instructions that the prover then proves the execution of, which is why the guest is ~330k instructions (2^19 padded) and why its opcode mix is what the recursion benchmark reports. Two consequences:
...
`A_NEW`, `E_NEW` and `W[t]` are pinned (materialized into their own slots), because `a' = T1 + Sigma_0(a) + Maj` reads `a` four times over, so an unpinned bit's affine support quadruples every round: `4^64`. `T1` is deliberately NOT pinned (its operands are all pinned already, so its support stays ~70 terms and inlining it saves 2,048 rows). With those three pin families the substituted matrices would carry 1.09M nonzeros, against BLAKE2s's ~89M for pinning nothing.

**Neither matrix is ever built.** `flock::sha2` describes the circuit twice, forwards and backwards, and both walks are `O(circuit)`: `forward_walk` threads `WireWord`s to give every row its `(A_0 w, B_0 w)` pair, which answers the verifier's `bilinear_form` and, kept per row, feeds recursion's column contraction; `marginal_walk_side` is its reverse-mode transpose, and gives lincheck's prover the whole column marginal `(A_0 + a B_0)^T u`. `gf2` holds the gadget pair (`walk_*` / `back_*`) and nothing else. The two are cross-checked by the protocol itself, lincheck's terminal identity being exactly the assertion that they agree, and directly by `marginal_walk_transposes_the_forward_walk`.

Density therefore costs nothing on either side, and the pins now buy only the bounded per-row support the layout is tiled around. The zk.golf `gf2-sha256-compress-canonical` frontier is 22,215 rows and gets there by dropping every pin; that would need `K_LOG = 15` all the same (`Ch`, `Maj` and the adds are already over `2^14`), so it would buy slots inside a block that is not shrinking.
