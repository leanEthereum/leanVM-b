...
- The one hash function is **SHA3-256**, in `primitives::hash`: byte-exact FIPS 202, with a `const fn` portable `Keccak-f[1600]` and a two-way batched path on aarch64's `FEAT_SHA3`, where `BCAX` is chi in one instruction. Surfaces: `permute`, `hash_block` for the 64-byte case (the Fiat-Shamir chain, the Merkle parent), `hash` / `Hasher` for anything else, `hash_md` for the chain below, and `hash_many_md*` for the PCS Merkle tree.
- **The VM proves one sponge step per opcode**: `permute(prev ^ (msg ‖ 0…0))`, absorbing a 136-byte rate block into a 1600-bit state. The XOR is free over GF(2), so folding the absorb into the circuit costs nothing and makes one instruction worth 136 bytes. A 64-byte message is one such step from the zero state with a constant pad, and so IS plain SHA3-256; that covers the Fiat-Shamir chain and every Merkle parent.
- **Anything longer than 64 bytes uses `hash_md`, not the sponge.** `st' = SHA3-256(st ‖ group32)`. Not because the sponge is unavailable but because the guest cannot lay a rate block out: seventeen lanes have no 128-bit memory-cell alignment, and forcing a nine-cell grouping would re-layout every buffer the guest hashes, the PCS leaf image included. 32 bytes is two cells, which is exactly what the guest holds. The caller binds the length; XMSS's tweak carries it explicitly.
...
| `flock`           | batched R1CS over GF(2) for Keccak: zerocheck + lincheck                |
...
Heavy benches and measurement harnesses are `#[ignore]`d; run by name with `-- --ignored --nocapture`: `sha3_batch_prove_verify`, `pcs_throughput`, `aggregate_three_levels`, `aggregate_statement_binds`, `aggregate_hints_bind`, `aggregate_rejects_a_bad_signature`, `print_whir_query_counts`, `encoding_grinding_bits`.

## Benchmarking

The benchmarks we care about:
- `cargo run --release -- xmss --n-signatures 450 --log-inv-rate 1 --repeat 3`
...
The third is worth understanding before touching the verifier. `guests/aggregate.py` is not Python that runs; it is the zkDSL, which `lean_compiler` lowers to the VM's six-opcode ISA (`XOR`, `MUL`, `SET`, `DEREF`, `JUMP`, `KECCAK`) over write-once memory. So every verifier step, Fiat-Shamir absorption, sumcheck fold, Merkle path, field inverse, becomes VM instructions that the prover then proves the execution of, which is why the guest is ~330k instructions (2^19 padded) and why its opcode mix is what the recursion benchmark reports. Two consequences:
...
`flock::hash` encodes one sponge step, `permute(prev ^ (msg || 0...0))`, in `2^16` rows, 42,881 of them useful.

**`2^16` is a floor, not a choice.** Keccak's only nonlinear step is chi, and its multiplicative complexity is exactly 5 per five-bit row: the five outputs have quadratic parts equal to five distinct degree-two monomials, which are linearly independent, while four products of affine forms span at most four dimensions. So 1,600 AND rows a round, 38,400 over 24 rounds, and nothing in the encoding moves it. theta, rho, pi, iota and the absorb are all affine and cost no rows at all.

**No pins, and no cascade.** Every chi output IS an AND wire and therefore committed by construction, so between one AND layer and the next the affine depth is a single theta, about eleven terms a bit. The pin families an ARX hash needs have no analogue here; the only lin-id rows in the block are the output state, and they exist so the VM can read the result.

**Neither matrix is ever built.** The circuit is described twice, both `O(circuit)`: `forward_walk` threads wire values to give every row its `(A_0 w, B_0 w)` pair, and `marginal_walk_side` is its reverse-mode transpose, giving lincheck's prover the whole column marginal. `gf2` holds only what any walk needs; the gadgets live with the circuit, chi's AND being the only nonlinear one. The two directions are cross-checked by lincheck's terminal identity and directly by `marginal_walk_transposes_the_forward_walk`.

**A Keccak lane is a flock word**, so witness generation is whole-`u64` stores with no bit shuffling, and every region is padded to a whole number of 128-bit VM cells. The pad words are empty rows, so the R1CS forces them to zero and the bus in turn forces the VM cell's matching lane to zero.
