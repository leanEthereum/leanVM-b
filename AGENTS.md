...
`flock::hash` encodes one compression in `2^15` rows, 29,113 of them useful. That is twice the block BLAKE2s occupied, and the factor is unavoidable: `Ch`, `Maj` and the adds need ~22,000 AND rows however they are encoded, which is already over `2^14`. So a proof's flock share roughly doubled with the switch.
