...
class Program:
    ...
    def parse(cls, data: dict[str, Any]) -> "Program":
        ...
        bytecode_used = data.get("bytecode_used", len(operations) - 1)
        require(
            isinstance(bytecode_used, int)
            and not isinstance(bytecode_used, bool)
            and 0 <= bytecode_used < len(operations),
            "bytecode_used must identify a prefix ending before the sentinel",
        )
        return cls(operations, bytecode_used)

    def digest(self) -> tuple[F128, F128]:
        words = [F128.new(len(self.operations), 1), F128(self.bytecode_used)]
...
def build_layout(program: Program, memory_used: int, row_counts: Sequence[int]) -> Layout:
    require(
        2 <= memory_used <= 1 << 32
    ...
    # Reproduce cpu::layout::jagged_column_blocks. Columns may share a row-major
    # block only when they have the same public height source and occur in exactly
    # the same opening groups (bus point, table rho point, and public-input point).
    ...
    for local in BLAKE3_VALUES:
        sources[b3 + local] = None

    signatures: list[list[int]] = [[] for _ in kappas]
    group_of_source: dict[tuple[Any, ...], int] = {}
    for block in (*push, *pull, *count):
        if block.owner is not None:
            block_source = ("table-log", block.owner[0])
        elif any(coordinate.public is not None for coordinate in block.coordinates):
            block_source = ("constant-log", bytecode_log)
        elif any(coordinate.column in {0, 1} for coordinate in block.coordinates):
            block_source = ("memory-log",)
        else:
            block_source = ("constant-log", 0)
        group = group_of_source.setdefault(block_source, len(group_of_source))
        for coordinate in block.coordinates:
            column = coordinate.column
            if column is None:
                column = coordinate.generator_column
            if column is not None and sources[column] is not None:
                signatures[column].append(group)
    next_group = len(group_of_source)
    for table, (base, width) in enumerate(zip(BASES, WIDTHS)):
        for column in range(base, base + width):
            if sources[column] is not None:
                signatures[column].append(next_group)
        next_group += 1
    signatures[0].append(next_group)
    signatures = [sorted(set(signature)) for signature in signatures]

    blocks: list[list[int]] = [[3]]
    committed = [column for column in range(len(kappas)) if column != 3 and kappas[column] is not None]
    ...
    for first in committed:
        ...
        group = [
            ...
            and sources[column] == sources[first]
    ...
    logical_log = 0
    for block in blocks:
        width_log = (len(block) - 1).bit_length()
        variables = int(kappas[block[0]])
...
PointClaim = JaggedPointClaim | StridedPointClaim


@dataclass(frozen=True)
class JaggedClaimBatch:
    members: tuple[int, ...]
    offset: int
    height: int
...
def jagged_indicator_with_weights(
    ...
    """Evaluate ``1[index = start + row and index < end]`` as a width-four program."""
    require(
        0 <= start <= end <= 1 << len(index_point)
        and len(row_weights) <= len(index_point),
        "invalid Jagged interval",
    )
    state = [ONE, ZERO, ZERO, ZERO]
    for bit in range(len(index_point) + 1):
        b = index_point[bit] if bit < len(index_point) else ZERO
        c_bit = bool(start >> bit & 1)
        d_bit = bool(end >> bit & 1)
        a_weights = row_weights[bit] if bit < len(row_weights) else (ONE, ZERO)
        b_weights = (ONE + b, b)
        next_state = [ZERO, ZERO, ZERO, ZERO]
        for state_index, state_weight in enumerate(state):
            carry = bool(state_index & 1)
            comparison = bool(state_index & 2)
            for a_bit in range(2):
                for b_bit in range(2):
                    total = a_bit + int(carry) + int(c_bit)
                    if b_bit != total & 1:
                        continue
                    next_carry = bool(total >> 1)
                    next_comparison = comparison if bool(b_bit) == d_bit else d_bit
                    next_index = int(next_carry) + 2 * int(next_comparison)
                    next_state[next_index] += (
                        state_weight * a_weights[a_bit] * b_weights[b_bit]
                    )
        state = next_state
    return state[2]


def jagged_indicator(
    row_point: Sequence[F128], start: int, end: int, index_point: Sequence[F128]
) -> F128:
    return jagged_indicator_with_weights(
        tuple((ONE + coordinate, coordinate) for coordinate in row_point),
        start,
        end,
        index_point,
    )
...
def point_claim_eq(claim: PointClaim, point: Sequence[F128]) -> F128:
    ...
    result = ONE
    for bit, coordinate in enumerate(point[: claim.stride_log]):
        result *= coordinate if claim.slot >> bit & 1 else ONE + coordinate
    for expected, coordinate in zip(claim.point, point[claim.stride_log : block_variables]):
        result *= ONE + expected + coordinate
    selector = claim.offset >> block_variables
    for bit, coordinate in enumerate(point[block_variables:]):
        result *= coordinate if selector >> bit & 1 else ONE + coordinate
    return result


def geometric_claim_weights(
    claims: Sequence[PointClaim], gamma: F128
) -> tuple[list[F128], list[JaggedClaimBatch]]:
    """Match the native verifier's geometric batching of complete row-major blocks."""
    count = len(claims)
    ranks = [-1] * count
    batch_members: list[tuple[tuple[int, ...], int, int, int]] = []
    next_rank = 0
    ...
    for members, offset, height, selector_len in batch_members:
        first = claims[members[0]]
        assert isinstance(first, JaggedPointClaim)
        geometric = gamma
...
def verify_stacked_opening(
    ...
    point_gamma = transcript.sample()
    point_scales, jagged_batches = geometric_claim_weights(point_claims, point_gamma)
    target += sum(
    ...
    def evaluate_basis(prefix: Sequence[F128], residual_log: int) -> list[F128]:
        ...
        for vertex in range(1 << residual_log):
            ...
            for batch in jagged_batches:
                value += batch.scale * jagged_indicator_with_weights(
                    batch.row_weights,
                    batch.offset,
            ...
            for claim_index, (scale, claim) in enumerate(zip(point_scales, point_claims)):
                if claim_index not in grouped:
                    value += scale * point_claim_eq(claim, point)
            result.append(value)
        return result
...
def verify_execution(statement: dict[str, Any], proof: Proof) -> None:
    ...
    layout = build_layout(program, memory_used, row_counts)
