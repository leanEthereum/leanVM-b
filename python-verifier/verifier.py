...
class Transcript:
    def __init__(self, proof: Proof, label: bytes, statement: Sequence[E]) -> None:
        ...
        self.absorb_bytes(b"leanvm-b/transcript/v5-sha2")
...
R1CS_DIGEST = bytes.fromhex("6017e55a27fab620adedfcfb3e0d076969692ee85431578e6b767c3cd709877e")
...
FLOCK_NUM_LINCHECK_ROUNDS = SHA2_R1CS_LOG_SIZE - FLOCK_K_SKIP
QFLOCK_SLOT_BITS = SHA2_R1CS_LOG_SIZE - LOG_PACKING
...
def verify_flock_lincheck(zc: ZerocheckResult, transcript: Transcript) -> tuple[MultilinearPoint, tuple[E, ...]]:
    ...
    terminal = (
        sha2_bilinear(alpha, e_row, w_col) + alpha_sq * eq_eval(chi_in, chi_in_prime) * dot(skip_weights, s) + alpha_cu * w_col[SHA2_CONSTANT_COLUMN]
    )
...
def verify_execution(bytecode: Sequence[K], public_input: Digest, proof: Proof) -> None:
    ...
    # 7] SHA-256 validity, its 64 claims ring-switched, then the one opening that
    # discharges every claim.
    flock_point, flock_s = verify_flock(SHA2_R1CS_LOG_SIZE + layout.table_logs[SHA2.opcode], transcript)
