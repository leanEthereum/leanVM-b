...
Recursion consists in a proof that $n$ sub-proofs exist, for a shared leanISA bytecode and $n$ different public inputs $\pi_0, \ldots, \pi_{n-1}$.

The implemented N-to-1 aggregation runs the verifier of \S\ref{sec:e2e-unrolled} \emph{inside} the VM: one compiled guest replays the transcripts, sumchecks, ring switches, and stacked WHIR openings of $n_{\mathrm{rec}}\ge1$ ordinary VM proofs for a fixed inner program. The proofs may have different announced table and commitment sizes; the guest dispatches among the certified shapes without recompilation. Base-field bookkeeping uses ordinary machine words, extension verifier arithmetic uses three-word values and the explicit extension opcodes, and Fiat--Shamir hashing uses the \texttt{BLAKE3} opcode. One class of operations, however, does not fit.
