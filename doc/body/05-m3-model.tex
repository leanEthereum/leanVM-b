...
Each table must satisfy its own constraints on every one of its own rows. Instead of one zerocheck per table, over boolean hypercubes of different sizes, we batch them into a single sumcheck.

Table $T_j$, for $1\le j\le\ntab$, has $2^{\tau_j}$ rows and $\ncons{j}$ constraints of degree at most $d$. Write $\sigma=\sum_j\ncons{j}$ for the total number of constraints and $n=\max_j\tau_j$ for the tallest table's height.

\paragraph{One challenge and one point.} The verifier samples $\eta\in\E$ and fixes a point $r\in\E^{n}$ (in the assembled protocol $r$ is not freshly sampled but taken from the bus, see \emph{Nonzero claims} below and \S\ref{sec:leafstack}). Table $T_j$ mixes its constraints with the powers of $\eta$ from $o_j=\ncons{1}+\dots+\ncons{j-1}$ on, and is tested at the length-$\tau_j$ prefix of $r$:
\[
  C_j\;=\;\sum_{i=1}^{\ncons{j}}\eta^{\,o_j+i-1}\,C_{j,i},
  \qquad
  S_j\;=\;\widetilde{C_j}\bigl(r_{<\tau_j}\bigr)\;=\;\sum_{x\in\cube{\tau_j}}C_j(x)\,\eq(r_{<\tau_j},x).
\]
The ranges of $\eta$-powers are \emph{disjoint}, so no table's violation can be cancelled by another's, and proving $\sum_j S_j=0$ proves every $C_{j,i}$ vanishes on every row of its own table, up to Schwartz--Zippel error $(n+\sigma)/|\E|$ over $r$ and $\eta$ (Lemma~\ref{lem:sz}). (The identities keep disjoint ranges throughout. \S\ref{sec:leafstack} appends three more entries per table that deliberately \emph{share} their powers, since only their total over tables has to be pinned; that lengthens the $\eta$-degree, and the error, by $3$.)
...
\]
Because $\sum_x\prod_i x_i=1$ on any boolean hypercube, $\sum_{x\in\cube n}f_j(x)=S_j$, so with $F=\sum_j f_j$ the whole statement is the single claim $\sum_{x\in\cube n}F(x)=0$, proven by $n$ rounds of sumcheck on $F$.

The rounds bind $X_0$ first and $X_{n-1}$ last, so every table is active from the start and $T_j$ \emph{departs} after round $\tau_j$, having bound $X_0,\dots,X_{\tau_j-1}$. The last $n-\tau_j$ rounds touch only variables it does not use, where $f_j$ contributes the line $Y\,w_j\,S_j$ with $w_j$ its accumulated weight and $S_j=0$. Either direction gives $T_j$ the same weight,
\[
  w_j\;=\;\prod_{i<\tau_j}\eq(r_i,\rho_i)\cdot\prod_{i\ge\tau_j}\rho_i ,
\]
and lands its claim on the nested point $\rho_{<\tau_j}$; only the order in which the factors accumulate differs. Binding upward is what makes a table's padding rows a constant run in every round, not just before the first fold: see \emph{Padding} in \S\ref{sec:gp}.
...
The one consequence for the rounds is that a departed table's line $Y\,w_j\,S_j$ no longer vanishes. Write $X_\ell$ for the variable a round binds and $Y$ for it while still an unknown. The departed tables' lines sum to $Y\,u$ for the single scalar $u=\sum_{\tau_j\le\ell}w_j\,S_j$, and with $p$ the active tables' cofactor, of degree at most $2$, the round polynomial is the cubic
\[
  h(Y)\;=\;\eq(r_\ell,Y)\,p(Y)+Y\,u .
\]
The prover sends $h$ whole, at $\{0,1,\gen,\gen^{2}\}$: four $\E$-elements a round, one more than \S\ref{sec:gkr}'s $\eq$-trick. That one is the price of the nonzero claims rather than of sending $h$ whole, for the trick cannot save it: the verifier has no way to form $u$, so $u$ would travel beside the cofactor instead. The rounds are then vanilla sumcheck, checking $h(0)+h(1)=c$ against the running claim $c$ and folding $c\leftarrow h(\rho_\ell)$, with no $\eq$ reapplied to the message. Prover work is unchanged: $p$ is quadratic, so its fourth value interpolates from the other three rather than costing another pass over the rows.

Departing rather than waiting also means the prover is never told what a table sums to: $S_j$ enters $u$ only once $T_j$ has bound all of its own variables, at which point $w_j\,S_j$ is a value it has just computed. Only the total $\sum_j S_j$, the batch's target, is formed in advance, and \S\ref{sec:leafstack} has the verifier derive that from the bus.
...
\]
every coordinate $\btup_i$ being a $\K$-element, so each summand is a mixed $\K\times\E$ product.
...
Being one fixed row is also what keeps the padding out of the prover's costs. The leaves of \eqref{eq:gp} past a block's real rows are that one factor, so \S\ref{sec:gkr}'s tree is built from the real rows alone. The same holds in \S\ref{sec:air}'s batch once the rounds bind upward: the padding occupies the tail of the row index and a round pairs \emph{adjacent} rows, so the tail's pairs have that one row on both sides. Binding a variable sends a pair $(v_{\mathrm{lo}},v_{\mathrm{hi}})$ to $v_{\mathrm{lo}}+\rho\,(v_{\mathrm{lo}}+v_{\mathrm{hi}})$ (Fact~\ref{fact:eq}), which in characteristic $2$ is $v$ again when both halves are $v$, so the tail is still that row after every fold. Its entire contribution to a round is therefore $C_j(\mathrm{pad})$, one evaluation for the whole proof, times the $\eq$ mass of the tail, which is $O(\tau_j)$ multiplies from the binary expansion of the boundary. The prover sums and folds the real prefix only, so both \S\ref{sec:gp} and \S\ref{sec:air} cost what the trace holds rather than what it is padded to. Note that pairing $x$ with $x+2^{\tau_j-1}$ instead, which is what binding downward does, would isolate nothing: $\tau_j=\lceil\log_2(\text{rows})\rceil$ leaves more than half the cube real, so every such pair has a real half.

\subsection{Proving the grand product (GKR)}\label{sec:gkr}

Take one side of \eqref{eq:gp}. Its tuples give leaves $v_k=\gamma-\pi_\alpha(\btup_k)$, padded with $1$s to a power of two, $2^\mu$ in all. Their product $P=\prod_kv_k$ is the root of a binary product tree: layer $0$ is the leaves, layer $i$ has $2^{\mu-i}$ nodes, and layer $\mu$ is $P$. Let $\widetilde V_i:\E^{\mu-i}\to\E$ denote the multilinear extension of layer $i$.
...
\]
where $B_j(x)=\sum_{b\in\mathcal B}\eq(\mathsf{sel}_b,\zeta^{b}_{\mathrm{hi}})\bigl(\gamma-\sum_i\alpha^{i}c_{b,i}(x)\bigr)$, a polynomial of degree at most $d$ in $T_j$'s columns, as every bus coordinate is (\S\ref{sec:m3}) every coefficient of which the verifier forms itself (on the counts side $\alpha=1$ and $\gamma=0$, \S\ref{sec:memchan}, so it is a bare sum of count columns). The right-hand side is exactly a claim of \S\ref{sec:air}'s kind, with a nonzero value; its degree is the $d=2$ the batch already pays for the identities. So hand the three $B_j$, one per side, to the batch and run it at $r=\zeta$. They take three powers of $\eta$ placed after all $\sigma=\sum_j\ncons{j}$ identity powers of \S\ref{sec:air} and \emph{shared} by every table, so with a table's real constraints vanishing,
\[
