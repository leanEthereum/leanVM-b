...
\paragraph{Tables and columns.} Recall that a column of height $2^{\kappa}$ is a function $\cube{\kappa}\to\K$, that a table is a group of columns of equal height, all of them committed, and that a quantity a row derives from its columns is no column of its own (Definition~\ref{def:column}). The arithmetization fixes the number of tables $\ntab$ and, for each table $T_j$, its number of columns $\ncol{j}$, its $\ncons{j}$ constraints and its $\nfl{j}$ bus flushes. The table heights are not fixed: the prover announces each table's log height $\tau_j$ at the beginning of the proof, and every one of its $2^{\tau_j}$ rows is a row the program really executed (\S\ref{sec:e2e-pad}).

\paragraph{Constraints.} The constraints of $T_j$ are $C_{j,1},\dots,C_{j,\ncons{j}}$, each a polynomial of degree at most $d$ in the $\ncol{j}$ columns ($d=2$ throughout), required to vanish on every row. A derived quantity costs no column here: the return target $\gen^{2}\cdot\pc$ of \S\ref{sec:tab-deref} is a degree-one term of the constraint that uses it, like any other.

\paragraph{The bus.} All interactions (between tables) share a single \emph{bus}, a channel carrying tuples of up to $m=12$ $\K$-elements (shorter ones are zero-padded). Table $T_j$ has $\nfl{j}$ flushes $\btup_{j,1},\dots,\btup_{j,\nfl{j}}$, each is either a \emph{push} or a \emph{pull}. Each one of the $m$ coordinates in each tuple is a fixed polynomial of degree at most $d$ in the table's columns: a constant, a column, the scalar multiple $\gen^{k}\cdot\cnt$ carrying an advanced count (\S\ref{sec:lookup}), the product $\gen^{k}\fp\cdot o$ carrying an address (\S\ref{sec:memchan}). A few \emph{boundary} blocks belong to no table, the state boundary and each array's seed and finalization; their coordinates are constants, public components, the index column (\S\ref{sec:idxcol}), or a $\K$-multiple of a single column, so of degree at most one, which is what lets \S\ref{sec:leafstack} settle them by evaluation instead of through the row sumcheck of \S\ref{sec:air}.
