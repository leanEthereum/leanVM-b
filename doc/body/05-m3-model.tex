...
\paragraph{Tables and columns.} Recall that a column of height $2^{\kappa}$ is a function $\cube{\kappa}\to\K$, that a table is a group of columns of equal height (Definition~\ref{def:column}). The arithmetization fixes the number of tables $\ntab$ and, for each table $T_j$, its number of columns $\ncol{j}$, its $\ncons{j}$ constraints and its $\nfl{j}$ bus flushes. The table heights are not fixed, and are announced by the prover at the beginning of the proof, padded to the next power of two $2^{\tau_j}$.

\paragraph{Constraints.} The constraints of $T_j$ are $C_{j,1},\dots,C_{j,\ncons{j}}$, each a polynomial of degree at most $d$ ($d=2$ throughout) in the $\ncol{j}$ columns, required to vanish on every row, including the padding rows.
