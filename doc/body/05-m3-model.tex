...
The arithmetization follows the M3 model (multi-multiset matching)~\cite{m3}. Rows are unordered, and an instance is accepted exactly when every table's constraints hold and the bus balances.

\paragraph{Tables and columns.} Recall that a column of height $2^{\kappa}$ is a function $\cube{\kappa}\to\K$ and a table is a group of columns of equal height, each column either committed or virtual (Definition~\ref{def:column}). The arithmetization fixes the number of tables $\ntab$ and, for each table $T_j$, its number of columns $\ncol{j}$, its number of constraints $\ncons{j}$ and its number of bus flushes $\nfl{j}$, together with the constraints and the flushes themselves: all of this is public and independent of the execution being proven. The heights are not fixed. The prover chooses how many rows each $T_j$ carries, pads it to $2^{\tau_j}$ (\S\ref{sec:e2e-pad}), and announces the row counts before any challenge is drawn; the verifier checks them against public caps (\S\ref{sec:memchan}) and derives every later shape from them (\S\ref{sec:e2e-unrolled}).
...
\paragraph{The bus.} All interactions share a single \emph{bus}, a channel carrying tuples of $m$ $\K$-elements. The flushes of $T_j$ are $\btup_{j,1},\dots,\btup_{j,\nfl{j}}$: fixed tuples whose coordinates are public constants that do not vary with the row (the first coordinate is always one of these, the domain separator below), the row's own columns, committed or virtual, and the products of \S\ref{sec:m3-addr}. Each flush is fixed once and for all as a \emph{push} onto the bus or a \emph{pull} off it, and every row performs all $\nfl{j}$ of them. A few fixed \emph{boundary} tuples, belonging to no table, are pushed or pulled directly. The width $m$ is that of the widest tuple, twelve (the bytecode tuple: separator, $\pc$, count, opcode, and eight operand/immediate slots); shorter tuples are zero-padded.
...
\paragraph{Per-instruction tables.} Each instruction has its own table, with columns $\pc$, $\fp$, $\mathit{next\_pc}$, $\mathit{next\_fp}$, the operands and the read values. The operand \emph{addresses} are not columns (\S\ref{sec:m3-addr}). For every instruction except \texttt{JUMP}, the successor is virtual: $\mathit{next\_fp}=\fp$ is a copy of $\fp$, and $\mathit{next\_pc}=\gen\cdot\pc$ is the free increment of \S\ref{sec:vm}. Each row makes the same kinds of flush: on the state interaction (\S\ref{sec:state}) it pulls $\tup{\dsep{ST},\pc,\fp}$ and pushes $\tup{\dsep{ST},\mathit{next\_pc},\mathit{next\_fp}}$; it makes one memory flush per cell read (\S\ref{sec:memchan}) and one bytecode flush for the instruction at $\pc$ (\S\ref{sec:bytecode}). The concrete tables are in \S\ref{sec:tables}.

\paragraph{Addresses as products.}\label{sec:m3-addr} An fp-relative reference names the cell at $\fp\cdot o$ (\S\ref{sec:vm}), so every memory flush needs that address as a bus coordinate. Committing it as a column $a$ would cost one column per cell read plus one degree-$2$ constraint $a=\fp\cdot o$ per column to say what it holds. Instead we widen what a coordinate may be: alongside constants and columns, a coordinate may be the product $\gen^{k}\,c_{a}\,c_{b}$ of two of the row's columns. Then the address IS the coordinate, no column can disagree with it, and the binding constraint has nothing left to say. The $\gen^{k}$ factor addresses a consecutive cell for free, as in \S\ref{sec:vm}, so \texttt{BLAKE3}'s paired cells cost nothing extra. Across the seven tables this removes $19$ columns and the $19$ constraints that bound them, leaving \texttt{SET}, \texttt{BLAKE3} and \texttt{PACK64X2} with no constraint at all.

What it costs is that a table's bus contribution becomes quadratic in its columns rather than linear. Two places feel this, and neither pays for it. The batch of \S\ref{sec:air} already carries degree-$2$ constraints, so the round polynomial's degree is unchanged (\S\ref{sec:leafstack}). And the prover, which reads a linear contribution straight off the column evaluations it already computes, must sum each product over the table's rows: one extra $\eq$-weighted pass per distinct column pair, against the one it already makes per column.
...
\]
where $B_j(x)=\sum_{b\in\mathcal B}\eq(\mathsf{sel}_b,\zeta^{b}_{\mathrm{hi}})\bigl(\gamma-\sum_i\alpha^{i}c_{b,i}(x)\bigr)$, a degree-$2$ polynomial in $T_j$'s columns (degree $1$ but for the products of \S\ref{sec:m3-addr}) every coefficient of which the verifier forms itself (on the counts side $\alpha=1$ and $\gamma=0$, \S\ref{sec:memchan}, so it is a bare sum of count columns). The right-hand side is exactly a claim of \S\ref{sec:air}'s kind, with a nonzero value; its degree is the $d=2$ the batch already pays for the identities. So hand the three $B_j$, one per side, to the batch and run it at $r=\zeta$. They take three powers of $\eta$ placed after all $\sigma=\sum_j\ncons{j}$ identity powers of \S\ref{sec:air} and \emph{shared} by every table, so with a table's real constraints vanishing,
\[
