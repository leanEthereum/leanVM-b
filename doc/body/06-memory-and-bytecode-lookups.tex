...
The interaction carries $\tup{\dsep{MEM},\addr,\cnt,\val}$, every coordinate in $\K$: the address and count are $\gen$-powers, and the value is one $64$-bit memory word. The prover commits a single memory-value column $M$ ($2^{h}$ cells) and the counts ("in the exponent", i.e. powers of $\gen$). With $A[i]$ the number of reads of address $i$, the flushes are:
\begin{itemize}
