...
\begin{enumerate}
\item \textbf{Prover.} Commits every column: each table's state, operands, read values and counts; the three memory limbs; the two finalize counts; and flock's packed witness $q_\flock$ (\S\ref{sec:tab-blake3}). A program that never executes \texttt{BLAKE3} still runs flock's least instance count of dummy compressions, so the proof shape stays uniform. The prover stacks the columns into $q$ and sends the commitment (\S\ref{sec:stacking}, Annex~\ref{annex:pcs}).
...
\item \textbf{Prover \& Verifier.} Run the table sumcheck, $n=\max_j\tau_j$ rounds, a table of $2^{\tau}$ rows joining at round $n-\tau$. The target is the one the verifier derived from the three leaf claims. Each round costs four $\E$-elements. The end yields one claim per column of every table, at that table's own point $\rho_{<\tau_j}$; these enter the pool, and the bus forms are read off them. \texttt{BLAKE3}'s value columns are not committed here: their claims route to $q_\flock$ (\S\ref{sec:tab-blake3}).
\end{enumerate}
