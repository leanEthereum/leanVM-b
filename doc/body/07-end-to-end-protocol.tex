...
\paragraph{Opening.}
\begin{enumerate}
\item \textbf{Prover \& Verifier.} Each pooled claim is one evaluation of a committed column at a point, its value already supplied. The verifier removes any fixed public-padding contribution and, via the interval indicator of \S\ref{sec:stacking}, turns the real-prefix claim into a weighted sum $\sum_w W_j(w)\,\widetilde q(w)=c_j$ over the dense jagged witness. (The ring-switched claims arrive already in this form, their weights supported on the $q_\pkd$ prefix.)
\item \textbf{Verifier.} Sends the batching challenge $\lambda\in\E$.
\item \textbf{Prover \& Verifier.} Fold the $J$ claims into $W_\lambda=\sum_j\lambda^{j} W_j$ and $C_\lambda=\sum_j\lambda^{j} c_j$, and run the inner-product PCS opening on $\sum_w W_\lambda(w)\,\widetilde q(w)=C_\lambda$ (\S\ref{sec:rs-dense}), the verifier evaluating $W_\lambda$ itself. This one opening discharges every pooled claim; there is no separate reduction sumcheck.
