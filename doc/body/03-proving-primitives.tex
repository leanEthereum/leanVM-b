...
\]
of degree at most $d$, the verifier checks $h_j(0)+h_j(1)\qeq s_{j-1}$, samples $\rho_j\leftarrow\E$ and takes $s_j=h_j(\rho_j)$. It ends holding $s_{\kappa}=P(\rho)$, for the caller to settle. A false $s$ survives with probability at most $d\kappa/|\E|$.
\end{fact}
...
\paragraph{Two savings on a round message~\cite{Gru24}.} A round polynomial of degree $D$ would take $D+1$ values, but the round check $h_j(0)+h_j(1)\qeq s_{j-1}$ already pins one of them to the incoming claim, so $D$ are enough. The same idea applies to the known factor: a zerocheck's summand carries $\eq(r,\cdot)$, which the verifier can form itself, so only the cofactor travels and it is one degree lower. A zerocheck round on a degree-$d$ cofactor therefore costs $d$ field elements.
