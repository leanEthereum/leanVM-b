...
\]
By Fact~\ref{fact:eq} that sum is $\mle{g}(r)$, for $g$ the table of $P$'s values on the boolean hypercube: the zero polynomial if $P$ vanishes there, and otherwise a nonzero multilinear, which a random $r$ sends to $0$ with probability at most $\kappa/|\E|$ (Lemma~\ref{lem:sz}).
\end{definition}
...
\[
  ...
  \qquad c\in\E .
\]
The commitment is over $\K$, the opening over $\E$.
\end{definition}

We use WHIR~\cite{ACFY25} / Ligerito~\cite{NA25}, whcih are inner-product commitment schemes. The two are essentially the same scheme when WHIR is instantiated with interleaved codes and Ligerito with the Reed--Solomon codes. Specified in annex \ref{annex:pcs}
