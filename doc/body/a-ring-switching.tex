...
Turning $64$ evaluation claims over $\E$ into one opening of a commitment whose values are $64$ bits wide. This annex is the general construction; \S\ref{sec:ringswitch} instantiates it on the packed flock witness $q_\flock$.
...
\[
Q_\flock:\cube{\nflock}\to\K,\qquad Q_\flock(u)=\sum_{i=0}^{63}Q_i(u)\,x^i.
\]
The commitment scheme proves statements of one shape. Given a target $T\in\E$ and a transparent weight $W:\cube{\nflock}\to\E$, one that the verifier can evaluate for itself, an opening proves
\[
\sum_{u\in\cube{\nflock}}W(u)\,Q_\flock(u)=T.
\]
...
\begin{equation}
\begin{split}
\sum_{u\in\cube{\nflock}}\Phi\bigl(\eq(r,u)\bigr)\,Q_\flock(u)
