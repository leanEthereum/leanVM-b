...
\section{The polynomial commitment scheme}\label{annex:pcs}

\begin{center}
\fbox{\textbf{Warning: AI-assisted writing, some parts have not been properly reviewed yet.}}
\end{center}

A self-contained description of the WHIR\slash Ligerito polynomial commitment scheme \cite{ACFY25,NA25} over binary fields, with a round-by-round soundness analysis in the Johnson list-decoding regime (nothing new). The two schemes coincide when WHIR is instantiated with interleaved codes and Ligerito with the Reed--Solomon codes described here. Reed--Solomon encoding is by the additive NTT in the novel polynomial basis~\cite{LCH14}, as used in~\cite{DP26}. Following Flock~\cite{Flock26}, we do not use out-of-domain (OOD) sampling at commitment, so the PCS is only list binding. \S\ref{sec:rs-dense} fixes the instantiation this VM uses.

\paragraph{Symbols.} Each symbol means one thing across the whole document, so where the main matter had already spent a letter, the object below carries a free one. The papers this annex follows use the middle column; \path{crates/pcs} uses those names too.
...
$\rate_i$      & $\rho_i$     & code rate ($\rho$ is a sumcheck point in \S\ref{sec:air})\\
$\rexp_i$      & $R_i$        & rate exponent, $\rate_i=2^{-\rexp_i}$\\
$\rad_i$       & $\gamma_i$   & proximity radius ($\gamma$ is the bus challenge, \S\ref{sec:gp})\\
$\slack_i$     & $\eta_i$     & Johnson slack ($\eta$ batches the constraints, \S\ref{sec:air})\\
...
$\blen$        & $n$          & code block length ($n$ is the tallest table, \S\ref{sec:air})\\
...
$\beta$        & $b$          & bit width of $\F$ ($b$ is a \texttt{JUMP} column)\\
\hline
\end{tabular}
\end{center}

\noindent Bound variables keep the short names: $i,j,q$ index levels, rounds and queries; $u,v$ are boolean hypercube points and $x$ a domain element; and $a$, $d_i$, $\varphi$, $t$, $B_1$, $B_2$, $\iota$ are local to the single proof or algorithm that introduces them.
...
\paragraph{Notation.}
Throughout, $\F = \F_{2^{\beta}}$ is a binary field; $\log$ is base $2$; $[\blen] := \{1,\dots,\blen\}$. Query indices are sampled i.i.d.\ uniformly (with replacement).

\subsubsection{Multilinear extensions}

Every $f : \cube{\nv} \to \F$ has a unique multilinear extension $\mle{f}(x) = \sum_{u \in \cube{\nv}} f(u) \cdot \eq(u, x)$, where $\eq(u, x) := \prod_{i}(u_i x_i + (1-u_i)(1-x_i))$; we call $f$ the \emph{table} of $\mle{f}$ and freely identify a multilinear polynomial with its table.

\begin{fact}[Partial evaluation is a fold of the table]\label{fact:fold}
For every $f : \cube{\nv} \to \F$, $1 \le \ell \le \nv$, $\fc \in \F^{\ell}$, and $x \in \cube{\nv - \ell}$: $\ \mle{f}(\fc, x) = \sum_{u \in \cube{\ell}} \eq(\fc, u) \cdot f(u, x)$. That is, the table of $\mle{f}(\fc, \cdot)$ is the $\eq(\fc, \cdot)$-weighted combination of the rows $f(u, \cdot)$.
\end{fact}

\begin{fact}[Schwartz--Zippel for multilinears]\label{fact:sz}
Distinct multilinear $\mle{f} \neq \mle{g}$ in $\nv$ variables satisfy $\Pr_{z \leftarrow \F^{\nv}}[\mle{f}(z) = \mle{g}(z)] \le \nv / |\F|$.
\end{fact}

\begin{definition}[Weighted claim]\label{def:claim}
A \emph{weighted claim} about $g : \cube{\nv} \to \F$ is a pair $(W, c)$ asserting $\inner{W}{g} := \sum_{x \in \cube{\nv}} W(x)\, g(x) = c$; the weight $W$ is \emph{MLE-friendly} if $\mle{W}$ is evaluable anywhere in $\mathrm{poly}(\nv)$ operations. An evaluation claim $\mle{g}(r) = c$ is the case $W = \eq(r, \cdot)$.
\end{definition}

\subsubsection{Codes, interleaving, and list decoding}\label{sec:codes}

For a domain $\dom \subseteq \F$, $|\dom| = \blen$, and $\kdim \le \blen$, let $\RS[\F, \dom, \kdim] := \{ (p(x))_{x \in \dom} : \deg p < \kdim \}$, of rate $\rate := \kdim/\blen$. Message lengths are powers of two, $\kdim = 2^{\kappa}$, messages are indexed by $\cube{\kappa}$, and an \emph{encoder} is a linear injection $\Enc : \F^{\cube{\kappa}} \to \F^{\dom}$ onto the code (fixed concretely in \S\ref{sec:binary-rs}).

\begin{definition}[Interleaved words, lists, folds]\label{def:interleaved}
An \emph{interleaved word} is a matrix $\orc \in \F^{\cube{\ell} \times \dom}$; it is an \emph{interleaved codeword} if every row $\orc[u, \cdot]$ is a codeword. $\orc$ \emph{agrees} with an interleaved codeword $U$ on $\aset \subseteq \dom$ if $\orc[u,x] = U[u,x]$ for all $u$ and all $x \in \aset$; it is \emph{$\rad$-close} to $U$ if they agree on some $\aset$ with $|\aset| \ge (1-\rad)\blen$; and $\lst_\rad(\orc)$ is the set of interleaved codewords $\rad$-close to $\orc$. An interleaved codeword $U$ \emph{decodes} to the table $g_U : \cube{\ell + \kappa} \to \F$ with $U[u, \cdot] = \Enc(g_U(u, \cdot))$; distinct $U$ decode to distinct $g_U$, hence distinct $\mle{g_U}$. For $\fc \in \F^{j}$, $j \le \ell$, the \emph{fold} $\orc_\fc \in \F^{\cube{\ell - j} \times \dom}$ is $\orc_\fc[u, x] := \sum_{\iota \in \cube{j}} \eq(\fc, \iota)\, \orc[(\iota, u), x]$ (for $j = \ell$, a single word in $\F^{\dom}$).
\end{definition}

\begin{fact}[Folding commutes with decoding]\label{fact:fold-decode}
If $U$ is an interleaved codeword then so is $U_\fc$, with $\mle{g_{U_\fc}} = \mle{g_U}(\fc, \cdot)$ (linearity of $\Enc$ plus Fact~\ref{fact:fold}).
\end{fact}

\begin{theorem}[Johnson bound]\label{thm:johnson-interleaved}
For every interleaved word $\orc$ of $\RS[\F,\dom,2^\kappa]$ (rate $\rate$) and every $\slack \in (0, 1-\sqrt{\rate})$: $\ |\lst_{1-\sqrt{\rate}-\slack}(\orc)| \le \frac{1}{2\slack\sqrt{\rate}}$. (see \S\ref{app:johnson} for a self-contained proof)
\end{theorem}
...
For $\rexp \ge 1$ and $\kappa + \rexp \le \beta$, \S\ref{app:ntt} constructs, following~\cite{LCH14} as presented in~\cite{DP26}, an $\Ftwo$-subspace $\dom \subseteq \F$ of size $\blen = 2^{\kappa+\rexp}$ and an encoder $\Enc_{\kappa,\rexp} : \F^{\cube{\kappa}} \to \F^{\dom}$ onto $\RS[\F, \dom, 2^{\kappa}]$, of rate $\rate = 2^{-\rexp}$, with the two properties the protocol uses:
...
\item \emph{MLE-friendly columns, for the verifier:} for every $x \in \dom$, the weight $W_x : \cube{\kappa} \to \F$, defined by $\Enc_{\kappa,\rexp}(g)[x] = \inner{W_x}{g}$ (the relation being valid for arbitrary $g$), admits an MLE efficiently evaluable ($O(\kappa)$ multiplications and additions (Lemma~\ref{lem:colweight})).
\end{itemize}
...
\ \par\smallskip
\emph{Parameters.} The scheme commits to $f : \cube{\nv} \to \F$ across $\nlev \ge 1$ \emph{levels} $i = 0, \dots, \nlev-1$ plus a plaintext final level $\nlev$: variable counts $\nv = \nv_0 > \nv_1 > \dots > \nv_\nlev \ge 0$ with fold factors $\ell_i := \nv_i - \nv_{i+1} \ge 1$ and $\nv_\nlev$ small (e.g.\ $\nv_\nlev \le 5$); per level, a rate $\rate_i = 2^{-\rexp_i}$, $\rexp_i \ge 1$, giving the encoder $\Enc_i := \Enc_{\kappa_i, \rexp_i}$ of \S\ref{sec:binary-rs} with $\kappa_i := \nv_{i+1}$, i.e.\ the code $\mathcal{C}_i := \RS[\F, \dom_i, 2^{\kappa_i}]$ over its domain $\dom_i$ of size $\blen_i = 2^{\kappa_i + \rexp_i}$ (requires $\beta \ge \kappa_i + \rexp_i$); and a query count $\nq_i$. (The proximity radii $\rad_i \in (0, 1 - \sqrt{\rate_i})$ appear only in the analysis.)

\smallskip
\textbf{Commit}$(f : \cube{\nv} \to \F)$: $\mathbf{P}$ sends the oracle $\orc^{(0)}[u, \cdot] := \Enc_0(f(u, \cdot))$, $u \in \cube{\ell_0}$. No OOD sample is taken.

\smallskip
\textbf{Open}$\bigl(\{(W_\tau, c_\tau)\}_{\tau \in [J_0]}\bigr)$, $J_0 \ge 1$ weighted claims about $f$: set $f^{(0)} := f$. For $i = 0, 1, \dots, \nlev - 1$:
\begin{enumerate}[itemsep=0.5pt,topsep=2pt]
\item \emph{(batch)} $\mathbf{V}$ sends $\lambda \leftarrow \F$; the \emph{batched claim} is $(W, \rc{0}) := \bigl(\sum_{\tau \in [J_i]} \lambda^{\tau-1} W_\tau,\; \sum_{\tau} \lambda^{\tau-1} c_\tau\bigr)$.
\item \emph{(sumcheck~\cite{LFKN92})} For $j = 1, \dots, \ell_i$: $\mathbf{P}$ sends $h_j \in \F[X]$ with $\deg h_j \le 2$, honestly $h_j(X) = \sum_{x} \mle{W}(\fc^{<j}, X, x)\, \mle{f^{(i)}}(\fc^{<j}, X, x)$ where $\fc^{<j} := (\fc_1, \dots, \fc_{j-1})$; $\mathbf{V}$ checks $h_j(0) + h_j(1) = \rc{j-1}$ and sends $\fc_j \leftarrow \F$; both set $\rc{j} := h_j(\fc_j)$.\\
Set $\fc := (\fc_1, \dots, \fc_{\ell_i})$, $c' := \rc{\ell_i}$, and, for $x \in \cube{\nv_{i+1}}$: \ $f^{(i+1)}(x) := \mle{f^{(i)}}(\fc, x)$, \ $W'(x) := \mle{W}(\fc, x)$. The \emph{residual claim} $(W', c')$ is a weighted claim about the folded table $f^{(i+1)}$.
\item If $i < \nlev - 1$:
...
\end{enumerate}
\item If $i = \nlev - 1$: \emph{(final)} $\mathbf{P}$ sends the table $\ffinal : \cube{\nv_\nlev} \to \F$; $\mathbf{V}$ sends $\nq_{\nlev-1}$ i.i.d.\ columns $\qset_{\nlev-1} \leftarrow \dom_{\nlev-1}$, computes $c_q$ from $\orc^{(\nlev-1)}$ as in 3(c), and checks $\sum_{x \in \cube{\nv_\nlev}} W'(x)\, \ffinal(x) = c'$ and $\Enc_{\nlev-1}(\ffinal)[x_q] = c_q$ for all $q \in [\nq_{\nlev-1}]$.
\end{enumerate}
$\mathbf{V}$ accepts iff every check above passed.
\end{protocol}

All weights in the protocol are MLE-friendly and multilinear, so every sumcheck summand $\mle{W} \cdot \mle{f^{(i)}}$ has individual degree at most $2$.
...
Suppose that for every level $i$: $\rad_i = 1 - \sqrt{\rate_i} - \slack_i$ with $\slack_i \in (0, 1-\sqrt{\rate_i})$, $L_i := \tfrac{1}{2\slack_i\sqrt{\rate_i}}$, and $\epsilon_i := \mcanumi{i}/|\F|$ is the mutual correlated agreement error of Theorem~\ref{thm:mca-johnson} below for $\mathcal{C}_i$ at radius $\rad_i$. Then the Protocol~\ref{fig:protocol} is a $L_0$-list binding PCS, and its opening phase $\relopen$ has the following RBR soundness error at each verifier message:
\[
\begin{array}{llcl}
\textnormal{batching, level } i: & \varepsilon^{\mathrm{batch}}_i &=& \dfrac{(J_i - 1)\, L_i}{|\F|} , \\[2ex]
\textnormal{fold challenge } \fc_j, \textnormal{ level } i: & \varepsilon^{\mathrm{fold}}_{i,j} &=& \dfrac{2 L_i}{|\F|} + 2^{\ell_i - j}\, \epsilon_i , \\[2ex]
...
Let $\RS[\F,\dom,\kdim]$ have block length $\blen = |\dom|$ and relative minimum distance $\dmin := 1 - (\kdim-1)/\blen$. Suppose
\[
...
\[
\Pr_{\fc \leftarrow \F}\!\left[\; \exists \aset \subseteq \dom,\ |\aset| \ge (1-\rad) \blen :\ \begin{array}{l} \orc_\fc \text{ agrees with the code on } \aset, \text{ and} \\ \orc \text{ does not agree with the interleaved code on } \aset \end{array} \right]
\;\le\; \frac{\rad \blen + 1}{|\F|}.
\]
\end{theorem}
...
Let $\RS[\F,\dom,\kdim]$ have block length $\blen = |\dom|$ and set $\rate := (\kdim-1)/\blen$, the \emph{slightly reduced rate} (\cite{BCHKS25} parametrize by the degree bound $\kdim - 1$, not the dimension $\kdim$). Let $\rad \in (0, 1-\sqrt{\rate})$, and set $\slack := 1 - \sqrt{\rate} - \rad$ and $\mcapar := \max\{\lceil \tfrac{\sqrt{\rate}}{\slack} \rceil, 3\}$ (denoted $m$ in~\cite{BCHKS25}). For every two-row word $\orc$,
\[
\Pr_{\fc \leftarrow \F}\!\left[\; \exists \aset \subseteq \dom,\ |\aset| \ge (1-\rad) \blen :\ \begin{array}{l} \orc_\fc \text{ agrees with the code on } \aset, \text{ and} \\ \orc \text{ does not agree with the interleaved code on } \aset \end{array} \right] \;\le\; \frac{\mcanum}{|\F|},
\]
where
\[
\mcanum \;=\; \frac{2(\mcapar + \tfrac12)^5 + 3(\mcapar + \tfrac12)\rad\rate}{3\rate^{3/2}} \cdot \blen \;+\; \frac{\mcapar + \tfrac12}{\sqrt{\rate}}
\]
(``$\orc_\fc$ agrees with the code on $\aset$'' means some codeword equals $\orc_\fc$ on $\aset$; ``$\orc$ agrees with the interleaved code on $\aset$'' means some pair of codewords equals the two rows on $\aset$).
\end{theorem}

We write $\epsilon := \mcanum/|\F|$ for this MCA error at radius $\rad$. MCA is exactly what Lemma~\ref{lem:fold-list} needs: folding should not create \emph{new} nearby codewords.

\begin{lemma}[Folding preserves lists]\label{lem:fold-list}
Let $\RS[\F,\dom,\kdim]$ have rate $\rate$, let $\rad \in (0, 1-\rate)$, and let $\epsilon$ be an MCA error at radius $\rad$. For every interleaved word $\orc \in \F^{\cube{\ell} \times \dom}$, $\ell \ge 1$:
\[
\Pr_{\fc \leftarrow \F}\Bigl[\, \lst_\rad(\orc_\fc) \neq \{ U_\fc : U \in \lst_\rad(\orc) \} \,\Bigr] \;\le\; 2^{\ell-1} \epsilon .
\]
\end{lemma}
...
\[
\bigl(\, \mle{W}(\fc^{\le j}, \cdot),\; \rc{j} \,\bigr) \qquad \text{about tables } g : \cube{\nv_i - j} \to \F,
\]
where $W$, $\rc{0}$, and $\rc{j} = h_j(\fc_j)$ are read off the transcript as in Protocol~\ref{fig:protocol}; an interleaved codeword satisfies a claim if its decoded table does. The round-$0$ claim is the batched claim, and the round-$\ell_i$ claim is the residual claim $(W', c')$.
...
Assume that every $U \in \lst_{\rad_i}(\orc^{(i)})$ violates at least one of the $J_i$ claims $(W_\tau, c_\tau)$. Fix such a $U$ and let $d_\tau := \inner{W_\tau}{g_U} - c_\tau$ be its discrepancies, not all zero. Against the batched claim, $U$ has discrepancy
\[
\sum_{\tau \in [J_i]} \lambda^{\tau - 1} \bigl( \inner{W_\tau}{g_U} - c_\tau \bigr) \;=\; \sum_{\tau \in [J_i]} \lambda^{\tau - 1} d_\tau ,
\]
a nonzero polynomial in $\lambda$ of degree less than $J_i$, which vanishes for at most $J_i - 1$ challenges. Union bound over the at most $L_i$ list members: except with probability $(J_i - 1) L_i / |\F|$, every $U$ violates the round-$0$ claim.
...
\]
be the honest round polynomial for $U$; it has degree at most $2$, since $\mle{W}$ and $\mle{g_U}$ are multilinear. Summing over $X \in \{0, 1\}$ turns the multilinear evaluations back into table values, so
\[
...
\begin{itemize}[itemsep=1pt]
\item $B_1$: $h_j(\fc_j) = H_U(\fc_j)$ for some $U \in \lst$. For each $U$, two distinct polynomials of degree at most $2$ agree on at most $2$ points, so $\Pr[B_1] \le 2 L_i / |\F|$.
...
Fix an $\Ftwo$-basis $e_0, \dots, e_{\beta-1}$ of $\F$ with $e_0 = 1$. This appendix constructs the encoder of \S\ref{sec:binary-rs} and proves its two properties; the material follows~\cite{LCH14} as presented in~\cite{DP26}. For $u \in \cube{\nv}$ we write $\langle u \rangle := \sum_i u_i 2^i$ for the integer with bits $u$ (bit $0$ first).

\subsubsection{Subspace polynomials, novel basis, encoder}

For $0 \le i \le \beta$ let $\subsp_i := \langle e_0, \dots, e_{i-1} \rangle_{\Ftwo}$ and define the \emph{subspace vanishing polynomial} $\vansub_i(X) := \prod_{u \in \subsp_i}(X + u)$, of degree $2^i$.

\begin{lemma}\label{lem:subspace}
For $i < \beta$: (i) $\vansub_{i+1}(X) = \vansub_i(X)\bigl(\vansub_i(X) + \vansub_i(e_i)\bigr)$ with $\vansub_i(e_i) \neq 0$; (ii) $\vansub_i(X + Y) = \vansub_i(X) + \vansub_i(Y)$ as a polynomial identity; in particular the map $\vansub_i : \F \to \F$ is $\Ftwo$-linear.
\end{lemma}

\begin{proof}
By induction on $i$; the base case is $W_0(X) = X$. For (i), split the product defining $\vansub_{i+1}$ along $\subsp_{i+1} = \subsp_i \sqcup (e_i + \subsp_i)$:
\[
...
Let $\Wn{i} := \vansub_i / \vansub_i(e_i)$ (the hat on $W$ denotes normalization, not multilinear extension), so $\Wn{i}(e_i) = 1$ and $\Wn{0} = X$ (using $e_0 = 1$). By linearity, $\Wn{i}(x) = \sum_c x_c \Wn{i}(e_c)$ for $x = \sum_c x_c e_c$, so given the table of values $\Wn{i}(e_c)$ (precomputed via the recursion (i)), $\Wn{i}$ is evaluated anywhere in $O(b)$ additions.

\begin{definition}[Novel basis and encoder~\cite{LCH14}]\label{def:enc}
For $0 \le j < 2^{\kappa}$ with bits $j_i$, let $\novel_j(X) := \prod_i \Wn{i}(X)^{j_i}$; then $\deg \novel_j = j$, so $(\novel_j)_{j < 2^\kappa}$ is a (triangular) basis of the polynomials of degree less than $2^{\kappa}$. Let $\dom := \langle e_0, \dots, e_{\kappa+\rexp-1} \rangle_{\Ftwo}$ and, for a message $g : \cube{\kappa} \to \F$,
\[
\mpoly_g(X) := \sum_{u \in \cube{\kappa}} g(u) \cdot \novel_{\langle u \rangle}(X), \qquad \Enc_{\kappa,\rexp}(g) := \bigl(\mpoly_g(x)\bigr)_{x \in \dom},
\]
a linear injection onto $\RS[\F, \dom, 2^{\kappa}]$ of rate $\rate = 2^{-\rexp}$. (The table is read \emph{directly} as novel-basis coefficients; no conversion is applied.)
\end{definition}
...
\[
\ntmap_d(X) \;:=\; \frac{\vansub_d(e_d)^2}{W_{d+1}(e_{d+1})} \cdot X(X+1).
\]
