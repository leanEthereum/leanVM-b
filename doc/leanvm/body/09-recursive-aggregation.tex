...
\item \textbf{the flock matrices}: the \texttt{KECCAK} relation is proven by flock (\S\ref{sec:tab-keccak}, Annex~\ref{annex:flock}), whose lincheck ends by evaluating $\widetilde A_0$ and $\widetilde B_0$, the per-block R1CS matrices: $32$-variable multilinears, far too dense to materialize.
\end{itemize}
