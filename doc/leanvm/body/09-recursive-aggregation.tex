...
\item \textbf{the flock matrices}: the \texttt{SHA2} relation is proven by flock (\S\ref{sec:tab-sha2}, Annex~\ref{annex:flock}), whose lincheck ends by evaluating $\widetilde A_0$ and $\widetilde B_0$, the per-block R1CS matrices: $30$-variable multilinears with $\approx 1.1\mathrm{M}$ nonzero Boolean entries between them.
\end{itemize}
...
\]
and ends in the reduced claims $\widetilde A_0(r^\ast)$ and $\widetilde B_0(r^\ast)$, at a common point. The in-circuit cost is that of the sumcheck verifier: $30$ cheap rounds. A carried claim enters with the plain weight above. A fresh one enters with the weight of \eqref{flock:eq:matrix-form}, $\erow\otimes \wcol$ on the row and column halves, at relative weight $\lcalpha$ of $B_0$ against $A_0$; its four tensor factors are evaluated at $r^\ast$ from the child's $\zskip$, $\fc_{\mathrm{in}}$, $s$ and $\fc'_{\mathrm{in}}$, all of which ride the aggregation statement. The prover never forms the $2^{30}$-sized boolean hypercube, and never reads a matrix entry either: both contractions it needs are circuit walks (\S\ref{flock:matrices}). The first $15$ rounds (the row variables) enter through the column contraction $m_t[i]=\sum_{j\in\mathrm{row}_i}\eq(r_t^{\mathrm{col}},j)$, which is the forward walk at the claim's column weights, keeping each row's value rather than contracting it; the last $15$ contract each matrix along its rows at the row point just fixed, which is the backward walk. Each is $O(\text{circuit})$, so the whole batch costs one forward walk per claim and two backward walks, plus $O(t\cdot2^{15})$ multiplications on dense tables of size $2^{15}$, against the $O(2^{30})$ field operations and memory of a dense sumcheck and the $\approx 1.1\mathrm{M}$ nonzeros a sparse one would iterate. The bytecode sumcheck is the same, only smaller, dense, and skipping the nine of sixteen stacking slots that are structurally zero.
