...
The \emph{signer-set digest} is BLAKE2s of the following string, whole $64$-byte blocks by construction, where each list's digest is itself BLAKE2s of that list alone (the keys of one group, $32$ bytes each; the SPHINCS claims, $64$ bytes each):
\begin{center}
\begin{tabular}{@{}ll@{}}
\hline
block $0$ & a domain tag, then $\gen^{\#\mathrm{XMSS\ groups}}$, $\gen^{\#\mathrm{SPHINCS\ claims}}$\\
block $1$ & the SPHINCS claim list's digest, then two zero cells\\
per XMSS group & $\mathrm{epoch}$, $\gen^{\#\mathrm{keys}}$, then the group's message\\
& then that group's key list's digest, and two zero cells\\
\hline
\end{tabular}
\end{center}
