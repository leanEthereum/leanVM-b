...
Over $\kappa$ rounds, sumcheck reduces a claim $\sum_{x\in\cube{\kappa}}P(x)=s$, on a polynomial $P$ of degree at most $d$ in each variable, to one evaluation $P(\fc)$ at a random $\fc\in\E^{\kappa}$. Round $j$ carries a claim $s_{j-1}$, with $s_0=s$: the prover sends the univariate
\[
