...
Over $\kappa$ rounds, sumcheck reduces a claim $\sum_{x\in\cube{\kappa}}P(x)=\rc{0}$, on a polynomial $P$ of degree at most $d$ in each variable, to one evaluation $P(\fc)$ at a random $\fc\in\E^{\kappa}$. Round $j$ carries the running claim $\rc{j-1}$: the prover sends the univariate
\[
  h_j(Y)=\sum_{x\in\cube{\kappa-j}}P(\fc_1,\dots,\fc_{j-1},Y,x)
\]
of degree at most $d$, the verifier checks $h_j(0)+h_j(1)\qeq\rc{j-1}$, samples $\fc_j\leftarrow\E$ and takes $\rc{j}=h_j(\fc_j)$. It ends holding $\rc{\kappa}=P(\fc)$, for the caller to settle. A false $\rc{0}$ survives with probability at most $d\kappa/|\E|$.
\end{fact}

\begin{definition}[Zerocheck]\label{def:zerocheck}
To prove that a polynomial $P$ vanishes on $\cube{\kappa}$, the verifier samples $r\in\E^{\kappa}$ uniformly and the two parties run sumcheck on
\[
...
\paragraph{Two savings on a round message~\cite{Gru24}.} A round polynomial of degree $D$ travels as its coefficients, and would take $D+1$ of them, but the round check $h_j(0)+h_j(1)\qeq s_{j-1}$ already pins one to the incoming claim, so $D$ are enough. The same idea applies to the known factor: a zerocheck's summand carries $\eq(r,\cdot)$, which the verifier can form itself, so only the cofactor travels and it is one degree lower. A zerocheck round on a degree-$d$ cofactor therefore costs $d$ field elements.
