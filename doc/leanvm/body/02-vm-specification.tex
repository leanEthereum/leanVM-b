...
\]
There is no block-length field: the byte counter is cumulative through the block being absorbed, so it is what says how many of the $64$ bytes are message, and only a final block may be partial. For a one-block $64$-byte hash, the chaining value is the parameterized BLAKE2s-256 IV (digest length $32$, unkeyed, fanout and depth $1$) and
\[
  \md=(64,\;2^{32}-1,\;0),
\]
the final-block flag being all ones as RFC 7693~\cite{blake2} sets it on the last block, and the last-node flag zero: nothing here uses BLAKE2s's tree mode. A longer standard BLAKE2s hash of any length passes each compression result back through $o_V$ and sets the metadata immediate to the running byte counter, with the final-block flag on the last block. BLAKE2s has no chunk tree and no parent-node mode, so that one chain covers every input length: this is why a single compression opcode is a complete hash primitive here. The BLAKE2s table (\S\ref{sec:tab-blake2s}) wires its memory and bytecode operands into a flock~\cite{flock-impl} R1CS proof.
