...
\paragraph{Instruction set.} The machine implements six instructions. Below, $o,o_A,\dots$ are fp-relative references in $\K$, $k$ is an immediate operand in $\E$.
\begin{center}
...
\texttt{DEREF}         & $[\alpha,\beta,\gamma;\,s]$ & $\mem[\loc{\alpha}\cdot\beta]=\srcsel{s}$ (see below)\\
\texttt{JUMP}          & $[o_c,o_d,o_f]$ & conditional jump (see below)\\
\texttt{BLAKE2S}        & $[o_0,o_1,o_2,o_3,o_V,o_C,\md]$ & BLAKE2s compression (see below)\\
\hline
\end{tabularx}
\end{center}
...
\[
  \srcsel{s}\;=\;
  \begin{cases}
    \loc{\gamma} & s=\texttt{deref\_cell}[\gamma] \quad(\text{the local cell at offset }\gamma \in \K),\\
    \gen^{2}\cdot\pc & s=\texttt{deref\_pc} \quad(\text{the current program counter advanced by two}),\\
    \fp & s=\texttt{deref\_fp} \quad(\text{the current frame pointer}).
  \end{cases}
\]

Additionally, it asserts $\loc{\alpha} \in \K$.
...
\texttt{BLAKE2S} is the standard BLAKE2s compression, consuming a $64$-byte message block and a $32$-byte chaining value and producing $32$ bytes. Each $128$-bit chunk occupies one full $\E$ memory cell through the canonical embedding $a_0+a_1y\mapsto a_0+a_1y+0y^2$; using a cell as a BLAKE2s operand therefore constrains its top limb to zero. The four message chunks are addressed \emph{independently} by references $o_0,\dots,o_3$ (chunk $i$ at $\loc{o_i}=\fp\cdot o_i$), while $o_V$ names two \emph{consecutive} chaining-value cells $\loc{o_V},\loc{\gen\cdot o_V}$. The $256$-bit result is written to the consecutive cells $\loc{o_C},\loc{\gen\cdot o_C}$, so each row accesses eight cells. The $128$-bit immediate $\md \in \E$ (with top lane zero) packs the standard compression metadata in little-endian order,
\[
