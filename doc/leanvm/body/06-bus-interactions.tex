...
The bytecode is an array of $\nprog$ instructions, $\nprog\le2^{32}$ (\S\ref{sec:e2e-bc}). Bus interaction uses domain separation $\sep=\dsep{BC}$. An instruction is an opcode plus six operand or immediate slots, so $\ncomp=7$: the widest entry on the bus, which is what fixes $m=10$ (\S\ref{sec:m3}). Six and not eight because \texttt{SHA2} carries no immediate: the two slots a byte counter and finalization flags would occupy were zero for every other opcode. The program is public, i.e. not committed, the verifier can evaluate it directly.
