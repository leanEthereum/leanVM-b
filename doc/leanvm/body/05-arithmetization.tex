...
\]
the verifier computing the right side itself, at soundness error $\bigl(\taumax+\nside+\sum_j\ncons{j}\bigr)/|\E|$ (Corollary~\ref{cor:idtest}). Rounds bind $X_{\taumax-1}$ first, so $T_j$ joins only at round $\taumax-\tau_j$: hence \emph{back-loaded}. The end yields one claim per column, $T_j$'s at $\fc_{<\tau_j}$, settled by the PCS opening (\S\ref{sec:stacking}).
