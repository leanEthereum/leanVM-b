...
The bus balances when its pushed and pulled tuples form the same multiset (\S\ref{sec:m3}). Random challenges, $\alpha_0,\dots,\alpha_3$ then $\beta$, reduce this to a single product.
...
\emph{Product check.} Both sides carry the same number of tuples, and padding each with factors of $1$, which change no product, makes that number a power of two, $2^{\mu}$. The verifier samples $\beta\in\E$, and balance becomes
\begin{equation}
...
\]
The same sumcheck then reduces $\widetilde V_i(r)$ to the four values $\widetilde V_{i-2}(a,b,\fc)$, which the prover sends and two fresh challenges $(u_0,u_1)$ combine into one claim $\widetilde V_{i-2}(u_0,u_1,\fc)$, two layers down. Its cofactor has degree four, so a round costs four $\E$-elements (\S\ref{sec:prelim}), against three for the binary tree above.
