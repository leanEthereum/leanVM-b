...
The ISA makes it possible to use values of $\fp$ outside the range $\{\gen^{0},\dots,\gen^{2^{h}-1}\}$. As long as every memory access of the form $\loc{o}$ (for some offset $o$) is in range, i.e.\ $\fp\cdot o=\gen^{i}$ with $i<2^{h}$, the execution is valid. Yet, reasoning about such out-of-range values of $\fp$ is tricky. The initial $\fp$ is in range, since the state boundary condition fixes $\fp_{0}=\gen^{0}$ (\S\ref{sec:state}), but nothing guarantees it for the ones a \texttt{JUMP} sets. We thus suggest the following programming convention: whenever a \texttt{JUMP} updates $\fp$, the program should guarantee that the new value is in range. The calling convention above does it for free:

\begin{itemize}
\item On a call, $\fp\gets\loc{o_f}$, where $\loc{o_f}=\mathit{nfp}$ is hinted, hence arbitrary. But the callee frame was initialized with the \texttt{DEREF} $\mem[\mathit{nfp}]\gets\gen^{2}\cdot\pc$, whose memory access at address $\mathit{nfp}$ forces $\mathit{nfp}=\gen^{i}$ with $i<2^{h}$ (Corollary~\ref{cor:addrrange}).
