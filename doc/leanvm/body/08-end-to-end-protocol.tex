...
The public input is the first two memory cells $\mem[\gen^{0}],\mem[\gen^{1}]$, each a $192$-bit word, fixed and agreed by both parties. Those two cells span the subcube of the address hypercube $\cube{\kmem}$ with the $\kmem-1$ high coordinates $0$ and the low one free, so on \emph{each} memory limb $\mem_\ell$ ($\ell\in\{0,1,2\}$, \S\ref{sec:memchan}) the MLE restricts there to the line $\widetilde{\mathrm{PI}}_\ell(r)=(1+r)\,\mem[\gen^{0}]_\ell+r\,\mem[\gen^{1}]_\ell$, which the verifier forms itself from the public words. It samples $r_m\in\E$ and checks all three limbs,
\[
  \mle{\mem_\ell}(r_m,0,\dots,0)\;\qeq\;\widetilde{\mathrm{PI}}_\ell(r_m),\qquad \ell\in\{0,1,2\}.
\]
The prover sends the three claimed values, the verifier checks each against the line, and the three claims are deferred to the PCS (\S\ref{sec:stacking}).
...
\item \textbf{Prover \& Verifier.} The prover sends one evaluation per memory limb; the verifier checks the three against the line and pools them as claims on $\mem_0,\mem_1,\mem_2$ at $(r_m,0,\dots,0)$ (\S\ref{sec:e2e-pi}).
\end{enumerate}
