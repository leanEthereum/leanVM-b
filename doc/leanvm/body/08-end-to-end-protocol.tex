...
\paragraph{Tags.} Coordinate $0$ of every tuple is a domain separator naming its interaction, and coordinate $3$ of a bytecode tuple is an opcode naming its instruction; both take the values of \S\ref{sec:omc}.

\subsection{Bytecode encoding}\label{sec:e2e-bc}

The program is public. The bus interaction carrying the bytecode lookup has the following shape:
\[
...
\texttt{BLAKE2S}        & $\opc{B3}$  & $o_{m_0}$ & $o_{m_1}$ & $o_{m_2}$ & $o_{m_3}$ & $o_{\mathit{cv}}$ & $o_{\mathit{out}}$ & $\md_0$ & $\md_1$\\
\hline
\end{tabular}
\end{center}
