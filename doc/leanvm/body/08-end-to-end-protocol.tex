...
\item \textbf{Prover \& Verifier.} Run the table sumcheck, $\taumax=\max_j\tau_j$ rounds, a table of $2^{\tau}$ rows joining at round $n-\tau$. The target is the one the verifier derived from the three leaf claims. Each round costs four $\E$-elements. The end yields one claim per column of every table, at that table's own point $\fc_{<\tau_j}$; these enter the pool, and the bus forms are read off them. \texttt{SHA2}'s value columns are not committed here: their claims route to $\qflock$ (\S\ref{sec:tab-sha2}).
\end{enumerate}
