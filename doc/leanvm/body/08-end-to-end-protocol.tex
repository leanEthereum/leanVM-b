...
\]
Every column is $\K$-valued, and so is every constraint on them (\S\ref{sec:prelim}); a machine word is an $\E$-element only in the sense that its three $\K$-lanes are read as one, and challenges are $\E$-valued for soundness. Every bus tuple has width $m=10$ (the bytecode tuple); shorter tuples are zero-padded in their high coordinates (\S\ref{sec:m3}).
...
\[
  \opc{XOR}=1,\quad \opc{MUL}=\gen,\quad \opc{SET}=\gen^{2},\quad \opc{DRF}=\gen^{3},\quad \opc{JMP}=\gen^{4},\quad \opc{SHA}=\gen^{5},\quad \opc{PACK}=\gen^{6}.
\]
...
\]
Coordinate $3$ holds the opcode and coordinates $4,\ldots,9$ hold six operands or immediate lanes. Each reference operand is a $\gen$-power. Immediates occupy their $\K$-lanes: $k_0,k_1,k_2$ for \texttt{SET\_CONSTANT}. Six rather than eight because \texttt{SHA2} carries no immediate: the two slots a byte counter and finalization flags would need were zero for every other opcode.
\begin{center}
\begin{tabular}{llllllll}
\hline
Instruction & $3$ & $4$ & $5$ & $6$ & $7$ & $8$ & $9$\\
\hline
\texttt{XOR}           & $\opc{XOR}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$ & $0$\\
\texttt{MUL\_NATIVE}   & $\opc{MUL}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$ & $0$\\
\texttt{SET\_CONSTANT} & $\opc{SET}$ & $o$   & $k_0$ & $k_1$ & $k_2$ & $0$ & $0$\\
\texttt{DEREF}         & $\opc{DRF}$ & $\alpha$ & $\beta$ & $\gamma$ & $f_{\pc}$ & $f_{\fp}$ & $0$\\
\texttt{JUMP}          & $\opc{JMP}$ & $o_c$ & $o_d$ & $o_f$ & $0$ & $0$ & $0$\\
\texttt{PACK64X2}      & $\opc{PACK}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$ & $0$\\
\texttt{SHA2}          & $\opc{SHA}$ & $o_0$ & $o_1$ & $o_2$ & $o_3$ & $o_V$ & $o_C$\\
\hline
\end{tabular}
\end{center}
The program is padded to $\nprog=2^{\kbc}$ instructions (\S\ref{sec:idxcol}) and read as one $\K$-valued multilinear $P$ in $\kbc+4$ variables: the low $\kbc$ index the instruction, the high four its slot. Slot $i$ holds tuple coordinate $i$: the opcode at $3$, the operands and immediates at $4,\dots,9$, the rest zero. Writing slot bits low first, $P(z,1,1,0,0)$ is instruction $z$'s opcode, $P(z,0,0,1,0)$ its first operand, and $P(z,0,0,0,0)=P(z,0,0,1,1)=0$.
...
\item \textbf{Prover \& Verifier.} Run the table sumcheck, $n=\max_j\tau_j$ rounds, a table of $2^{\tau}$ rows joining at round $n-\tau$. The target is the one the verifier derived from the three leaf claims. Each round costs four $\E$-elements. The end yields one claim per column of every table, at that table's own point $\rho_{<\tau_j}$; these enter the pool, and the bus forms are read off them. \texttt{SHA2}'s value columns are not committed here: their claims route to $\qflock$ (\S\ref{sec:tab-sha2}).
\end{enumerate}
...
\paragraph{SHA-256 validity.}
\begin{enumerate}
\item \textbf{Prover \& Verifier.} Reduce the executed SHA-256 constraints to one evaluation claim on $\qflock$ (\S\ref{sec:tab-sha2}, Annex~\ref{annex:flock}).
