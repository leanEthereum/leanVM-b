...
\]
Coordinate $3$ holds the opcode and coordinates $4,\ldots,11$ hold eight operands or immediate lanes. Each reference operand is a $\gen$-power. Immediates occupy their $\K$-lanes: $k_0,k_1,k_2$ for \texttt{SET\_CONSTANT}. \texttt{KECCAK} has none, a permutation carrying no counter and no flags.
\begin{center}
...
\texttt{KECCAK}        & $\opc{B3}$  & $o_0$ & $o_1$ & $o_2$ & $o_3$ & $o_R$ & $o_P$ & $o_C$ & $0$\\
\hline
\end{tabular}
\end{center}
...
\item \textbf{Prover \& Verifier.} Run the table sumcheck, $\taumax=\max_j\tau_j$ rounds, a table of $2^{\tau}$ rows joining at round $n-\tau$. The target is the one the verifier derived from the three leaf claims. Each round costs four $\E$-elements. The end yields one claim per column of every table, at that table's own point $\fc_{<\tau_j}$; these enter the pool, and the bus forms are read off them. \texttt{KECCAK}'s value columns are not committed here: their claims route to $\qflock$ (\S\ref{sec:tab-keccak}).
\end{enumerate}
...
\paragraph{Keccak validity.}
\begin{enumerate}
\item \textbf{Prover \& Verifier.} Reduce the executed Keccak constraints to one evaluation claim on $\qflock$ (\S\ref{sec:tab-keccak}, Annex~\ref{annex:flock}).
