...
Flock~\cite{Flock26} proves many executions of one Boolean circuit at once, in our case the SHA-256 compression. Since we do not follow their protocol exactly, we describe our version here.

\subsection{Statement and commitment}\label{flock:statement}

One SHA-256 compression has a Boolean R1CS witness block of $2^{\kblock}$ bits, where $\kblock=15$ ($29{,}113$ of those rows carry data, the rest are zero padding). Its fixed matrices are $A_0,B_0,C_0\in\Ftwo^{2^{\kblock}\times 2^{\kblock}}$, with $C_0=I$. The witness includes the circuit inputs, outputs, the $\mathrm{Ch}$ and $\mathrm{Maj}$ AND values, addition carries, the materialized message-schedule and round-state words and one constant-one position; every other linear wire is substituted away. Within-block coordinates are the low-order coordinates and batch coordinates are the high-order coordinates throughout this annex, and a Boolean tuple is also read as an integer, lowest coordinate first.
...
\begin{align*}
\qext a(\zskip,\rho_{\mathrm{in}},\rho_{\mathrm{out}})&=\sum_{\ksk\in\cube{\kskip}}\lag_{\ksk}(\zskip)\,\mle a(\ksk,\rho_{\mathrm{in}},\rho_{\mathrm{out}})\\
&=\sum_{\ksk}\lag_{\ksk}(\zskip)\sum_{\kin}\eq(\rho_{\mathrm{in}},\kin)\sum_{t\in\cube{\kbatch}}\eq(\rho_{\mathrm{out}},t)\,a\bigl((\ksk,\kin),t\bigr)\\
&=\sum_{\ksk,\kin}\lag_{\ksk}(\zskip)\eq(\rho_{\mathrm{in}},\kin)\sum_{j\in\cube{\kblock}}A_0((\ksk,\kin),j)\underbrace{\sum_{t\in\cube{\kbatch}}\eq(\rho_{\mathrm{out}},t)\,z(j,t)}_{=\;\mle z(j,\rho_{\mathrm{out}})}\\
&=\sum_{j\in\cube{\kblock}}\Bigl(\underbrace{\sum_{\ksk,\kin}\lag_{\ksk}(\zskip)\,\eq(\rho_{\mathrm{in}},\kin)\,A_0((\ksk,\kin),j)}_{=\;\qext A_0(\zskip,\rho_{\mathrm{in}},j)}\Bigr)\mle z(j,\rho_{\mathrm{out}}).
\end{align*}
\end{proof}
...
\qext z(\zskip,\rho_{\mathrm{in}},\rho_{\mathrm{out}})=\sum_{j\in\cube{\kblock}}\lag_{\jsk}(\zskip)\,\eq(\rho_{\mathrm{in}},\jin)\,\mle z(j,\rho_{\mathrm{out}}),
\end{equation}
...
\begin{split}
v_a+\lcalpha v_b+\lcalpha^2v_c+\lcalpha^3\qeq\sum_{j\in\cube{\kblock}}\Bigl(&\qext A_0(\zskip,\rho_{\mathrm{in}},j)+\lcalpha\qext B_0(\zskip,\rho_{\mathrm{in}},j)\\
&+\lcalpha^2\lag_{\jsk}(\zskip)\eq(\rho_{\mathrm{in}},\jin)+\lcalpha^3[j=512]\Bigr)\mle z(j,\rho_{\mathrm{out}}).
\end{split}
\end{equation}

\vspace{2mm}

The parties start by running $\kblock - \kskip = 14 - 6 = 8$ sumcheck rounds on $\eqref{flock:eq:lincheck}$, binding the $\jin$ coordinates (leaving the skip index $\jsk$ unfolded), with challenges $\rho'_{\mathrm{in}}$ in coordinate order (the reverse of their round order). Let $R_{\mathrm{lc}}\in\E$ be the resulting sumcheck claim.

The prover sends $(s_0,\ldots,s_{63})\in\E^{64}$, claiming $s_i=\mle z(i,\rho'_{\mathrm{in}},\rho_{\mathrm{out}})$. These 64 claims will later be proven by Ring-Switching.

The verifier finally needs to check:

\begin{equation}\label{flock:eq:lincheck-terminal}
\begin{split}
R_{\mathrm{lc}}\qeq\sum_{i=0}^{63}\Bigl(&\qext A_0(\zskip,\rho_{\mathrm{in}},i,\rho'_{\mathrm{in}})+\lcalpha\,\qext B_0(\zskip,\rho_{\mathrm{in}},i,\rho'_{\mathrm{in}})\\
&+\lcalpha^2\lag_i(\zskip)\,\eq(\rho_{\mathrm{in}},\rho'_{\mathrm{in}})+\lcalpha^3[i=0]\,\eq(\rho'_{\mathrm{in}},8)\Bigr)s_i.
\end{split}
\end{equation}
...
e_{\mathrm{row}}(\ksk,\kin)=\lag_{\ksk}(\zskip)\,\eq(\rho_{\mathrm{in}},\kin),
\end{equation}
and likewise for $B_0$.

Writing $w_{\mathrm{col}}(\jsk,\jin)=s_{\jsk}\,\eq(\rho'_{\mathrm{in}},\jin)$ with $(\jsk,\jin) \in\cube{\kskip} \times \cube{\kblock-\kskip}$, each matrix term in $\eqref{flock:eq:lincheck-terminal}$ is simplified as follows:
\begin{equation}\label{flock:eq:matrix-form}
\sum_{i=0}^{63}\qext A_0(\zskip,\rho_{\mathrm{in}},i,\rho'_{\mathrm{in}})\,s_i=\sum_{k,j\in\cube{\kblock}}A_0(k,j)\,e_{\mathrm{row}}(k)\,w_{\mathrm{col}}(j).
\end{equation}

One forward walk of the SHA-256 circuit returns \eqref{flock:eq:matrix-form} for both matrices, in $O(\text{circuit})$ field operations, never touching the roughly $1.1$ million nonzero entries they hold. The $c$ term of \eqref{flock:eq:lincheck-terminal} is $\lcalpha^2\langle e_{\mathrm{row}},w_{\mathrm{col}}\rangle$ in the same notation, and needs no walk: both sides are tensors, so it contracts to $\lcalpha^2\eq(\rho_{\mathrm{in}},\rho'_{\mathrm{in}})\sum_i\lag_i(\zskip)s_i$.
...
\[
s_i \qeq \mle z(i,\rho'_{\mathrm{in}},\rho_{\mathrm{out}})\qquad\text{for }i\in\cube{\kskip}.
\]
...
\end{enumerate}
What Flock leaves is one family of $64$ claims for Ring-Switching (\S\ref{flock:ringswitch}): $(s_i \qeq \mle z(i, \rho'_{\mathrm{in}},\rho_{\mathrm{out}}))_{0 \leq i \leq 63}$.
\end{protocol}
...
\[
s_i=\mle z(i,\rho'_{\mathrm{in}},\rho_{\mathrm{out}}),\qquad i\in\cube{\kskip},
\]
...
\[
\bigl(v_a+\qext a(\zskip,\rho)\bigr)+\lcalpha\bigl(v_b+\qext b(\zskip,\rho)\bigr)+\lcalpha^2\bigl(v_c+\qext z(\zskip,\rho)\bigr)+\lcalpha^3\bigl(1+\mle z(512,\rho_{\mathrm{out}})\bigr)=0,
\]
degree three in $\lcalpha$, all four coefficients fixed before $\lcalpha$ is drawn. Acceptance therefore pins them all, except with $3/|\E|$ (Lemma~\ref{lem:sz}): $v_a$, $v_b$ and $v_c$ are the true evaluations, and $\mle z(512,\rho_{\mathrm{out}})=1$, which is \eqref{flock:eq:const-one} except with $\kbatch/|\E|$ more, $\rho_{\mathrm{out}}$ being random.

With all three pinned, the zerocheck's terminal identity \eqref{flock:eq:zc-terminal} becomes a real check again: it fixes $R_{\mathrm{zc}}$ to the true value of the summand at $\rho$.
