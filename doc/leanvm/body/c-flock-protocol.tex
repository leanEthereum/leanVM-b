...
One forward walk of the SHA-256 circuit returns \eqref{flock:eq:matrix-form} for both matrices (\S\ref{flock:matrices}), never touching the roughly $1.1$ million nonzero entries they hold. Its committed wires are $768$ free inputs, the constant, $5{,}888$ committed $\oplus$ wires (the pins of $W[t]$, $A_{\mathrm{new}}$ and $E_{\mathrm{new}}$, and the output chaining value), and $22{,}456$ $\wedge$ wires: the $4{,}096$ bits of $\mathrm{Ch}$ and $\mathrm{Maj}$ over the $64$ rounds, and one per product bit of the $360$ modular additions of the schedule, the rounds and the feed-forward. At one multiplication per committed wire on the $A$ side, one per $\wedge$ wire on the $B$ side, and one for the constant factor the remaining $B$ rows share, that is $51{,}570$ field multiplications in $O(\text{circuit})$ work. The $c$ term of \eqref{flock:eq:lincheck-terminal} is $\lcalpha^2\inner{\erow}{\wcol}$ in the same notation, and needs no walk: both sides are tensors, so it contracts to $\lcalpha^2\eq(\fc_{\mathrm{in}},\fc'_{\mathrm{in}})\sum_i\lag_i(\zskip)s_i$.
...
$A_0$ and $B_0$ encodes a Boolean circuit, but are never materialized.
...
$A_0$, $B_0$ and $C_0=I$ corresponding R1CS constraints faithfully encode the circuit.
