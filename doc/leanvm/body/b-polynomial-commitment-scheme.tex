...
For $\rexp \ge 1$ and $\kappa + \rexp \le 64$, \S\ref{app:ntt} constructs, following~\cite{LCH14} as presented in~\cite{DP24}, an $\Ftwo$-subspace $\dom \subseteq \K$ of size $\blen = 2^{\kappa+\rexp}$ and an encoder $\Enc_{\kappa,\rexp} : \E^{\cube{\kappa}} \to \E^{\dom}$ onto $\RS[\E, \dom, 2^{\kappa}]$, of rate $\rate = 2^{-\rexp}$, with the two properties the protocol uses:
...
Fix the $\Ftwo$-basis $e_c := x^c$, $0 \le c < 64$, of $\K$, the first $64$ vectors of Annex~\ref{annex:rs}'s basis of $\E$, so that $e_0 = 1$. This appendix constructs the encoder of \S\ref{sec:binary-rs} and proves its two properties; the material follows~\cite{LCH14} as presented in~\cite{DP24}. For $u \in \cube{\nv}$ we write $\langle u \rangle := \sum_i u_i 2^i$ for the integer with bits $u$ (bit $0$ first).
...
\item the prover can thus preprocess the internal state of \shaeth{} after hashing those zeros, and then hash the useful part of each leaf starting from this state. Since \shaeth{} takes the message length first, that state depends only on the leaf's (fixed) width and on how many whole blocks of zeros precede the data, so it is one compression chain computed once for the whole tree.
\end{itemize}
