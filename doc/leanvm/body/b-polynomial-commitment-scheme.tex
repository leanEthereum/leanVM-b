...
\smallskip
\textbf{Commit}$(f : \cube{\nv} \to \K)$: $\mathbf{P}$ sends the oracle $\orc^{(0)}[u, \cdot] := \Enc_0(f(u, \cdot))$, $u \in \cube{\ell_0}$, whose entries are $\K$ elements, three times smaller than the $\E$ entries of every deeper oracle. No OOD sample is taken.

\emph{Coordinate order.} Throughout the protocol a multilinear's leading arguments are the ones folded first, so at level $i$ the row index $u$ of $\orc^{(i)}$ carries the $\ell_i$ coordinates the sumcheck of step 2 binds. Which coordinates of the committed object those are is the caller's choice, and it matters: \S\ref{sec:pcs-opt} takes them to be the TOP $\ell_0$ coordinates of the stacked witness of \S\ref{sec:stacking}, which is what makes that stack's zero padding whole rows of $\orc^{(0)}$.
...
\item \emph{(sumcheck~\cite{LFKN92})} For $j = 1, \dots, \ell_i$: $\mathbf{P}$ sends $h_j \in \E[X]$ with $\deg h_j \le 2$, honestly $h_j(X) = \sum_{x} \mle{W}(\fc^{<j}, X, x)\, \mle{f^{(i)}}(\fc^{<j}, X, x)$ where $\fc^{<j} := (\fc_1, \dots, \fc_{j-1})$; $\mathbf{V}$ checks $h_j(0) + h_j(1) \qeq \rc{j-1}$ and sends $\fc_j \leftarrow \E$; both set $\rc{j} := h_j(\fc_j)$.\\
...
\[
\sum_{\tau = 1}^{J_i} \lambda^{\tau - 1} \bigl( \inner{W_\tau}{g_U} - c_\tau \bigr) \;=\; \sum_{\tau = 1}^{J_i} \lambda^{\tau - 1} d_\tau ,
\]
a nonzero polynomial in $\lambda$ of degree less than $J_i$, which vanishes for at most $J_i - 1$ challenges. Union bound over the at most $L_i$ list members: except with probability $(J_i - 1) L_i / |\E|$, every $U$ violates the round-$0$ claim.
...
After the last fold challenge of level $i < \nlev - 1$, we know: every codeword within $\rad_i$ of the single row $c := \orc^{(i)}_\fc \in \E^{\dom_i}$ violates the residual claim $(W', c')$. The prover now sends the new oracle $\orc^{(i+1)}$, which nothing yet ties to $c$; the next two verifier messages transfer the violation property to the new oracle's list.
...
Four prover-side savings. None of them changes the protocol of \S\ref{sec:protocol} or any bound of \S\ref{sec:soundness}.

\paragraph{The padding is free.} The committed $f$ is the stack of \S\ref{sec:stacking}: $P$ placed words, then zeros up to $2^{\nv}$. Let the row index be the stack's top $\ell_0$ coordinates. A row is then a contiguous block of the stack, the padding is whole rows, and only $\nrows=\lceil P/2^{\nv-\ell_0}\rceil$ of the $2^{\ell_0}$ rows hold data. So
\begin{itemize}[itemsep=1pt,topsep=2pt]
\item the encoder runs on $\nrows$ rows, the others encoding to zero;
\item a leaf ships $\nrows$ words, and the verifier fills in the zeros;
\item taking a leaf's rows from the top down puts those zeros first, so the whole blocks among them hash to one chaining value, computed once for the tree.
\end{itemize}
BLAKE2s compresses with the byte counter and the final flag (\S\ref{sec:tab-blake2s}), which is why the zeros must lead: only a prefix can be shared. Each saving stops at one half, because $\nv=\lceil\log_2 P\rceil$ keeps $\nrows>2^{\ell_0-1}$. And the fold now binds the stack's top coordinates first, so a weight written in stack coordinates is evaluated at the terminal point rotated left by $\ell_0$.

\paragraph{Bind the lowest row bit first.} Rows $2t$ and $2t+1$ then pair, the live rows stay a prefix, and the $\ell_0$ rounds cost a fraction $\nrows2^{-\ell_0}$ of a dense fold. The other order pairs row $t$ with $t+2^{\ell_0-1}$, and since $\nrows>2^{\ell_0-1}$ every output row holds data, so the support is full after one round and the cost is $(1+\nrows2^{-\ell_0})/2$. At $\nrows=37$ and $\ell_0=6$: $0.58$ against $0.79$.

\paragraph{Touch the codeword once.} A row is contiguous in the stack and a leaf is contiguous in the oracle, so one of the two must be reordered. The message is the cheaper side, at $2^{-\rexp_0}$ of the codeword, so the prover writes it into the codeword's own first replica and encodes it there. Replication comes free with that: each block at layer $\rexp_0$ is one replica (\S\ref{sec:ntt}), so the encoder's first pass gathers the message itself. Each sumcheck round likewise emits the next round's message from its own output, before that leaves cache.

\paragraph{Batch a level's query weights once.} A level's $\nq_i$ consistency claims share one product over its message coordinates; only the queried column's novel-basis chain differs. The prover builds their batch once per level rather than once per query, and the succinct verifier never builds it at all, evaluating the same weight in closed form (Lemma~\ref{lem:colweight}).
