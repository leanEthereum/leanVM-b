...
\paragraph{Notation.}
Throughout, $\F = \F_{2^{\beta}}$ is a binary field; $\log$ is base $2$; $[\blen] := \{1,\dots,\blen\}$. Query indices are sampled i.i.d.\ uniformly (with replacement). The multilinear vocabulary is that of \S\ref{sec:prelim}, read with $\F$ in place of $\E$: multilinears and their extensions (Definition~\ref{def:mle}), $\eq$ and its interpolation weights (Definition~\ref{def:eq}, Fact~\ref{fact:eq}), the inner product $\inner{\cdot}{\cdot}$ (Definition~\ref{def:inner}), weighted claims and MLE-friendliness (Definition~\ref{def:ipcs}), and Schwartz--Zippel (Lemma~\ref{lem:sz}).
...
\emph{Coordinate order.} Throughout the protocol a multilinear's leading arguments are the ones folded first, so at level $i$ the row index $u$ of $\orc^{(i)}$ carries the $\ell_i$ coordinates the sumcheck of step 2 binds. Which coordinates of the committed object those are is the caller's choice, and it matters: \S\ref{sec:stacking} takes them to be the TOP $\ell_0$ coordinates of the stacked witness, which is what makes the stack's zero padding whole rows of $\orc^{(0)}$.
...
\item \emph{(sumcheck~\cite{LFKN92})} For $j = 1, \dots, \ell_i$: $\mathbf{P}$ sends $h_j \in \F[X]$ with $\deg h_j \le 2$, honestly $h_j(X) = \sum_{x} \mle{W}(\fc^{<j}, X, x)\, \mle{f^{(i)}}(\fc^{<j}, X, x)$ where $\fc^{<j} := (\fc_1, \dots, \fc_{j-1})$; $\mathbf{V}$ checks $h_j(0) + h_j(1) \qeq \rc{j-1}$ and sends $\fc_j \leftarrow \F$; both set $\rc{j} := h_j(\fc_j)$.\\
Set $\fc := (\fc_1, \dots, \fc_{\ell_i})$, $c' := \rc{\ell_i}$, and, for $x \in \cube{\nv_{i+1}}$: \ $f^{(i+1)}(x) := \mle{f^{(i)}}(\fc, x)$, \ $W'(x) := \mle{W}(\fc, x)$. The \emph{residual claim} $(W', c')$ is a weighted claim about the folded multilinear $f^{(i+1)}$.
\item If $i < \nlev - 1$:
...
\paragraph{Rows the prover may omit.} If $f$ vanishes on all but $\nrows$ of the $2^{\ell_0}$ row indices, the remaining rows of $\orc^{(0)}$ are zero because $\Enc_0$ is linear, so the prover need not encode them: it commits the $\nrows$ rows that carry data and supplies zeros for the others wherever step 3(c) reads a column. Nothing above changes, and in particular nothing in \S\ref{sec:soundness} does: the oracle, its list, and every bound are still the full $2^{\ell_0}$-row ones. A prover that fills the omitted rows with anything at all commits $f + \delta$ for some $\delta$ supported on them, and if every weight of the batched claim vanishes on those rows then $\inner{W}{f + \delta} = \inner{W}{f}$, so it has proved the same claims. \S\ref{sec:stacking} arranges both conditions, and that is where the saving is: the omitted rows are the stack's zero padding.
...
Let $\Wn{i} := \vansub_i / \vansub_i(e_i)$ (the hat on $W$ denotes normalization, not multilinear extension), so $\Wn{i}(e_i) = 1$ and $\Wn{0} = X$ (using $e_0 = 1$). By linearity, $\Wn{i}(x) = \sum_c x_c \Wn{i}(e_c)$ for $x = \sum_c x_c e_c$, so given the values $\Wn{i}(e_c)$ (precomputed via the recursion (i)), $\Wn{i}$ is evaluated anywhere in $O(b)$ additions.
...
\]
a linear injection onto $\RS[\F, \dom, 2^{\kappa}]$ of rate $\rate = 2^{-\rexp}$. (The message is read \emph{directly} as novel-basis coefficients; no conversion is applied.)
\end{definition}
