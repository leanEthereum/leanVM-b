...
A self-contained description of the WHIR\slash Ligerito polynomial commitment scheme \cite{ACFY25,NA25} over binary fields, with a round-by-round soundness analysis in the Johnson list-decoding regime. The two schemes coincide when WHIR is instantiated with interleaved codes and Ligerito with the Reed--Solomon codes described here. Reed--Solomon encoding is by the additive NTT in the novel polynomial basis~\cite{LCH14}, as used in~\cite{DP26}. Following Flock~\cite{Flock26}, we do not use out-of-domain (OOD) sampling at commitment, so the PCS is only list binding. The two fields are those of \S\ref{sec:vm}: a commitment is to a $\K$-valued multilinear, and an opening runs over $\E$.
...
$\rate_i$      & $\rho_i$     & code rate (elsewhere in the document $\rho$ is a sumcheck challenge)\\
...
\item \emph{fast encoding, for the prover:} $\Enc_{\kappa,\rexp}$ is computed by the additive NTT with $\tfrac12 \kappa \blen$ multiplications and fewer than $(\kappa + 1)\, \blen$ additions (Proposition~\ref{prop:ntt}); its twiddles lie in $\K$, so an $\E$-valued message pays the same count in $\K \times \E$ products;
