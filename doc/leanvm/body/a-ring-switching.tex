...
Ring switching was introduced in \cite{DP24}. We present a variant tailored to our use case: transforming a PCS for $\K$ into a PCS for $\Ftwo$ (transforming a $\K$-valued multilinear PCS, with $\nflock$ variables and opening claims in $\E$, into a commitment protocol to 64 $\Ftwo$-valued multilinears, each with $\nflock$ variables and sharing opening claims in $\E$).
...
What arrives is not of that shape. It is $64$ claimed values $s_i\in\E$, one per bit polynomial, at a point $r\in\E^{\nflock}$ they all share.
\[
s_i \qeq \mle{Q_i}(r)=\sum_{u\in\cube{\nflock}}\eq(r,u)\,Q_i(u),\qquad 0\le i<64,
\]

Ring switching~\cite{DP24} turns those $64$ claims into a single statement the opening can prove.
...
\label{eq:transpose}
\end{equation}
The $x^i$ are an $\F_2$-basis of $\K$, so some $\Delta_w\neq0$. The cheating prover succeeds if $\sum_{w=0}^{191}\Phi(e_w)\,\Delta_w=0$. We must thus draw $\Phi$ to make this event unlikely.
...
\]
is $\F_2$-linear for any choice of coefficients. Every $\F_2$-linear map $\E\to\E$ arises this way, and exactly once. Distinct tuples of $(c_k)$ give distinct maps: their difference would be a nonzero polynomial of degree at most $2^{191}$ with $2^{192}$ roots. There are $|\E|^{192}$ tuples and as many maps. Call $\supp=\{k:c_k\neq0\}\subseteq\{0,\dots,191\}$ the \emph{support} of $\Phi$. Substituting into the check above,
\begin{equation}
\sum_{w=0}^{191}\Phi(e_w)\,\Delta_w=\sum_{w=0}^{191}\sum_{k\in\supp}c_k\,e_w^{2^k}\,\Delta_w=\sum_{k\in\supp}c_k\,\pair_k,
\qquad
...
First, split $\pair_k$ over $\K$. Write $w=64j+i$, so that $e_w^{2^k}=x^{i2^k}\,\bigl(y^{2^k}\bigr)^{j}$ and
\[
\pair_k=\sum_{j=0}^{2}\bigl(y^{2^k}\bigr)^{j}\,\pair_{j,k},
\qquad
\pair_{j,k}:=\sum_{i=0}^{63}\Delta_{64j+i}\,x^{i2^k}\in\K,\qquad 0\le j<3.
\]
Squaring is a bijection of $\K$ onto itself, so $y^{2^k}\in\K$ would force $y\in\K$, hence $\E=\K[y]=\K$. By Lemma~\ref{lem:kbasis} applied to $y^{2^k}$, the set $\bigl\{1,y^{2^k},\bigl(y^{2^k}\bigr)^{2}\bigr\}$ is a $\K$-basis of $\E$, and $\pair_k=0$ forces $\pair_{0,k}=\pair_{1,k}=\pair_{2,k}=0$.
...
We need a random $\Phi$ of support $\{0,\dots,63\}$, cheap to apply. Instead of drawing its $64$ coefficients independently, we derive them from six challenges.
...
\]
so by Schwartz--Zippel (Lemma~\ref{lem:sz}) the check passes with probability less than $2^{32}/2^{192}=2^{-160}$.

\subsection{Evaluating the weight}\label{rs:weight}

An opening reduces $\sum_{u\in\cube{\nflock}}W(u)\,\qflock(u)=T$ to a claim about $\qflock$ at a random point $r'\in\E^{\nflock}$, and the verifier must evaluate the multilinear extension of $W$ at $r'$, written $\mle{W}(r')$, by itself. One fact turns that into a short formula.

\emph{Fact (Frobenius passes through $\eq$ on the boolean hypercube).} Squaring respects sums and products and fixes bits, so for $u\in\cube{\nflock}$, by Definition~\ref{def:eq},
\[
...
Expand $W$ at $r'$ with \eqref{eq:ck} and this fact:
\[
\begin{aligned}
...
\end{aligned}
\]
The inner sum is $\eq(r^{2^k},r')$, since $\eq(u,\cdot\,)$ is its own multilinear extension for every $u$. By Definition~\ref{def:eq} again,
\[
\boxed{\ \
...
The construction above rests on generalizing ring switching~\cite{DP24} to extension degrees that are not powers of two, an idea of Lev Soukhanov, \textit{[[alloc]init]}. The map $\Phi$ follows~\cite{Flock26}.
