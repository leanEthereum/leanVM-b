...
\newcommand{\kblock}{\kappa_{\mathrm{block}}} % variables in one SHA-256 witness block
\newcommand{\kbatch}{\kappa_{\mathrm{batch}}} % log number of SHA-256 instances
\newcommand{\shaeth}{\ensuremath{\mathrm{sha2\_eth}}} % the machine's hash: length-prefixed MD over SHA-256's C
\newcommand{\ivbase}{\ensuremath{\mathrm{IV}_{\mathrm{eth}}}} % sha2_eth's initial chaining value, SHA256(Ethereum)
\newcommand{\ivlen}{\ensuremath{\mathrm{iv}}}         % iv(n) = C(IV_eth, len(8n)), the per-length constant state
\newcommand{\lenblock}{\ensuremath{\mathrm{len}}}     % the 512-bit big-endian length block
