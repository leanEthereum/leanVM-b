...
Turning $64$ evaluation claims over $\E$ into one opening of a commitment whose values are $64$ bits wide. This annex is the general construction; \S\ref{sec:ringswitch} instantiates it on the packed flock witness $q_\pkd$.
...
The witness is $64$ bit-valued multilinears $Q_0,\dots,Q_{63}:\cube{\npkd}\to\{0,1\}$. We commit to them packed, one $\K$ element per point:
\[
Q_\pkd:\cube{\npkd}\to\K,\qquad Q_\pkd(u)=\sum_{i=0}^{63}Q_i(u)\,x^i.
\]
The commitment scheme proves statements of one shape. Given a target $T\in\E$ and a transparent weight $W:\cube{\npkd}\to\E$, one that the verifier can evaluate for itself, an opening proves
\[
\sum_{u\in\cube{\npkd}}W(u)\,Q_\pkd(u)=T.
\]
The weight is $\E$-valued, the committed values are $\K$-valued, and the sum is taken in $\E$.

What arrives is not of that shape. It is $64$ claimed values $s_i\in\E$, one per bit polynomial, at a point $r\in\E^{\npkd}$ they all share. An honest prover sends
\[
s_i=\mle{Q_i}(r)=\sum_{u\in\cube{\npkd}}\eq(r,u)\,Q_i(u),\qquad 0\le i<64,
\]
where $\eq$ is the equality indicator of \S\ref{sec:prelim}, here in its general form
\[
\eq:\E^{\npkd}\times\E^{\npkd}\to\E,
\qquad
\eq(a,b)=\prod_{n=1}^{\npkd}\bigl(a_nb_n+(1+a_n)(1+b_n)\bigr),
\]
...
\begin{equation}
\begin{split}
\sum_{u\in\cube{\npkd}}\Phi\bigl(\eq(r,u)\bigr)\,Q_\pkd(u)
&=\sum_{i=0}^{63}x^i\sum_{u\in\cube{\npkd}}Q_i(u)\,\Phi\bigl(\eq(r,u)\bigr)\\
&=\sum_{i=0}^{63}x^i\,\Phi\Bigl(\sum_{u\in\cube{\npkd}}Q_i(u)\,\eq(r,u)\Bigr)
...
\item the commitment scheme opens $Q_\pkd$ against the weight $W:\cube{\npkd}\to\E$, $W(u)=\Phi\bigl(\eq(r,u)\bigr)$, with target $T$; if that opening accepts, the verifier accepts the $64$ claims.
\end{enumerate}
...
\]
so by Schwartz--Zippel over $\E$ the check passes with probability less than $2^{32}/2^{192}=2^{-160}$, before the opening's own soundness error.

\subsection{Evaluating the weight}\label{rs:weight}

An opening reduces $\sum_{u\in\cube{\npkd}}W(u)\,Q_\pkd(u)=T$ to a claim about $Q_\pkd$ at a random point $r'\in\E^{\npkd}$, and the verifier must evaluate the multilinear extension of $W$ at $r'$, written $\mle{W}(r')$, by itself. Two facts turn that into a short formula.
...
\emph{Fact 2 (Frobenius passes through $\eq$ on the cube).} Squaring respects sums and products and fixes bits, so for $u\in\cube{\npkd}$, by Fact~1,
\[
\eq(r,u)^{2^k}=\prod_{n=1}^{\npkd}(1+r_n+u_n)^{2^k}=\prod_{n=1}^{\npkd}\bigl(1+r_n^{2^k}+u_n\bigr)=\eq\bigl(r^{2^k},u\bigr),
...
=\sum_{u\in\cube{\npkd}}\Phi\bigl(\eq(r,u)\bigr)\,\eq(r',u)\\
&=\sum_{k=0}^{63}c_k\sum_{u\in\cube{\npkd}}\eq\bigl(r^{2^k},u\bigr)\,\eq(r',u).
\end{aligned}
\]
