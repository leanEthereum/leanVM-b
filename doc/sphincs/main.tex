...
\[
  \Th(P,\tw,M)=\Truncate_n\!\left(\hash(\tw^{\lvert M\rvert}\concat P\concat M)\right),
\]
where $\tw^{\lvert M\rvert}$ is $\tw$ carrying $M$'s length, as the next definition says, and $\hash$ is applied to a string zero-filled to whole $32$-byte groups. $\hash$ and the code $\mathcal C$ of Section~\ref{sec:ots} are those of~\cite{leanVMb}, with a different target sum; there $\hash$ of a long string is a chain over those groups rather than one sponge, which is what a verifier can recompute one memory cell at a time.
...
\]
fourteen bytes of fields and two that $\Th$ overwrites with $\lvert M\rvert$, writing $\tw^{\lvert M\rvert}$ for the result, so no caller below carries a length. It is bound because the hashed string is zero-filled to whole $32$-byte groups: without it a short payload and a longer zero-tailed one would be the same string. The byte-wide fields cap $d\leq256$ and $k\leq257$; the 32-bit fields are never near their range here.
\end{definition}
