...
// ---------------------------------------------------------------------------
// Challenger shim (same convention as ligerito_k / ring_switch_k): the
// Challenger trait speaks GHASH F128 as 16 uniform transcript bytes; every
...
pub fn open_batch_mixed_ligerito_stacked_k<Ch: Challenger>(
    challenger: &mut Ch,
    stack: &[F64],
    ...
    for claim in point_claims {
        challenger.observe_label(b"flock-pcs-packed-direct-k-v0");
        observe_ext(challenger, claim.value());
    }
    let gammas_pd = sample_ext_vec(challenger, point_claims.len());
...
pub fn verify_opening_batch_mixed_ligerito_stacked_k<Ch: Challenger>(
    challenger: &mut Ch,
    ...
    // return false rather than panicking (`verify_succinct` asserts the
    ...
    for (claim, rs_proof) in ring.claims.iter().zip(proof.ring_switches.iter()) {
        match ring_switch_k::verify_succinct(claim.value, &claim.prefix_weights, rs_proof, challenger)
        {
            Ok(out) => rs_outputs.push(out),
            Err(_) => return false,
        }
    }
    let gammas_rs = sample_ext_vec(challenger, n_rs);
    let mut target = F128T::ZERO;
    for (out, g) in rs_outputs.iter().zip(gammas_rs.iter()) {
        target += *g * out.sumcheck_claim;
    }
...
mod tests {
    ...
    use crate::pcs::ligerito_k::{commit_k, inner_product_base_ext, k_configs_for};
    use crate::pcs::pack_k::{LOG_PACKING_K, pack_witness_k};
    use crate::pcs::ring_switch_k::{claim_check, eq_prefix_weights, fold_1b_rows_k};
    ...
    fn build_instance(seed: u64) -> Instance {
        ...
        let yr_log_n = log_n - pc.initial_k - pc.recursive_ks.iter().sum::<usize>();
        assert!(
