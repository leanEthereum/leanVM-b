...
pub fn placements_of_blocks(
    ...
    // the K-PCS minimum required by Ligerito's level ladder.
...
mod tests {
    ...
    fn row_major_block_is_one_multilinear_claim() {
        ...
        let z_col = F128T::new(17, 0);
        let row_point = [F128T::new(19, 0), F128T::new(23, 0)];
        let mut block_point = vec![z_col];
        block_point.extend(row_point);
        let block_eq = ::pcs::ligerito_k::build_eq_table_ext(&block_point);
        let block_eval = q[..6]
            .iter()
            .zip(&block_eq)
            .fold(F128T::ZERO, |acc, (&v, &e)| acc + e.mul_base(v));

        let row_eq = ::pcs::ligerito_k::build_eq_table_ext(&row_point);
        let eval0 = cols[0]
            .iter()
            .zip(&row_eq)
            .fold(F128T::ZERO, |acc, (&v, &e)| acc + e.mul_base(v));
        let eval1 = cols[1]
            ...
            .zip(&row_eq)
            .fold(F128T::ZERO, |acc, (&v, &e)| acc + e.mul_base(v));
        assert_eq!(block_eval, (F128T::ONE + z_col) * eval0 + z_col * eval1);
    }
}
