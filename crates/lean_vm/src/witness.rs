...
pub fn stack_q(cols: &[Column], placements: &[Placement], pads: &[F128], m: usize) -> Vec<F128> {
    use rayon::prelude::*;
...
pub(crate) fn stack(cols: &[Column]) -> Stacked {
    ...
    let q = stack_q(cols, &placements, &vec![F128::ZERO; cols.len()], m);
