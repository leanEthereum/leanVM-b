...
/// Under `LEANVM_PROFILE`, report how the bus blocks are distributed: per side the
/// total entry and block counts and the tree height, then the same split by the
/// owning table (blocks owned by no table are the framework's boundary, memory and
/// bytecode blocks). Attributes bus cost to the opcode that generates it.
fn dump_bus_layout(l: &Layout, owners: &BlockOwners) {
    let sides = [("push", &l.push), ("pull", &l.pull), ("count", &l.count)];
    for (name, blocks) in sides {
    ...
    for (side, (side_name, blocks)) in sides.into_iter().enumerate() {
        ...
        for bucket in 0..=tables::N_TABLES {
            if nblocks[bucket] != 0 {
                let name = Stats::TABLES.get(bucket).copied().unwrap_or("framework");
                eprintln!(
                    "[bus]   {side_name:<5}/{name:<12}: {:>10} entries, {} blocks",
...
/// the public-input claim, located in its committed slot. Both sides assemble it
/// here, so a claim can never shift by one element.
fn finish_claims(
    l: &Layout,
...
mod tests {
    ...
    /// 256-bit inputs (`a` at cells 4..8, `b` at cells 8..12, one 64-bit word per
