...
fn sigmas(bus: &[Vec<F128>; 3], form_pows: [F128; 3]) -> Vec<F128> {
    (0..tables::tables().len())
        .map(|t| (0..3).fold(F128::ZERO, |acc, s| acc + form_pows[s] * bus[s][t]))
        .collect()
}
...
fn eta_form_pows(eta: F128) -> [F128; 3] {
...
pub fn prove(program: &Program, public_input: [F128; 2]) -> (Proof, Stats) {
    ...
    let table_claims = tracing::info_span!("Prove constraints").in_scope(|| {
        ...
        constraints::prove(&airs(&l.taus, &bus.forms, form_pows), &mut cols, eta, &bus.point, &sigma, &mut ps)
    });
    if prof {
...
pub fn verify(
    ...
    let target = (0..3).fold(F128::ZERO, |a, s| a + form_pows[s] * bus.totals[s]);
