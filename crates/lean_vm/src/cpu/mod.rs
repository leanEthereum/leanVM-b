...
use crate::pcs;
use crate::tables::{
    self, FillCtx, FlushBuilder, OP_DEREF, OP_JUMP, OP_KECCAK, OP_MUL, OP_SET, OP_XOR, SEP_BYTECODE, SEP_MEM, SEP_STATE,
};
...
/// cached on `Program`, SHA3-256 over the stacked multilinear
...
fn keccak_value_slot(col: usize) -> Option<usize> {
    let base = schema().base[tables::KECCAK_TABLE];
...
/// `[XOR, MUL, SET, DEREF, JUMP, Keccak]`, and the
...
impl Stats {
    ...
    pub const TABLES: [&'static str; tables::N_TABLES] = ["XOR", "MUL", "SET", "DEREF", "JUMP", "KECCAK"];
...
pub fn prove(program: &Program, public_input: [F192; 2], log_inv_rate: usize) -> (Proof, Stats) {
    ...
    // Warm the shape-dependent Keccak R1CS setup concurrently with the earlier proving stages. A no-Keccak program still uses the padding shape.
...
fn slot_claims(l: &Layout, claims: &[ColumnClaim]) -> Vec<pcs::SlotClaim> {
    claims
        ...
        .map(|c| {
            ...
            // dense QFLOCK block).
            if let Some(slot) = keccak_value_slot(c.col) {
...
mod tests {
    ...
    fn keccak_program(msg: &[u64; 17]) -> Program {
        ...
        for k in 0..total - executed - 1 {
            prog.push(Op::Set {
                o: OUT + STATE_CELLS as u32 + k as u32,
                k: F192::ONE,
            });
        }
        prog.push(Op::Xor { a: 0, b: 0, c: 0 }); // sentinel
        assert_eq!(prog.len(), total);
