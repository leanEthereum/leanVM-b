...
fn program_digest(prog: &[Op], bytecode_used: usize) -> [F64; 4] {
    ...
    let mut words: Vec<F64> = Vec::with_capacity(7 * prog.len() + 3);
...
fn slot_claims(l: &Layout, claims: &[ColumnClaim]) -> Vec<pcs::SlotClaim> {
    // The three public-input limb claims are last (`verify`/`prove` push them
    // after the bus and constraint claims) and are the only claims that are not
    // whole-column evaluations. Their point is `(r, 0, .., 0)`, so `eq` is
    // supported on rows 0 and 1 alone. Those two dense cells are
    ...
    claims
        ...
        .map(|(index, c)| {
            ...
            if index >= public_start {
                debug_assert!(c.point[1..].iter().all(|&x| x == F192::ZERO));
                return pcs::SlotClaim::Strided {
