...
/// instruction-prefix length (BLAKE3 of every instruction's canonical encoding:
/// opcode, operands, and the DEREF store-mode), as two field elements. Seeded
...
/// Announce the prover's public sizes (the non-default memory prefix, the table
/// row counts, and the PCS rate) by writing them onto the scalar stream, which
/// binds them into the sponge and lets the verifier reconstruct the layout. The
/// public statement (program + input) is not announced here; it seeds the
/// transcript at construction (see [`transcript_seed`]). The boundary states and
/// per-table log-sizes (`taus`) are derived (constants from the program, and
/// `padlen(row_counts)`), so they need no separate binding, and the logical
/// memory log is canonically `max(MIN_LOG_MEM, ceil_log2(mem_used))`; the real
/// bytecode prefix is fixed by the program and already bound by
/// [`transcript_seed`].
