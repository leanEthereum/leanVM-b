...
/// ([`crate::blake3_flock`]).
fn blake3_compress(va: [F64; 4], vb: [F64; 4], vcv: [F64; 4], metadata: [F64; 2]) -> [F64; 4] {
    let metadata = F192::new(metadata[0].0, metadata[1].0, 0);
...
/// instruction-prefix length. The bytecode encoding is canonical (opcode,
/// operands, and the DEREF store-mode). The digest is seeded into the
/// transcript alongside the public input, so EVERY challenge depends on the
/// exact program and on which public suffix is declared padding.
///
...
fn program_digest(prog: &[Op], bytecode_used: usize) -> [F64; 4] {
    // VM-native: encode the program as a field-element slice and hash it with
    // the same slice hash that recursion can evaluate using the VM instructions.
    let mut words: Vec<F64> = Vec::with_capacity(6 * prog.len() + 3);
    words.push(F64(prog.len() as u64));
    words.push(F64(6));
    // The real instruction prefix: Jagged commits only that prefix of the
    // bytecode-finalize column, so which suffix is declared padding is part of
    // the statement and must be bound before any challenge.
    words.push(F64(bytecode_used as u64));
    for op in prog {
        // Fixed-width injective encoding: opcode, six address/offset slots,
        // and two base-field immediate words (BLAKE3 metadata uses both).
        let (tag, a, b, c, d, e, f, k0, k1) = match *op {
            Op::Xor { a, b, c } => (0u8, a, b, c, 0, 0, 0, F64::ZERO, F64::ZERO),
            ...
            Op::Blake3 { ins, cv, out, metadata } => {
                (9, ins[0], ins[1], ins[2], ins[3], cv, out, metadata[0], metadata[1])
            }
        };
        words.push(F64(a as u64 | ((b as u64) << 32)));
        words.push(F64(c as u64 | ((d as u64) << 32)));
        words.push(F64(e as u64 | ((f as u64) << 32)));
        words.push(k0);
        words.push(k1);
        words.push(F64(tag as u64));
    }
...
/// Announce the prover's public sizes by writing them onto the scalar stream
/// (which binds them into the sponge and lets the verifier reconstruct the
/// layout). The public statement (program + input) is not announced here — it
/// seeds the transcript at construction (see [`transcript_seed`]). The boundary
/// states and per-table log-sizes (`taus`) are derived (constants from the
/// program, and `padlen(row_counts)`), so they need no separate binding.
///
/// Announce the non-default memory prefix, the table row counts, and the PCS
/// rate. The logical memory log is canonically
...
pub fn prove(program: &Program, public_input: [F64; 4], log_inv_rate: usize) -> (Proof, Stats) {
    ...
    // BLAKE3 ↔ flock (§blake3_flock), single PCS: q_flock is ALWAYS a column in
    ...
    let table_claims = crate::stage!("Prove constraints", || {
        ...
        // reads them again (`prove_balance` is done, and `QFLOCK < N_SHARED` is never
...
mod tests {
    ...
    fn blake3_program(a: [F64; 4], b: [F64; 4]) -> Program {
        let mut prog: Vec<Op> = a
            ...
            .collect();
        prog.extend(
            crate::blake3_flock::IV
                .into_iter()
                .enumerate()
                .map(|(k, v)| Op::Set { o: 24 + k as u32, k: v }),
        );
        let metadata = md();
        prog.push(Op::Blake3 {
            ins: [4, 6, 8, 10],
            cv: 24,
            out: 12,
            metadata: [F64(metadata.c0), F64(metadata.c1)],
        });
        // 16 slots: 13 executed so far; 2 filler SETs step the pc to 15 (halt);
    ...
    fn blake3_self_hash_aliased_operands() {
        ...
        prog.extend(
            crate::blake3_flock::IV
                .into_iter()
                .enumerate()
                .map(|(k, v)| Op::Set { o: 16 + k as u32, k: v }),
        );
        let metadata = md();
        prog.push(Op::Blake3 {
            ins: [4, 6, 4, 6],
        ...
        for k in 0..6u32 {
            prog.push(Op::Set { o: 20 + k, k: F64::ONE }); // fillers step pc to the sentinel
        }
