...
pub fn fs_seed(program: &Program) -> [F128T; 2] {
    ...
    [F128T::new(word(0), word(8)), F128T::new(word(16), word(24))]
}
...
fn transcript_seed(program: &Program, pi: &[F128T; 2]) -> [F128T; 4] {
...
pub struct Program {
    ...
    pub(crate) witness: HashMap<String, Vec<Vec<F128T>>>,
...
fn block_owners(log_bytecode: usize, sides: [usize; 3]) -> [Vec<Option<(usize, usize)>>; 3] {
    ...
    let mut it = src.into_iter().map(|(source, _)| source.checked_sub(2).map(|t| (t, sch.base[t])));
...
fn table_spans() -> Vec<(usize, usize)> {
    let sch = schema();
    tables::tables().iter().enumerate().map(|(t, tb)| (sch.base[t], tb.n_committed_columns())).collect()
}
        ...
        .map(|(t, (&table, &tau))| {
            ...
            constraints::Air {
                ...
                eval: Box::new(move |p, vals| {
                    ...
                    bus.iter().zip(form_pows).fold(air, |acc, (form, w)| acc + w * form.eval(vals))
                }),
            }
        })
        .collect()
...
fn sigmas(bus: &[Vec<F128T>; 3], form_pows: [F128T; 3]) -> Vec<F128T> {
    (0..tables::N_TABLES)
        .map(|t| (0..3).fold(F128T::ZERO, |acc, s| acc + form_pows[s] * bus[s][t]))
        .collect()
}
...
/// `η^{base+s}` per side, the batch's target is `Σ_s η^{base+s}·R_s` for
...
fn eta_form_pows(eta: F128T) -> [F128T; 3] {
...
pub fn prove(program: &Program, public_input: [F128T; 2]) -> (Proof, Stats) {
    ...
    let table_claims = tracing::info_span!("Prove constraints").in_scope(|| {
        ...
        constraints::prove(&airs(&l.taus, &bus.forms, form_pows), cols, eta, &bus.point, &sigma, &mut ps)
    });
    if prof {
...
pub fn verify(
    ...
    let spans = table_spans();
    let bus = leaf::verify_balance(&l.push, &l.pull, &l.count, &l.pad, &owners, &spans, &mut vs)
        .map_err(Error::Bus)?;
    ...
    let target = (0..3).fold(F128T::ZERO, |a, s| a + form_pows[s] * bus.totals[s]);
    let table_claims = constraints::verify(&airs(&l.taus, &bus.forms, form_pows), zc_eta, &bus.point, target, &mut vs)
...
mod tests {
    ...
    /// A K-embedded 128-bit immediate (high lane zero).
    fn w(x: u64) -> F128T {
        F128T::new(x, 0)
    }

    /// Pack two 64-bit flock words into one 128-bit machine word (lo, hi lanes).
    fn cell(lo: F64, hi: F64) -> F128T {
        F128T::new(lo.0, hi.0)
    }
    ...
    fn blake3_program(a: [F64; 4], b: [F64; 4]) -> Program {
        ...
        // a → cells 2,3 and b → cells 4,5 (each 128-bit word = two flock lanes).
        prog.push(Op::Set { o: 2, k: cell(a[0], a[1]) });
        ...
        for k in 0..10u32 {
            prog.push(Op::Set { o: 16 + k, k: F128T::ONE });
        }
    ...
    fn blake3_rejects_tampered_validity() {
        ...
        lig.ligerito.sumcheck_transcript[0].u_0 += F128T::ONE;
        assert!(
    ...
    /// A 128-bit-word MUL: the E-product of two full machine words is proven and
    /// verified. Exercises the tower-product constraint (both lanes nonzero).
    #[test]
    fn mul_128bit_word() {
        let x = F128T::new(0x0123_4567_89ab_cdef, 0xfeed_face_dead_beef);
        let y = F128T::new(0x9999_aaaa_bbbb_cccc, 0x1357_9bdf_2468_ace0);
        let prog = vec![
            Op::Set { o: 2, k: x },
            Op::Set { o: 3, k: y },
            Op::Mul { a: 2, b: 3, c: 4 },
