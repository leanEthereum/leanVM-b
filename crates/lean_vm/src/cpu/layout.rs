...
pub struct Layout {
    ...
    pub pi: [F128T; 2],
    pub taus: [usize; tables::N_TABLES], // per-table log row counts, in `tables::tables` order
...
fn jagged_column_blocks(log_bytecode: usize, bytecode_used: usize, sides: [&[Block]; 3]) -> Vec<Vec<usize>> {
    ...
    signatures[MEM_LO].push(next_group); // public-input claims
...
mod tests {
    ...
    fn shared_default_suffixes_use_jagged_prefixes() {
        let prog = vec![Op::Set { o: 0, k: F128T::ZERO }; 8];
        let bytecode_used = 3;
        let mem_used = 5;
        let l = layout(
            &prog,
        ...
        assert_eq!(l.placements[MEM_LO].height, mem_used);
        assert_eq!(l.placements[MEM_HI].height, mem_used);
        assert_eq!(l.placements[MFCNT].height, mem_used);
        assert_eq!(l.placements[BFCNT].height, bytecode_used);
        assert_eq!(l.pad[MEM_LO], F64::ZERO);
        assert_eq!(l.pad[MEM_HI], F64::ZERO);
