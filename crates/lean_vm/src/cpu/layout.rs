...
pub const MFCNT: usize = 1;
pub const BFCNT: usize = 2;
...
fn jagged_column_blocks(log_bytecode: usize, bytecode_used: usize, sides: [&[Block]; 3]) -> Vec<Vec<usize>> {
    ...
    signatures[MEM].push(next_group);
...
/// reconstruct exactly the same structure (§8, §9).
pub fn layout(
    prog: &[Op],
    bytecode_used: usize,
    log_mem: usize,
...
impl Program {
    pub(crate) fn build(&self, exec: &Execution) -> Witness {
        ...
        crate::stage!("Fill columns", || {
            ...
            // Shared base-field memory column: cells are K-valued, so the image is
            // the committed column as it stands.
        ...
        let (q_flock, flock_reduction) = crate::stage!("Build q_flock", || {
            let blocks: Vec<_> = tr
                .blake3
                .iter()
                .map(|r| {
                    let metadata = F192::new(r.metadata[0].0, r.metadata[1].0, 0);
                    crate::blake3_flock::compression(r.va, r.vb, r.vcv, metadata)
                })
                .collect();
            crate::blake3_flock::build_qflock_prepared(&blocks)
        });
        cols[QFLOCK] = q_flock;
        ...
        let q = crate::stage!("Stack witness", || {
            witness::stack_q(&cols, &l.placements, &l.pad, l.m)
        });
        Witness {
            cols,
            q,
...
mod tests {
    ...
    fn shared_default_suffixes_use_jagged_prefixes() {
        let prog = vec![Op::Set { o: 0, k: F64::ZERO }; 8];
        let bytecode_used = 3;
        let mem_used = 5;
        let l = layout(
            &prog,
        ...
        assert_eq!(l.placements[MEM].height, mem_used);
        assert_eq!(l.pad[MEM], F64::ZERO);
