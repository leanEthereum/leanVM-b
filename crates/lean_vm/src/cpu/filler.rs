...
pub const MIN_ROWS: [usize; N_TABLES] = [1, 1, 1, 1, 1, 1, 1, 1, 8];
...
/// scratch run a dummy writes, four words wide because an extension dummy writes three and
/// the `BLAKE3` dummy reads its chaining value there; and the digest and message runs,
/// placed clear of it.
pub mod frame {
    ...
    /// What a dummy writes: four words, since an extension dummy writes a three-word run
    /// and the `BLAKE3` dummy reads a four-word chaining value here.
    pub const SCRATCH: u32 = 4;
    /// The `BLAKE3` dummy's four-word output run, clear of the scratch cells so a digest
    /// never becomes the next traversal's chaining value.
    pub const DIGEST: u32 = 8;
    /// The `BLAKE3` dummy's four two-word message chunks, never written, so every
    /// traversal compresses the same input.
    pub const MSG: u32 = 12;
    /// Cells a block's frame occupies.
    pub const CELLS: u32 = 20;
}
...
mod tests {
    ...
    fn a_solve_lands_every_table_on_a_power_of_two() {
        let cases: [[usize; N_TABLES]; 6] = [
            ...
            [125_000, 286_000, 0, 0, 341_000, 508_000, 114_000, 130_000, 0],
            ...
            [1 << 17, 1 << 12, 1 << 10, 1000, 1 << 19, 1 << 16, 1 << 11, 1 << 16, 8],
            [1, 2, 3, 4, 5, 6, 7, 8, 9],
    ...
    fn the_fill_is_short() {
        let base = [125_000, 286_000, 40_000, 60_000, 341_000, 508_000, 90_000, 114_000, 130_000];
