...
impl QuaternaryLayerState {
    fn new(mut values: Vec<F192>, width: usize) -> Self {
        ...
        values.resize(values.len().max(4), F192::ONE);
    ...
    fn round_message(&self, equality: &[F192]) -> [F192; 4] {
        ...
        let mut message = if half >= PAR_THRESHOLD {
            ...
            (0..half).map(summand).fold([F192Unreduced::ZERO; 4], xor)
        };
    ...
    fn fold(&mut self, challenge: F192) {
        ...
        if stored_rows == 1 {
            ...
            let folds = mul_four([0, 1, 2, 3].map(|child| self.values[child] + F192::ONE), [challenge; 4]);
            for child in 0..4 {
                self.next[child] = self.values[child] + folds[child];
            }
        ...
        if rows >= PAR_THRESHOLD {
            self.next
                ...
                .for_each(|(row, destination)| {
                    ...
                    let hi = lo + 4;
                    let folds = mul_four(
                        [0, 1, 2, 3].map(|child| self.values[lo + child] + self.values[hi + child]),
                        [challenge; 4],
                    );
                    for child in 0..4 {
                        destination[child] = self.values[lo + child] + folds[child];
                    }
                });
        } else {
            for row in 0..rows {
                ...
                for child in 0..4 {
                    self.next[4 * row + child] = self.values[lo + child] + folds[child];
                }
            }
        }
        std::mem::swap(&mut self.values, &mut self.next);
...
mod tests {
    ...
    fn implicit_identity_suffix_matches_dense_padding() {
        for mu in 3..=10 {
            ...
            let leaves: [Vec<F192>; 3] = depths.map(|depth| {
                (0..1usize << depth)
                    .map(|row| F192::new((3 + row + depth * 10_007) as u64, row as u64, depth as u64))
                    .collect()
            });
