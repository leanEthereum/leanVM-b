...
use primitives::field::{F128T, F128TUnreduced};
use primitives::multilinear::{eq_table as build_eq, interp, quartic_eval_from_eq, shrink_eq_low};
use rayon::prelude::*;

#[inline]
fn mul_pair(a: [F128T; 2], b: [F128T; 2]) -> [F128T; 2] {
    [a[0] * b[0], a[1] * b[1]]
}

#[inline]
fn mul_four(a: [F128T; 4], b: [F128T; 4]) -> [F128T; 4] {
...
#[inline]
fn mul_unreduced_four(a: F128T, b: [F128T; 4]) -> [F128TUnreduced; 4] {
...
fn build_layers(leaves: Vec<F128T>, mu: usize) -> Vec<Vec<F128T>> {
    assert!(leaves.len().is_power_of_two());
    assert!(leaves.len() <= 1usize << mu);
    let mut layers: Vec<Vec<F128T>> = (0..=mu).map(|_| Vec::new()).collect();
    layers[0] = leaves;
    let mut level = 0;
    while level + 2 <= mu {
...
#[inline]
fn quartic_summand(lines: [[F128T; 2]; 4], equality: F128T) -> [F128TUnreduced; 4] {
...
impl QuaternaryLayerState {
    ...
    /// `(q(0)+q(1), [X²]q, [X³]q, [X⁴]q)`.
    fn round_message(&self, equality: &[F128T]) -> [F128T; 4] {
        let stored_rows = self.values.len() / 4;
        let half = stored_rows / 2;
        let summand = |row: usize| -> [F128TUnreduced; 4] {
        ...
        let xor = |mut left: [F128TUnreduced; 4], right: [F128TUnreduced; 4]| {
        ...
        let mut message = if half >= PAR_THRESHOLD {
            (0..half)
                .into_par_iter()
                .map(summand)
                .reduce(|| [F128TUnreduced::ZERO; 4], xor)
        } else {
            (0..half).map(summand).fold([F128TUnreduced::ZERO; 4], xor)
        };
        ...
        if stored_rows == 1 && self.logical_rows > 1 {
            let lines = [0, 1, 2, 3].map(|child| {
                ...
                [at_zero, at_zero + F128T::ONE]
            });
            message = xor(message, quartic_summand(lines, equality[0]));
        }
        message.map(F128TUnreduced::reduce)
    }

    fn fold(&mut self, challenge: F128T) {
        let stored_rows = self.values.len() / 4;
        let rows = (stored_rows / 2).max(1);
        self.next.resize(4 * rows, F128T::ZERO);
        if stored_rows == 1 {
            debug_assert!(self.logical_rows > 1);
            let [fold0, fold1] = mul_pair([self.values[0] + F128T::ONE, self.values[1] + F128T::ONE], [challenge; 2]);
            let [fold2, fold3] = mul_pair([self.values[2] + F128T::ONE, self.values[3] + F128T::ONE], [challenge; 2]);
...
pub fn prove_product_triple(leaves: [Vec<F128T>; 3], ps: &mut ProverState) -> ProductTriple {
...
mod tests {
    ...
    fn mle_eval(table: &[F128T], point: &[F128T]) -> F128T {
        let mut folded = table.to_vec();
        for &challenge in point {
            let half = folded.len() / 2;
            for i in 0..half {
                folded[i] = interp(folded[2 * i], folded[2 * i + 1], challenge);
            }
    ...
    fn quartic_round_message_matches_direct_evaluation() {
        for width in [2, 4, 8, 16] {
            let below: Vec<F128T> = (0..4 * width)
                .map(|i| F128T::new((17 * i + width + 1) as u64, (i * i + 3) as u64))
                .collect();
            let state = QuaternaryLayerState::new(below, width);
            let equality: Vec<F128T> = (0..width / 2)
                .map(|i| F128T::new((31 * i + 5) as u64, (7 * i + 1) as u64))
                .collect();
            let [difference, c2, c3, c4] = state.round_message(&equality);
            let direct = |point: F128T| {
                (0..width / 2).fold(F128T::ZERO, |sum, row| {
                    let values = [0, 1, 2, 3]
    ...
    fn radix_four_roundtrip_at_even_and_odd_depths() {
        for mu in 0..=10 {
            let leaves: [Vec<F128T>; 3] = [0, 1, 2].map(|lane| {
                (0..1usize << mu)
                    .map(|row| F128T::new((1 + row + lane * 100_003) as u64, 0))
                    .collect()
            });
            let expected_roots = leaves
                .each_ref()
                .map(|lane| lane.iter().copied().fold(F128T::ONE, |product, value| product * value));
    ...
    fn implicit_identity_suffix_matches_dense_padding() {
        for mu in 3..=10 {
            ...
            let leaves: [Vec<F128T>; 3] = depths.map(|depth| {
                (0..1usize << depth)
                    .map(|row| F128T::new((3 + row + depth * 10_007) as u64, 0))
                    .collect()
            });
            let dense = leaves.each_ref().map(|lane| {
                let mut padded = lane.clone();
                padded.resize(1 << mu, F128T::ONE);
                padded
            });
            ...
            for lane in 0..3 {
                ...
                assert_eq!(
                    ...
                    dense[lane]
                        .iter()
                        .copied()
                        .fold(F128T::ONE, |product, value| product * value)
                );
            }
            let proof = ps.into_proof();
