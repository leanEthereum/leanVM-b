//! The tweakable hash `Th(P, tw, M) = Truncate_n(sha2_eth(tw | P | M))`, and the
...
//! one-time leaf. Those are unchanged from the BLAKE2s scheme this replaces,
//! because `sha2_eth` binds the length by starting from it, and at every one of
//! these fixed sizes that starting value is a compile-time constant
//! (`primitives::hash::iv_for_len`).
...
pub fn th_digests(pp: &PublicParam, tw: &Tweak, values: &[Digest]) -> Digest {
    let mut hasher = primitives::hash::Hasher::new(TWEAK_LEN + PUBLIC_PARAM_LEN + values.len() * N);
