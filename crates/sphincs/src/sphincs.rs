...
pub fn message_digest(pp: &PublicParam, root: &Digest, rho: &Randomizer, m: &Message) -> (u64, [u32; K]) {
    // `th`'s construction, not a streaming sponge: only the 32-byte-group chain
    // is what the VM's 64-byte opcode can reproduce.
    let mut msg = [0u8; TWEAK_LEN + PUBLIC_PARAM_LEN + RANDOMIZER_LEN + N + MESSAGE_LEN];
    let payload = RANDOMIZER_LEN + N + MESSAGE_LEN;
    let mut at = 0;
    for part in [
        &tweak_with_payload_len(&tweak(TWEAK_MSG, 0, 0, 0, 0), payload)[..],
        pp,
        rho,
        root,
        m,
    ...
    let whole = primitives::hash::hash_md(&msg);
    let digest = &whole[..DIGEST_BYTES];
