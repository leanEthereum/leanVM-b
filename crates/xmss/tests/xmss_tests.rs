...
/// The multi-block WOTS public-key hash is `sha2_eth` of `tweak | pp |
/// payload`, assembled independently here and streamed in unrelated chunks: the
/// length is fixed up front, so the chunking cannot change the result.
#[test]
fn multi_block_tweak_hash_is_standard_sha2() {
    ...
    let mut hasher = primitives::hash::Hasher::new(input.len());
