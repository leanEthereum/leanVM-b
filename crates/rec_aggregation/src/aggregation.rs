...
fn tweak_table(epoch: u32) -> Vec<xmss::Tweak> {
    ...
    for level in 0..LOG_LIFETIME {
        ...
        tweaks.push(make_tweak(TWEAK_TYPE_MERKLE, (level + 1) as u32, parent_index, 2 * DIGEST_LEN));
    }
    tweaks
}
...
fn tagged_hash(label: &[u8], lanes: impl Iterator<Item = u64>) -> [F192; 2] {
    ...
    pack_hash_state(&primitives::hash::hash_md(&bytes))
}
...
/// The Keccak table's virtual value columns, in `hash_flock::SLOTS` order.
