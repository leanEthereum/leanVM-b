...
fn prove_inner(
    ...
    assert!(
        stats.base_counts.iter().all(|&count| count > 0),
...
fn gen_verify(
    ...
    let hints = vec![
        ...
        ("col_sort_order".to_string(), base_words(col_sort_order)),
...
fn recursion_soundness_binds() {
    ...
    let tampers: Vec<(&str, usize, F64)> = vec![
        ...
        ("stream", 0, F64((lean_vm::cpu::MIN_LOG_MEM - 1) as u64)), // native memory floor
        ("stream", 3, F64(1u64 << 32)), // first row count: stream values use three F64 limbs
        ("claim_nover", 0, g_pow(5)),
        ("pi_cplen", 0, g_pow(2)),
