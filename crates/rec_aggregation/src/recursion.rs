...
impl RecursiveStatement {
    fn public_input(&self, inner_environment: [F64; 4]) -> [F64; 4] {
        ...
        sponge.observe(nsub_word(self.sub_statements.len()).into());
...
/// The `K` scalar a coordinate carries beside its columns: the constant itself, or
/// the `g^k` a `GCol`/`Prod` scales by. Zero for every other kind.
...
fn gen_verify(
    ...
    let log_mem = log_mem.max(lean_vm::cpu::MIN_LOG_MEM);
    let l = lean_vm::cpu::layout(
        &program.prog,
        program.bytecode_used(),
        log_mem,
    ...
    let stack_mu = l.m;
    let vcfg = pcs::whir_config::WhirSecurityConfig::derive_config_with_log_inv_rate(
    ...
    let hints = vec![
        ...
        // (zeta/rho) and low_len(+SLOT_STRIDE_LOG for qflock) <= lenris.
...
/// Candidate committed log-sizes the guest dispatches over, for a program whose
/// bytecode has `kbc` log-rows. Jagged packs only real prefixes, so a small
/// execution can now reach the PCS floor rather than the former aligned-stack
/// minimum of 22; tight packing can never exceed the old aligned layout, so
/// the ceiling is unchanged.
fn mu_range(kbc: usize) -> (usize, usize) {
...
fn placeholder_map(program: &Program) -> BTreeMap<String, String> {
    ...
    // A table coord's terms: the guest rebuilds it as `Σ_terms`, which is what lets
    // a value the row derives ride the bus instead of costing a column.
    let (mut coord_toff, mut coord_tcount): (Vec<usize>, Vec<usize>) = (vec![], vec![]);
    let (mut term_type, mut term_const): (Vec<u64>, Vec<u64>) = (vec![], vec![]);
    let (mut term_col_a, mut term_col_b): (Vec<usize>, Vec<usize>) = (vec![], vec![]);
    ...
    for blocks in sides.iter() {
        for blk in blocks.iter() {
            ...
            for c in &blk.coords {
                ...
                // Only a FRAMEWORK block's Col/GCol raises a claim; a table's
                // coords are settled by the zerocheck, off its column evaluations,
                // which the terms below index locally.
                ...
                // A table's coord becomes terms, so a value the row derives costs
                // terms rather than a column; a framework one has none, having
                ...
                ct.push(coord_kind(c));
                cval.push(coord_scale(c).0);
                fpv.push(lean_vm::leaf::coord_pad_value(c, &l.pad).0);
            }
        }
        sblk.push(nblocks);
    }
    ...
    for (t, table) in lean_vm::tables::tables().iter().enumerate() {
        for c in 0..table.n_committed_columns() {
            ...
            if l.placements[col].is_virtual() {
                ...
                cpslot.push(lean_vm::blake3_flock::SLOTS[p]);
                cprowkey.push((3, 0, 0));
            } else {
                cpbuf.push(1);
    ...
    for i in 0..ncl {
        ...
        for j in i..ncl {
            ...
            {
                assert!(
                    by_slot[cpblockslot[j]].replace(j).is_none(),
                    "duplicate claim for one Jagged block slot"
                );
            }
        }
        assert!(
            by_slot.iter().all(Option::is_some),
            "Jagged membership partition must produce complete blocks"
        );
    ...
    let oshape = |m: usize, log_inv_rate: usize| {
        let vc =
            pcs::whir_config::WhirSecurityConfig::derive_config_with_log_inv_rate(m + pcs::LOG_PACKING, log_inv_rate)
                .and_then(|s| s.to_prover_verifier_configs())
    ...
    ps(
        ...
        ints(
            &ordered_heights
                .iter()
                .map(|(_, width_log)| *width_log)
                .collect::<Vec<_>>(),
        ),
    );
    assert!(
        ordered_heights.iter().all(|(source, width_log)| !matches!(
            source,
            lean_vm::cpu::ColHeightSource::BytecodeRows(_)
        ) || *width_log == 0),
    ...
    ps(
        ...
        ints(
            &(0..34)
                .map(|bit| (program.bytecode_used() >> bit) & 1)
                .collect::<Vec<_>>(),
        ),
    );
    ps(
    ...
    );
    {
        ...
        // that shares it (`open_stacked`). A memory cell holds one base-field
        // word here, so level 0's n lanes take n cells. Deeper levels load 3 raw
        // tower limbs per word into `lanes` (3n cells) and then pack them into a
        // leaf row that is also 3n cells wide, so the two coincide and 3n covers
        // both. (On 128-bit cells these were n/2 and 3n/2, and `lanes` alone
        // forced the deeper term to 3n; sizing it at 3n/2 under-provisioned
        // once a deeper interleave exceeded level 0's.)
...
fn run_recursion_with_rates(
    ...
    // Only the final measured pass of each stage is traced (the harness passes
    // `last`, so the earlier passes run quiet).
...
/// rather than supplied as named witness streams, so they need no tamper here.
#[test]
#[ignore]
fn recursion_soundness_binds() {
    ...
    let tampers: Vec<(&str, usize, F64)> = vec![
        ...
        ("nsub", 0, F64::ONE),          // g^0: dropping a sub-proof must not keep the statement
...
fn recursion_generic_many() {
    ...
    // The ARITY is generic too, now that it is a hint rather than a placeholder:
    // the same bytecode aggregates a different number of sub-proofs.
    for arity in [2usize, 3] {
        let inner: Vec<(usize, usize)> = vec![(4, 1 << 12); arity];
        let rates = vec![lean_vm::pcs::LOG_INV_RATE; arity];
        let batch = build_batch(&inner, &rates, lean_vm::pcs::LOG_INV_RATE);
        let (recursive_proof, _) = batch.prove(&mut guest);
        recursive_proof
            .verify(&batch.program0)
            .expect("complete recursive proof verifies");
        eprintln!("  verified: arity {arity}");
    }
    eprintln!(
        "all {} shapes and two further arities verified by the SAME guest bytecode",
