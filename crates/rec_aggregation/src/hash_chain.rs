//! End-to-end proof of an unrolled zkDSL SHA-256 hash chain.
...
fn chain_source(steps: usize, unroll: usize) -> String {
    ...
    for step in 1..=unroll {
        ...
        body.push_str(&format!(
            "        sha2(h{previous}, h{previous}, h{step})\n",
            previous = step - 1
        ));
    }
    for word in 0..2 {
...
fn sha2_hash_chain() {
    ...
    for (name, &c) in ["XOR", "MUL", "SET", "DEREF", "JUMP", "SHA2"].iter().zip(&stats.counts) {
        let pow = if c == 0 {
            "0".to_string()
        } else {
