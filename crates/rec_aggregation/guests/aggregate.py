...
SP_TIP_CELLS = SP_V
SP_LEAF_GROUPS = (2 + SP_V) / 2       # prefix (tweak, pp) + V tips, two cells a 32-byte group
SP_N_FTS = SP_K - 1                   # the forest drops the last index's tree
SP_ROOT_GROUPS = (2 + SP_N_FTS) / 2
...
# The tweak's last two bytes are the hashed payload's length, at bits 112..128,
# so lane 1's bits 48..64. `Th` sets it rather than its caller because the
# hashed string is zero-filled to whole 32-byte groups: without it a short
# payload and a longer zero-tailed one would hash the same string. Every call
# site's length is a constant, so each is one compile-time literal.
SP_LEN_MUL = 2 ** 112
SP_LEN_ONE = SP_LEN_MUL * 16            # a chain value, or a few-time secret
SP_LEN_PAIR = SP_LEN_MUL * 32           # two digests: a Merkle node
SP_LEN_ENC = SP_LEN_MUL * 20            # the signed value and LE_32(c)
SP_LEN_TIPS = SP_LEN_MUL * (16 * SP_V)  # a one-time key's chain tips
SP_LEN_ROOTS = SP_LEN_MUL * (16 * SP_N_FTS)
SP_LEN_MSG = SP_LEN_MUL * 64            # the randomizer, the root and the message
...
def open_stacked(m_idx: Const, fs0, fs1, target, commit_root_0, commit_root_1, cursor):
    ...
    for lvl in unroll(0, LIG_N_LEVELS[m_idx]):
        ...
        for xe in mul_range(1, GEN ** LIG_QUERIES[m_idx * LIG_MAX_LEVELS + lvl]):
            ...
            for jg in unroll(2, 2 * LIG_LEAF_BLOCKS[m_idx * LIG_MAX_LEVELS + lvl]):
                leaf_digest = StackBuf(2)
                sha3_64(leaf_hash_state, packed_row[2 * jg:2 * jg + 2], leaf_digest)
...
def verify_sig(message, tweak_table, merkle_bits, pk_ptr):
    ...
    # Encoding digest D = hash_md(tweak | pp | msg | randomness | zero-pad), 96
    # bytes: the chain's 64-byte first link, then one 32-byte group (24 bytes of
    ...
    sha3_64(tweak_pp, msg_block, after_msg)
    rand_block = StackBuf(WORDS_PER_BLOCK)
    hint_witness(rand_block, "rand")
    # The spec's pad: cell 1 is randomness bytes 16..24 then 8 zero bytes.
    # `assert_in_k` reads its operands as (lo, 0, 0), so this is what pins cell 1's
    # high lane to zero where the hash reads it as (lo, hi, 0).
    assert_in_k(rand_block[1], 0)
    digest = StackBuf(WORDS_PER_BLOCK)
    sha3_64(after_msg, rand_block, digest)
    ...
    # WOTS public-key leaf = hash_md over prefix + V tips: the chain's 64-byte
    # first link, then one link per 32-byte group.
    ...
    sha3_64(pk_tweak_pp, tips[0:2], leaf)
    for q in unroll(2, 2 * WOTS_PK_BLOCKS):
        next_leaf = StackBuf(WORDS_PER_BLOCK)
        sha3_64(leaf, tips[2 * q - 2:2 * q], next_leaf)
        leaf = next_leaf
    ...
    for l in unroll(0, LOG_LIFETIME):
        ...
        sha3_64(merkle_tweak_pp, children, parent)
        node = parent[0]
...
def sp_walk(value, tw_base, pp, k: Const):
    ...
    for s in unroll(k, SP_CHAIN_STEPS):
        step_tweak = StackBuf(WORDS_PER_BLOCK)
        step_tweak[0] = tw_base + s * SP_P_MUL + SP_LEN_ONE
        step_tweak[1] = pp
        out = StackBuf(WORDS_PER_BLOCK)
        sha3_64(step_tweak, block, out)
        block = StackBuf(WORDS_PER_BLOCK)
...
def sp_ots_leaf(tw_pos, pp, msg):
    ...
    enc_tweak[0] = tw_pos + SP_TW_ENC + SP_LEN_ENC
    enc_tweak[1] = pp
    enc_block = StackBuf(WORDS_PER_BLOCK)
    enc_block[0] = msg
    enc_block[1] = ctr[0]
    digest = StackBuf(WORDS_PER_BLOCK)
    sha3_64(enc_tweak, enc_block, digest)
    ...
    leaf_tweak[0] = tw_pos + SP_TW_LEAF + SP_LEN_TIPS
    leaf_tweak[1] = pp
    leaf = StackBuf(WORDS_PER_BLOCK)
    sha3_64(leaf_tweak, tips[0:2], leaf)
    for q in unroll(2, SP_LEAF_GROUPS):
        next_leaf = StackBuf(WORDS_PER_BLOCK)
        sha3_64(leaf, tips[2 * q - 2:2 * q], next_leaf)
        leaf = next_leaf
    return leaf[0]
...
def verify_sig_sphincs(signer):
    ...
    for kappa in unroll(0, SP_N_FTS):
        ...
        fts_tweak[0] = SP_TW_FTS_LEAF + SP_LEN_ONE + kappa * SP_LAY_MUL + idx_tau + sp_bit_field(bits, leaf_off, SP_A, SP_J_POS)
        fts_tweak[1] = pp
        fts_leaf = StackBuf(WORDS_PER_BLOCK)
        sha3_64(fts_tweak, secret, fts_leaf)
        node = fts_leaf[0]
        for level in unroll(0, SP_A):
            ...
            node_tweak[0] = SP_TW_FTS_NODE + SP_LEN_PAIR + kappa * SP_LAY_MUL + SP_P_LEVEL[level + 1] + idx_tau + sp_bit_field(bits, leaf_off + level + 1, SP_A - level - 1, SP_J_POS)
            node_tweak[1] = pp
            parent = StackBuf(WORDS_PER_BLOCK)
            sha3_64(node_tweak, children, parent)
            node = parent[0]
        roots[kappa] = node
    roots_tweak = StackBuf(WORDS_PER_BLOCK)
    roots_tweak[0] = SP_TW_FTS_ROOTS + SP_LEN_ROOTS + idx_tau
    roots_tweak[1] = pp
    fts_key = StackBuf(WORDS_PER_BLOCK)
    sha3_64(roots_tweak, roots[0:2], fts_key)
    for q in unroll(2, SP_ROOT_GROUPS):
        next_key = StackBuf(WORDS_PER_BLOCK)
        sha3_64(fts_key, roots[2 * q - 2:2 * q], next_key)
        fts_key = next_key
    signed = fts_key[0]
    ...
    for step in unroll(0, SP_D):
        ...
        for level in unroll(0, SP_HEIGHTS[lay]):
            ...
            children[1] = sibling[0] + m
            node_tweak = StackBuf(WORDS_PER_BLOCK)
            node_tweak[0] = SP_TW_NODE + SP_LEN_PAIR + lay * SP_LAY_MUL + SP_P_LEVEL[level + 1] + tau_field + sp_bit_field(bits, leaf_index_off + level + 1, SP_HEIGHTS[lay] - level - 1, SP_J_POS)
            node_tweak[1] = pp
            parent = StackBuf(WORDS_PER_BLOCK)
            sha3_64(node_tweak, children, parent)
            node = parent[0]
...
def statement_digest(seed_0, seed_1, n_xmss_g, n_sphincs_g, pk_hash, msg, epoch_digest, defer):
    ...
    st = StackBuf(2)
    sha3_64(cells[0:2], cells[2:4], st)
    for g in unroll(2, 2 * STMT_BLOCKS):
        nxt = StackBuf(2)
        sha3_64(st, cells[2 * g:2 * g + 2], nxt)
        st = nxt
    return st[0], st[1]
...
def hash_key_range(state_0, state_1, keys_ptr, half_g, odd_g):
    ...
    for xp in mul_range(1, half_g):
        ...
        state = chain * pair
        sha3_64(state[0:2], keys[0:2], state[2:4])
        sha3_64(state[2:4], keys[2:4], state[4:6])
    ...
    if odd_g == 1:
        ...
        sha3_64(paired_end[0:2], last[0:2], out)
    return out[0], out[1]
...
def hash_sphincs_range(state_0, state_1, entries_ptr, n_g):
    ...
    for xe in mul_range(1, n_g):
        ...
        state = chain * quad
        sha3_64(state[0:2], entry[0:2], state[2:4])
        sha3_64(state[2:4], entry[2:4], state[4:6])
    end = chain * (n_g ** 4)
...
def hash_child_keys(state_0, state_1, keys_ptr, cover, base, limit_g, half_g, odd_g):
    ...
    for xp in mul_range(1, half_g):
        ...
        sha3_64(state[0:2], key_a[0:2], state[2:4])
        sha3_64(state[2:4], key_b[0:2], state[4:6])
    paired_end = chain * (half_g ** 4)
    out = StackBuf(WORDS_PER_BLOCK)
    if odd_g == 1:
        out[0] = paired_end[1]
        ...
        sha3_64(paired_end[0:2], key_last[0:2], out)
    return out[0], out[1]
...
def main():
    ...
    sha3_64(tag[0:2], tag[2:4], epoch_state)
    ...
    for t in unroll(0, N_TWEAK_BLOCKS):
        ...
        mid_state = StackBuf(WORDS_PER_BLOCK)
        sha3_64(epoch_state, blk[0:2], mid_state)
        next_state = StackBuf(WORDS_PER_BLOCK)
        sha3_64(mid_state, blk[2:4], next_state)
    ...
    sha3_64(pk_seed[0:2], pk_seed[2:4], pk_iv)
    ...
    for xc in mul_range(1, n_children_g):
        ...
        sha3_64(sub_seed[0:2], sub_seed[2:4], sub_iv)
