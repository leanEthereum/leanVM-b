...
def challenge_from_state(state):
    ...
    # Both words are SHA-256 outputs, so their top limbs are already zero. Only
...
def verify_sub(pi_0, pi_1, seed_0, seed_1, g_logs_pow2, g_squares, defer_out):
    ...
    # coordinates. A SHA2 dominated inner proof (every real XMSS aggregation:
    # qflockv = tau_5 + SLOT_STRIDE_LOG) pushes qflockv past lenris, so the top
    # rs_nover coordinates continue into the residual challenges. rs_nover is
    # hinted and pinned exactly as the point claims pin theirs: rs_low =
    # qflockv - rs_nover and rs_len = lenris + rs_nover - qflockv are divisions
    # off it, so the two range checks plus the either/or leave
    # rs_nover = max(0, qflockv - lenris).
...
def sp_ots_leaf(tw_pos, pp, msg):
    ...
    enc_block[1] = ctr[0]
    digest = StackBuf(WORDS_PER_BLOCK)
    sha2(enc_tweak, enc_block, digest, msg_bytes=52)
    ...
    leaf_tweak[1] = pp
    leaf = StackBuf(WORDS_PER_BLOCK)
    sha2(leaf_tweak, tips[0:2], leaf, msg_bytes=64 * SP_LEAF_BLOCKS)
...
def verify_sig_sphincs(signer):
    ...
    sha2(msg_tweak, rho_root, prefix, msg_bytes=96)
    msg_block = StackBuf(WORDS_PER_BLOCK)
    ...
    zero_block[1] = 0
    digest = StackBuf(WORDS_PER_BLOCK)
    sha2(msg_block, zero_block, digest, cv=prefix)
    ...
    for kappa in unroll(0, SP_N_FTS):
        ...
        sha2(fts_tweak, secret, fts_leaf, msg_bytes=48)
        node = fts_leaf[0]
        for level in unroll(0, SP_A):
            ...
            node_tweak[1] = pp
            parent = StackBuf(WORDS_PER_BLOCK)
            sha2(node_tweak, children, parent)
            node = parent[0]
        roots[kappa] = node
    ...
    sha2(roots_tweak, roots[0:2], fts_key, msg_bytes=64 * SP_ROOT_BLOCKS)
    for q in unroll(1, SP_ROOT_BLOCKS):
        next_key = StackBuf(WORDS_PER_BLOCK)
        sha2(roots[4 * q - 2:4 * q], roots[4 * q:4 * q + 2], next_key, cv=fts_key)
        fts_key = next_key
    signed = fts_key[0]
...
def statement_digest(seed_0, seed_1, n_xmss_g, n_sphincs_g, pk_hash, msg, epoch_digest, defer):
    ...
    #
    # Fixed-length preimage, so a plain sha2_eth beats the Fiat-Shamir chain, which spent a
...
def hash_key_range(state_0, state_1, keys_ptr, half_g, odd_g):
    ...
    if odd_g == 1:
        ...
        sha2(paired_end[0:2], last[0:2], out)
    return out[0], out[1]
...
def hash_sphincs_range(state_0, state_1, entries_ptr, n_g):
    ...
    for xe in mul_range(1, n_g):
        ...
        state = chain * quad
        sha2(state[0:2], entry[0:2], state[2:4])
        sha2(state[2:4], entry[2:4], state[4:6])
    end = chain * (n_g ** 4)
...
def hash_child_keys(state_0, state_1, keys_ptr, cover, base, limit_g, half_g, odd_g):
    ...
    if odd_g == 1:
        ...
        sha2(paired_end[0:2], key_last[0:2], out)
    return out[0], out[1]
...
def main():
    ...
    sha2(pk_seed[0:2], pk_seed[2:4], pk_iv)
    ...
    for xc in mul_range(1, n_children_g):
        ...
        sha2(sub_seed[0:2], sub_seed[2:4], sub_iv)
