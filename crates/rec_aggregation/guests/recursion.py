...
# A GCol coord's free g^k, as a value (the exponent cannot be a runtime list read).
COORD_GVAL = COORD_GVAL_PLACEHOLDER
...
# CLAIM_QPKD_SLOT_BITS contains the fixed packed-column slot bits for every logical
...
# Ring-switch coefficient factorization for the F64-polynomial / quadratic
# tower coordinate basis. Each 13-value row contains the base-field main
# term, six product factors, four sparse corrections, then the two relative
# trace-dual tower factors.
RS_COEFF_ORBIT_WIDTH = 13
...
def check_128_bits_decomposition(bits_ptr, v):
    ...
    # the F128T COORDINATE basis (hint_decompose_bits emits coordinate bits).
    acc = 0
...
def open_stacked(m_idx: Const, fs0, fs1, target, commit_root_0, commit_root_1, cursor):
    ...
    for lvl in unroll(0, LIG_N_LEVELS[m_idx]):
        ...
        for xe in mul_range(1, GEN ** LIG_QUERIES[m_idx * LIG_MAX_LEVELS + lvl]):
            ...
            row_dot = 0
            if lvl == 0:
                # Level-0 rows are base-field F64, embedded one-per-word (c1=0) for
                # the E-valued dot. The committed Merkle leaf holds them PACKED
                # (two 8-byte lanes per cell), so re-pack the embedded lanes into
                # a contiguous cell run for the standard leaf hash; the dot still
                # consumes the individual embedded lanes. PACK64X2 reads both
                # source cells through the memory bus as `(lo, 0)`, so the packs
                # also prove every hinted lane is genuinely F64: arithmetic
                # packing (e0 + e1·Y) would hash the same word for many
                # non-canonical (e0, e1) pairs and leave the dot unbound.
                ...
                # Standard BLAKE3 of the packed row (a power of two of 64-byte
...
def verify_sub(pi_0, pi_1, seed_0, seed_1, g_logs_pow2, g_squares, defer_out):
    ...
    # tables are shared read-only tables built once in main; the deferred-claim
    # data is written to `defer_out`.
    #
    ...
    #   5. ONE batched zerocheck for all the tables, n = max_t tau_t rounds at the
    ...
    dims_g = HeapBuf(N_TABLES + 1)  # [g^log_mem, g^tau_0 .. g^tau_{N_TABLES-1}], all computed
    ...
    for t in unroll(0, N_TABLES):
        ...
        for k in unroll(0, N_TABLE_COLS[t]):
            ...
            claim_idx += 1
        # the table's AIR constraint at the final point. col_evals is indexed by LOCAL
        # committed-column index, and each identity takes its own power out of the
        # table's disjoint eta range. A memory word is a 128-bit E-value
        # e128(lo, hi) = lo + hi*Y, so it costs two lane columns. The formulas mirror
        # tables.rs eval_constraint.
        if t == TABLE_XOR:
            va = (col_evals[8] + col_evals[9] * Y_TOWER)
            vb = (col_evals[10] + col_evals[11] * Y_TOWER)
            vc = (col_evals[12] + col_evals[13] * Y_TOWER)
        ...
        if t == TABLE_MUL:
            ...
            constraint_eval = eta_pows[ETA_OFFSET[t] + 0] * (col_evals[5] + col_evals[1] * col_evals[2]) + eta_pows[ETA_OFFSET[t] + 1] * (col_evals[6] + col_evals[1] * col_evals[3]) + eta_pows[ETA_OFFSET[t] + 2] * (col_evals[7] + col_evals[1] * col_evals[4]) + eta_pows[ETA_OFFSET[t] + 3] * (vc + va * vb)
        if t == TABLE_SET:
            constraint_eval = eta_pows[ETA_OFFSET[t] + 0] * (col_evals[5] + col_evals[1] * col_evals[2])
        if t == TABLE_DEREF:
            v2 = (col_evals[11] + col_evals[12] * Y_TOWER)
            v3 = (col_evals[13] + col_evals[14] * Y_TOWER)
        ...
        if t == TABLE_JUMP:
            c = (col_evals[10] + col_evals[11] * Y_TOWER)
            d = (col_evals[12] + col_evals[13] * Y_TOWER)
            ff = (col_evals[14] + col_evals[15] * Y_TOWER)
            w = (col_evals[20] + col_evals[21] * Y_TOWER)
            ft = GEN * col_evals[0]
            ...
            ind_def = eta_pows[ETA_OFFSET[t] + 3] * (col_evals[22] + c * w)
            ind_nz = eta_pows[ETA_OFFSET[t] + 4] * (c * (col_evals[22] + 1))
            sel_pc = eta_pows[ETA_OFFSET[t] + 5] * (col_evals[2] + col_evals[22] * d + (col_evals[22] + 1) * ft)
            sel_fp = eta_pows[ETA_OFFSET[t] + 6] * (col_evals[3] + col_evals[22] * ff + (col_evals[22] + 1) * col_evals[1])
        ...
        # sum_b eq_hi(b) * (gamma + sum_i alpha^i * coord_i), each coord read off
        # col_evals at its local index. This is what replaces opening those columns
        # at zeta. A GCol applies its baked free factor g^k.
        for sd in unroll(0, N_GKR_SIDES):
            form = 0
            for b in unroll(0, N_BLOCKS):
                if BLOCK_SIDE[b] == sd:
                    if BLOCK_REAL_TABLE[b] == t:
                        ...
                        for i in unroll(0, BLOCK_COORD_COUNT[b]):
                            ...
                            if COORD_TYPE[BLOCK_COORD_OFF[b] + i] == COORD_KIND_GCOL:
                                cv = COORD_GVAL[BLOCK_COORD_OFF[b] + i] * col_evals[COORD_COL_LOCAL[BLOCK_COORD_OFF[b] + i]]
                            if sd == COUNT_SIDE:
                                inner += cv
                            else:
                                inner += apow * cv
    ...
    for i in unroll(0, LOG2_FIELD_BITS):
        ...
        rv = fs[0]
        r_dprime[GEN ** i] = rv
    # Only base-field indices 0..3 occur in the sparse correction. Their four
    # high base-index bits are all zero, so share that equality factor.
    correction_weights = StackBuf(4)
    correction_high = GEN ** 0
    for t in unroll(2, K_SKIP):
        correction_high *= 1 + r_dprime[GEN ** t]
    for i in unroll(0, 4):
        correction_weight = correction_high
        for t in unroll(0, 2):
            if (i // (2 ** t)) % 2 == 1:
                correction_weight *= r_dprime[GEN ** t]
            else:
                correction_weight *= 1 + r_dprime[GEN ** t]
        correction_weights[i] = correction_weight
    # Factor the 64-coordinate polynomial-basis transform, then apply the
    # two-coordinate relative trace-dual factor of E/K.
    for k in unroll(0, FIELD_BITS):
        orbit_off = RS_COEFF_ORBIT_WIDTH * k
        base_main = RS_COEFF_ORBITS[orbit_off]
        for t in unroll(0, K_SKIP):
            base_main *= 1 + r_dprime[GEN ** t] * RS_COEFF_ORBITS[orbit_off + 1 + t]
        base_correction = 0
        for i in unroll(0, 4):
            base_correction += correction_weights[i] * RS_COEFF_ORBITS[orbit_off + 1 + K_SKIP + i]
        tower_factor = RS_COEFF_ORBITS[orbit_off + 11] + r_dprime[GEN ** K_SKIP] * RS_COEFF_ORBITS[orbit_off + 12]
        c_table[GEN ** k] = (base_main + base_correction) * tower_factor
    for rs in unroll(0, 2):
    ...
    for j in unroll(0, N_CLAIMS):
        ...
        if CLAIM_POINT_BUF[j] == POINT_BUF_QPKD:
            ...
            # A value column is referenced only by its own table's bus blocks, and
            # those are settled by the zerocheck, so the instance point of a q_pkd
            # slot claim is always the batch's rho (asserted where the descriptors
            # are built).
            zptr = rho
    ...
    # Walk runtime coordinates outside and the fixed 128 Frobenius powers
    # inside: each coordinate loads its opening challenge once, evolves z by
    # squaring in registers, and advances one contiguous 128-product row.
    ...
    for rs in unroll(0, 2):
        ...
        for x_round in mul_range(1, qpkdv_g):
            ...
            one_plus_q = 1 + fold_challenges[x_round]
            prod_row = prod_chains * x_round ** FIELD_BITS
            prod_row_next = prod_row * GEN ** FIELD_BITS
            for k in unroll(0, FIELD_BITS):
                prod_row_next[GEN ** k] = prod_row[GEN ** k] * (zv + one_plus_q)
                if k != FIELD_BITS - 1:
                    zv *= zv
        prod_final = prod_chains * qpkdv_g ** FIELD_BITS
        e_acc = 0
        for k in unroll(0, FIELD_BITS):
            e_acc += c_table[GEN ** k] * prod_final[GEN ** k]
        rs_eq_vals[rs] = e_acc
    ...
    # || certified placement bits): coord_k = m_k * ov_k + (1 + m_k) * bit_k with
