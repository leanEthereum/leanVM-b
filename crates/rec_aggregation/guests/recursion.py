...
# term for a plain kind, several for a value the row derives (§sec:m3). A
# framework coord has none, having decomposed into a pooled claim.
...
def open_stacked(m_idx: Const, fs0, fs1, fs2, fs3, target: Ext, commit_root_0, commit_root_1, commit_root_2, commit_root_3, cursor, sumcheck_out, inner_out, yr_at_tail_out):
    ...
    #
    # Returns (fold_challenges, final_msg, yr_log_n_g = g^yr_log_n,
    # yr_pad_g = g^(YR_LOG_CAP - yr_log_n), fold_cap_g = g^lenris,
    # tail_challenges); the three-word sumcheck_target, inner_total, and
    # yr_at_tail leave through the caller's out-buffers.
    ...
    for j in unroll(0, LIG_YR_LOG_LEN[m_idx] - 1):
        ...
        sumcheck_target = eadd(emul(eadd(emul(round_quad_a, tail_c), round_quad_b), tail_c), round_quad_c)
        fs, msg_a, msg_cursor = fs_next(fs, msg_cursor)
        fs, msg_b, msg_cursor = fs_next(fs, msg_cursor)
        round_quad_c = msg_a
        round_quad_b = eadd(sumcheck_target, msg_b)
    ...
    sumcheck_target = eadd(emul(eadd(emul(round_quad_a, tail_last), round_quad_b), tail_last), round_quad_c)
    ...
    for j in unroll(0, LIG_YR_LOG_LEN[m_idx]):
        ...
        sstore(tail_w, 2 * j, eadd([1, 0, 0], tail_c))
        sstore(tail_w, 2 * j + 1, tail_c)
    ...
    # inverse alone: hoist them out of the query loop into one row per level
    # (the fold coords, then the tail coords, which together are exactly the
    # level's LIG_LOG_MSG_COLS coordinates). Each query then folds in one
    # MUL_192 multiply-add per coordinate against the running chain, with no
    # per-query basis vector stored at all: the chain recurrence and the
    # accumulation fuse into a single pass over t.
    ...
    for lvl in unroll(0, LIG_N_LEVELS[m_idx]):
        for t in unroll(0, LIG_RESIDUAL_PREFIX_LEN[m_idx * LIG_MAX_LEVELS + lvl]):
            ...
            estore(basis_a * GEN ** (3 * (lvl * LIG_LOG_MSG_COLS_CAP + t)), eadd_base(1, fold_c))
            estore(basis_b * GEN ** (3 * (lvl * LIG_LOG_MSG_COLS_CAP + t)), emul(fold_c, vanish_inv))
        for j in unroll(0, LIG_YR_LOG_LEN[m_idx]):
            slot = lvl * LIG_LOG_MSG_COLS_CAP + LIG_RESIDUAL_PREFIX_LEN[m_idx * LIG_MAX_LEVELS + lvl] + j
            tail_c = eload(tail_challenges * GEN ** (3 * j))
            inv_idx = m_idx * LIG_MAX_VANISH_LEN + LIG_VANISH_OFF[m_idx * LIG_MAX_LEVELS + lvl] + LIG_RESIDUAL_PREFIX_LEN[m_idx * LIG_MAX_LEVELS + lvl] + j
            vanish_inv = [LIG_VANISH_INVS[3 * inv_idx], LIG_VANISH_INVS[3 * inv_idx + 1], LIG_VANISH_INVS[3 * inv_idx + 2]]
            estore(basis_a * GEN ** (3 * slot), eadd_base(1, tail_c))
            estore(basis_b * GEN ** (3 * slot), emul(tail_c, vanish_inv))
    ...
    for lvl in unroll(0, LIG_N_LEVELS[m_idx]):
        ...
        for xr in mul_range(1, GEN ** LIG_QUERIES[m_idx * LIG_MAX_LEVELS + lvl]):
            ...
            # t = 0: the chain IS the base-field query position, so the
            # multiply-add takes the cheaper base-scalar mode.
            basis_a0 = eload(basis_a * GEN ** (3 * lvl * LIG_LOG_MSG_COLS_CAP))
            basis_b0 = eload(basis_b * GEN ** (3 * lvl * LIG_LOG_MSG_COLS_CAP))
            prefix_eq = eadd(basis_a0, emul_base(basis_scalar, basis_b0))
            for t in unroll(1, LIG_LOG_MSG_COLS[m_idx * LIG_MAX_LEVELS + lvl]):
                ...
                basis_at = eload(basis_a * GEN ** (3 * (lvl * LIG_LOG_MSG_COLS_CAP + t)))
                basis_bt = eload(basis_b * GEN ** (3 * (lvl * LIG_LOG_MSG_COLS_CAP + t)))
                prefix_eq = emul(prefix_eq, eadd(basis_at, emul(basis_bt, basis_chain)))
    ...
    for ood_lvl in unroll(1, LIG_N_LEVELS[m_idx]):
        ...
        for os in unroll(0, LIG_OOD_SAMPLES[m_idx * LIG_MAX_LEVELS + ood_lvl]):
            ...
            for t in unroll(0, LIG_YR_LOG_LEN[m_idx]):
                ...
                tail_c = eload(tail_challenges * GEN ** (3 * t))
                scalar = emul(scalar, eadd(eadd([1, 0, 0], zt), tail_c))
...
def verify_sub(pi_0, pi_1, pi_2, pi_3, seed_0, seed_1, seed_2, seed_3, g_logs_pow2, g_squares, defer_out):
    ...
    # exponent.
    ext_assert_eq(sload(gkr_roots, PUSH_SIDE), sload(gkr_roots, PULL_SIDE))
    ...
    estore(zeta_eq_chain, [1, 0, 0])
    for xk in mul_range(1, g_bus_mu):
        xk3 = xk ** 3
        factor = eadd([1, 0, 0], eadd(eload(zeta * xk3), eload(fold_challenges * xk3)))
        estore(zeta_eq_chain * xk3 * GEN ** 3, emul(eload(zeta_eq_chain * xk3), factor))
    rho_eq_chain = HeapBuf(3 * (SIZE_BITS + 1))
    estore(rho_eq_chain, [1, 0, 0])
    for xk in mul_range(1, g_zc_n):
        xk3 = xk ** 3
    ...
    estore(zeta_slot_eq_chain, [1, 0, 0])
    for xk in mul_range(1, g_bus_mu):
        xk3 = xk ** 3
        factor = eadd([1, 0, 0], eadd(eload(zeta * xk3), eload(ris_slot * xk3)))
        estore(zeta_slot_eq_chain * xk3 * GEN ** 3, emul(eload(zeta_slot_eq_chain * xk3), factor))
    rho_slot_eq_chain = HeapBuf(3 * (SIZE_BITS + 1))
    estore(rho_slot_eq_chain, [1, 0, 0])
    for xk in mul_range(1, g_zc_n):
        xk3 = xk ** 3
        factor = eadd([1, 0, 0], eadd(eload(rho * xk3), eload(ris_slot * xk3)))
        estore(rho_slot_eq_chain * xk3 * GEN ** 3, emul(eload(rho_slot_eq_chain * xk3), factor))
    claim_weights = HeapBuf(3 * N_CLAIMS)
    pi_point = HeapBuf(3 * SIZE_BITS)
    estore(pi_point, rm0)
    estore(pi_point * GEN ** 3, rm1)
    ...
    for j in unroll(0, N_CLAIMS):
        ...
        if CLAIM_POINT_BUF[j] == POINT_BUF_RHO:
            low_eq = eload(rho_eq_chain * low_len_g ** 3)
        if CLAIM_POINT_BUF[j] == POINT_BUF_PI:
    ...
    for j in unroll(0, N_CLAIMS):
        ...
        for k in unroll(0, YR_LOG_CAP):
            ...
            slot_coord = eadd(emul_base(mask_bit, overlap), ebase((1 + mask_bit) * slot_bit))
            tail_c = eload(tail_challenges * GEN ** (3 * k))
            tail_eq = emul(tail_eq, eadd([1, 0, 0], eadd(slot_coord, tail_c)))
    ...
    for k in unroll(0, YR_LOG_CAP):
        ...
        tail_c = eload(tail_challenges * GEN ** (3 * k))
        rs_tail_eq = emul(rs_tail_eq, eadd_base(1 + yb, tail_c))
    rs_hi = rs_yslot_bits * yr_log_n_g
    for xk in mul_range(1, yr_pad_g):
        assert rs_hi[xk] == 0  # zero-pin coords beyond final_msg's log-length
    inner_sum = eadd(inner_sum, emul(rs_weight, rs_tail_eq))
