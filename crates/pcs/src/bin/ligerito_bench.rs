...
fn configs_for(log_n: usize) -> (ProverConfig, VerifierConfig) {
    LigeritoSecurityConfig::derive_config(log_n + 7)
        .expect("derive the shipped config")
        .to_prover_verifier_configs()
        .expect("prover/verifier configs")
}

/// eq(point, y) table, LSB-first indexing.
...
fn run_f128(log_n: usize, verify_once: bool) -> Timing {
    ...
    for _ in 0..SAMPLES {
        ...
        let mut ps = pcs::ProverState::new(b"ligerito-bench", &[]);
        let t = Instant::now();
        let proof = pcs::ligerito::multilevel_prover_with_basis(
            &pc,
            witness.clone(),
            b.clone(),
            target,
        ...
        ps.hint_opening(proof);
        let proof_t = ps.into_proof();

        if verify_once && !checked {
            checked = true;
            let root = c.root;
            let mut vs = pcs::VerifierState::new(b"ligerito-bench", &proof_t, &[]);
            let point_v = point.clone();
            let ok = pcs::ligerito::multilevel_verifier_with_basis_succinct(
                &vc,
                vs.next_opening().expect("the one opening"),
                log_n,
                target,
                &root,
            ...
            assert!(ok.is_some(), "F128 verification failed at log_n={log_n}");
        }
        black_box(proof_t);
    }
    Timing {
...
fn run_k(log_n: usize, verify_once: bool) -> Timing {
    ...
    for _ in 0..SAMPLES {
        ...
        let mut ch = Sponge::new(b"ligerito-bench-k", &[]);
        let t = Instant::now();
        ...
        if verify_once && !checked {
            checked = true;
            let mut vch = Sponge::new(b"ligerito-bench-k", &[]);
            let point_v = point.clone();
