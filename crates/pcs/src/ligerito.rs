...
fn add_sumcheck_msg(ps: &mut ProverState, msg: &SumcheckMessage) {
    ps.add_scalar(crate::as_e(msg.u_0));
    ps.add_scalar(crate::as_e(msg.u_2));
}
...
fn next_sumcheck_msg(vs: &mut VerifierState<'_>) -> Option<SumcheckMessage> {
    let u_0 = crate::as_ghash(vs.next_scalar().ok()?);
    let u_2 = crate::as_ghash(vs.next_scalar().ok()?);
...
fn multilevel_prover_with_basis_impl(
    ...
    ps.observe_scalar(crate::as_e(target));
    ...
    for j in 0..initial_k {
        ...
        let r = crate::as_ghash(ps.sample());
    ...
    // the interleaved list before any of L0's queries are drawn.
    {
        ...
        for _ in 0..ood_count(1) {
            let z = ps.sample_vec(n1).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
            ...
            ps.add_scalar(crate::as_e(y));
            add_sumcheck_msg(ps, &intro);
            let beta = crate::as_ghash(ps.sample());
            sc_prover.glue(beta);
        }
        if trace {
            t_ood += _t.elapsed();
    ...
    let alpha_0 = ps.sample_vec(log2_ceil(num_queries_0)).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
    let _t = std::time::Instant::now();
    ...
    let beta_0 = crate::as_ghash(ps.sample());
    ...
    for i in 0..r {
        ...
        for j in 0..k_i {
            ...
            let ri = crate::as_ghash(ps.sample());
        ...
        // OOD binding for the L_{i+2} commit (same as the L1 block above).
        {
            ...
            for _ in 0..ood_count(i + 2) {
                let z = ps.sample_vec(n_next).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
                let eq_z = build_eq_table(&z);
                let (intro, y) = sc_prover.introduce_new_with_eval(eq_z);
                ps.add_scalar(crate::as_e(y));
                add_sumcheck_msg(ps, &intro);
        ...
        let alpha_i = ps.sample_vec(log2_ceil(num_queries_i)).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
        let _t = std::time::Instant::now();
        ...
        let beta_i = crate::as_ghash(ps.sample());
    ...
    F: Fn(&[F128], usize) -> Vec<F128>,
{
    ...
    vs.observe_scalar(crate::as_e(target));
    ...
    for j in 0..initial_k {
        ...
        let ri = crate::as_ghash(vs.sample());
        r_lane_fold.push(ri);
    ...
    for _ in 0..ood_count(1) {
        let z = vs.sample_vec(log_n - initial_k).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
        let y = crate::as_ghash(vs.next_scalar().ok()?);
        let intro_msg = next_sumcheck_msg(vs)?;
        let intro_quad = RoundQuad::from_msg(intro_msg, y);
        let beta = crate::as_ghash(vs.sample());
    ...
    let alpha_0 = vs.sample_vec(log2_ceil(num_queries_0)).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
    let _t = std::time::Instant::now();
    ...
    let beta_0 = crate::as_ghash(vs.sample());
    ...
    for i in 0..r {
        ...
        if i == r - 1 {
            let yr: Vec<F128> = vs.next_scalars(1 << n_current).ok()?.into_iter().map(crate::as_ghash).collect();
            ...
            let alpha_last = vs.sample_vec(log2_ceil(num_queries_last)).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
            if trace {
                t_sample_q += _t.elapsed();
            }
            let _t = std::time::Instant::now();
            ...
            let beta_last = crate::as_ghash(vs.sample());
        ...
        for _ in 0..ood_count(i + 2) {
            let z = vs.sample_vec(n_current).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
            let y = crate::as_ghash(vs.next_scalar().ok()?);
            let intro_msg = next_sumcheck_msg(vs)?;
            let intro_quad = RoundQuad::from_msg(intro_msg, y);
            let beta = crate::as_ghash(vs.sample());
        ...
        let alpha_i = vs.sample_vec(log2_ceil(num_queries_i)).into_iter().map(crate::as_ghash).collect::<Vec<_>>();
        ...
        let beta_i = crate::as_ghash(vs.sample());
...
mod tests {
    ...
    fn stateful_sumcheck_single_basis_roundtrip() {
        ...
        for _ in 0..(n - 1) {
            let r = crate::as_ghash(ch.sample());
            ris.push(r);
        ...
        let r_last = crate::as_ghash(ch.sample());
        let mut t_r = h;
    ...
    fn stateful_sumcheck_introduce_glue() {
        ...
        let r0 = crate::as_ghash(ch.sample());
        ...
        let alpha = crate::as_ghash(ch.sample());
        prover.glue(alpha);
    ...
    fn induce_sumcheck_poly_consistent_with_codeword() {
        ...
        let msg: Vec<F128> = (0..msg_cols).map(|_| crate::as_ghash(ch.sample())).collect();
        ...
        while queries.len() < num_queries {
            let q = (ch.sample().c0 as usize) % block_len;
            if !queries.contains(&q) {
                queries.push(q);
            }
        }
        let opened_rows: Vec<Vec<F128>> = queries.iter().map(|&q| vec![codeword[q]]).collect();
        let alpha = ch.sample_vec(log2_ceil(queries.len())).into_iter().map(crate::as_ghash).collect::<Vec<F128>>();
    ...
    fn induce_sumcheck_poly_via_ntt_matches_dense() {
        ...
        for (si, &(log_msg, log_inv_rate, log_int, n_queries)) in shapes.iter().enumerate() {
            ...
            let opened_rows: Vec<Vec<F128>> = (0..nq)
                .map(|_| ch.sample_vec(num_interleaved).into_iter().map(crate::as_ghash).collect::<Vec<F128>>())
                .collect();
            let v_challenges = ch.sample_vec(log_int).into_iter().map(crate::as_ghash).collect::<Vec<F128>>();
            let alpha = ch.sample_vec(log2_ceil(nq.max(1))).into_iter().map(crate::as_ghash).collect::<Vec<F128>>();
            let sks_vks = eval_sk_at_vks(log_msg);
        ...
        for &log_d in &[6usize, 11, 12, 14, 16, 18] {
            for &nq in &[1usize, 5, 43, 218] {
                ...
                while positions.len() < nq {
                    let p = (ch.sample().c0 as usize) % n;
                    if !positions.contains(&p) {
                        positions.push(p);
                        values.push(crate::as_ghash(ch.sample()));
                    }
                }
    ...
    fn induce_sumcheck_poly_with_interleaving_and_v_challenges() {
        ...
        let poly: Vec<F128> = (0..poly_len).map(|_| crate::as_ghash(ch.sample())).collect();
        ...
        let v_challenges: Vec<F128> = (0..log_interleaved).map(|_| crate::as_ghash(ch.sample())).collect();
    ...
    fn induce_sumcheck_evaluate_at_residual_matches_dense() {
        ...
        let opened_rows: Vec<Vec<F128>> = (0..num_queries)
            .map(|_| (0..num_interleaved).map(|_| crate::as_ghash(rng.sample())).collect())
            .collect();
        let v_challenges: Vec<F128> = (0..log_num_interleaved)
            .map(|_| crate::as_ghash(rng.sample()))
            .collect();
        let alpha: Vec<F128> = (0..log2_ceil(num_queries))
            .map(|_| crate::as_ghash(rng.sample()))
            .collect();
        let ris_for_basis: Vec<F128> = (0..prefix_len).map(|_| crate::as_ghash(rng.sample())).collect();
    ...
    fn final_level_binding_pins_yr_to_committed_codeword() {
        ...
        let yr: Vec<F128> = (0..msg_cols).map(|_| crate::as_ghash(rng.sample())).collect();
    ...
    fn ligerito_ood_and_fold_grinding_roundtrip_and_tamper() {
        ...
        let poly: Vec<F128> = (0..(1usize << log_n)).map(|_| crate::as_ghash(rng.sample())).collect();
        let z: Vec<F128> = (0..log_n).map(|_| crate::as_ghash(rng.sample())).collect();
        ...
        bad_ood.stream[first_ood_idx] += crate::as_e(F128::ONE);
        ...
        bad_nonce.stream[fold_nonce_0_idx] += crate::as_e(F128::new(0xDEAD_BEEF, 0));
    ...
    fn multilevel_prover_with_basis_roundtrip_batched_claims() {
        ...
        let poly: Vec<F128> = (0..(1usize << log_n)).map(|_| crate::as_ghash(rng.sample())).collect();
        let z1: Vec<F128> = (0..log_n).map(|_| crate::as_ghash(rng.sample())).collect();
        let z2: Vec<F128> = (0..log_n).map(|_| crate::as_ghash(rng.sample())).collect();
        let g1 = crate::as_ghash(rng.sample());
        let g2 = crate::as_ghash(rng.sample());
        ...
        let eval_b_residual = move |ris: &[F128], yr_log_n: usize| -> Vec<F128> {
            ...
            (0..yr_len)
                .map(|y| {
                    ...
                    g1 * crate::eq_eval_ghash(&z1, &point)
                        + g2 * crate::eq_eval_ghash(&z2, &point)
                })
                .collect()
        };
        let bundle = p_ch.into_proof();
