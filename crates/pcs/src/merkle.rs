...
/// ([`primitives::sha2::zero_prefix_state`]) and each leaf hashes only what
...
/// Leaves the batched `sha2_eth` consumes in one whole batch: the widest backend
/// this build targets, which is also a multiple of the narrower widths and of the
/// aarch64 crypto path's interleave.
const BATCH_LEAVES: usize = primitives::sha2::LANES;
...
fn hash_leaves_padded_rows_uninit(
    ...
    // the zero padding and then its row. Under the length-first ordering that
    // prefix depends only on the image's width, so the whole tree shares one
    // chain. Both the sharing and the batched hasher need whole blocks, so an
    // image that is not one (only the small ring-switch shapes) shares nothing and
    // takes the scalar arm below. Every real leaf width is `2^k >= 8`.
    ...
    let state = primitives::sha2::zero_prefix_state(leaf_words * 8, zero_blocks);
    ...
    for_each_hash_group(out, |lo, outputs| {
        ...
        for (t, chunk) in outputs.chunks_mut(per_tile).enumerate() {
            ...
            if whole_blocks {
                ...
                primitives::sha2::hash_many_dyn_from_state(bytes, staged * 8, &state, out_bytes);
            } else {
...
/// Words of `F64` per `sha2_eth` block.
