...
use primitives::field::F128T as F128;
...
mod tests {
    use super::*;
    use crate::ligerito_k::build_eq_table_ext as build_eq;

    fn mle_eval(table: &[F128], point: &[F128]) -> F128 {
        assert_eq!(table.len(), 1usize << point.len());
        let mut folded = table.to_vec();
        let mut len = folded.len();
        for &r in point {
            for i in 0..len / 2 {
                folded[i] = folded[2 * i] + r * (folded[2 * i] + folded[2 * i + 1]);
            }
            len /= 2;
        }
        folded[0]
