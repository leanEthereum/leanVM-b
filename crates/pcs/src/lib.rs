...
/// Lane lift between the tower-typed transcript scalars (`E = F128T`) and this
/// crate's GHASH-typed `F128` world: a raw 16-byte reinterpretation (lo↔c0,
/// hi↔c1) — NOT the field isomorphism. The transcript only ever binds/serves
/// uniform bytes; all GHASH arithmetic stays on the `F128` side.
#[inline]
pub fn as_e(x: F128) -> F128T {
    F128T::new(x.lo, x.hi)
}
/// Inverse lane lift of [`as_e`].
#[inline]
pub fn as_ghash(x: F128T) -> F128 {
    F128::new(x.c0, x.c1)
}
...
/// `eq(r, x)` over the GHASH-typed `F128` (mirror of
/// `primitives::multilinear::eq_eval`, which is `E = F128T`-typed).
pub(crate) fn eq_eval_ghash(r: &[F128], x: &[F128]) -> F128 {
...
fn stack_claim_eq_at(claim: &StackClaim, x: &[F128]) -> F128 {
    match claim {
        StackClaim::Slot { offset, low_point, .. } => {
            let n = low_point.len();
            let mut e = eq_eval_ghash(low_point, &x[..n]);
            let sel = offset >> n;
        ...
        StackClaim::StridedSlot { offset, slot, stride_log, point, .. } => {
            ...
            e *= eq_eval_ghash(point, &x[*stride_log..block_vars]);
        ...
        StackClaim::Point { point, .. } => eq_eval_ghash(point, x),
    }
}
...
pub fn open_batch_mixed_ligerito_stacked(
    ...
    for claim in stack_pd {
        ps.observe_scalar(as_e(claim.value()));
    }
    let gammas_pd: Vec<F128> = (0..stack_pd.len()).map(|_| as_ghash(ps.sample())).collect();
...
pub fn verify_opening_batch_mixed_ligerito_stacked(
    ...
    let r_dprime: Vec<F128> = if n_rs > 0 {
        vs.sample_vec(LOG_PACKING).into_iter().map(as_ghash).collect()
    } else {
        Vec::new()
    };
    ...
    let rs_outputs: Vec<ring_switch::RingSwitchVerifierOutput> = (0..n_rs)
        ...
        .collect();
    let gammas_rs: Vec<F128> = (0..n_rs).map(|_| as_ghash(vs.sample())).collect();
    let mut target_combined = F128::ZERO;
    ...
    for claim in stack_pd {
        vs.observe_scalar(as_e(claim.value()));
    }
    let gammas_pd: Vec<F128> = (0..stack_pd.len()).map(|_| as_ghash(vs.sample())).collect();
    for (claim, g) in stack_pd.iter().zip(gammas_pd.iter()) {
