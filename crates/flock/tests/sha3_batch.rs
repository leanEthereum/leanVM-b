...
fn sha3_batch_prove_verify() {
    ...
    let prove_pass = || {
        ...
        let mut ps = ProverState::new(b"flock-sha3-batch", &[]);
    ...
    let (_, verify_time) = Plan::new(plan.repeat, 0).measure_quiet(|_final_pass| {
        let mut vs = VerifierState::new(b"flock-sha3-batch", &transcript, &[]);
    ...
    println!(
        "\nFlock Keccak-f[1600] batch proving, {} permutations (2^{n_log} slots)",
        pretty_integer(n)
    );
    ...
    let perms_per_second = (n as f64 / prove_s).round() as u64;
    println!(
        "  throughput                      : {:>14} permutations/s{}",
        pretty_integer(perms_per_second),
        prove.spread()
    );
    println!(
        "  (~{:.1} XMSS/s equivalent at 138 permutations/signature)",
        n as f64 / prove_s / 138.0
    );
}
