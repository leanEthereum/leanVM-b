        ...
        |x_hi| {
            ...
            let (a_chunk, b_chunk) = unsafe { (a_chunks.get(x_hi), b_chunks.get(x_hi)) };
            {
                ...
                for x_lo in 0..lo_size {
                    ...
                    let a0 = fold_row(table, a_packed, x0g);
                    let b0 = fold_row(table, b_packed, x0g);
                    let a1 = fold_row(table, a_packed, x1g);
                    let b1 = fold_row(table, b_packed, x1g);
    ...
    let sum1 = parallel::map_reduce(
        ...
        |x_hi| {
            ...
            for x_lo in 0..lo_size {
                ...
                let x1l = x0l + 1;
                if ((pair_idx_base + x_lo) & pair_in_block_mask) >= useful_pairs_inclusive {
                    c_chunk[x0l] = F192::ZERO;
                    c_chunk[x1l] = F192::ZERO;
                    continue;
                }
                let x0g = base + 2 * x_lo;
                let c0 = fold_row(table, c_packed, x0g);
                let c1 = fold_row(table, c_packed, x0g + 1);
                c_chunk[x0l] = c0;
                c_chunk[x1l] = c1;
...
mod tests {
    ...
    ///
    /// Covers three padding shapes: SHA-256 as encoded here (k_log=15,
    /// useful=29113), a denser 2^15 block (useful=31401), and Keccak
    /// (k_log=16, useful=42560).
    #[test]
    fn uni_skip_fold_round_pair_padded_matches_dense() {
        const K_SKIP: usize = 6;
        let cases: &[(usize, usize, usize)] = &[(18, 15, 29_113), (18, 15, 31_401), (19, 16, 42_560)];
    ...
    /// coarser skip does not reach them at the SHA-256 shape.
    #[test]
    fn uni_skip_fold_round_single_padded_matches_dense() {
        const K_SKIP: usize = 6;
        let cases: &[(usize, usize, usize)] = &[(18, 15, 29_113), (17, 14, 16_000), (18, 15, 31_401)];
        for &(m, k_log, useful_bits) in cases {
