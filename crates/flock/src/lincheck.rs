...
// [`CscCircuit`] (the cached column-major transpose of SHA-256's `(A_0, B_0)`,
// see `BlockR1cs::csc_lincheck_circuit`), the prover's marginal fold, and
// `sha2::WalkLincheckCircuit`, whose `bilinear_form` lets the verifier skip
...
///
/// On SHA-256's matrices (k = 2^14, ~16.7M nonzeros) this measures ~1.7× faster
...
fn partial_fold_packed_z_fast_padded(
    ...
    // with allocation plus tree-reduce XOR traffic (SHA-256's k = 2^14 is
...
mod tests {
    ...
    fn partial_fold_padded_matches_dense() {
        // (m, k_log, useful_bits)
        let cases: &[(usize, usize, usize)] = &[
            // SHA-256's own shape: k_log = 14, useful = 15409, not byte-aligned.
            (17, 14, 15_409),
