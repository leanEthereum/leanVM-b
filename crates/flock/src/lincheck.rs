...
fn prove_padded_inner<O>(
    ...
    let captured_z_vec: Option<Vec<F192>> = if capture_z_vec { Some(z_vec.clone()) } else { None };
...
mod tests {
    ...
    fn sparse_row_fold_matches_dense() {
        ...
        let eq_table: Vec<F192> = rng.ext_vec(k);
