...
# heap cells `buf·i`, `buf·i·g` (one MUL folds `i` into the pointer). A BLAKE3
# chain over heap pairs, addressed by the loop counter: pair k sits at cells
# g^{2k}, g^{2k+1}, and pair k+1 = H(pair k, pair k). Published: H³(5, 7).
...
def main():
    ...
    for i in mul_range(1, GEN ** 3):
        b = i * i  # pair k at cells g^{2k}, g^{2k+1}
