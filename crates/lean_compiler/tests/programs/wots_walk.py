...
# over heap slices; the walker also builds g^{4n} at runtime (unrolled MULs)
# to read its final quad back through g-power indexing. The recomputation at
# the end lands on an already-written StackBuf quad, so write-once turns the
# hash into a digest assertion; the dead `if` branch holds an impossible
# assert that must never execute. Published: the first two words of H²(5, 7).
...
def main():
    ...
    v[0] = t0
    v[1] = t1
    blake3(buf[4:8], buf[4:8], v)  # recompute H(quad1, quad1): asserts v[0:2] == (t0, t1)
    p = GEN ** 0
    p[1] = t0
    p[GEN] = t1
    return
