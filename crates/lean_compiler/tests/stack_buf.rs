...
";
    ...
    assert_eq!(rows(src, want, "BLAKE3"), 1, "one BLAKE3 instruction");
...
";
    ...
    let (proof, _) = prove(&program, want, lean_vm::pcs::LOG_INV_RATE);
    assert_eq!(rows(src, want, "BLAKE3"), 2);
...
";
    ...
    let (proof, _) = prove(&program, want, lean_vm::pcs::LOG_INV_RATE);
    assert_eq!(rows(src, want, "BLAKE3"), 0, "no BLAKE3 here");
