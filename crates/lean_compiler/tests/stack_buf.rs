...
";
    ...
    let want = [F128T::new(word(0), word(8)), F128T::new(word(16), word(24))];
    let (proof, stats) = prove(&program, want);
...
";
    ...
    for flag in [0, 1] {
        ...
        program.set_witness("flag", vec![vec![F128T::new(flag, 0)]]);
...
";
    ...
    let d = digest(&block);
    let want = [F128T::new(d[0].0, d[1].0), F128T::new(d[2].0, d[3].0)];
    warm_setup(1);
    let (proof, _) = prove(&program, want);
