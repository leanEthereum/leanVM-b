...
fn a_proof_roundtrips_through_bytes() {
    ...
    // A SHA-256 height below flock's instance floor describes a layout the
    // arithmetization cannot express, and all three verifiers reject it there.
    let sha2 = lean_vm::cpu::Stats::TABLES.iter().position(|&t| t == "SHA2").unwrap();
    let mut sub_floor = decoded;
    sub_floor.stream[1 + sha2] = F192::new(2, 0, 0);
    assert!(
        matches!(verify(&program, &pi, &sub_floor), Err(Error::PublicInput)),
        "the announced SHA-256 height must reach flock's instance floor"
    );
}
