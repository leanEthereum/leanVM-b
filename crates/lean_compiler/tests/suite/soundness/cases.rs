...
///
/// The digest constant comes from [`print_hash_digest`], not from a hand
...
/// print_hash_digest -- --ignored --nocapture`. Kept so the constant above is
/// reproducible rather than folklore.
#[test]
#[ignore = "prints a constant; not a check"]
fn print_hash_digest() {
    let src = "\
def main():
    m = StackBuf(4)
    hint_witness(m, \"msg\")
