...
/// `zkDSL.md` §sha2: "If `out` was already written, the statement *asserts*
...
///
/// Regression test: the `SHA2` output arm named the raw run, so a `StackBuf`
...
#[test]
fn prewritten_sha2_out_asserts_the_digest() {
    check_pair(&Pair {
        name: "prewritten_sha2_out_asserts_the_digest",
        why: "zkDSL.md §sha2: a pre-written `out` turns the hash into a verification.",
        a: "\
def main():
    v = StackBuf(2)
    hint_witness(v, \"w\")
...
",
        trials: {
            let digest = super::cases::digest_5_7();
            vec![
                two(digest[0], digest[1]),
                // Anything else must be rejected by both spellings.
                two(F192::ZERO, F192::ZERO),
                two(digest[0], F192::ZERO),
                two(g(3), g(5)),
            ]
        },
    });
