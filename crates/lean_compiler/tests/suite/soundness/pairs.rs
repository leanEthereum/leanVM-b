...
/// `zkDSL.md` §SHA3-256: "If `out` was already written, the statement *asserts*
...
///
/// Regression test: the `sha3_64` output arm named the raw run, so a `StackBuf`
...
#[test]
fn prewritten_hash_out_asserts_the_digest() {
    check_pair(&Pair {
        name: "prewritten_hash_out_asserts_the_digest",
        why: "zkDSL.md §SHA3-256: a pre-written `out` turns the hash into a verification.",
        a: "\
def main():
    v = StackBuf(2)
    hint_witness(v, \"w\")
