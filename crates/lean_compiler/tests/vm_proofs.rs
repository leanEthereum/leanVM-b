...
fn a_proof_roundtrips_through_bytes() {
    ...
    bad_rate.stream[1 + lean_vm::tables::N_TABLES] = F192::new(5, 0, 0);
    assert!(
    ...
    let mut sub_floor = decoded.clone();
    sub_floor.stream[1 + lean_vm::tables::BLAKE3_TABLE] = F192::new(2, 0, 0);
    assert!(
        matches!(verify(&program, &pi, &sub_floor), Err(Error::PublicInput)),
        "the announced BLAKE3 height must reach flock's instance floor"
    );

    // Every announcement is one base-field word: a nonzero high limb is not a size.
    for announcement in 0..=lean_vm::tables::N_TABLES + 1 {
