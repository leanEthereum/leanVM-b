...
pub enum FillerOp {
    ...
    /// `SET s = 0`.
    Set,
    /// `XOR_192 s, s -> s`: pins the scratch run to `m[s..s+3] + m[s..s+3] = 0`.
    AddExt,
    /// `MUL_192 s, s -> s`: pins it to `m[s..s+3]^2`, so to `0` given the above.
    MulExt,
    /// `DEREF_192` through the frame's pointer cell: the three-word run at memory cell
    /// `0`, the public input's, equals the scratch run.
    DerefExt,
...
/// them.
pub const TABLES: [(u8, FillerOp); 9] = [
    (0, FillerOp::Xor),
    (1, FillerOp::Mul),
    (2, FillerOp::AddExt),
    (3, FillerOp::MulExt),
