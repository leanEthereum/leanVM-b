...
struct FnLower<'a> {
    ...
    /// instruction's destination, a `sha3_64` output, or a hint destination. A
...
impl FnLower<'_> {
    ...
    fn emit(&mut self, op: LOp) {
        ...
        match op {
            ...
            LOp::Keccak { c, .. } => {
                ...
                // and the rest are dead, but the instruction writes every one of them.
                for o in c..c + lean_vm::hash_flock::STATE_CELLS as u32 {
                    self.phys.insert(o);
                }
                true
            }
    ...
    /// cells: a `sha3_64` output, or a hint destination. Those consumers do not go
    ...
    fn sha3_pad64(&mut self) -> Off {
        ...
        // Every cell, the all-zero ones included. An unwritten cell reads as zero,
        // so skipping them would compute the same digest, but then the instruction
        // reads cells nothing writes, which is what `cpu::prove` rejects: the
        // emitted code has to say what it relies on. Once per scope either way.
    ...
    fn stack_store(&mut self, dst: Off, val: &Expr) {
        ...
        // pre-written `sha3_64` output assert the digest.
    ...
    fn lower_sha3_64(&mut self, args: &[Expr]) {
        ...
        let st = self.alloc_stack(lean_vm::hash_flock::STATE_CELLS as u32);
    ...
    /// [`lean_vm::hash_flock::STATE_CELLS`] cells, `block` spans
    /// [`lean_vm::hash_flock::RATE_CELLS`].
    fn lower_keccak(&mut self, args: &[Expr]) {
        assert_eq!(args.len(), 3, "keccak takes three arguments: (prev, block, out)");
        let prev = self.keccak_run(&args[0], lean_vm::hash_flock::STATE_CELLS);
        let block = self.keccak_run(&args[1], lean_vm::hash_flock::RATE_CELLS);
        let out = self.keccak_run(&args[2], lean_vm::hash_flock::STATE_CELLS);
        self.emit(LOp::Keccak {
            ins: [block, block + 1, block + 2, block + 3],
            rest: block + lean_vm::hash_flock::IN_CELLS as u32,
            prev,
            c: out,
        });
    }
    ...
    fn sha3_zero_state(&mut self) -> Off {
        ...
        // Written out for the same reason as [`Self::sha3_pad64`]'s zero cells: the
        // absorb reads all thirteen, so all thirteen have to be written.
