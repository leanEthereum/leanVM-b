...
def hash_pair(buf, k: Const):
    h = StackBuf(2)
    sha2(buf[k * 2:k * 2 + 2], buf[k * 2:k * 2 + 2], h)
    return h[0], h[1]
```
...
cvb = obs(cvb, v)        # exactly 3 ops: two tag writes + one sha2
```
...
All memory is **write-once**: a cell is set once; a second write of the same value is a no-op, of a different value a proof failure. This turns stores into equality assertions and is used throughout (publishing, `sha2` outputs). Reading a cell nobody ever writes yields an unconstrained value (fixed to zero at the end of witness generation): don't.
...
**Compile-time indices are bounds-checked.** When the whole index is a compile-time exponent and the pointer resolves to a declared `HeapBuf` (directly, or through shifted aliases like `row = buf * GEN ** k`), the compiler rejects `index >= size`, and the same for the spans of `hint_witness` and `sha2` slices. **Runtime** indices are not checked (their value is unknown at compile time): there the buffer remains a region convention, and a stray access surfaces at proving time as a write-once conflict or wild deref.
...
`buf[lo:hi]` names a run of cells (`hi` exclusive). `sha2` operands must span exactly two cells; `hint_witness` accepts any supported literal length. Two forms:
...
```python
h = StackBuf(2)
sha2(a, b, h)                    # sha2_eth of (a, b) written into h
sha2(t[0:2], t[x:x + 2], t[4:6])  # slices of one large StackBuf
sha2(h, hb[0:2], hb[2:4])         # HeapBuf slices, input and output
sha2(hb[i:i + 2], h, hb[j:j + 2])  # runtime-indexed heap slices (i, j g-powers)

# An 80-byte hash as two blocks. Keyword values are compile-time.
block0 = [1, 2, 3, 4]  # 64 bytes
tail = [5, 0, 0, 0]    # 16 more, the rest of the block zero-filled
sha2(block0[0:2], block0[2:4], cv, msg_bytes=80)
...
Omitting both selects `iv_for_len(64)`, the one-block hash. Each distinct constant chaining value costs two `SET`s, emitted at the first such hash on each runtime path and reused thereafter.

There is no block-length field, so only the last block may be partial and the program must zero-fill the bytes past the message's real length, which the compression circuit does not enforce. An `n`-byte hash is therefore `ceil(n / 64)` instructions: `msg_bytes=n` on the first, `cv=` on each of the rest.
...
The compression, including its chaining value, is proven by the flock-derived SHA-256 R1CS (`crates/flock`, see `doc.pdf` §SHA-256); one instruction is one 64-byte-block compression.
...
| `sha2(a, b, out, ...)` | 1; plus two `SET`s once per frame per distinct constant chaining value; message/CV words are read in place, +1 `DEREF` per heap input or CV word, +1 `MUL` per runtime slice start |
