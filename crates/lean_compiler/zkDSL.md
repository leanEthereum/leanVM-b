...
- the same trap follows a **name** across a boundary, and there the compiler picks a reading for you rather than warning. A global constant and a `Const` argument are both evaluated as integers and substituted, so `STEP = 3 + 1` is `4` everywhere it appears while the identical `x = 3 + 1` written inside a function is the field element `2`; and a compile-time `if` folds on the integer reading while the runtime test compares field values, so `if K == 4` with `K = 3 + 1` enters an arm whose own condition is false as a value. Where a constant may be read either way, write it so the two readings coincide: a literal, a power of two, a product, or `GEN ** k`, never a sum of overlapping integers.
