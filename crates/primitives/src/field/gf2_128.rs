...
pub mod aarch64 {
    ...
    /// the AVX2 and AVX-512 paths used by the radix-four GKR prover.
    ///
    /// # Safety
...
pub mod x86_64 {
    ...
    #[cfg(all(target_feature = "vpclmulqdq", target_feature = "avx2"))]
    #[target_feature(enable = "vpclmulqdq", enable = "avx2")]
    pub unsafe fn ghash_mul_unreduced_vec2_clmul(
        a: [F128; 2],
        b: [F128; 2],
    ) -> [F256Unreduced; 2] {
        // SAFETY: the function carries the required target features; each
        // unaligned load spans exactly two repr(C) F128 values.
        unsafe {
    ...
    /// Batch multiply 4× F128, one multiplication in each 128-bit lane of an
    /// AVX-512 register.
    ///
    /// # Safety
    ...
    pub unsafe fn ghash_mul_vec4_clmul(a: [F128; 4], b: [F128; 4]) -> [F128; 4] {
        // SAFETY: the function carries all required target features; the
        // unaligned loads and store cover exactly four repr(C) F128 values.
        unsafe {
    ...
    pub unsafe fn ghash_mul_unreduced_vec4_clmul(
        a: [F128; 4],
        b: [F128; 4],
    ) -> [F256Unreduced; 4] {
        // SAFETY: the function carries the required target features and every
        // load/store spans exactly four repr(C) F128 values.
        unsafe {
            let x = _mm512_loadu_si512(a.as_ptr().cast());
            let y = _mm512_loadu_si512(b.as_ptr().cast());
