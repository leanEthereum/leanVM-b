...
\vspace{-3em}
\begin{center}
Moving leanVM-b's data to $\K=\F_2[x]/(x^{64}+x^4+x^3+x+1)$ inside
$\E=\K[y]/(y^2+y+x^{61})$:\\
...
\textbf{Addendum (shipped design is now hybrid).} This note is the rationale for
the field \emph{stack} $\K=\F_{2^{64}}\subset\E=\F_{2^{128}}$ and the
base-field-commit / extension-open PCS, which the machine keeps. But machine
\emph{words} were subsequently re-widened to $128$ bits (elements of $\E$) so a
recursion guest can do native $128$-bit field arithmetic: a memory word, an
immediate, and a hashed value are $\E$-valued, while \emph{addresses, the pc/fp,
and counters stay in the $64$-bit subfield $\K$}. A word is committed as its two
$\K$-lanes (two ordinary $\K$-columns), so the commitment layer and $120$-bit
soundness here are unchanged; \texttt{MUL\_NATIVE} is multiplication in $\E$,
and \texttt{BLAKE3} addresses one cell per $128$-bit chunk. Where this note says
``every VM quantity is a $\K$-element'' (\S\ref{sec:vm-transport}) read
``addresses/pc/counters are $\K$; data words are $\E$, carried as two $\K$-lanes.''
The shipped spec is \texttt{misc/doc.tex}.
}}\end{center}
...
\item \textbf{Randomness lives in $\E$.} Every challenge ($\alpha,\gamma,
  ...
  not move. Under $\K\hookrightarrow\E$, $k\mapsto k+0\cdot y$, $\K$-valued
...
$\K=\F_{2^{64}}=\F_2[x]/(x^{64}+x^4+x^3+x+1)$, with
$x^{64}=x^4+x^3+x+1$ and $\mathtt{0x1B}=x^4+x^3+x+1$. One
...
$\E=\K[y]/(y^2+y+x^{61})$, with
$\mathrm{Tr}_{\K/\F_2}(x^{61})=1$. Elements are $e_0+e_1y$ with
...
(These bullets describe the pure-64-bit step; the shipped hybrid supersedes the
word-width ones — see the Addendum. Kept for the rationale.)
\begin{itemize}
\item \texttt{MUL\_NATIVE} becomes the $\K$-product (one \texttt{PMULL}
  ...
  degree 2 as before). \emph{[Hybrid: words are $\E$, so this is the $\E$ tower
  product — still degree $2$ on the value lanes.]}
\item \texttt{BLAKE3}: a 256-bit value now spans \emph{four} consecutive
  ...
  flock witness (\S\ref{sec:pipeline}). \emph{[Hybrid: a $128$-bit chunk is one
  cell, so a $256$-bit value is \emph{two} cells and the row touches six.]}
\item Public input: $256$ bits now occupy memory cells $0..3$ (a 2-variable
  ...
  square. \emph{[Hybrid: the two public-input words are cells $0,1$; the
  verifier checks the $\widetilde{\mathrm{PI}}$ line on each of the two memory
  lanes.]}
\item Compiler/programs: immediates are 64-bit; XMSS digests are 4 words;
  ...
  \emph{[Hybrid: immediates are $128$-bit; a $256$-bit value is $2$ cells.]}
\end{itemize}

\subsection{Protocol randomness moves to $\E$}

All challenges are sampled from $\E$ (16-byte transcript reads as $e_0+e_1y$,
$e_0,e_1\in\K$); all sumchecks, zerochecks and GKR passes run
...
\paragraph{Status.} All work packages below have landed on
\texttt{field-exploration}; \texttt{cargo testall} is green end to end
(including \texttt{xmss\_vm} and \texttt{hash\_chain}). Two deltas
against the plan as written. First, the flock boundary is cleaner than
anticipated: the zerocheck (with \(\varphi_8\) and the univariate skip)
stays entirely in the GHASH representation, and its output claims cross
into the tower through an explicit field isomorphism
(\path{vendor/flock-core/src/field/iso_f128.rs}), so \WP6 required no
changes to the zerocheck stack at all; the ring switch itself was built
fresh at \(f=64\), \(e=128\) with generalized prefix weights
(\path{pcs/ring_switch_k.rs}, \path{pcs/stack_open_k.rs}). Second,
\texttt{flock-prover} needed no witness re-layout: \(\mathtt{M\_BASE}=640\)
is already 64-aligned, so \(q_{\mathrm{pkd}}\) over $\K$ is the lo/hi
flattening of the old packed words and only the slot indices doubled.
Measured on the demo workload (Fibonacci, $2^{21}$ cycles): 1.2M cycles/s
proving, 10\,ms verification, 570\,KiB proof.
...
sample $\E$ challenges (16 bytes, $e_0+e_1y$ with $e_0,e_1\in\K$), domain
...
\item \textbf{Representation swap breadth} (\WP2, \WP4, \WP6). Everything
  typed \texttt{F128} changes meaning: the two representations
...
\item \textbf{$\E\times\E$ parity} (\WP1). Multiplication in $\E$ is expected
