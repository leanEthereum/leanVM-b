...
\paragraph{Instruction set.} The machine implements eight instructions. Below, $o,o_A,\dots$ are fp-relative references and $k$ a base-field immediate.
\begin{center}
\begin{tabular}{lll}
\hline
...
\texttt{BLAKE3} compresses $64$ bytes to $32$. Each operand is four consecutive $\K$ words (256 bits): $a$ begins at $\loc{o_A}$, $b$ at $\loc{o_B}$, and the digest $c$ at $\loc{o_C}$. Only each run's first address is committed; its remaining addresses are virtual multiples by $\gen,\gen^2,\gen^3$. Each row accesses twelve memory words. The BLAKE3 table (\S\ref{sec:tab-blake3}) connects them to the memory, state, and bytecode interactions, while Flock~\cite{flock} proves the compression relation.
...
Compresses two 256-bit operands to a 256-bit digest. Each value is four
consecutive $\K$ words. The committed starting addresses are
$a_A=\fp o_A$, $a_B=\fp o_B$, and $a_C=\fp o_C$; addresses at offsets
$1,2,3$ are virtual multiples by $\gen,\gen^2,\gen^3$. Thus each row accesses
twelve memory cells while committing only three address columns.
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_A,o_B,o_C$; starting addresses $a_A,a_B,a_C$; twelve per-word memory counts; bytecode count $r_{\mathrm{bc}}$. The twelve values live in $q_\pkd$.
\item \textbf{Constraints:} the three address relations $a_X=\fp o_X$. Flock proves $c=\mathrm{BLAKE3}(a,b)$.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{B3},o_A,o_B,o_C,0,0}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
  \item \textbf{Memory:} twelve reads, at $\gen^j a_X$ for $X\in\{A,B,C\}$ and $j\in\{0,1,2,3\}$, each paired with the count multiplied by $\gen$.
  \end{itemize}
\end{itemize}
...
\[
  \opc{ADD}=1,\quad \opc{MUL}=\gen,\quad \opc{SET}=\gen^{2},\quad \opc{DRF}=\gen^{3},\quad \opc{JMP}=\gen^{4},\quad \opc{B3}=\gen^{5},\quad \opc{ADD\_EXT}=\gen^{6},\quad \opc{MUL\_EXT}=\gen^{7}.
\]
...
\paragraph{Local constraints.} Run once per table (\S\ref{sec:air}); the eight runs share no challenges.
\begin{enumerate}
...
How the zkDSL compiler (\texttt{crates/lean\_compiler}; surface language in \texttt{crates/lean\_compiler/zkDSL.md}) programs the eight-instruction ISA. Two themes run through every pattern: \emph{write-once memory is an assertion mechanism} (a second write of a cell is an equality check, an unwritten cell is prover-chosen), and \emph{indices live in the exponent} (a logical index $i$ is the element $\gen^{i}$, so incrementing, offsetting, and address formation are single field multiplications).
