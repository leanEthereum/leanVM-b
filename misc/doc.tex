...
The ring-switch-specific identities and their recursive-verification reductions
are derived separately in the
\href{https://github.com/leanEthereum/leanVM-b/releases/download/ring-switching-latest/ring-switching.pdf}{ring-switching note}.

\subsection{Deferred evaluation claims}\label{sec:deferred-claims}

Three places during verification would otherwise require a large \emph{fixed} multilinear computation:
\begin{itemize}
...
\item \textbf{the trace-dual Frobenius transform}: the ring-switch front uses the fixed table
\[
 D(k,i)=\delta_i^{2^k},\qquad k,i\in\{0,\ldots,127\},
\]
where $(\delta_i)$ is the trace-dual basis. At the shared seven-coordinate challenge $r''_t$, its linearized-polynomial coefficients are
$c_t(k)=\sum_iD(k,i)\eq(r''_t,i)$. Forming all $128$ coefficients directly inside the VM costs a $128\times128$ contraction.
\end{itemize}
Each computation is one pass over a fixed object. This takes milliseconds natively, but in-circuit it would dwarf the entire remaining verifier, in cycles and in bytecode size alike.
...
There are thus four fixed polynomials: the bytecode, the two matrices $A_0,B_0$, and the $14$-variable multilinear extension $\widetilde D$. Once all $n_{\mathrm{rec}}$ sub-proofs are verified, a fresh Fiat--Shamir transcript is fed with all deferred points, values, and coefficient vectors. Three sumchecks batch them: one for the bytecode, one shared by the two matrices, and one for $D$. The four reduced evaluations are written to the recursive statement. Only the outermost verifier, which runs natively, evaluates the fixed polynomials.
...
For the trace-dual table, the prover supplies each $c_t$ as advice and the guest uses it immediately in the ring-switch checks. Soundness is restored in the fresh aggregation transcript. After absorbing every $c_t$ and $r''_t$, it samples batching scalars $\gamma_t$ and a random seven-coordinate row point $\rho$, and forms
\[
 C=\sum_t\gamma_t\sum_k\eq(\rho,k)c_t(k)
   =\sum_i \widetilde D(\rho,i)\,
      \underbrace{\sum_t\gamma_t\eq(r''_t,i)}_{W(i)}.
\]
A seven-round product sumcheck reduces this identity to
$C^\ast=\widetilde D(\rho,r^\ast)W(r^\ast)$. The guest evaluates $W(r^\ast)$ itself and forwards only the single fixed-table evaluation $\widetilde D(\rho,r^\ast)$. Consequently the expensive fixed $128\times128$ construction and contraction disappear from the VM; the in-circuit replacement is one $128$-entry coefficient MLE evaluation per sub-proof plus seven sumcheck-verifier rounds.

Two further ring-switch operations are transparent but proof-dependent rather than fixed. For each of the two $128$-word messages $s_{t,a}$, the opening target contains
\[
 T_{t,a}=\sum_{j=0}^{127}2^j\sum_{k=0}^{127}c_t(k)\,s_{t,a}(j)^{2^k}.
\]
Computing both values directly costs two more $128\times128$ contractions per sub-proof. The guest instead uses advised $T_{t,a}$ values, then copies $(r''_t,s_{t,0},s_{t,1},T_{t,0},T_{t,1})$ into the recursive statement; these are already-public transcript messages plus two field elements. The statement hash binds them to the outer VM proof. The native verifier reconstructs $c_t$ from $r''_t$, recomputes both displayed identities, and rejects on any mismatch. This adds $265$ field elements (about $4.2$ KiB) per sub-proof to the transparent statement while removing the two quadratic contractions from the recursion VM.

At the Ligerito terminal, the ring-switch weight also contains, for each claim,
\[
 E_{t,a}=\sum_{k=0}^{127}c_t(k)
   \prod_j\bigl(z_{t,a,j}^{2^k}+1+q_j\bigr),
\]
where $z_{t,a}$ is derived from the flock challenges and $q$ is the prefix of the opening's fold challenges. The guest likewise uses advised $E_{t,a}$ values and exports the two $z$ vectors, their shared $q$, and the two results. Their common length is separately bound and is at most $40$. The outer verifier evaluates the displayed formula with the already-reconstructed $c_t$. This removes the last two proof-dependent $128\times |q|$ ring-switch contractions at a statement cost of only $3|q|+2$ field elements per sub-proof.
