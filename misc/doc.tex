...
\paragraph{Memory.} Memory is immutable from the program's point of view: an array of $2^{h}$ cells, each holding one $192$-bit word (an $\E$-element), addressed by the $2^{h}$ powers $\gen^{0},\gen^{1},\dots,\gen^{2^{h}-1}$ (the addresses are $\K$-elements). The size exponent $h$ is fixed per execution, with $16\le h\le 32$. Witness generation may initialize an unset cell once; writing an initialized cell asserts that the value is unchanged.
...
\texttt{BLAKE3}        & $[o_0,o_1,o_2,o_3,o_V,o_C,\mu]$ & standard BLAKE3 compression (below)\\
\hline
\end{tabularx}
\end{center}

\texttt{PACK64X2} groups the low limbs of two physical $\E$-valued cells into one canonical $128$-bit cell. Its memory interactions read the sources as $(a_0,0,0)$ and $(b_0,0,0)$ and the destination as $(a_0,b_0,0)$. Bus balance therefore proves that both source words lie in $\K$; this is not merely a witness-generation check. The instruction is useful when a program receives serialized $\K$-limbs through an $\E$-valued interface.
...
\begin{equation}
  ...
  \qquad
  \Phi_\rho(a)=\sum_{k=0}^{63}\rho^{k}a^{2^k},
  \qquad
  \lambda_w:=\Phi_\rho(e_w).
  \label{eq:rs-phi}
\end{equation}
...
The map is specified by its \(64\) coefficients, not by the \(192\) weights \(\lambda_w\) it induces. Either determines the other, but only the coefficient count sets the verifier's cost, and no sound map has fewer than \(64\) coefficients.

\paragraph{Soundness.} The coordinate transpose is a bijection \(\E^{64}\simeq\K^{192}\) of \(12288\)-dimensional \(\F_2\)-vector spaces. If the transmitted \(s_i\) disagree with the committed witness, transposing their difference yields a nonzero vector \((\Delta_w)_{w<192}\in\K^{192}\), fixed before \(\rho\) is sampled. The batched discrepancy is
\[
  \sum_{w=0}^{191}\lambda_w\Delta_w=\sum_{k=0}^{63}\rho^{k}W_k,
  \qquad
  W_k:=\sum_{w=0}^{191}e_w^{2^k}\Delta_w .
\]
Writing \(w=64\nu+i\) splits \(W_k\) over the \(\K\)-basis \(\{1,\vartheta^{2^k},\vartheta^{2\cdot2^k}\}\) of \(\E\), whose \(\K\)-coordinates are \(\sum_{i<64}x^{i2^k}\Delta_{64\nu+i}\) for \(\nu<3\). Those coefficient matrices are Moore matrices of the basis \(x^0,\dots,x^{63}\) of \(\K/\F_2\), hence invertible, so \(\Delta\neq0\) forces \(W_k\neq0\) for some \(k\). The discrepancy is then a nonzero polynomial in \(\rho\) of degree below \(64\), and the batching error is
\[
  \Pr\Bigl[\textstyle\sum_k\rho^kW_k=0\Bigr]\le\frac{63}{2^{192}}<2^{-186}.
\]
Sampling \(\rho\) from \(\K\) would provide only about \(58\) bits of soundness; hence \(\rho\) is sampled from \(\E\). The full derivation, including why no map with fewer than \(64\) coefficients is sound, is in \href{https://github.com/leanEthereum/leanVM-b/blob/main/misc/ring-switching.pdf}{\texttt{ring-switching.pdf}}.
...
For succinct evaluation the verifier needs \(\Phi_\rho\) as a linearized polynomial, which \eqref{eq:rs-phi} already is: \(c_k=\rho^k\) for \(k<64\), and \(c_k=0\) above. The coefficients therefore cost \(63\) multiplications and no precomputed table, and one evaluation of \(\Phi_\rho\) costs \(63\) squarings and \(64\) multiplications. Defining the map by its weights instead, say \(\lambda_w=\rho^w\), leaves all \(192\) coefficients nonzero and needs a closed form to construct them, which triples both this evaluation and the target \(T\). The in-VM verifier computes that target as \(\sum_{i<64}x^i\Phi_\rho(s_i)\), straight from the transmitted rows, so it never forms the coordinates \(t_w\) or the weights \(\lambda_w\).
