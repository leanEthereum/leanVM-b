...
For succinct evaluation, represent \(\Phi_\rho\) by its linearized polynomial
\[
  \Phi_\rho(a)=\sum_{k=0}^{191}c_ka^{2^k}.
\]
For each coordinate $w$, let $d_w$ be the unique field element for which $\operatorname{Tr}_{\E/\F_2}(d_wa)$ extracts bit $w$ of $a$. Then
\[
  c_k=\sum_{w=0}^{191}\rho^w d_w^{2^k}.
\]
The implementation does not evaluate this dense sum. Writing $w=64j+i$ makes the coordinate-extraction basis factor across $\F_2\subset\K\subset\E$, so $c_k=A_kB_k$. The three-term tower factor $B_k$ costs two multiplications, while the $64$-term base-field factor collapses by $\sum_{i<64}u^i=(1+u)^{63}$ in characteristic two. All Frobenius-orbit constants are fixed by the field basis and baked into the verifier. This computes every $c_k$ in constant work and then evaluates both Flock weights together. The full derivation is in \href{https://github.com/leanEthereum/leanVM-b/blob/main/misc/ring-switching.pdf}{\texttt{ring-switching.pdf}}.
