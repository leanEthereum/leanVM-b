...
\[
  \opc{XOR}=1,\quad \opc{MUL}=\gen,\quad \opc{SET}=\gen^{2},\quad \opc{DRF}=\gen^{3},\quad \opc{JMP}=\gen^{4},\quad \opc{B3}=\gen^{5},\quad \opc{PACK}=\gen^{7}.
\]
The internal transcript-packing variant of \texttt{BLAKE3} uses the reserved tag $\gen^6$, so \texttt{PACK64X2} uses $\gen^7$.
