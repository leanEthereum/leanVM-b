...
\texttt{SHA256}        & $[o_0,o_1,o_2,o_3,o_C]$ & $c=\mathrm{SHA256}(a,b)$ on independently-addressed single-cell chunks (below)\\
\hline
\end{tabular}
\end{center}

\texttt{SHA256} compresses $64$ bytes to $32$. Each $128$-bit chunk occupies one full $\E$ memory cell through the canonical embedding $a_0+a_1y\mapsto a_0+a_1y+0y^2$; using a cell as a SHA256 input or output therefore constrains its top limb to zero. References $o_0,\dots,o_3$ independently address the four input cells, with $\loc{o_i}=\fp\cdot o_i$. The operands are $a=\loc{o_0}\Vert\loc{o_1}$ and $b=\loc{o_2}\Vert\loc{o_3}$. The input cells need not be adjacent. The digest occupies the consecutive cells $\loc{o_C}$ and $\loc{\gen\cdot o_C}$. Each row therefore accesses six cells. The SHA256 table (\S\ref{sec:tab-sha256}) connects them to the memory, state, and bytecode interactions. Flock~\cite{flock} proves $c=\mathrm{SHA256}(a,b)$ with an R1CS proof of the compression circuit.
...
\iffalse % Deferred future instructions (u32, SHA256): commented out for now.
...
No column is committed on its own. The columns are $\K$-valued multilinears over boolean hypercubes of various sizes, concatenated into one witness $q$; one region of the stack is flock's packed Boolean witness $q_\pkd$, $64$ bits per $\K$-word (\S\ref{sec:tab-sha256}). Order the $T$ columns $\widehat P_1,\dots,\widehat P_T$ by size, $\kappa_1\ge\dots\ge\kappa_T$ variables, and lay their truth tables end to end,
\[
...
\subsection{\texttt{SHA256}}\label{sec:tab-sha256}
...
Flock~\cite{flock} proves $c=\mathrm{SHA256}(a,b)$ with a batched R1CS over a Boolean witness. The witness is packed into $q_\pkd$, with $64$ bits per $\K$-element. The concrete ring switch of \S\ref{sec:ringswitch} maps its $\F_2$ claims to one $\E$-weighted claim on $q_\pkd$, which joins the batched opening (\S\ref{sec:e2e-unrolled}).

The words $a,b,c$ are contained in $q_\pkd$. Each $64$-bit flock word occupies one packed slot, and each SHA256 cell contributes its two low limbs; the top limb is canonically zero. The twelve limbs $v_{A0},\dots,v_{C3}$ therefore need no separate commitment. Each is a strided point evaluation of $q_\pkd$. Linking these embedded values to the full memory interaction implicitly forces the top limb of every SHA256-accessed cell to zero. The R1CS matrices fix the SHA256 IV, counter, block length, and flags, so they prove the compression of $a\Vert b$ without separate pin openings.
\begin{itemize}
...
\item \textbf{Constraints:} the five degree-$2$ relations $a_i=\fp\cdot o_i$ for $i\in\{0,1,2,3\}$ and $a_C=\fp\cdot o_C$. Flock proves $c=\mathrm{SHA256}(a,b)$.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
...
The program is public. Instruction $\pc$ fills coordinates $3$ through $8$ of its bytecode tuple $\tup{\dsep{BC},\pc,\cnt,\cdot}$. Coordinate $3$ holds the opcode, and coordinates $4,5,6$ hold three operands. Coordinates $7,8$ hold the \texttt{DEREF} flags $(f_{\pc},f_{\fp})$, the \texttt{SHA256} operands $(o_3,o_C)$, or zero. Each operand is a $\gen$-power or an immediate. For \texttt{SET\_CONSTANT}, coordinates $5,6$ hold the two $\K$-lanes of the immediate.
\begin{center}
...
\texttt{SHA256}        & $\opc{B3}$  & $o_0$ & $o_1$ & $o_2$ & $o_3$ & $o_C$\\
\hline
\end{tabular}
\end{center}
...
\paragraph{SHA256 validity.}
\begin{enumerate}
\item \textbf{Prover \& Verifier.} Reduce the executed SHA256 constraints to two evaluation claims on $q_\pkd$ (\S\ref{sec:tab-sha256}).
...
The implemented N-to-1 aggregation runs the verifier of \S\ref{sec:e2e-unrolled} \emph{inside} the VM: one compiled guest replays the transcripts, sumchecks, ring switches, and stacked Ligerito-K openings of $n_{\mathrm{rec}}\ge1$ ordinary VM proofs for a fixed inner program. The proofs may have different announced table and commitment sizes; the guest dispatches among the certified shapes without recompilation. The VM is well suited to this because $\E$ is its machine-word field and its Fiat--Shamir hash is the \texttt{SHA256} opcode. One class of operations, however, does not fit.
...
\item \textbf{the flock matrices}: the \texttt{SHA256} relation is proven by flock (\S\ref{sec:tab-sha256}), whose lincheck ends by evaluating $\widetilde A_0$ and $\widetilde B_0$, the per-block R1CS matrices: $28$-variable multilinears with $\approx 21\mathrm{M}$ nonzero Boolean entries between them.
\end{itemize}
...
Flock's SHA256 argument produces claims about Boolean multilinears. The VM packs them into \(\K\)-words for commitment and opens the resulting claims over \(\E\). We use the following specialization of ring switching~\cite{cryptoeprint:2024/504}:
\[
