...
\texttt{BLAKE3}        & $[o_{V0},o_{V1},o_0,o_1,o_2,o_C,\mu]$ & $\bigl(\loc{o_C},\loc{\gen\cdot o_C}\bigr)=\mathrm{Compress}\bigl(\loc{o_{V0}},\loc{o_{V1}};\loc{o_0},\loc{o_1},\loc{o_2},\loc{\gen\cdot o_2};\mu\bigr)$ (below)\\
\hline
\end{tabular}
\end{center}

\texttt{BLAKE3} is the standard BLAKE3 compression, consuming a $64$-byte message block and a $32$-byte chaining value and producing $32$ bytes. The chaining-value words at $\loc{o_{V0}},\loc{o_{V1}}$ and the first two message words at $\loc{o_0},\loc{o_1}$ are addressed independently. The second message half occupies the consecutive cells $\loc{o_2},\loc{\gen\cdot o_2}$, and the result similarly occupies $\loc{o_C},\loc{\gen\cdot o_C}$. This allocation makes a keyed BLAKE3 key---used as the initial chaining value---cheap to assemble from a public parameter and tweak without copying either half. The immediate $\mu\in\F_{2^{128}}$ packs the standard compression metadata in little-endian order,
\[
  \mu=\mathit{counter}_{64}\;\Vert\;\mathit{block\_len}_{32}\;\Vert\;\mathit{flags}_{32}.
\]
For a one-block $64$-byte hash, the chaining value is the standard IV and $\mu=(0,64,\textsc{chunk-start}|\textsc{chunk-end}|\textsc{root})$. Keyed mode instead uses the $256$-bit key as the initial chaining value and adds \textsc{keyed-hash} to every compression's flags. Within one $1024$-byte chunk, a longer standard BLAKE3 hash passes each compression result back through $o_{V0},o_{V1}$ and chooses the length and flags prescribed by BLAKE3. Still longer inputs additionally use BLAKE3's chunk counters and binary tree of parent compressions. The BLAKE3 table (\S\ref{sec:tab-blake3}) wires its memory and bytecode operands into a flock~\cite{flock} R1CS proof.
...
Compresses a $64$-byte message block (four $128$-bit words) under a $32$-byte chaining value to the $32$-byte result at $c$. The CV addresses $a_{V0}=\fp\cdot o_{V0}$, $a_{V1}=\fp\cdot o_{V1}$ and first-half message addresses $a_0=\fp\cdot o_0$, $a_1=\fp\cdot o_1$ are independent. The second message half occupies $a_2=\fp\cdot o_2$ and $\gen a_2$; the output similarly occupies $a_C=\fp\cdot o_C$ and $\gen a_C$. The immediate $\mu$ packs the $64$-bit counter, $32$-bit block length, and $32$-bit flags required by the BLAKE3 specification.
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_{V0},o_{V1},o_0,o_1,o_2,o_C$; base addresses $a_{V0},a_{V1},a_0,a_1,a_2,a_C$; memory counts $r_{A0},r_{A1},r_{B0},r_{B1},r_{V0},r_{V1},r_{C0},r_{C1}$; bytecode count $r_{\mathrm{bc}}$. The nine values above are virtual columns backed by $q_{\mathrm{pkd}}$. The successor addresses $\gen a_2$ and $\gen a_C$ are virtual.
\item \textbf{Constraints:} the six address bindings $a_X=\fp\cdot o_X$ for $X\in\{V0,V1,0,1,2,C\}$, all degree $2$. The compression relation itself is handled by flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{B3},o_{V0},o_{V1},o_0,o_1,o_2,o_C,v_\mu}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
  \item \textbf{Memory:} eight reads, each \pull\ at count $r$ and \push\ at $\gen\cdot r$: CV words at $a_{V0},a_{V1}$, first message words at $a_0,a_1$, the second message half at $a_2,\gen a_2$, and the result at $a_C,\gen a_C$.
  \end{itemize}
\end{itemize}
...
\texttt{BLAKE3}        & $\opc{B3}$  & $o_{V0}$ & $o_{V1}$ & $o_0$ & $o_1$ & $o_2$ & $o_C$ & $\mu$\\
\hline
\end{tabular}
\end{center}
