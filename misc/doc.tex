...
\paragraph{The matrices: evaluated in-circuit by the circuit walk.} The lincheck's terminal quantity is the batched bilinear form $\alpha\,u^{\!\top} A_0\,w + u^{\!\top} B_0\,w$, with $u$ the row weights fixed by the zerocheck point (univariate-skip Lagrange $\times$ $\eq$ tensor) and $w$ the column weights fixed by the lincheck's own challenges ($z$-partials $\times$ $\eq$ tensor). Although the \emph{substituted} matrices have $\approx 21\mathrm{M}$ nonzeros, the walk of flock's \S Circuit walking computes the form at the cost of one pass over the \emph{unsubstituted} compression circuit: the guest threads the 56-G BLAKE3 cascade as $\E$-valued wire values over the virtual witness $w$ (a committed slot contributes $w[\mathrm{slot}]$, an intermediate wire its running linear combination), accumulating each row's $u[i]\,(\alpha\,\langle A_i, w\rangle + \langle B_i, w\rangle)$ where the builder would emit that row. Three regroupings exploit the weight factorizations $u = u_{\mathrm{lo}}\otimes u_{\mathrm{hi}}$, $w = z\otimes w_{\mathrm{hi}}$: the $A$- and $B$-sides accumulate separately so $\alpha$ multiplies once at the very end; within a $64$-aligned run of consecutive rows $u_{\mathrm{hi}}$ is constant, so inner loops pay only the $u_{\mathrm{lo}}$ product and $u_{\mathrm{hi}}$ closes the run; and the $B{=}[z_{\mathrm{const}}]$ rows (lin-id, out, message) reduce to $w_{\mathrm{const}}\cdot\sum u$ over fixed row sets, prefix-table window sums of $1$-$2$ multiplications per $64$-window (the message rows' $A$-side even factors fully, row index $=$ column index). Cost per sub-proof: $\approx116\mathrm{k}$ cycles ($15\mathrm{k}$ of them the $2^{14}$ column-weight table), about $9\%$ of the guest; the routing schedule is baked as compile-time tables, so no matrix data is ever committed, hinted, or deferred.

\paragraph{The bytecode: deferred.} The bytecode is not a fixed-shape object of the VM (it \emph{is} the statement), so its claim is forwarded: the evaluation point and claimed value go to the public input, to be checked externally. A sub-proof may itself be an aggregation carrying a deferred claim, so verifying it yields two claims: the one read from its public input, and the fresh one produced by the verification itself. Once all $n_{\mathrm{rec}}$ sub-proofs are verified, a fresh Fiat-Shamir transcript is fed with all the points and values, and the claims are batched by one dense sumcheck over the bytecode variables; the reduced claim is written to the public input, for the next level of recursion (zeros when $n_{\mathrm{rec}}=0$). Only the outermost verifier, which runs natively, ever evaluates the bytecode itself.
