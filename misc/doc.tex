...
\texttt{BLAKE3}        & $[o_0,o_1,o_2,o_3,o_C]$ & $\bigl(\loc{o_C},\loc{\gen\cdot o_C}\bigr)=\mathrm{BLAKE3}\bigl(\loc{o_0},\loc{o_1},\loc{o_2},\loc{o_3}\bigr)$ (below)\\
\hline
\end{tabular}
\end{center}

\texttt{BLAKE3} compresses $64$ bytes to $32$. The four $128$-bit input words are addressed \emph{independently} by references $o_0,\dots,o_3$ (word $i$ at $\loc{o_i}=\fp\cdot o_i$) — the two $256$-bit operands are $\loc{o_0}\Vert\loc{o_1}$ and $\loc{o_2}\Vert\loc{o_3}$ — so a caller hashing values that live in non-adjacent cells (a tweak in the heap, a public parameter in a register, a zero pad) need not first copy them into consecutive cells. The $256$-bit digest is written to the two \emph{consecutive} cells of $c$, $\loc{o_C}$ and $\loc{\gen\cdot o_C}$. The compression is \emph{fully proven}: the BLAKE3 table (\S\ref{sec:tab-blake3}) wires the six cells onto the memory, state, and bytecode interactions, and the relation is discharged by an R1CS proof of the compression circuit, produced by flock~\cite{flock} (\S\ref{sec:tab-blake3}).
...
No column is committed on its own. The columns are $\F_{2^{128}}$-valued multilinears over boolean hypercubes of various sizes, concatenated into one witness $q$. Order the $T$ columns $\widehat P_1,\dots,\widehat P_T$ by size, $\kappa_1\ge\dots\ge\kappa_T$ variables, and lay their truth tables end to end,
\[
...
Compresses the $64$ input bytes (four $128$-bit words) to the $32$ output bytes at $c$. The four input words are addressed \emph{independently}: $a_0=\fp\cdot o_0$, $a_1=\fp\cdot o_1$, $a_2=\fp\cdot o_2$, $a_3=\fp\cdot o_3$, so the caller may hash values that live in non-adjacent cells with no assembling copies. The output takes the two consecutive cells $a_C=\fp\cdot o_C$ and its successor $\gen\cdot a_C$ (a virtual $\times\gen$ of the committed base, \S\ref{sec:prelim}). The four input cells are read; the two $c$ cells are the written digest, tied into committed memory like any other access.

The compression $c=\mathrm{BLAKE3}(a,b)$ is \emph{fully proven} by flock~\cite{flock}, which proves a batched R1CS of the circuit against a committed Boolean witness $w$. We commit flock's ring-switched~\cite{cryptoeprint:2024/504} witness $\widehat w$ over $\F_{2^{128}}$ alongside our own columns, in one stacked commitment (\S\ref{sec:rs-dense}). The words $a,b,c$ therefore already sit inside $\widehat w$, at fixed byte-aligned slots, so we do not re-commit the six value cells $v_{A0},\dots,v_{C1}$: each is a read from $\widehat w$ with appropriate boolean-selection.
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_0,o_1,o_2,o_3,o_C$; input/output addresses $a_0,a_1,a_2,a_3,a_C$; memory counts $r_{A0},r_{A1},r_{B0},r_{B1},r_{C0},r_{C1}$; bytecode count $r_{\mathrm{bc}}$. The six values $v_{A0},v_{A1},v_{B0},v_{B1},v_{C0},v_{C1}$ are committed "inside" $w$, see above. Only the output successor address $\gen\cdot a_C$ is virtual. (Independent input words cost four address columns where three sufficed for the old contiguous form — no new constraint degree.)
\item \textbf{Constraints:} the five address bindings $a_i=\fp\cdot o_i$ ($i\in\{0,1,2,3\}$) and $a_C=\fp\cdot o_C$ — all degree $2$. The words $v_{X0},v_{X1}$ carry no constraint here, their correctness (the relation $c=\mathrm{BLAKE3}(a,b)$) is handled by Flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{B3},o_0,o_1,o_2,o_3,o_C}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
  \item \textbf{Memory:} six reads, each \pull\ at count $r$ and \push\ at $\gen\cdot r$. The four inputs \pull\ $\tup{\dsep{MEM},a_i,r,v}$ at their own addresses; the output \pull\ $\tup{\dsep{MEM},a_C,r_{C0},v_{C0}}$ and $\tup{\dsep{MEM},\gen\cdot a_C,r_{C1},v_{C1}}$.
  \end{itemize}
\end{itemize}
...
\paragraph{Field, generator, and bus.} The field is $\F_{2^{128}}$ in the GHASH representation $\F_2[x]/(x^{128}+x^7+x^2+x+1)$. The generator is $\gen=x$ (has order $2^{128}-1$). Every bus tuple has width $m=9$ (the bytecode tuple); shorter tuples are zero-padded in their high coordinates (\S\ref{sec:m3}).
...
\paragraph{Opening.}
\begin{enumerate}
\item \textbf{Prover \& Verifier.} Each pooled claim is one evaluation of a committed column at a point, its value already supplied; via the stacking selectors (\S\ref{sec:stacking}) it becomes a weighted sum $\sum_w W_j(w)\,\widehat q(w)=c_j$ over the stacked witness.
...
\end{itemize}
Either evaluation is one pass over a fixed object, milliseconds natively; in-circuit, a pass over the nonzeros would dwarf the entire remaining verifier. The two cases resolve differently.

\paragraph{The matrices: batched, then walked in-circuit.} Each sub-proof's lincheck ends in the batched bilinear form $\alpha\,u^{\!\top} A_0\,w + u^{\!\top} B_0\,w$, with $u$ the row weights fixed by the zerocheck point (univariate-skip Lagrange $\times$ $\eq$ tensor) and $w$ the column weights fixed by the lincheck's own challenges ($z$-partials $\times$ $\eq$ tensor). These per-sub claims never leave the guest: the aggregation transcript batches all of them through the $28$-round matrix sumcheck (below, as before), and the guest then settles the sumcheck's terminal quantities $\widetilde A_0(r^\ast), \widetilde B_0(r^\ast)$ \emph{itself}, by ONE evaluation per aggregation via the walk of flock's \S Circuit walking. Although the \emph{substituted} matrices have $\approx 21\mathrm{M}$ nonzeros, the walk computes both values at the cost of one pass over the \emph{unsubstituted} compression circuit: the guest threads the 56-G BLAKE3 cascade as $\E$-valued wire values over the virtual witness $w = \eq(r^\ast_{\mathrm{col}},\cdot)$ (a committed slot contributes $w[\mathrm{slot}]$, an intermediate wire its running linear combination), accumulating each row's $\langle A_i, w\rangle$ and $\langle B_i, w\rangle$, weighted by $u[i] = \eq(r^\ast_{\mathrm{row}}, i)$, where the builder would emit that row. Three regroupings exploit the tensor factorizations $u = u_{\mathrm{lo}}\otimes u_{\mathrm{hi}}$, $w = w_{\mathrm{lo}}\otimes w_{\mathrm{hi}}$ at the $64\times256$ split: the $A$- and $B$-sides accumulate separately (yielding both terminal values in one pass, no batching challenge inside the walk); within a $64$-aligned run of consecutive rows $u_{\mathrm{hi}}$ is constant, so inner loops pay only the $u_{\mathrm{lo}}$ product and $u_{\mathrm{hi}}$ closes the run; and the $B{=}[z_{\mathrm{const}}]$ rows (lin-id, out, message) reduce to $w_{\mathrm{const}}\cdot\sum u$ over fixed row sets, prefix-table window sums of $1$-$2$ multiplications per $64$-window (the message rows' $A$-side even factors fully, row index $=$ column index). Cost: $\approx117\mathrm{k}$ cycles per aggregation \emph{level} ($15\mathrm{k}$ of them the $2^{14}$ column-weight table), independent of $n_{\mathrm{rec}}$; the routing schedule is baked as compile-time tables, so no matrix data is ever committed, hinted, or forwarded to the public input.

\paragraph{The bytecode: deferred.} The bytecode is not a fixed-shape object of the VM (it \emph{is} the statement), so its claim is forwarded: the evaluation point and claimed value go to the public input, to be checked externally. A sub-proof may itself be an aggregation carrying a deferred claim, so verifying it yields two claims: the one read from its public input, and the fresh one produced by the verification itself. Once all $n_{\mathrm{rec}}$ sub-proofs are verified, a fresh Fiat-Shamir transcript is fed with all the points and values, and the claims are batched by one dense sumcheck over the bytecode variables (the matrix claims by their own $28$-round sumcheck, discharged in-circuit as above); the reduced bytecode claim is written to the public input, for the next level of recursion (zeros when $n_{\mathrm{rec}}=0$). Only the outermost verifier, which runs natively, ever evaluates the bytecode itself.
