...
\paragraph{Setup.} Fixed and shared in advance (\S\ref{sec:e2e-const} through \S\ref{sec:e2e-pi}): the field and constants; the public program; the initial state $(\pc_0,\fp_0)$ and final state $(\pc_{\mathrm{final}},\fp_{\mathrm{final}})$; and the public input $\mem[0],\mem[1]$.  The prover announces the used-memory prefix; the logical memory size is canonically $2^h$ for $h=\max(16,\lceil\log_2 h_{\rm mem}\rceil)$.

\paragraph{Commitment.}
\begin{enumerate}
\item \textbf{Prover.} Builds all committed columns (field-valued): each table's state, operands, addresses, read values, and counts (plus $w,b,\mathit{next\_pc},\mathit{next\_fp}$ for \texttt{JUMP}); the memory column $M$; and the memory and bytecode finalize counts. Counts are always committed $\gen$-powers (\S\ref{sec:memchan}); the public program and the virtual increments $\gen\cdot\pc,\gen\cdot\cnt$ are not. It announces the used-memory prefix and each table's non-padded row count, packs the corresponding real prefixes into the Jagged witness $q$, and sends its dense commitment (\S\ref{sec:stacking}, \S\ref{sec:rs-dense}).  These sizes determine the Jagged boundaries; their power-of-two ceilings still determine the logical AIR and grand-product leaf layouts (\S\ref{sec:leafstack}). Padding rows are fixed defaults (\S\ref{sec:e2e-pad}); the verifier restores their MLE contributions during opening.
\end{enumerate}
