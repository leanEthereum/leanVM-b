...
\texttt{JUMP}          & $[o_c,o_d,o_f;\,\delta,t]$ & conditional jump (below)\\
...
\texttt{JUMP} reads a condition $c=\loc{o_c}$, an arbitrary field element, and branches on whether it is zero. When $c\neq0$ it transfers control, $\fp\gets\loc{o_f}$ and $\pc$ to the destination; when $c=0$ it falls through, $\pc\gets\gen\cdot\pc$ and $\fp\gets\fp$. The destination is either \emph{indirect}, $\pc\gets\loc{o_d}$ (a code address held in a cell, flag $\delta=0$), or an \emph{immediate}, $\pc\gets\gen^{t}$ (flag $\delta=1$): a compile-time-constant target reached with no \texttt{SET} to build it.
...
A conditional jump on whether the condition $c=\loc{o_c}$ is nonzero, with $c$ an arbitrary field element. It transfers to $\pc\gets d$, $\fp\gets f$ when $c\neq0$ (the new frame $f=\loc{o_f}$), and falls through, $\pc\gets\gen\cdot\pc$, $\fp\gets\fp$, when $c=0$. The destination $d$ is either the cell value $\loc{o_d}$ (indirect, flag $\delta=0$) or the immediate $\gen^{t}$ (direct, $\delta=1$); a helper column $d=\delta\cdot\gen^{t}+(1-\delta)\loc{o_d}$ carries it, keeping the pc-selection below degree $2$. The flag $\delta$ is a public bytecode field, hence $\{0,1\}$ by construction. The branch is taken according to a committed boolean indicator $b=[\,c\neq0\,]$, which a prover-supplied inverse $w$ certifies through the two degree-$2$ constraints
\[
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp,\mathit{next\_pc},\mathit{next\_fp}$; operands $o_c,o_d,o_f$; addresses $a_c,a_d,a_f$; values $c,f$ and the effective destination $d$; the immediate $\gen^{t}$ and flag $\delta$; inverse $w$; indicator $b$; memory counts $r_c,r_d,r_f$; bytecode count $r_{\mathrm{bc}}$. ($w$ and $b$ are witness columns, neither read from memory nor in the bytecode; the fall-through $s=\gen\cdot\pc$ is virtual.)
\item \textbf{Constraints:} the three addresses $a_X=\fp\cdot o_X$; the destination select $d=\delta\cdot\gen^{t}+(1-\delta)\,d_{\mathrm{cell}}$ where $d_{\mathrm{cell}}=\loc{o_d}$; the indicator pair $b=c\cdot w$ and $c\,(b+1)=0$; and the two selections $\mathit{next\_pc},\mathit{next\_fp}$ above. All degree $2$.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\mathit{next\_pc},\mathit{next\_fp}}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{JMP},o_c,o_d,o_f,\gen^{t},\delta}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$. (The two \texttt{DEREF}-flag slots carry $\gen^{t}$ and $\delta$.)
  \item \textbf{Memory} ($X\in\{c,d_{\mathrm{cell}},f\}$): \pull\ $\tup{\dsep{MEM},a_X,r_X,X}$, \push\ $\tup{\dsep{MEM},a_X,\gen\cdot r_X,X}$. In direct mode $o_d=0$, so the $d_{\mathrm{cell}}$ read touches the frame's cell $0$ and is discarded.
  \end{itemize}
\end{itemize}
...
The program is public. Instruction $\pc$ fills coordinates $3$ through $8$ of its bytecode tuple $\tup{\dsep{BC},\pc,\cnt,\cdot}$: an opcode in coordinate $3$, three operands in coordinates $4,5,6$, and coordinates $7,8$ carrying the two \texttt{DEREF} flags $(f_{\pc},f_{\fp})$ — or, for \texttt{BLAKE3}, its fourth and fifth operands $(o_3,o_C)$, or, for \texttt{JUMP}, its immediate target $\gen^{t}$ and direct flag $\delta$ — ($0$ for every other opcode). Each operand is a $\gen$-power or an immediate.
\begin{center}
...
\texttt{JUMP}          & $\opc{JMP}$ & $o_c$ & $o_d$ & $o_f$ & $\gen^{t}$ & $\delta$\\
