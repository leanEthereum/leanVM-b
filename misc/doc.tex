...
\paragraph{Instruction set.} The machine implements nine instructions. Below, $o,o_A,\dots$ are fp-relative references and $k$ a base-field immediate.
\begin{center}
\begin{tabular}{lll}
\hline
Instruction & Operands & Semantics\\
\hline
\texttt{ADD}           & $[o_A,o_B,o_C]$ & $\loc{o_C}=\loc{o_A}+\loc{o_B}$ in $\K$\\
\texttt{MUL}           & $[o_A,o_B,o_C]$ & $\loc{o_C}=\loc{o_A}\cdot\loc{o_B}$ in $\K$\\
\texttt{ADD\_EXT}      & $[o_A,o_B,o_C]$ & three-word extension addition\\
\texttt{MUL\_EXT}      & $[o_A,o_B,o_C]$ & three-word extension multiplication\\
\texttt{SET\_CONSTANT} & $[o,k]$         & $\loc{o}=k$ \quad($k\in\K$)\\
\texttt{DEREF}         & $[\alpha,\beta,\gamma;\,s]$ & $\mem[\loc{\alpha}\cdot\beta]=\srcsel{s}$\\
\texttt{DEREF\_EXT}    & $[\alpha,\beta,\gamma]$ & three-word equality at $\loc{\alpha}\cdot\beta$ and $\loc{\gamma}$\\
\texttt{JUMP}          & $[o_c,o_d,o_f]$ & conditional jump (below)\\
\texttt{BLAKE3}        & $[o_{A,0},o_{A,1},o_{B,0},o_{B,1},o_C]$ & $c=\mathrm{BLAKE3}(a,b)$ (below)\\
\hline
\end{tabular}
\end{center}

For \texttt{ADD\_EXT} and \texttt{MUL\_EXT}, each operand reference names its first coordinate. If $a_A=\fp o_A$, the other two addresses are the virtual columns $\gen a_A$ and $\gen^2a_A$; only $a_A$ is committed. The same convention applies to $B$ and $C$.

\texttt{BLAKE3} compresses $64$ bytes to $32$. Each 256-bit input is split into two independently addressed 128-bit chunks: $a$ begins at $\loc{o_{A,0}}$ and $\loc{o_{A,1}}$, and $b$ at $\loc{o_{B,0}}$ and $\loc{o_{B,1}}$. Each chunk contains two consecutive $\K$ words, so its second address is the virtual multiple by $\gen$. The digest $c$ is four consecutive words beginning at $\loc{o_C}$, with its remaining addresses virtual multiples by $\gen,\gen^2,\gen^3$. Each row therefore commits five starting addresses and accesses twelve memory words. The BLAKE3 table (\S\ref{sec:tab-blake3}) connects them to the memory, state, and bytecode interactions, while Flock~\cite{flock} proves the compression relation.
...
\texttt{DEREF\_EXT} is the three-word cell mode: it equates the heap run beginning at $\loc{\alpha}\beta$ with the local run beginning at $\loc{\gamma}$. Only the two base addresses are committed; their successors are virtual multiples by $\gen$ and $\gen^2$. Write-once witness generation fills whichever run is unset.
...
\subsection{\texttt{ADD}}\label{sec:tab-xor}
...
\item \textbf{Flushes:}
  ...
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{ADD},o_A,o_B,o_C,0,0}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
...
Identical to \texttt{ADD} but for the base-field product:
\begin{itemize}
\item \textbf{Columns:} as \texttt{ADD}.
\item \textbf{Constraints:} the three addresses and $v_C=v_Av_B$ in $\K$.
\item \textbf{Flushes:} as \texttt{ADD}, with opcode $\opc{MUL}$.
\end{itemize}

\subsection{\texttt{ADD\_EXT} and \texttt{MUL\_EXT}}\label{sec:tab-ext}
...
$v_C=v_A+v_B$ or $v_C=v_Av_B$ in $\E$. The bytecode shape remains the opcode
and three starting offsets. In the zkDSL, \texttt{sub\_ext} aliases addition
and \texttt{div\_ext} is a checked multiplication with the quotient
back-solved by witness generation.
...
\subsection{\texttt{DEREF\_EXT}}\label{sec:tab-deref-ext}

\texttt{DEREF\_EXT} bundles the three scalar cell-mode dereferences used for an extension word. It reads the pointer $p$ at $a_1=\fp\alpha$, the heap run $(v_{2,0},v_{2,1},v_{2,2})$ beginning at $a_2=p\beta$, and the local run $(v_{3,0},v_{3,1},v_{3,2})$ beginning at $a_3=\fp\gamma$. The successors $\gen a_2,\gen^2a_2$ and $\gen a_3,\gen^2a_3$ are virtual columns.
Its constraints are the three base-address relations and the packed equality
\[
  \operatorname{ext}(v_{2,0},v_{2,1},v_{2,2})
  =\operatorname{ext}(v_{3,0},v_{3,1},v_{3,2}).
\]
The row flushes one pointer access and all six run-cell accesses to memory, plus the usual state and bytecode interactions. Thus one VM cycle transfers one three-word extension value without weakening the 64-bit memory-word invariant.
...
output starts at the committed address $a_C=\fp o_C$, with successor addresses
$\gen^j a_C$ for $j\in\{1,2,3\}$. Thus each row accesses twelve memory cells
while committing five address columns.
...
The twelve $64$-bit words of $a,b,c$ are contained in $q_\pkd$ and need no
...
R1CS matrices fix the BLAKE3 IV, counter, block length, and flags.
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_{A,0},o_{A,1},o_{B,0},o_{B,1},o_C$; their five starting addresses; twelve per-word memory counts; bytecode count $r_{\mathrm{bc}}$. The twelve values live in $q_\pkd$.
\item \textbf{Constraints:} the five address relations $a_X=\fp o_X$. Flock proves $c=\mathrm{BLAKE3}(a,b)$.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{B3},o_{A,0},o_{A,1},o_{B,0},o_{B,1},o_C}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
  \item \textbf{Memory:} twelve reads: two at $a_{X,i},\gen a_{X,i}$ for each input chunk, and four at $\gen^j a_C$ for $j\in\{0,1,2,3\}$, each paired with the count multiplied by $\gen$.
  \end{itemize}
\end{itemize}
...
\]
Committed columns and machine words are $\K$-valued; challenges are $\E$-valued (\S\ref{sec:prelim}). Every bus tuple has width $m=9$ (the bytecode tuple); shorter tuples are zero-padded in their high coordinates (\S\ref{sec:m3}).
...
\[
\begin{aligned}
  \opc{ADD}&=1,& \opc{MUL}&=\gen,& \opc{SET}&=\gen^{2},& \opc{DRF}&=\gen^{3},& \opc{JMP}&=\gen^{4},\\
  \opc{B3}&=\gen^{5},& \opc{ADD\_EXT}&=\gen^{6},& \opc{MUL\_EXT}&=\gen^{7},& \opc{DRF\_EXT}&=\gen^{8}.&&
\end{aligned}
\]

\subsection{Bytecode encoding}\label{sec:e2e-bc}

The program is public. Instruction $\pc$ fills coordinates $3$ through $8$ of its bytecode tuple $\tup{\dsep{BC},\pc,\cnt,\cdot}$. Coordinate $3$ holds the opcode, coordinates $4,5,6$ hold three operands, and coordinates $7,8$ hold the \texttt{DEREF} flags or zero. Each operand is a $\gen$-power or a base-field immediate.
\begin{center}
\begin{tabular}{lllllll}
\hline
Instruction & coord.\ $3$ & coord.\ $4$ & coord.\ $5$ & coord.\ $6$ & coord.\ $7$ & coord.\ $8$\\
\hline
\texttt{ADD}           & $\opc{ADD}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$\\
\texttt{MUL}           & $\opc{MUL}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$\\
\texttt{ADD\_EXT}      & $\opc{ADD\_EXT}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$\\
\texttt{MUL\_EXT}      & $\opc{MUL\_EXT}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$\\
\texttt{SET\_CONSTANT} & $\opc{SET}$ & $o$   & $k$ & $0$ & $0$ & $0$\\
\texttt{DEREF}         & $\opc{DRF}$ & $\alpha$ & $\beta$ & $\gamma$ & $f_{\pc}$ & $f_{\fp}$\\
\texttt{DEREF\_EXT}    & $\opc{DRF\_EXT}$ & $\alpha$ & $\beta$ & $\gamma$ & $0$ & $0$\\
\texttt{JUMP}          & $\opc{JMP}$ & $o_c$ & $o_d$ & $o_f$ & $0$ & $0$\\
\texttt{BLAKE3}        & $\opc{B3}$  & $o_{A,0}$ & $o_{A,1}$ & $o_{B,0}$ & $o_{B,1}$ & $o_C$\\
\hline
\end{tabular}
\end{center}
So the program is six public columns (opcode and five operand slots), whose MLEs the verifier forms itself. For \texttt{DEREF}, the final two slots are the store-mode flags; for \texttt{BLAKE3}, all five are addresses. Writing $\mathit{instr}$ for the six, the bytecode seed at $\pc$ pushes $\tup{\dsep{BC},\pc,1,\mathit{instr}}$ and an instruction table pulls it with its opcode hardcoded (\S\ref{sec:tables}); balance then forces each table's operands to match the program at its $\pc$.
...
\paragraph{Setup and statement binding.} Fixed and shared in advance (\S\ref{sec:e2e-const} through \S\ref{sec:e2e-pi}): the fields and constants; the instance caps (\S\ref{sec:memchan}); the public program and its length; the initial and final state; and the four public words. Before any challenge, the transcript is seeded by the public input and an environment digest binding the bytecode and BLAKE3 circuit family. The prover then announces $h$ and the nine real opcode-row counts.

\paragraph{Commitment.}
\begin{enumerate}
\item \textbf{Prover.} Builds the $\K$-valued table columns, the single memory-word column $M$, the memory and bytecode finalize counts, and flock's packed witness $q_\pkd$. Extension operands contribute three value columns but only one committed starting-address column per operand; successor addresses are virtual. It stacks these into $q$, sends the dense commitment, and announces each table's non-padded row count.
\end{enumerate}
...
The assertion \texttt{assert a != b} first computes $a+b$ with \texttt{ADD}. A conditional \texttt{JUMP} skips a poison path exactly when this value is nonzero. Equality falls through to a jump to $\gen^{-1}$, outside the committed bytecode cube, so the bytecode bus cannot balance any purported continuation. This proves inequality without an inverse hint.
...
A counted loop keeps its counter in the exponent: it starts at $\gen^{\mathrm{lo}}$, advances by one multiplication $i\gets\gen\cdot i$, and exits on reaching $\gen^{\mathrm{hi}}$. The body is lowered as a tail-recursive helper function whose exit test \emph{is} the recursive call's \texttt{JUMP} condition (the \texttt{ADD} $i+\gen^{\mathrm{hi}}$ is nonzero exactly while the loop must continue), so an iteration costs one call plus two instructions, with no is-zero gadget. Loop state is threaded through captured arguments or a heap buffer.

\subsection{Conditionals}\label{sec:prog-conditionals}

\texttt{if}/\texttt{else} on a field equality is one \texttt{ADD} plus one conditional \texttt{JUMP}: the taken jump goes to whichever block the test should \emph{not} fall into ($=$ falls into \emph{then}, $\neq$ into \emph{else}), so no negation is ever computed. Intra-function jump targets are bytecode addresses $\gen^{\mathrm{entry}+i}$, backpatched at layout.
