...
Each table must satisfy its own constraints on every one of its own rows. Instead of $k$ zerochecks over cubes of $k$ different sizes, we use sumcheck batching to perform only a single sumcheck.
...
\paragraph{One challenge and one point.} The verifier samples $\eta\in\E$ and $r\in\E^{n}$. Table $T_j$ mixes its constraints with the powers of $\eta$ from $o_j=s_1+\dots+s_{j-1}$ on, and is tested at the length-$\tau_j$ prefix of $r$:
\[
  C_j\;=\;\sum_{i=1}^{s_j}\eta^{\,o_j+i-1}\,C_{j,i},
  \qquad
  S_j\;=\;\widetilde{C_j}\bigl(r_{<\tau_j}\bigr)\;=\;\sum_{x\in\cube{\tau_j}}C_j(x)\,\eq(r_{<\tau_j},x).
\]
The ranges of $\eta$-powers are disjoint. Consequently, a nonzero constraint in any table makes $\sum_j S_j$ a nonzero polynomial in $(\eta,r)$, except with Schwartz--Zippel error at most $(n+\sigma)/|\E|$.
...
\begin{enumerate}
\item \textbf{Verifier.} Sends the constraint-batch challenge $\eta$, each table taking a disjoint range of its powers, and the zerocheck point $r\in\E^n$, of which table $T_j$ uses the prefix $r_{<\tau_j}$.
\item \textbf{Prover \& Verifier.} Run the back-loaded batched zerocheck, an $n$-round sumcheck on $\sum_j\sum_x C_j(x)\,\eq(r_{<\tau_j},x)=0$ in which a table of $2^{\tau}$ rows joins at round $n-\tau$. Its end needs every table's committed column evaluations, each at that table's own reduced point $\rho_{<\tau_j}$, all of them prefixes of one $\rho$; these enter the pool.
\end{enumerate}
