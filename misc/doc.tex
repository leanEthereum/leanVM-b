...
Each instruction has a table row; the seven opcodes use six tables because
\texttt{XOR} and \texttt{MUL\_NATIVE} share the arithmetic table below.

\subsection{Arithmetic: \texttt{XOR} and \texttt{MUL\_NATIVE}}\label{sec:tab-xor}\label{sec:tab-mul}

The two arithmetic instructions share one table. A selector $s\in\K$ is $0$ for \texttt{XOR} and $1$ for \texttt{MUL\_NATIVE}; these are also their bytecode opcode tags. A materialized word $p\in\E$ stores the product $v_Av_B$, keeping every identity quadratic.
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_A,o_B,o_C$; addresses $a_A,a_B,a_C$; values $v_A,v_B,v_C,p$ (each three $\K$-limbs); selector $s$; memory counts $r_A,r_B,r_C$; bytecode count $r_{\mathrm{bc}}$.
\item \textbf{Constraints:} the three addresses $a_X=\fp\cdot o_X$ ($\K$), followed by
\[
 s(s+1)=0,\qquad p+v_Av_B=0,\qquad
 v_C+s p+(1+s)(v_A+v_B)=0.
\]
Thus $s=0$ selects $v_C=v_A+v_B$, while $s=1$ selects $v_C=p=v_Av_B$. Reassembling each word as $v=v_0+v_1y+v_2y^2$ makes these $\E$-relations degree at most two in the committed $\K$-limb columns.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},s,o_A,o_B,o_C,0,0}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
...
\[
  \opc{XOR}=0,\quad \opc{MUL}=1,\quad \opc{SET}=\gen^{2},\quad \opc{DRF}=\gen^{3},\quad \opc{JMP}=\gen^{4},\quad \opc{B3}=\gen^{5},\quad \opc{PACK}=\gen^{6}.
\]
...
\end{tabular}
\end{center}
So the program is six public columns (opcode, three operands, two flags), whose MLEs the verifier forms itself. Writing $\mathit{instr}$ for the six, the bytecode seed at $\pc$ pushes $\tup{\dsep{BC},\pc,1,\mathit{instr}}$. Each instruction table pulls it with its fixed opcode, except the shared arithmetic table, which pulls with its boolean selector $s$; balance then forces the table's opcode/selector, operands, and flags to match the program at its $\pc$.
