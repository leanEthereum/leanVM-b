...
No ordinary trace column is committed on its own.  We use the Jagged PCS construction of Hemo et al.~\cite{jagged-pcs}: if column $i$ has $h_i$ real rows, let $t_0=0$ and $t_{i+1}=t_i+h_i$.  The commitment witness contains only the real rows, tightly concatenated,
\[
  q \;=\; P_0[0..h_0] \,\Vert\, P_1[0..h_1] \,\Vert\,\dots\,\Vert\, P_{T-1}[0..h_{T-1}] \,\Vert\, 0,
\]
and is zero-padded only once, to $2^M$ where $M=\max\{M_{\min},\lceil\log_2 t_T\rceil\}$.  Thus commitment work and committed area depend on the actual trace area $\sum_i h_i$, not on the sum of the columns' separately rounded power-of-two heights.

For a Boolean dense index $w$, column $i$ and row $z$ correspond exactly when
\[
  w=t_i+z \quad\text{and}\quad w<t_{i+1}.
\]
Let $J_i(z,w)$ be the multilinear extension of this predicate.  It is the width-four read-once branching program from the Jagged PCS construction: its state records the carry of $t_i+z$ and the most-significant comparison seen between $w$ and $t_{i+1}$, while reading bits least-significant first.  The real part of a column evaluation is consequently the inner-product claim
\[
  \sum_{w\in\cube M} J_i(\zeta,w)\,\widehat q(w).
\]
...
\]
Unlike the generic adapter in~\cite{jagged-pcs}, our underlying PCS already accepts a verifier-evaluable inner-product weight.  We therefore evaluate the batched Jagged indicators directly and introduce no separate Jagged reduction sumcheck.
