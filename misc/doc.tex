...
\paragraph{Instruction set.} The machine implements five instructions. Below, $o,o_A,\dots$ are fp-relative references and $k$ an immediate operand.
\begin{center}
\begin{tabular}{lll}
\hline
...
\texttt{BLAKE3}      & (varies)            & $64$-byte to $32$-byte compression\\
\hline
\end{tabular}
\end{center}

\noindent These operate on $32$-bit integers (\texttt{u32}) rather than native field elements, and so require integer addition with carries, which the field arithmetic does not provide. Their full semantics are fixed later. \todo{specify the \texttt{u32\_MUL} and \texttt{BLAKE3} semantics later.}
...
Each of the five instructions has its own table.
...
\[
  \opc{XOR}=1,\quad \opc{MUL}=\gen,\quad \opc{SET}=\gen^{2},\quad \opc{DRF}=\gen^{3},\quad \opc{JMP}=\gen^{4}.
\]
...
\paragraph{Local constraints.} Run once per table (\S\ref{sec:air}); the five runs share no challenges.
\begin{enumerate}
