...
\[
  ...
  \qquad |\E|=2^{192}.
\]
A \emph{memory word}, including a cell value, immediate, or hash value, is an element $v=v_0+v_1y+v_2y^2\in\E$ with $v_0,v_1,v_2\in\K$. Each address, program counter $\pc$, frame pointer $\fp$, and read counter is in $\K$. Addition is bitwise \textsc{xor}; multiplication is in $\K$ or $\E$. Protocol randomness is sampled from $\E$ (\S\ref{sec:prelim}). Each physical committed column (\S\ref{sec:rs}) is $\K$-valued, so a data word occupies three columns.
...
\]
Committed columns and their truth tables are $\K$-valued; extensions are evaluated at $\E$-points; protocol randomness is sampled from $\E$. A $192$-bit \emph{value} column is represented by $V_0,V_1,V_2:\cube\ell\to\K$ with $V=V_0+V_1y+V_2y^2$. Address, register, and counter columns are single $\K$-columns.
...
\textbf{Tables and columns.} A \emph{table} has one full row cube, but a committed column may store a shorter prefix when its omitted suffix is publicly fixed to zero (the arithmetic upper-limb optimization of \S\ref{sec:tab-xor}). Each column is \emph{committed} or \emph{virtual}: a virtual column is a $\K$-linear function of the committed columns of its table, carried with no commitment of its own. Such a map, extended $\E$-linearly, commutes with the multilinear extension, $\widetilde{L(x)}(\zeta)=L\bigl(\widetilde x(\zeta)\bigr)$, so one opening of the committed columns evaluates the virtual one.
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_A,o_B,o_C$; addresses $a_A,a_B,a_C$; values $v_A,v_B,v_C$ (each three $\K$-limbs); memory counts $r_A,r_B,r_C$; bytecode count $r_{\mathrm{bc}}$.
\item \textbf{Short upper-limb commitment:} rows for which any of $v_A,v_B,v_C$ lies outside the embedded base field $\K$ are placed first; base-only rows and then padding follow. If $2^{\tau_E}$ is the smallest power of two covering that first region and the full table has $2^\tau$ rows, only the prefixes of length $2^{\tau_E}$ of $v_{A,1},v_{A,2},v_{B,1},v_{B,2},v_{C,1},v_{C,2}$ are committed. Their suffix is the public zero vector. Thus, at a full-table point $r\in\E^\tau$,
\[
 \widetilde V_{i,\mathrm{full}}(r)
 =\widetilde V_{i,\mathrm{prefix}}(r_0,\ldots,r_{\tau_E-1})
   \prod_{j=\tau_E}^{\tau-1}(1+r_j),\qquad i\in\{1,2\}.
\]
The memory bus makes the cutoff sound: a row outside the prefix carries literal zero upper limbs, so it cannot balance a memory access to a genuinely extension-valued word. The prover announces $\tau_E$ (one value for \texttt{XOR}, one for \texttt{MUL}); the verifier checks $0\le\tau_E\le\tau$. Choosing a larger prefix is valid but only increases prover cost.
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operand $o$; immediate $k$ ($192$-bit, three $\K$-limbs $k_0,k_1,k_2$); address $a$; memory count $r$; bytecode count $r_{\mathrm{bc}}$.
...
\item \textbf{Flushes:}
  ...
  \item \textbf{Memory:} \pull\ $\tup{\dsep{MEM},a,r,k}$, \push\ $\tup{\dsep{MEM},a,\gen\cdot r,k}$ (the value slot is the constant $k$, with all three limbs).
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $\alpha,\beta,\gamma$; flags $f_{\pc},f_{\fp}$; addresses $a_1,a_2,a_3$; pointer $p$ (a memory word, three $\K$-limbs); values $v_2,v_3$ (each three $\K$-limbs); memory counts $r_1,r_2,r_3$; bytecode count $r_{\mathrm{bc}}$.
...
\item \textbf{Flushes:}
  ...
  \item \textbf{Memory:} read $\tup{\dsep{MEM},a_A,r_A,v_A,0,0}$ and $\tup{\dsep{MEM},a_B,r_B,v_B,0,0}$, then read the destination as $\tup{\dsep{MEM},a_C,r_C,v_A,v_B,0}$; each pull is paired with the corresponding push at count $\gen\cdot r$. The literal zeros make the two source range assertions and the destination packing exact through bus balance.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_0,o_1,o_2,o_3,o_C$; input/output addresses $a_0,a_1,a_2,a_3,a_C$; per-cell memory counts $r_0,\dots,r_3$ (the four input cells) and $r_{C0},r_{C1}$ (the two output cells); bytecode count $r_{\mathrm{bc}}$. The twelve nonzero-capable value limbs $v_{A0},\dots,v_{C3}$ (two per value cell) are committed "inside" $q_\pkd$, see above; each cell's third limb is zero. The successor address $\gen\cdot a_C$ is virtual.
...
\item \textbf{Flushes:}
  ...
  \item \textbf{Memory:} six reads, each \pull\ at count $r$ and \push\ at $\gen\cdot r$, its embedded value carrying two limbs from $q_\pkd$ and a zero top limb (\S\ref{sec:memchan}). The four input cells \pull\ $\tup{\dsep{MEM},a_i,r_i,v_i}$; the output \pull s $\tup{\dsep{MEM},a_C,r_{C0},v_{C0}}$ and $\tup{\dsep{MEM},\gen\cdot a_C,r_{C1},v_{C1}}$.
  \end{itemize}
\end{itemize}
...
\paragraph{Setup and statement binding.} Fixed and shared in advance (\S\ref{sec:e2e-const} through \S\ref{sec:e2e-pi}): the fields and constants; the instance caps (\S\ref{sec:memchan}); the public program and its length; the initial state $(\pc_0,\fp_0)$ and final state $(\pc_{\mathrm{final}},\fp_{\mathrm{final}})$; and the public input $\mem[0],\mem[1]$. Before any challenge, the transcript is seeded by the public input and one environment digest that binds both the exact bytecode and flock's BLAKE3 circuit family. The prover then announces $h$, the seven real opcode-row counts, and the two arithmetic extension-prefix logs $\tau_{E,\mathrm{XOR}},\tau_{E,\mathrm{MUL}}$; the verifier checks them against the caps and their respective full-table logs and derives every table, stack, and leaf shape. Thus neither a different program nor an adaptively chosen shape can reuse the proof's later challenges.

\paragraph{Commitment.}
\begin{enumerate}
\item \textbf{Prover.} Builds all committed columns ($\K$-valued; a $192$-bit value is three $\K$-limb columns, \S\ref{sec:prelim}): each table's state, operands, addresses, read-value limbs, and counts (plus $w,b,\mathit{next\_pc},\mathit{next\_fp}$ for \texttt{JUMP}); the three memory limbs $M_0,M_1,M_2$; the memory and bytecode finalize counts; and flock's packed witness $q_\pkd$ (\S\ref{sec:tab-blake3}), always present (a program that executes no \texttt{BLAKE3} carries one padding instance, keeping the proof shape uniform). Counts are always committed $\gen$-powers (\S\ref{sec:memchan}); the public program and the virtual increments $\gen\cdot\pc,\gen\cdot\cnt$ are not. The six arithmetic upper-limb columns use the shortened prefixes of \S\ref{sec:tab-xor}; every other non-virtual column keeps its full cube. The prover stacks these into $q$ and sends the dense commitment (\S\ref{sec:stacking}, \S\ref{sec:rs-dense}). The verifier has already checked the announced caps and $\tau_E\le\tau$ and reconstructed the same placements. Padding rows are a fixed default (\S\ref{sec:e2e-pad}); the announced sizes fix the surplus the verifier later divides out (\S\ref{sec:gp}).
\end{enumerate}
