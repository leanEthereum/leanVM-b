...
Take one side of \eqref{eq:gp}. Its tuples give $\E$-valued leaves $v_k=\gamma-\pi_\alpha(\sigma_k)$, padded with $1$s to a power of two, $2^\mu$ in all. Their product $P=\prod_kv_k$ is the root of a binary product tree: layer $0$ is the leaves, layer $i$ has $2^{\mu-i}$ nodes, and layer $\mu$ is $P$. Let $\widetilde V_i$ denote the MLE of layer $i$.
