...
\texttt{BLAKE3} is the standard BLAKE3 compression, consuming a $64$-byte message block and a $32$-byte chaining value and producing $32$ bytes. The four $128$-bit message cells are addressed \emph{independently} by references $o_0,\dots,o_3$ (cell $i$ at $\loc{o_i}=\fp\cdot o_i$), while $o_V$ names two \emph{consecutive} chaining-value cells $\loc{o_V},\loc{\gen\cdot o_V}$. The $256$-bit result is written to the consecutive cells $\loc{o_C},\loc{\gen\cdot o_C}$, so each row accesses eight cells. The $128$-bit immediate $\mu$ packs the standard compression metadata in little-endian order,
\[
...
Compresses a $64$-byte message block (four $128$-bit cells) under a $32$-byte chaining value to the $32$-byte result at $c$. The message cells are addressed independently at $a_i=\fp\cdot o_i$, while $a_V=\fp\cdot o_V$ is the base of the two consecutive chaining-value cells $a_V,\gen a_V$. The output similarly occupies $a_C=\fp\cdot o_C$ and $\gen a_C$. The $128$-bit immediate $\mu$ packs the $64$-bit counter, $32$-bit block length, and $32$-bit flags required by the BLAKE3 specification; its two $\K$-lanes $\mu_0,\mu_1$ ride the bytecode tuple.

Flock~\cite{flock} proves the compression relation with a batched R1CS over a Boolean witness. The witness is packed into $q_\pkd$, with $64$ bits per $\K$-element. Ring switching~\cite{cryptoeprint:2024/504} maps its $\F_2$ claims to one $\E$-weighted claim on $q_\pkd$, which joins the batched opening (\S\ref{sec:e2e-unrolled}).

The message, result, chaining value, and metadata are all contained in $q_\pkd$. Each $64$-bit flock word occupies one packed slot, and each $128$-bit cell occupies two slots. The eighteen lanes $v_{A0},\dots,v_{C3},v_{V0},\dots,v_{V3},\mu_0,\mu_1$ therefore need no separate commitment: each is a strided point evaluation of $q_\pkd$. The generalized R1CS matrices take the chaining value, counter, block length, and flags as free input rows; the bus and bytecode interactions bind them.
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_0,o_1,o_2,o_3,o_V,o_C$; base addresses $a_0,a_1,a_2,a_3,a_V,a_C$; per-cell memory counts $r_0,\dots,r_3$ (the four message cells), $r_{V0},r_{V1}$ (the two chaining-value cells), and $r_{C0},r_{C1}$ (the two output cells); bytecode count $r_{\mathrm{bc}}$. The eighteen value lanes above are committed "inside" $q_\pkd$, see above. The successor addresses $\gen\cdot a_V$ and $\gen\cdot a_C$ are virtual.
\item \textbf{Constraints:} the six degree-$2$ address bindings $a_i=\fp\cdot o_i$ for $i\in\{0,1,2,3\}$, $a_V=\fp\cdot o_V$, and $a_C=\fp\cdot o_C$. The compression relation itself is handled by flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  ...
  \item \textbf{Memory:} eight reads, each \pull\ at count $r$ and \push\ at $\gen\cdot r$, its value the cell's two $\K$-lanes (\S\ref{sec:memchan}): four independently addressed message cells, the chaining value at $a_V,\gen a_V$, and the result at $a_C,\gen a_C$.
  \end{itemize}
\end{itemize}
...
\]
Committed columns are $\K$-valued; machine words and challenges are $\E$-valued (\S\ref{sec:prelim}). Every bus tuple has width $m=12$ (the bytecode tuple); shorter tuples are zero-padded in their high coordinates (\S\ref{sec:m3}).
...
The program is public. Instruction $\pc$ fills coordinates $3$ through $11$ of its bytecode tuple $\tup{\dsep{BC},\pc,\cnt,\cdot}$: an opcode in coordinate $3$ and eight operand/immediate slots in coordinates $4,\ldots,11$. Each reference operand is a $\gen$-power; immediates ride two slots as their two $\K$-lanes (\texttt{SET\_CONSTANT}'s $k_0,k_1$ and \texttt{BLAKE3}'s $\mu_0,\mu_1$).
\begin{center}
...
\texttt{SET\_CONSTANT} & $\opc{SET}$ & $o$   & $k_0$ & $k_1$ & $0$ & $0$ & $0$ & $0$ & $0$\\
