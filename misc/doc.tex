...
\paragraph{The matrices: evaluated in-circuit by the circuit walk.} The lincheck's terminal quantity is the batched bilinear form $\alpha\,u^{\!\top} A_0\,w + u^{\!\top} B_0\,w$, with $u$ the row weights fixed by the zerocheck point (univariate-skip Lagrange $\times$ $\eq$ tensor) and $w$ the column weights fixed by the lincheck's own challenges ($z$-partials $\times$ $\eq$ tensor). Although the \emph{substituted} matrices have $\approx 21\mathrm{M}$ nonzeros, the walk of flock's \S Circuit walking computes the form at the cost of one pass over the \emph{unsubstituted} compression circuit: the guest threads the 56-G BLAKE3 cascade as $\E$-valued wire values over the virtual witness $w$ (a committed slot contributes $w[\mathrm{slot}]$, an intermediate wire its running linear combination), accumulating each row's $u[i]\,(\alpha\,\langle A_i, w\rangle + \langle B_i, w\rangle)$ where the builder would emit that row. Cost per sub-proof: $\approx145\mathrm{k}$ cycles ($33\mathrm{k}$ of them the two $2^{14}$ weight tables), about $11\%$ of the guest; the routing schedule is baked as compile-time tables, so no matrix data is ever committed, hinted, or deferred.
