...
The remaining ring-switch operations are proof-dependent rather than fixed, so
they are checked inside the recursive guest rather than appended to the
recursive statement. For each of the two $128$-word messages $s_{t,a}$, the
opening target contains
\[
...
\]
where $z_{t,a}$ is derived from the flock challenges and $q$ is the prefix of the opening's fold challenges.
The guest computes both $T_{t,a}$ and $E_{t,a}$ from the transcript-derived
inputs and the advised $c_t$. The aggregate coefficient sumcheck already binds
those coefficients to $r''_t$, so no additional proof-bound record is needed.
Thus a $2\mathbin{\to}1$ layer exports the same four reduced fixed-polynomial
evaluations at every depth: recursion can be iterated indefinitely without an
accumulating list of child-specific ring-switch checks.
