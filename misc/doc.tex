...
\texttt{BLAKE3}        & $[o_A,o_B,o_C]$ & $\bigl(\loc{o_C},\loc{\gen\cdot o_C}\bigr)=\mathrm{BLAKE3}\bigl(\loc{o_A},\loc{\gen\cdot o_A},\loc{o_B},\loc{\gen\cdot o_B}\bigr)$ (below)\\
\hline
\end{tabular}
\end{center}

\texttt{BLAKE3} compresses $64$ bytes to $32$. Each operand names a $256$-bit value laid out in \emph{two consecutive memory words}: the reference $o$ names the cell at $\fp\cdot o$ and its successor at $\gen\cdot\fp\cdot o=\fp\cdot(\gen\cdot o)$, written $\loc{o}$ and $\loc{\gen\cdot o}$. The instruction reads the two inputs $a,b$ ($512$ bits) and writes the digest to the two cells of $c$ ($256$ bits). The compression is \emph{fully proven}: the BLAKE3 table (\S\ref{sec:tab-blake3}) wires the six cells onto the memory, state, and bytecode interactions, and the relation $c=\mathrm{BLAKE3}(a,b)$ is discharged by an R1CS proof of the compression circuit, produced by flock~\cite{flock} (\S\ref{sec:tab-blake3}).
...
The six instructions are proven by five tables: \texttt{XOR} and \texttt{MUL\_NATIVE} share one arithmetic table (\S\ref{sec:tab-arith}); each of the four others has its own.

\subsection{\texttt{XOR} and \texttt{MUL\_NATIVE} (arithmetic)}\label{sec:tab-arith}

One table serves both arithmetic opcodes. \texttt{XOR} asserts $\loc{o_C}=\loc{o_A}+\loc{o_B}$ (the field sum) and \texttt{MUL\_NATIVE} asserts $\loc{o_C}=\loc{o_A}\cdot\loc{o_B}$ (the GHASH product); the executed rows of both share one block, and a committed per-row opcode column $\mathit{op}$ selects which identity holds. The two arithmetic opcodes thus share one block and one constraint system, at the cost of two extra columns (the committed opcode $\mathit{op}$ and the product $p$).
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_A,o_B,o_C$; addresses $a_A,a_B,a_C$; values $v_A,v_B,v_C$; memory counts $r_A,r_B,r_C$; bytecode count $r_{\mathrm{bc}}$; the opcode $\mathit{op}\in\{\opc{XOR},\opc{MUL}\}=\{1,\gen\}$; and the product $p=v_A\cdot v_B$.
\item \textbf{Constraints:}
  \begin{itemize}
  \item the three addresses $a_X=\fp\cdot o_X$;
  \item the product $p=v_A\cdot v_B$, broken out into its own column so the result identity stays degree $2$;
  \item the result selection, cleared of a field inverse by multiplying through by $\gen+1$:
    \[
      (\gen+1)\,(v_C+v_A+v_B)\;+\;(\mathit{op}+1)\,(p+v_A+v_B)\;=\;0,
    \]
    which for $\mathit{op}=1$ (\textsc{xor}) forces $v_C=v_A+v_B$ and for $\mathit{op}=\gen$ (\textsc{mul}) forces $v_C=p$;
  \item opcode validity $(\mathit{op}+\opc{XOR})\,(\mathit{op}+\opc{MUL})=(\mathit{op}+1)(\mathit{op}+\gen)=0$, pinning $\mathit{op}\in\{1,\gen\}$.
  \end{itemize}
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\mathit{op},o_A,o_B,o_C,0,0}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$. The opcode coordinate is the \emph{committed} column $\mathit{op}$, not a constant; the bus pins it to the program's opcode at $\pc$ (\S\ref{sec:e2e-bc}).
  \item \textbf{Memory} ($X\in\{A,B,C\}$): \pull\ $\tup{\dsep{MEM},a_X,r_X,v_X}$, \push\ $\tup{\dsep{MEM},a_X,\gen\cdot r_X,v_X}$.
  \end{itemize}
\end{itemize}

Padding rows take $\mathit{op}=\opc{XOR}=1$ (\S\ref{sec:e2e-pad}): an inert no-op $v_C=v_A+v_B$ on the all-else-zero row, so every constraint still vanishes.
...
Compresses the $64$ input bytes at $a$ and $b$ to the $32$ output bytes at $c$. Each operand names a $256$-bit value in two consecutive words, so the row touches six cells: the base addresses $a_A=\fp\cdot o_A$, $a_B=\fp\cdot o_B$, $a_C=\fp\cdot o_C$, and their successors $\gen\cdot a_A$, $\gen\cdot a_B$, $\gen\cdot a_C$ (each a virtual $\times\gen$ of its committed base, \S\ref{sec:prelim}). The two cells of $a,b$ are read inputs; the two cells of $c$ are the written digest, tied into committed memory like any other access.
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_A,o_B,o_C$; base addresses $a_A,a_B,a_C$; memory counts $r_{A0},r_{A1},r_{B0},r_{B1},r_{C0},r_{C1}$; bytecode count $r_{\mathrm{bc}}$. The six values $v_{A0},v_{A1},v_{B0},v_{B1},v_{C0},v_{C1}$ are committed "inside" $w$, see above. The successor addresses $\gen\cdot a_X$ are virtual.
\item \textbf{Constraints:} the three base addresses $a_X=\fp\cdot o_X$. The words $v_{X0},v_{X1}$ carry no constraint here, their correctness (the relation $c=\mathrm{BLAKE3}(a,b)$) is handled by Flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{B3},o_A,o_B,o_C,0,0}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
  \item \textbf{Memory:} six reads, each \pull\ at count $r$ and \push\ at $\gen\cdot r$. For each $X\in\{A,B,C\}$ the two words \pull\ $\tup{\dsep{MEM},a_X,r_{X0},v_{X0}}$ and \pull\ $\tup{\dsep{MEM},\gen\cdot a_X,r_{X1},v_{X1}}$, with the matching pushes at $\gen\cdot r$.
  \end{itemize}
\end{itemize}
...
\end{tabular}
\end{center}
So the program is six public columns (opcode, three operands, two flags), whose MLEs the verifier forms itself. Writing $\mathit{instr}$ for the six, the bytecode seed at $\pc$ pushes $\tup{\dsep{BC},\pc,1,\mathit{instr}}$ and an instruction table pulls it with its opcode fixed---a hardcoded constant for the four single-opcode tables, or a committed column pinned to $\{\opc{XOR},\opc{MUL}\}$ for the merged arithmetic table (\S\ref{sec:tab-arith})---so balance forces each table's opcode, operands \emph{and} flags to match the program at its $\pc$.
...
Each table is padded to a power of two. Padding rows are a fixed default, not all zeros (a zero count would self-cancel a read, \S\ref{sec:memchan}): every column zero but the read counts $\gen^{0}=1$ (and, in the arithmetic table, the opcode column $\mathit{op}=\opc{XOR}=1$, \S\ref{sec:tab-arith}), satisfying every constraint. The prover announces each table's real row count and the verifier divides the default surplus out of the bus product (\S\ref{sec:gp}).
