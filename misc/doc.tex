...
\texttt{BLAKE3}        & $[o_0,o_1,o_2,o_3,o_C]$ & $c=\mathrm{BLAKE3}(a,b)$ on independently-addressed two-word chunks (below)\\
\hline
\end{tabular}
\end{center}

\texttt{BLAKE3} compresses $64$ bytes to $32$. The four $128$-bit input chunks are addressed \emph{independently} by references $o_0,\dots,o_3$, each chunk spanning \emph{two consecutive memory words}: chunk $i$ occupies $\loc{o_i}=\fp\cdot o_i$ and its successor $\loc{\gen\cdot o_i}$. The two $256$-bit operands are $a=\loc{o_0}\Vert\loc{\gen\cdot o_0}\Vert\loc{o_1}\Vert\loc{\gen\cdot o_1}$ and $b$ likewise from $o_2,o_3$ — so a caller hashing values that live in non-adjacent cells (a tweak in the heap, a public parameter in a register, a zero pad) need not first copy them into consecutive cells. The $256$-bit digest is written to the \emph{four consecutive} cells $\loc{o_C},\dots,\loc{\gen^{3}\cdot o_C}$. The compression is \emph{fully proven}: the BLAKE3 table (\S\ref{sec:tab-blake3}) wires the twelve cells onto the memory, state, and bytecode interactions, and the relation $c=\mathrm{BLAKE3}(a,b)$ is discharged by an R1CS proof of the compression circuit, produced by flock~\cite{flock} (\S\ref{sec:tab-blake3}).
...
Compresses the $64$ input bytes to the $32$ output bytes at $c$. The four $128$-bit input chunks are addressed \emph{independently} — $a_0=\fp\cdot o_0,\dots,a_3=\fp\cdot o_3$, each chunk spanning the two cells $a_i$, $\gen\cdot a_i$ — so the caller may hash values that live in non-adjacent cells with no assembling copies. The output takes the four consecutive cells $a_C=\fp\cdot o_C$ and its successors $\gen^{k}\cdot a_C$, $k\in\{1,2,3\}$ (each a virtual $\times\gen^{k}$ of the committed base, \S\ref{sec:prelim}), so the row touches twelve cells: the eight input cells are read; the four $c$ cells are the written digest, tied into committed memory like any other access.
...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_0,o_1,o_2,o_3,o_C$; input/output addresses $a_0,a_1,a_2,a_3,a_C$; per-word memory counts $r_{A0},\dots,r_{A3}$, $r_{B0},\dots,r_{B3}$, $r_{C0},\dots,r_{C3}$; bytecode count $r_{\mathrm{bc}}$. The twelve values $v_{A0},\dots,v_{C3}$ are committed "inside" $q_\pkd$, see above. The successor addresses $\gen\cdot a_i$ and $\gen^{k}\cdot a_C$ are virtual. (Independent input chunks cost four address columns where three sufficed for the old contiguous form — no new constraint degree.)
\item \textbf{Constraints:} the five address bindings $a_i=\fp\cdot o_i$ ($i\in\{0,1,2,3\}$) and $a_C=\fp\cdot o_C$ — all degree $2$. The words $v_{X0},\dots,v_{X3}$ carry no constraint here, their correctness (the relation $c=\mathrm{BLAKE3}(a,b)$) is handled by flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} \pull\ $\tup{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{B3},o_0,o_1,o_2,o_3,o_C}$, \push\ the same at $\gen\cdot r_{\mathrm{bc}}$.
  \item \textbf{Memory:} twelve reads, each \pull\ at count $r$ and \push\ at $\gen\cdot r$. Input chunk $i$ \pull s $\tup{\dsep{MEM},a_i,r,v}$ and $\tup{\dsep{MEM},\gen\cdot a_i,r',v'}$ at its own base; the output \pull s $\tup{\dsep{MEM},\gen^{k}\cdot a_C,r_{Ck},v_{Ck}}$, $k\in\{0,\dots,3\}$, with each matching \push\ at $\gen\cdot r$.
  \end{itemize}
\end{itemize}
