...
Take one side of \eqref{eq:gp}. Its tuples give leaves $v_k=\gamma-\pi_\alpha(\sigma_k)$, padded with $1$s to a power of two, $2^\mu$ in all. Their product $P=\prod_kv_k$ is the root of a binary product tree: layer $0$ is the leaves, layer $i$ has $2^{\mu-i}$ nodes, and layer $\mu$ is $P$. Let $\widetilde V_i$ denote the MLE of layer $i$.
...
Three times during verification, a large \emph{fixed} multilinear must be evaluated at a random point:
\begin{itemize}
...
\item \textbf{the flock padding witness}: removing Flock's repeated public padding before the Jagged opening requires two evaluations of the fixed $2^{14}$-bit witness for the all-zero BLAKE3 block, one at each final Flock witness point.
\end{itemize}
Each evaluation is one pass over a fixed object. This takes milliseconds natively, but in-circuit it would dwarf the entire remaining verifier, in cycles and in bytecode size alike.
...
There are thus four fixed polynomials: the bytecode, the two matrices $A_0,B_0$, and the Boolean padding witness $p$. A sub-proof may itself be an aggregation carrying deferred claims, so verifying it yields the claims read from its public input together with the fresh claims produced by verification. Once all $n_{\mathrm{rec}}$ sub-proofs are verified, a fresh Fiat--Shamir transcript is fed with all points and values. Three sumchecks merge them: one for bytecode, one joint sumcheck for the matrices over their shared $28$ variables, and one $14$-round sumcheck for the two padding-witness claims produced by every Flock verification. The four reduced polynomial evaluations are written to the public input. Only the outermost native verifier evaluates the fixed polynomials themselves.

The padding sumcheck uses
\[
  P_p(x)=p(x)\sum_t\bigl(\gamma_{t,AB}W_{t,AB}(x)+\gamma_{t,C}W_{t,C}(x)\bigr),
\]
where each $W$ is the transparent quirky-point weight (six univariate Lagrange coordinates followed by eight multilinear coordinates). Its terminal weight is evaluated in-circuit in $O(n_{\mathrm{rec}}2^6)$ work, while the native prover makes one pass over the fixed $2^{14}$-entry table. Consequently recursion never evaluates that table directly.
