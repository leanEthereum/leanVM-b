...
No ordinary trace column is committed on its own. The columns are $\K$-valued multilinears over boolean hypercubes of various sizes, concatenated into one witness $q$ using the Jagged PCS construction of Hemo et al.~\cite{jagged-pcs}; one region is flock's packed Boolean witness $q_\pkd$, $64$ bits per $\K$-word (\S\ref{sec:tab-blake3}). Compatible equal-height columns are partitioned into power-of-two-width blocks (the ``fancy Jagged'' layout of that work): compatibility means that the columns occur in exactly the same opening groups. For a block of width $B=2^b$ and real height $h$, its columns are stored row-major,
\[
  (P_0[0],\ldots,P_{B-1}[0],P_0[1],\ldots,P_{B-1}[h-1]).
\]
The blocks themselves are tightly concatenated and the final vector is zero-padded only once, to $2^M$. A non-power-of-two compatible group is decomposed into power-of-two blocks, so this table packing adds no dummy cells. Opcode columns use their announced row counts. The two memory-image lanes and memory-finalize count use the announced used-memory prefix, with public suffixes $0$ and $1$; the bytecode-finalize count uses the program-bound real instruction prefix, with public suffix $1$. Thus $q_{\rm pkd}$ is the only full power-of-two committed column. If $A$ is the sum of the physical block lengths, then
\[
...
\item \textbf{Verifier.} Accepts iff the announced sizes passed the caps, the two roots agreed, each leaf decomposition and every sumcheck's final value matched the supplied evaluations, flock's reduction and ring switching verified, the public-input claim held, and the PCS opening verified.
\end{enumerate}
