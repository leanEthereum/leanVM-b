...
\paragraph{Field.} The machine works over two nested binary fields: the base $\K=\F_{2^{64}}=\F_2[x]/(x^{64}+x^4+x^3+x+1)$ (the standard low-weight pentanomial) and the degree-2 tower $\E=\K[y]/(y^2+y+x^{61})$ with $|\E|=2^{128}$ (\S\ref{sec:prelim}). The roles split by kind of quantity. A \emph{memory word} — the content of a cell, an immediate, a hashed value — is a $128$-bit element of $\E$. An \emph{address}, the program counter $\pc$, the frame pointer $\fp$, and every read \emph{counter} is a $64$-bit element of the subfield $\K$. Addition is the bitwise \textsc{xor} of two values ($128$-bit for words, $64$-bit for the $\K$-quantities); multiplication is the field's own product — in $\K$ the carry-less product folded by the pentanomial (one \texttt{PMULL} plus a fold), in $\E$ the degree-2 tower product. Protocol randomness is sampled from $\E$ (\S\ref{sec:prelim}). Keeping addresses and counters in the $\K$-subfield is deliberate: their arithmetic and their commitment (\S\ref{sec:rs}) cost $64$ bits, while only the data words cost $128$.
...
\texttt{MUL\_NATIVE}   & $[o_A,o_B,o_C]$ & $\loc{o_C}=\loc{o_A}\cdot\loc{o_B}$ \quad(field product in $\E$, the tower product)\\
...
\texttt{BLAKE3} compresses $64$ bytes to $32$. With $128$-bit words a $128$-bit chunk is one whole cell, so the four input chunks are addressed \emph{independently} by references $o_0,\dots,o_3$, each naming a \emph{single} memory word $\loc{o_i}=\fp\cdot o_i$. The two $256$-bit operands are $a=\loc{o_0}\Vert\loc{o_1}$ and $b=\loc{o_2}\Vert\loc{o_3}$ — so a caller hashing values that live in non-adjacent cells (a tweak in the heap, a public parameter in a register, a zero pad) need not first copy them into consecutive cells. The $256$-bit digest is written to the \emph{two consecutive} cells $\loc{o_C},\loc{\gen\cdot o_C}$. The row thus touches six cells. The compression is \emph{fully proven}: the BLAKE3 table (\S\ref{sec:tab-blake3}) wires those cells onto the memory, state, and bytecode interactions, and the relation $c=\mathrm{BLAKE3}(a,b)$ is discharged by an R1CS proof of the compression circuit, produced by flock~\cite{flock} (\S\ref{sec:tab-blake3}).
...
\paragraph{Words used as addresses.} A memory word is $128$-bit, but $\pc$, $\fp$, and addresses are $\K$-elements. So the three reads whose value becomes an address or register — the \texttt{DEREF} pointer $\loc{\alpha}$ (which forms $\loc{\alpha}\cdot\beta$), and \texttt{JUMP}'s destination $\loc{o_d}$ and frame $\loc{o_f}$ — must return a word in the $\K$-subfield (high lane zero, so a genuine $\gen$-power). This is enforced two ways: in circuit, the address/selection constraints (\S\ref{sec:tab-deref}, \S\ref{sec:tab-jump}) equate the word (times a $\K$-operand) to a single-lane $\K$ column, which forces its high lane to zero; and during witness generation the interpreter rejects any such word whose high lane is nonzero. An honest program only ever stores $\gen$-powers in those cells, so the condition is automatic.
...
\]
($x^{61}$ has trace $1$, so the polynomial is irreducible), of size $|\E|=2^{128}$: every soundness denominator below is $2^{128}$, unchanged from a design that would commit over $\F_{2^{128}}$ outright. $\K$ sits inside $\E$ as the basis-aligned subfield $e\mapsto e+0\cdot y$, and a product of a $\K$-value with an $\E$-value costs two $\K$-products. This convention is fixed once here and used throughout: \emph{committed} columns and their truth tables are $\K$-valued, extensions are evaluated at $\E$-points, and all protocol randomness is sampled from $\E$. A $128$-bit \emph{value} column (a memory word: the memory image, a read value, an immediate) is $\E$-valued; it is carried as its \emph{two $\K$-lanes} $V_0,V_1$ (with $V=V_0+V_1 y$), each a committed $\K$-column, so the committed witness stays entirely $\K$-valued. Its $\E$-value is reassembled as $V_0+V_1 y$ only where the protocol needs it (a degree-2 constraint on the lanes, \S\ref{sec:air}; a two-coordinate bus tuple, \S\ref{sec:memchan}), never in the commitment. Address, register, and counter columns are single $\K$-columns.
...
\item \textbf{Constraints:} the three addresses, and the product $v_C=v_A\cdot v_B$ (the tower product in $\E$: a single $\E$-relation on the value lanes, still degree $2$ in the lane columns — $v_{C,0}=v_{A,0}v_{B,0}+v_{A,1}v_{B,1}c$ and $v_{C,1}=v_{A,0}v_{B,1}+v_{A,1}v_{B,0}+v_{A,1}v_{B,1}$, with $c=x^{61}$).
...
Compresses the $64$ input bytes to the $32$ output bytes at $c$. With $128$-bit cells a $128$-bit chunk is one whole cell, so the four input chunks are addressed \emph{independently} — $a_0=\fp\cdot o_0,\dots,a_3=\fp\cdot o_3$, one cell each — and the caller may hash values that live in non-adjacent cells with no assembling copies. The output takes the two consecutive cells $a_C=\fp\cdot o_C$ and its successor $\gen\cdot a_C$ (a virtual $\times\gen$ of the committed base, \S\ref{sec:prelim}), so the row touches six cells: the four input cells are read; the two $c$ cells are the written digest, tied into committed memory like any other access.
...
The words $a,b,c$ already sit inside $q_\pkd$: each $64$-bit flock word is one whole packed word at a fixed slot of its compression instance, and a $128$-bit value cell is two such words — its two $\K$-lanes. So we do not re-commit the twelve value lanes $v_{A0},\dots,v_{C3}$ (two per cell over the six value cells): each is a point evaluation of $q_\pkd$ at its slot, and a few \emph{pinned} slots (the chaining value fixed to the BLAKE3 IV, the zero counter word, and the block-length/flags word) are checked against their public constants, making the proven compression a real BLAKE3 of the $64$-byte input $a\Vert b$.
\begin{itemize}
...
\item \textbf{Constraints:} the five address bindings $a_i=\fp\cdot o_i$ ($i\in\{0,1,2,3\}$) and $a_C=\fp\cdot o_C$ — all degree $2$ ($\K$-relations). The value lanes carry no constraint here; their correctness (the relation $c=\mathrm{BLAKE3}(a,b)$) is handled by flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
...
The program is public. Instruction $\pc$ fills coordinates $3$ through $8$ of its bytecode tuple $\tup{\dsep{BC},\pc,\cnt,\cdot}$: an opcode in coordinate $3$, three operands in coordinates $4,5,6$, and coordinates $7,8$ carrying the two \texttt{DEREF} flags $(f_{\pc},f_{\fp})$ — or, for \texttt{BLAKE3}, its fourth and fifth operands $(o_3,o_C)$ — ($0$ for every other opcode). Each operand is a $\gen$-power or an immediate; for \texttt{SET\_CONSTANT}, coordinates $5,6$ carry the two $\K$-lanes of the $128$-bit immediate.
\begin{center}
