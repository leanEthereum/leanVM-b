...
\texttt{MUL\_192}      & $[o_A,o_B,o_C]$ & three-word extension multiplication\\
\texttt{SET\_CONSTANT} & $[o,k]$         & $\loc{o}=k$ \quad($k\in\K$)\\
\texttt{DEREF}         & $[\alpha,\beta,\gamma;\,s]$ & $\mem[\loc{\alpha}\cdot\beta]=\srcsel{s}$\\
\texttt{DEREF\_192}    & $[\alpha,\beta,\gamma]$ & three-word equality at $\loc{\alpha}\cdot\beta$ and $\loc{\gamma}$\\
...
\texttt{DEREF\_192} is the three-word cell mode: it equates the heap run beginning at $\loc{\alpha}\beta$ with the local run beginning at $\loc{\gamma}$. Only the two base addresses are committed; their successors are virtual multiples by $\gen$ and $\gen^2$. Write-once witness generation fills whichever run is unset.
...
and three starting offsets. In the zkDSL, \texttt{xor\_192} exposes addition
and \texttt{div\_192} is a checked multiplication with the quotient
