...
Let $\RS[\F,S,k]$ have block length $n = |S|$ and relative minimum distance $\delta := 1 - (k-1)/n$. Suppose
\[
\delta \ge 3\sqrt{\frac{2}{n}}
\qquad\text{and}\qquad
\gamma \in \left[\frac{\delta}{3},\,\frac{\delta}{2} - \frac{3}{\delta n}\right].
\]
Then, for every two-row word $C$,
\[
\Pr_{s \leftarrow \F}\!\left[\; \exists A \subseteq S,\ |A| \ge (1-\gamma) n :\ \begin{array}{l} C_s \text{ agrees with the code on } A, \text{ and} \\ C \text{ does not agree with the interleaved code on } A \end{array} \right]
\;\le\; \frac{\gamma n + 1}{|\F|}.
\]
\end{theorem}
...
Let $\RS[\F,S,k]$ have block length $n = |S|$ and set $\rho := (k-1)/n$, the \emph{slightly reduced rate} (\cite{BCHKS25} parametrize by the degree bound $k - 1$, not the dimension $k$). Let $\gamma \in (0, 1-\sqrt{\rho})$, and set $\eta := 1 - \sqrt{\rho} - \gamma$ and $\theta := \max\{\lceil \tfrac{\sqrt{\rho}}{\eta} \rceil, 3\}$ (denoted $m$ in~\cite{BCHKS25}). For every two-row word $C$,
\[
