...
An arithmetization presents $N$ columns. Column $i$ is a multilinear
...
little-endian, with $\val a=\sum_j a_j2^j$. The counters
$N,m,n,k,h,b,B,L,t,e,A$ and $\beta$ are nonnegative integers throughout.
...
typically $3$ or $4$, and $b\le m-L$ for every block, so that a block's selector coordinates
are all consumed by the prefix. Its last check needs $\sum_{y\in\cube L}f(y)\,\pol W(\rho,y)$.
...
point is zero above its dimension, so that free case covers every residual coordinate the
block does not use.

\paragraph{Closed form when every residual row coordinate vanishes} With the high row bits pinned
...
for the automaton and at most $8(2^L-1)+5$ for the contraction, so roughly $7m$ per
