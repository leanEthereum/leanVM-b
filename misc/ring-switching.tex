...
\paragraph{Proof-dependent relations stay in-circuit.}
The same coefficients appear in two further verifier computations.  If
...
messages.  The recursive verifier therefore computes $T_{t,a}$ and $E_{t,a}$
itself from the advised $c_t$ and the transcript-derived inputs.  The aggregate
fixed-table sumcheck certifies every $c_t$, so these local computations are
sound without exporting any proof-specific records.

\paragraph{Constant-size composition.}
Only the fixed-table claims cross a recursion boundary, and the sumcheck above
merges arbitrarily many of them into one evaluation of $\widetilde D$.
Consequently a fixed-arity layer, in particular $2\mathbin{\to}1$, has
fixed-size recursive state at every depth.  The assistance removes the fixed
$\Theta(f^2)$ coefficient transform while retaining the proof-dependent
$\Theta(f^2+f\ell_t)$ checks inside each recursive verifier.
